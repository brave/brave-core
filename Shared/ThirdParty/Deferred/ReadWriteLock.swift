...
public final class CASSpinLock: ReadWriteLock {
  ...
  public func withWriteLock<T>(block: () -> T) -> T {
    ...
    repeat {
      ...
      if (state == 0 || state == Masks.WRITER_WAITING_BIT) && OSAtomicCompareAndSwap32Barrier(state, Masks.WRITER_BIT, _state) {
        break
      }
  ...
  public func withReadLock<T>(block: () -> T) -> T {
    ...
    repeat {
      ...
      if (state & Masks.MASK_WRITER_BITS) == 0 && OSAtomicCompareAndSwap32Barrier(state, state + 1, _state) {
        break
      }
    } while true
    ...
    repeat {
      ...
      let newState = ((state & Masks.MASK_READER_BITS) - 1) | (state & Masks.WRITER_WAITING_BIT)
