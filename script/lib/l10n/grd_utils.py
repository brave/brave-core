...
def braveify_grd_text(text,
    ...
    Args:
        ...
        brand: The brand name ('brave', 'brave_origin', or None for default)
    """
...
def braveify_grd_tree(source_xml_tree, branding_replacements_only, brand=None):
...
def braveify_grd_in_place(source_string_path, brand=None):
    """Takes in a grd file and replaces all messages and comments with Brave
       wording

    Args:
        source_string_path: Path to the GRD file to process
        brand: The brand name ('brave', 'brave_origin', or None to auto-detect)
    """
    # Auto-detect brand from filename if not specified
    if brand is None:
...
def update_xtbs_locally(grd_file_path, brave_source_root, only_for_lang):
    ...
    brand = get_brand_from_grd_name(grd_file_path)
