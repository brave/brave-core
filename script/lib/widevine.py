...
def generate_sig_file(input_file, output_file, flags):
    execute([
        SIGNATURE_GENERATOR_PY, '--input_file', input_file, '--output_file',
        output_file, '--flags', flags, '--certificate', SIGN_WIDEVINE_CERT,
        '--private_key', SIGN_WIDEVINE_KEY, '--private_key_passphrase',
