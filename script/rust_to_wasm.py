...
# specify wasm-bindgen-cli's path for wasm-pack (via --wasm_bindgen_cli_path),
...
# This is required since wasm-bindgen-cli is in root_build_dir
# (normally, we would add it to the $PATH in build/commands/lib/config.js).

# Example usage:
# python3 rust_to_wasm.py --wasm_bindgen_cli_path /path/to/wasm-bindgen-cli /path/to/wasm-pack [wasm-pack command line arguments...]
...
env = os.environ.copy()
env['PATH'] = args.wasm_bindgen_cli_path + os.pathsep + env['PATH']
