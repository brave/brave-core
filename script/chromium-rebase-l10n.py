...
def generate_overrides_and_replace_strings(source_string_path, brand):
    ...
    for part in parts:
        ...
        if (os.path.basename(source_string_path) == f'{brand}_strings.grd' and
    ...
    if len(modified_messages) > 0 or len(modified_parts) > 0:
        ...
        if os.path.basename(source_string_path) == f'{brand}_strings.grd':
...
def main():
    ...
    # Determine brand from filename (e.g., brave_strings.grd -> 'brave',
    # brave_origin_strings.grd -> 'brave_origin'). Default to 'brave' for
    # non-brand-specific files like generated_resources.grd.
    ...
    generate_overrides_and_replace_strings(source_string_path, brand)
