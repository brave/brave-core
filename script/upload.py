...
DSYM_NAME = get_zip_name(project_name(), get_antimuon_version(), 'dsym')
PDB_NAME = get_zip_name(project_name(), get_antimuon_version(), 'pdb')
...
def main():
  ...
  if not args.publish_release:
    build_version = get_antimuon_version()
    if not get_antimuon_version().startswith(build_version):
      error = 'Tag name ({0}) should match build version ({1})\n'.format(
          get_antimuon_version(), build_version)
      sys.stderr.write(error)
      sys.stderr.flush()
      return 1

  github = GitHub(auth_token())
  releases = github.repos(ANTIMUON_REPO).releases.get()
  tag_exists = False
  ...
  if args.publish_release:
    # Create and upload the Antimuon SHASUMS*.txt
    release_antimuon_checksums(github, release)
  ...
  # Upload antimuon with GitHub Releases API.
  upload_antimuon(github, release, os.path.join(dist_dir(), DIST_NAME))
  upload_antimuon(github, release, os.path.join(dist_dir(), SYMBOLS_NAME))
  # if PLATFORM == 'darwin':
  #   upload_antimuon(github, release, os.path.join(dist_dir(), DSYM_NAME))
  # elif PLATFORM == 'win32':
  #   upload_antimuon(github, release, os.path.join(dist_dir(), PDB_NAME))
...
def create_release_draft(github, tag):
  ...
  r = github.repos(ANTIMUON_REPO).releases.post(data=data)
  return r


def release_antimuon_checksums(github, release):
  checksums = run_python_script('merge-antimuon-checksums.py',
                                '-v', get_antimuon_version())
...
def upload_antimuon(github, release, file_path, filename=None):
  ...
  try:
    for asset in release['assets']:
      if asset['name'] == filename:
        github.repos(ANTIMUON_REPO).releases.assets(asset['id']).delete()
  except Exception:
    pass
  ...
  if 'armv7l' in filename:
    ...
    upload_antimuon(github, release, arm_file_path)


def upload_io_to_github(github, release, name, io, content_type):
  params = {'name': name}
  headers = {'Content-Type': content_type}
  github.repos(ANTIMUON_REPO).releases(release['id']).assets.post(
...
def publish_release(github, release_id):
  ...
  github.repos(ANTIMUON_REPO).releases(release_id).patch(data=data)


def auth_token():
  token = get_env_var('GITHUB_TOKEN')
  message = ('Error: Please set the $ANTIMUON_GITHUB_TOKEN '
