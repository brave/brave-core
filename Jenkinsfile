...
def setEnv() {
    ...
    if (branchExistsInOtherRepo) {
        ...
        OTHER_PR_DETAILS = readJSON(text: httpRequest(url: GITHUB_API + "/brave-browser/pulls?head=brave:" + OTHER_REPO_BRANCH, customHeaders: [[name: "Authorization", value: "token ${PR_BUILDER_TOKEN}"]]).content)[0]
        if (OTHER_PR_DETAILS) {
...
def startBraveBrowserBuild() {
    ...
    if (OTHER_PR_DETAILS) {
        build(job: "brave-browser-build-pr/PR-" + OTHER_PR_DETAILS.number, parameters: [string(name: "BUILD_STATUS", value: currentBuild.result)], propagate: false)
    }
}
