...
- (id)copyWithZone:(NSZone*)zone {
  ...
  config.overridenNumberOfSecondsBetweenReconcile =
      self.overridenNumberOfSecondsBetweenReconcile;
  return config;
}
...
- (void)reset {
  [[NSFileManager defaultManager]
      removeItemAtPath:self.configuration.stateStoragePath
                 error:nil];
  [[NSFileManager defaultManager] removeItemAtURL:DataController.shared.storeURL
                                            error:nil];
...
- (instancetype)initWithConfiguration:
    (BATBraveRewardsConfiguration*)configuration {
...
- (void)setupLedgerAndAds {
  NSString* adsStorage = [self.configuration.stateStoragePath
      stringByAppendingPathComponent:@"ads"];
  self.ads = [[self.adsClass alloc] initWithStateStoragePath:adsStorage];
  NSString* ledgerStorage = [self.configuration.stateStoragePath
      stringByAppendingPathComponent:@"ledger"];
  self.ledger =
      [[self.ledgerClass alloc] initWithStateStoragePath:ledgerStorage];
...
- (void)reportTabUpdated:(NSInteger)tabId
  ...
  [self.ads reportTabUpdated:tabId
                         url:url
...
- (void)reportPostData:(NSData*)postData
         ...
         firstPartyURL:(NSURL*)firstPartyURL
           referrerURL:(NSURL*)referrerURL {
  [self.ledger reportPostData:postData
                          url:url
                        tabId:tabId
                firstPartyURL:firstPartyURL
                  referrerURL:referrerURL];
