...
- (void)showNotification:(std::unique_ptr<ads::NotificationInfo>)info {
  if (info.get() != nullptr) {
    const auto notification =
        [[BATAdsNotification alloc] initWithNotificationInfo:*info.get()];
...
- (void)getAds:(const std::string&)category
      callback:(ads::OnGetAdsCallback)callback {
...
- (void)URLRequest:(const std::string&)url
       ...
       contentType:(const std::string&)content_type
            method:(const ads::URLRequestMethod)method
...
- (const std::string)loadJsonSchema:(const std::string&)name {
  ...
  const auto path =
      [bundle pathForResource:[NSString stringWithUTF8String:name.c_str()]
                       ofType:nil];
...
- (void)loadUserModelForLocale:(const std::string&)locale
                      callback:(ads::OnLoadCallback)callback {
  const auto bundle = [NSBundle bundleForClass:[BATBraveAds class]];
  const auto localeKey = [[[NSString stringWithUTF8String:locale.c_str()]
      substringToIndex:2] lowercaseString];
  const auto path = [[bundle pathForResource:@"locales" ofType:nil]
      stringByAppendingPathComponent:[NSString
                                         stringWithFormat:@"%@/user_model.json",
                                                          localeKey]];
  if (!path || path.length == 0) {
    callback(ads::Result::FAILED, "");
    return;
...
- (void)save:(const std::string&)name
    ...
    callback:(ads::OnSaveCallback)callback {
...
- (void)saveBundleState:(std::unique_ptr<ads::BundleState>)state
               callback:(ads::OnSaveCallback)callback {
...
- (std::unique_ptr<ads::LogStream>)log:(const char*)file
                                  line:(const int)line
                              logLevel:(const ads::LogLevel)log_level {
