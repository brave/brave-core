...
/// The number of seconds between overrides. Defaults to 0 (no override) which
/// means reconciles occur every 30 days (see:
/// bat-native-ledger/static_values.h/_reconcile_default_interval)
...
/// The default configuration. Channel is debug, no changes to ledger
/// configuration
///
/// State is stored in Application Support
@property(nonatomic, class, readonly)
    BATBraveRewardsConfiguration* defaultConfiguration NS_SWIFT_NAME(default);
/// The production configuration. Channel is production, no changes to ledger
/// configuration
///
/// State is stored in Application Support
@property(nonatomic, class, readonly)
    BATBraveRewardsConfiguration* productionConfiguration NS_SWIFT_NAME(
        production);
/// The testing configuration. Channel is debug & testing. Short retries are
/// enabled, number of seconds between reconciles is set to 30 seconds instead
/// of 30 days.
///
/// State is saved to a directory created in /tmp
@property(nonatomic, class, readonly)
    BATBraveRewardsConfiguration* testingConfiguration NS_SWIFT_NAME(testing);

@end

/// A container for handling Brave Rewards. Use `ads` to handle how many ads the
/// users see, when to display them. Use `ledger` to manage interactions between
/// the users wallet & publishers
...
/// Resets the ads & ledger (by purging its data). This should likely never be
/// used in production.
- (void)reset;
...
/// Create a BraveRewards instance with a given configuration and custom ledger
/// classes for mocking
- (instancetype)initWithConfiguration:
                    (BATBraveRewardsConfiguration*)configuration
...
- (void)reportLoadedPageWithURL:(NSURL*)url
                          tabId:(UInt32)tabId
                           html:(NSString*)html
    NS_SWIFT_NAME(reportLoadedPage(url:tabId:html:));
...
- (void)reportXHRLoad:(NSURL*)url
    ...
    NS_SWIFT_NAME(reportXHRLoad(url:tabId:firstPartyURL:referrerURL:));
...
- (void)reportPostData:(NSData*)postData
         ...
         firstPartyURL:(NSURL*)firstPartyURL
           referrerURL:(nullable NSURL*)referrerURL
    NS_SWIFT_NAME(reportPostData(_:url:tabId:firstPartyURL:referrerURL:));
;
