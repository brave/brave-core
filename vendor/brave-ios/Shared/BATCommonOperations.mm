...
- (uint32_t)createTimerWithOffset:(uint64_t)offset
  ...
  self.timers[[NSNumber numberWithUnsignedInt:timerID]] =
      [NSTimer scheduledTimerWithTimeInterval:offset
                                      repeats:false
...
- (void)loadURLRequest:(const std::string&)url
  ...
  // At the moment `headers` is ignored, as I'm not sure how to use an array of
  // strings to setup HTTP headers...
  ...
  task = [session
      dataTaskWithRequest:request
        completionHandler:^(NSData* _Nullable data,
          ...
          std::string json;
          if (data) {
            // Might be no reason to convert to an NSString back to a UTF8
            // pointer...
            json =
                std::string([[NSString alloc] initWithData:data
                                                  encoding:NSUTF8StringEncoding]
                                .UTF8String);
          }
          // For some reason I couldn't just do `std::map<std::string,
          // std::string> responseHeaders;` due to std::map's non-const key
          // insertion
  ...
  const auto result = [nscontents writeToFile:path
                                   atomically:YES
                                     encoding:NSUTF8StringEncoding
                                        error:&error];
  if (error) {
    NSLog(@"Failed to save ads data for %@: %@",
          filename,
...
- (std::string)loadContentsFromFileWithName:(const std::string&)name {
  ...
  const auto contents = [NSString stringWithContentsOfFile:path
                                                  encoding:NSUTF8StringEncoding
                                                     error:&error];
  if (error) {
    NSLog(@"Failed to load ads data for %@: %@",
          filename,
          error.localizedDescription);
    return "";
  }
