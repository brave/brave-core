...
+ (nullable BATPublisherInfo*)publisherInfoWithPublisherID:
...
/// Insert or update publisher info in the database given a BATPublisherInfo
/// object
...
/// Insert contribution info into the database given all the information for a
/// contribution
...
/// Get a list of publishers you have supported with one time tips given some
/// month and year
+ (NSArray<BATPublisherInfo*>*)oneTimeTipsPublishersForMonth:
                                   (BATActivityMonth)month
                                                        year:(int)year;
...
+ (void)insertOrUpdateActivitiesInfoFromPublishers:
    (NSArray<BATPublisherInfo*>*)publishers;

/// Get a list of publishers with activity info given some start, limit and
/// filter
+ (NSArray<BATPublisherInfo*>*)
    publishersWithActivityFromOffset:(uint32_t)start
...
+ (nullable BATPublisherInfo*)mediaPublisherInfoWithMediaKey:
    (NSString*)mediaKey;

/// Insert or update some media info given some media key and publisher ID that
/// it is linked to
...
+ (void)insertPendingContributions:
    (NSArray<BATPendingContribution*>*)contributions;
