...
- (NSURL*)storeURL {
  ...
  return [NSURL fileURLWithPath:[documentURL stringByAppendingPathComponent:
                                                 @"BraveRewards.sqlite"]];
}
...
- (instancetype)init {
  if ((self = [super init])) {
    ...
    NSAssert(model != nil,
             @"Error initializing managed object model from: %@",
             modelURL);
...
- (void)performOnContext:(NSManagedObjectContext*)context
                    task:(void (^)(NSManagedObjectContext* _Nonnull))task {
...
- (void)performOnContext:(NSManagedObjectContext*)context
  ...
  // Queue operation and saving is done in `performTask()` called at higher
  // level when a nil context is passed
  if (context) {
    task(context);
    return;
  }
  ...
  [self.operationQueue addOperationWithBlock:^{
    ...
    // performAndWait doesn't block main thread because it fires on
    // OperationQueue`s background thread.
