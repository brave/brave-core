...
#define BATLedgerBridge(                                              \
    __type, __objc_getter, __objc_setter, __cpp_getter, __cpp_setter) \
  -(__type)__objc_getter {                                            \
    return ledger->__cpp_getter();                                    \
  }                                                                   \
...
NSNotificationName const BATBraveLedgerNotificationAdded =
...
NS_INLINE ledger::ACTIVITY_MONTH BATGetPublisherMonth(NSDate* date) {
  const auto month = [[NSCalendar currentCalendar] component:NSCalendarUnitMonth
                                                    fromDate:date];
  return (ledger::ACTIVITY_MONTH)month;
}

NS_INLINE int BATGetPublisherYear(NSDate* date) {
  return (int)[[NSCalendar currentCalendar] component:NSCalendarUnitYear
                                             fromDate:date];
}
...
@property(nonatomic, copy, nullable) void (^walletInitializedBlock)
    (const ledger::Result result);
@property(nonatomic, copy, nullable) void (^walletRecoveredBlock)
    (const ledger::Result result,
     const double balance,
     const std::vector<ledger::Grant>& grants);
@property(nonatomic, copy, nullable) void (^grantCaptchaBlock)
...
BATLedgerReadonlyBridge(BOOL, isWalletCreated, IsWalletCreated)
  ...
  self.walletInitializedBlock = ^(const ledger::Result result) {
    ...
    if (result != ledger::WALLET_CREATED) {
      ...
      error = [NSError errorWithDomain:BATBraveLedgerErrorDomain
                                  code:result
                              userInfo:userInfo];
    }
    if (completion) {
...
- (void)fetchWalletDetails:(void (^)(BATWalletInfo*))completion {
  ledger->FetchWalletProperties(^(ledger::Result result,
                                  std::unique_ptr<ledger::WalletInfo> info) {
    const auto walletInfo = *info.get();
    dispatch_async(dispatch_get_main_queue(), ^{
      [self
          onWalletProperties:result
                        arg1:std::make_unique<ledger::WalletInfo>(walletInfo)];
      if (completion) {
...
- (void)onWalletProperties:(ledger::Result)result
                      arg1:(std::unique_ptr<ledger::WalletInfo>)arg1 {
...
- (NSString*)walletPassphrase {
  if (ledger->IsWalletCreated()) {
    return
        [NSString stringWithUTF8String:ledger->GetWalletPassphrase().c_str()];
  }
  return nil;
}
...
- (void)recoverWalletUsingPassphrase:(NSString*)passphrase
  ...
  const auto __weak weakSelf = self;
  self.walletRecoveredBlock = ^(const ledger::Result result,
                                const double balance,
                                const std::vector<ledger::Grant>& grants) {
    const auto strongSelf = weakSelf;
    if (!strongSelf) {
...
- (void)onRecoverWallet:(ledger::Result)result
                balance:(double)balance
                 grants:(const std::vector<ledger::Grant>&)grants {
...
- (void)addressesForPaymentId:
    (void (^)(NSDictionary<NSString*, NSString*>* _Nonnull))completion {
  ledger->GetAddressesForPaymentId(
...
BATLedgerReadonlyBridge(double,
                        ...
                        GetDefaultContributionAmount);

#pragma mark - Publishers

- (void)publisherInfoForId:(NSString*)publisherId
                completion:(void (^)(BATPublisherInfo* _Nullable))completion {
  ledger->GetPublisherInfo(
      std::string(publisherId.UTF8String),
      ^(ledger::Result result, ledger::PublisherInfoPtr info) {
        if (result == ledger::LEDGER_OK) {
          const auto* publisherInfo = info.get();
          if (publisherInfo != nullptr) {
            const auto info =
                [[BATPublisherInfo alloc] initWithPublisherInfo:*publisherInfo];
            dispatch_async(dispatch_get_main_queue(), ^{
              completion(info);
            });
          }
...
- (void)listActivityInfoFromStart:(unsigned int)start
  ...
  ledger->GetActivityInfoList(
      ...
      limit,
      cppFilter,
      ^(const ledger::PublisherInfoList& list, uint32_t nextRecord) {
        const auto publishers = NSArrayFromVector(
            &list, ^BATPublisherInfo*(const ledger::PublisherInfoPtr& info) {
...
- (void)activityInfoWithFilter:(nullable BATActivityInfoFilter*)filter
                    completion:
                        (void (^)(BATPublisherInfo* _Nullable info))completion {
  const auto cppFilter = filter ? filter.cppObj : ledger::ActivityInfoFilter();
  ledger->GetActivityInfo(
      cppFilter, ^(ledger::Result result, ledger::PublisherInfoPtr info) {
        if (result == ledger::LEDGER_OK) {
          const auto* publisherInfo = info.get();
          if (publisherInfo != nullptr) {
            completion([[BATPublisherInfo alloc]
                initWithPublisherInfo:*publisherInfo]);
          }
        } else {
          completion(nil);
        }
...
- (void)publisherActivityFromURL:(NSURL*)URL
  ...
  ledger->GetPublisherActivityFromUrl(
      1, visitData, std::string(publisherBlob.UTF8String));
}

- (void)mediaPublisherInfoForMediaKey:(NSString*)mediaKey
                           completion:(void (^)(BATPublisherInfo* _Nullable))
                                          completion {
  ledger->GetMediaPublisherInfo(
      std::string(mediaKey.UTF8String),
      ^(ledger::Result result, ledger::PublisherInfoPtr info) {
        if (result == ledger::LEDGER_OK) {
          const auto* publisherInfo = info.get();
          if (publisherInfo != nullptr) {
...
- (void)updateMediaPublisherInfo:(NSString*)publisherId
  ...
  ledger->SetMediaPublisherInfo(std::string(publisherId.UTF8String),
                                std::string(mediaKey.UTF8String));
}
...
- (void)updatePublisherExclusionState:(NSString*)publisherId
  ...
  ledger->SetPublisherExclude(std::string(publisherId.UTF8String),
                              (ledger::PUBLISHER_EXCLUDE)state);
}
  ...
  ledger->GetPublisherBanner(
      ...
      ^(std::unique_ptr<ledger::PublisherBanner> banner) {
        auto bridgedBanner = banner.get() != nullptr
                                 ? [[BATPublisherBanner alloc]
                                       initWithPublisherBanner:*banner.get()]
                                 : nil;
        dispatch_async(dispatch_get_main_queue(), ^{
...
- (nullable BATPublisherInfo*)currentActivityInfoWithPublisherId:
  ...
  return
      [[BATLedgerDatabase publishersWithActivityFromOffset:0
                                                     limit:1
                                                    filter:filter] firstObject];
}
...
- (void)tipPublisherDirectly:(BATPublisherInfo*)publisher
  ...
  ledger->DoDirectTip(std::string(publisher.id.UTF8String),
                      amount,
...
- (NSString*)notificationIDForGrant:(const ledger::Grant&)grant {
  const auto prefix =
      [self isGrantUGP:grant] ? @"rewards_grant_" : @"rewards_grant_ads_";
  const auto promotionId =
      [NSString stringWithUTF8String:grant.promotionId.c_str()];
...
- (void)onGrant:(ledger::Result)result grant:(const ledger::Grant&)grant {
  if (result == ledger::LEDGER_OK) {
    ...
    auto notificationKind = isUGP ? BATRewardsNotificationKindGrant
...
- (void)onGrantFinish:(ledger::Result)result grant:(const ledger::Grant&)grant {
  ...
  if (result == ledger::LEDGER_OK) {
    ledger::ReportType report_type =
        grant.type == "ads" ? ledger::ADS : ledger::GRANT;
...
- (NSDictionary<NSString*, BATBalanceReportInfo*>*)balanceReports {
  ...
  return NSDictionaryFromMap(
      reports, ^BATBalanceReportInfo*(ledger::BalanceReportInfo info) {
...
- (void)onReconcileComplete:(ledger::Result)result
                  viewingId:(const std::string&)viewing_id
                   category:(ledger::REWARDS_CATEGORY)category
                      probi:(const std::string&)probi {
...
- (void)reportXHRLoad:(NSURL*)url
  ...
  for (NSURLQueryItem* item in urlComponents.queryItems) {
    partsMap[std::string(item.name.UTF8String)] =
        std::string(item.value.UTF8String);
  }
...
- (void)adsDetailsForCurrentCycle:
    (void (^)(NSInteger adsReceived, double estimatedEarnings))completion {
  ledger->GetTransactionHistoryForThisCycle(
      ^(std::unique_ptr<ledger::TransactionsInfo> list) {
...
- (void)showBackupNotificationIfNeccessary {
  ...
  addNotificationOfKind:BATRewardsNotificationKindBackupWallet arguments:nil
...
- (void)addNotificationOfKind:(BATRewardsNotificationKind)kind
  ...
  if (!identifier || identifier.length == 0) {
    ...
    const auto idx = [self.mNotifications
        indexOfObjectPassingTest:^BOOL(BATRewardsNotification* _Nonnull obj,
                                       NSUInteger idx,
                                       BOOL* _Nonnull stop) {
  ...
  const auto notification = [[BATRewardsNotification alloc]
  ...
  // Post to observers
  [NSNotificationCenter.defaultCenter
      postNotificationName:BATBraveLedgerNotificationAdded
                    object:nil];
...
- (void)saveLedgerState:(const std::string&)ledger_state
                handler:(ledger::LedgerCallbackHandler*)handler {
  const auto result = [self.commonOps saveContents:ledger_state
                                              name:"ledger_state.json"];
  handler->OnLedgerStateSaved(result ? ledger::LEDGER_OK
                                     : ledger::NO_LEDGER_STATE);
}
...
- (void)savePublisherState:(const std::string&)publisher_state
                   handler:(ledger::LedgerCallbackHandler*)handler {
  const auto result = [self.commonOps saveContents:publisher_state
                                              name:"publisher_state.json"];
  handler->OnPublisherStateSaved(result ? ledger::LEDGER_OK
                                        : ledger::LEDGER_ERROR);
}

- (void)loadPublisherList:(ledger::LedgerCallbackHandler*)handler {
  const auto contents =
      [self.commonOps loadContentsFromFileWithName:"publisher_list.json"];
  if (contents.length() > 0) {
...
- (void)savePublishersList:(const std::string&)publisher_state
                   handler:(ledger::LedgerCallbackHandler*)handler {
  const auto result = [self.commonOps saveContents:publisher_state
                                              name:"publisher_list.json"];
  handler->OnPublishersListSaved(result ? ledger::LEDGER_OK
                                        : ledger::LEDGER_ERROR);
}

- (void)loadState:(const std::string&)name
         callback:(ledger::OnLoadCallback)callback {
...
- (void)resetState:(const std::string&)name
          callback:(ledger::OnResetCallback)callback {
  const auto key = [NSString stringWithUTF8String:name.c_str()];
  self.state[key] = nil;
...
- (void)saveState:(const std::string&)name
         ...
         callback:(ledger::OnSaveCallback)callback {
  const auto key = [NSString stringWithUTF8String:name.c_str()];
...
- (void)setTimer:(uint64_t)time_offset timerId:(uint32_t*)timer_id {
  const auto __weak weakSelf = self;
  const auto createdTimerID = [self.commonOps
      createTimerWithOffset:time_offset
                 timerFired:^(uint32_t firedTimerID) {
                   const auto strongSelf = weakSelf;
                   if (!strongSelf.commonOps) {
                     return;
                   }
...
- (void)loadURL:(const std::string&)url
    ...
    contentType:(const std::string&)contentType
         method:(const ledger::URL_METHOD)method
       callback:(ledger::LoadURLCallback)callback {
  std::map<ledger::URL_METHOD, std::string> methodMap{
      {ledger::GET, "GET"}, {ledger::POST, "POST"}, {ledger::PUT, "PUT"}};
...
- (void)fetchFavIcon:(const std::string&)url
          faviconKey:(const std::string&)favicon_key
            callback:(ledger::FetchIconCallback)callback {
...
- (std::unique_ptr<ledger::LogStream>)verboseLog:(const char*)file
                                            line:(int)line
                                       vlogLevel:(int)vlog_level {
  return std::make_unique<RewardsLogStream>(
      file, line, (ledger::LogLevel)vlog_level);
}

- (std::unique_ptr<ledger::LogStream>)log:(const char*)file
                                     line:(int)line
                                 logLevel:(const ledger::LogLevel)log_level {
...
- (void)handlePublisherListing:(NSArray<BATPublisherInfo*>*)publishers
  ...
  callback(VectorFromNSArray(publishers,
                             ^ledger::PublisherInfoPtr(BATPublisherInfo* info) {
...
- (void)getActivityInfoList:(uint32_t)start
                      limit:(uint32_t)limit
                     filter:(ledger::ActivityInfoFilter)filter
                   callback:(ledger::PublisherInfoListCallback)callback {
  const auto filter_ =
      [[BATActivityInfoFilter alloc] initWithActivityInfoFilter:filter];
  const auto publishers =
      [BATLedgerDatabase publishersWithActivityFromOffset:start
                                                    limit:limit
                                                   filter:filter_];

  [self handlePublisherListing:publishers
                         start:start
                         limit:limit
                      callback:callback];
}
...
- (void)getOneTimeTips:(ledger::PublisherInfoListCallback)callback {
  ...
  const auto publishers = [BATLedgerDatabase
      oneTimeTipsPublishersForMonth:(BATActivityMonth)BATGetPublisherMonth(now)
...
- (void)loadActivityInfo:(ledger::ActivityInfoFilter)filter
  ...
  [self handlePublisherListing:publishers
                         start:0
                         limit:2
                      callback:^(const ledger::PublisherInfoList& list,
                                 uint32_t) {
                        // activity info not found
                        if (list.size() == 0) {
                          // we need to try to get at least publisher info in
                          // this case this way we preserve publisher info
                          const auto publisherID =
                              [NSString stringWithUTF8String:filter.id.c_str()];
                          const auto info = [BATLedgerDatabase
                              publisherInfoWithPublisherID:publisherID];
                          if (info) {
                            callback(ledger::Result::LEDGER_OK,
                                     info.cppObj.Clone());
                          } else {
                            // This part diverges from brave-core. brave-core
                            // actually goes into an infinite loop here? Hope im
                            // missing something on their side where they don't
                            // even call this method unless there's a publisher
                            // with that ID in the ActivityInfo and
                            // PublisherInfo table...
...
- (void)loadMediaPublisherInfo:(const std::string&)media_key
  ...
  const auto publisher =
      [BATLedgerDatabase mediaPublisherInfoWithMediaKey:mediaKey];
  if (publisher) {
...
- (void)loadPublisherInfo:(const std::string&)publisher_key
  ...
  const auto publisher =
      [BATLedgerDatabase publisherInfoWithPublisherID:publisherID];
  if (publisher) {
    callback(ledger::Result::LEDGER_OK, publisher.cppObj.Clone());
  } else {
    callback(ledger::Result::NOT_FOUND, nullptr);
...
- (void)onExcludedSitesChanged:(const std::string&)publisher_id
                       exclude:(ledger::PUBLISHER_EXCLUDE)exclude {
  bool excluded = exclude == ledger::PUBLISHER_EXCLUDE::EXCLUDED;

  if (excluded) {
    const auto stamp = ledger->GetReconcileStamp();
    const auto publisherID =
        [NSString stringWithUTF8String:publisher_id.c_str()];
...
- (void)onPanelPublisherInfo:(ledger::Result)result
                        arg1:(std::unique_ptr<ledger::PublisherInfo>)arg1
...
- (void)onRemoveRecurring:(const std::string&)publisher_key
                 callback:(ledger::RecurringRemoveCallback)callback {
  const auto publisherID =
      [NSString stringWithUTF8String:publisher_key.c_str()];
  const auto success =
      [BATLedgerDatabase removeRecurringTipWithPublisherID:publisherID];
...
- (void)saveActivityInfo:(ledger::PublisherInfoPtr)publisher_info
  ...
  if (info != nullptr) {
    const auto publisher =
        [[BATPublisherInfo alloc] initWithPublisherInfo:*info];
...
- (void)saveContributionInfo:(const std::string&)probi
                ...
                publisherKey:(const std::string&)publisher_key
                    category:(const ledger::REWARDS_CATEGORY)category {
  [BATLedgerDatabase
      insertContributionInfo:[NSString stringWithUTF8String:probi.c_str()]
                       month:month
                        year:year
                        date:date
                publisherKey:[NSString
                                 stringWithUTF8String:publisher_key.c_str()]
                    category:(BATRewardsCategory)category];
}
...
- (void)saveMediaPublisherInfo:(const std::string&)media_key
  ...
  [BATLedgerDatabase
      insertOrUpdateMediaPublisherInfoWithMediaKey:
          [NSString stringWithUTF8String:media_key.c_str()]
                                       publisherID:[NSString
...
- (void)saveNormalizedPublisherList:(ledger::PublisherInfoList)normalized_list {
  const auto list = NSArrayFromVector(
      &normalized_list,
      ^BATPublisherInfo*(const ledger::PublisherInfoPtr& info) {
        return [[BATPublisherInfo alloc] initWithPublisherInfo:*info];
      });
...
- (void)savePendingContribution:(ledger::PendingContributionList)list {
  const auto list_ = NSArrayFromVector(
      &list,
      ^BATPendingContribution*(const ledger::PendingContributionPtr& info) {
        return
            [[BATPendingContribution alloc] initWithPendingContribution:*info];
      });
...
- (void)getCountryCodes:(const std::vector<std::string>&)countries
               callback:(ledger::GetCountryCodesCallback)callback {
}
...
- (void)onPanelPublisherInfo:(ledger::Result)result
               publisherInfo:(ledger::PublisherInfoPtr)publisher_info
                    windowId:(uint64_t)windowId {
}
...
- (void)removePendingContribution:(const std::string&)publisher_key
                        viewingId:(const std::string&)viewing_id
                        addedDate:(uint64_t)added_date
                         callback:
                             (const ledger::RemovePendingContributionCallback&)
                                 callback {
}
