...
bool StructTraits<ledger::mojom::VgBodyDataView, sync_pb::VgBodySpecifics>::
  ...
  if (!data.ReadCredsId(&creds_id) || !data.ReadTriggerId(&trigger_id) ||
      !data.ReadCreds(&creds) || !data.ReadBlindedCreds(&blinded_creds) ||
  ...
  out->set_trigger_id(std::move(trigger_id));
