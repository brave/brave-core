...
class BitflyerTest : public Test {
 private:
 ...
 protected:
  ...
  std::unique_ptr<Bitflyer> bitflyer_;
...
TEST_P(DisconnectBitflyerWallet, Paths) {
  const auto& params = GetParam();
  std::string bitflyer_wallet = std::get<1>(params);
  ...
  ON_CALL(*mock_ledger_client_, GetStringState(state::kWalletBitflyer))
      .WillByDefault(
          [&] { return FakeEncryption::Base64EncryptString(bitflyer_wallet); });

  ON_CALL(*mock_ledger_client_, SetStringState(state::kWalletBitflyer, _))
      .WillByDefault([&](const std::string&, const std::string& value) {
        bitflyer_wallet = *FakeEncryption::Base64DecryptString(value);
        return true;
      });
