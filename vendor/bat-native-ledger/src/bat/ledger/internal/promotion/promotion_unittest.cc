...
TEST_F(PromotionTest, TestInitialize) {
  // Arrange
  ON_CALL(*mock_ledger_client_, GetBooleanState(
  ...
  // Assert
  // See |EXPECT_CALL| in arrange, as this must happen before Act
}

TEST_F(PromotionTest, TestInitializeWithCorruptedPromotionMigration) {
  // Arrange
  ON_CALL(*mock_ledger_client_, GetBooleanState(
      ledger::kStatePromotionCorruptedMigrated))
...
TEST_F(PromotionTest, TestFetch) {
  // Arrange
  EXPECT_CALL(*mock_ledger_client_, LoadURL(_, _, _, _, _, _));
