...
TEST(MediaYouTubeTest, GetMediaIdFromUrl) {
  ...
  std::string media = braveledger_media::MediaYouTube::GetMediaIdFromUrl(data);
  ASSERT_EQ(media, "");

  // single element in the url
  data.url = "https://www.youtube.com/watch?v=44444444";

  media = braveledger_media::MediaYouTube::GetMediaIdFromUrl(data);
...
TEST(MediaYouTubeTest, GetUserFromUrl) {
  ...
  std::string user = braveledger_media::MediaYouTube::GetUserFromUrl(path);
  ASSERT_EQ(user, "");

  // path is just slash
  path = "/";

  user = braveledger_media::MediaYouTube::GetUserFromUrl(path);
...
TEST(MediaYouTubeTest, GetBasicPath) {
  ...
  std::string realPath = braveledger_media::MediaYouTube::GetBasicPath(path);
  ASSERT_EQ(realPath, "/gaming");

  path = "/watch?v=000000000000000";
  realPath = braveledger_media::MediaYouTube::GetBasicPath(path);
...
TEST(MediaYouTubeTest, GetNameFromChannel) {
  ...
  // quote
  resolve = braveledger_media::MediaYouTube::GetNameFromChannel("\"");
  ASSERT_EQ(resolve, std::string());
  ...
  std::string subject(json_envelope_open + "invalid\"json\"}" +
                      json_envelope_close);
  resolve = braveledger_media::MediaYouTube::GetNameFromChannel(subject);
...
TEST(MediaYouTubeTest, GetPublisherName) {
  ...
  // quote
  publisher_name = braveledger_media::MediaYouTube::GetPublisherName("\"");
  ASSERT_EQ(publisher_name, std::string());
  ...
  publisher_name = braveledger_media::MediaYouTube::GetPublisherName(subject);
...
TEST(MediaYouTubeTest, GetMediaDurationFromParts) {
  ...
  uint64_t duration(
      MediaYouTube::GetMediaDurationFromParts(parts, std::string()));
  ...
  parts.insert(std::pair<std::string, std::string>("st", start_times));
  parts.insert(std::pair<std::string, std::string>("et", end_times));
  duration = MediaYouTube::GetMediaDurationFromParts(parts, std::string());
...
TEST(MediaYouTubeTest, GetVideoUrl) {
  ...
  EXPECT_DEATH({ MediaYouTube::GetVideoUrl(media_id); },
...
#else
  ::testing::FLAGS_gtest_death_test_style = "threadsafe";
  EXPECT_DEATH({ MediaYouTube::GetChannelUrl(channel_id); },
