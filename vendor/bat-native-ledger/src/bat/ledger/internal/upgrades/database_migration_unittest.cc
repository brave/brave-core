...
class LedgerDatabaseMigrationTest : public BATLedgerTest {
 protected:
  ...
  void InitializeDatabaseAtVersion(int version) {
    ...
    GetTestLedgerClient()->database()->OpenInMemoryForTesting();
    ASSERT_TRUE(GetDB()->Execute(init_script.c_str()));
  }

  void RunUpgrades(int version = 0) {
    bool result = WaitFor(
        context().Get<UpgradeManager>().UpgradeToVersionForTesting(version));
    ASSERT_TRUE(result);
  }
...
TEST_F(LedgerDatabaseMigrationTest, SchemaCheck) {
  auto get_schema = [this]() {
    std::string schema = GetDB()->GetSchema();
    // Normalize white-space: collapse consecutive white-space and remove spaces
    // around commas and parenthesis.
    re2::RE2::GlobalReplace(&schema, "\\s\\s+", " ");
    re2::RE2::GlobalReplace(&schema, "\\s,(\\s)?|,\\s", ",");
    re2::RE2::GlobalReplace(&schema, "\\(\\s", "(");
    re2::RE2::GlobalReplace(&schema, "\\s\\)", ")");
    return schema;
  };

  // Skip a clean install by first upgrading to version 1 and then upgrading to
  // the current version.
  RunUpgrades(1);
  RunUpgrades();
  std::string upgrade_schema = get_schema();

  // Clear the database by closing it.
  WaitFor(context().Get<SQLStore>().Close());

  // Perform a clean install.
  RunUpgrades();
  std::string clean_schema = get_schema();

  // Verify that schemas are the same.
  EXPECT_EQ(upgrade_schema, clean_schema);
}
