...
void DatabaseVgBackupRestore::OnBackUpVgBodies(
  ...
  while (cit1 != cend) {
    ...
    vg_body.set_trigger_id(GetStringColumn(record_pointer, 1));
    vg_body.set_trigger_type(GetIntColumn(record_pointer, 2));
    vg_body.set_creds(GetStringColumn(record_pointer, 3));
    vg_body.set_blinded_creds(GetStringColumn(record_pointer, 4));
    vg_body.set_signed_creds(GetStringColumn(record_pointer, 5));
    vg_body.set_public_key(GetStringColumn(record_pointer, 6));
    vg_body.set_batch_proof(GetStringColumn(record_pointer, 7));
    vg_body.set_status(GetIntColumn(record_pointer, 8));

    while (cit1 != cit2) {
      record_pointer = cit1->get();

      sync_pb::VgBodySpecifics::Token token;
      token.set_token_id(GetInt64Column(record_pointer, 9));
      token.set_token_value(GetStringColumn(record_pointer, 10));
      token.set_value(GetDoubleColumn(record_pointer, 11));
      token.set_expires_at(GetInt64Column(record_pointer, 12));
...
void DatabaseVgBackupRestore::RestoreVgs(
    ...
    RestoreVgsCallback callback) const {
  auto transaction = type::DBTransaction::New();
  AlterTables(tables, transaction.get());
  DropIndices(indices, transaction.get());
  CreateTables(tables, transaction.get());
  CreateIndices(indices, transaction.get());
  ...
  for (auto& vg_body : vg_bodies) {
    ...
    BindString(command.get(), 1, vg_body.trigger_id());
