...
namespace ledger::endpoint::connect::test {

class PostConnectMock final : public endpoint::PostConnect {
 public:
  explicit PostConnectMock(LedgerImpl* ledger) : PostConnect(ledger) {}
  ~PostConnectMock() override = default;

 private:
  const char* Path() const override { return "/v3/wallet/mock/%s/connect"; }
};
...
TEST_P(PostConnect, Paths) {
  ...
  Request request{std::make_unique<PostConnectMock>(&mock_ledger_impl_)};
  EXPECT_TRUE(request);
