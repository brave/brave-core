...
void EmptyBalance::OnAllContributions(
  ...
  BLOG(ledger, ledger::LogLevel::LOG_ERROR) <<
      "Contribution SUM: " << contribution_sum;
...
void EmptyBalance::OnCreds(
    ...
    bat_ledger::LedgerImpl* ledger) {
  if (list.empty()) {
    BLOG(ledger, ledger::LogLevel::LOG_INFO) << "Creds batch list is emtpy";
  ...
  for (auto& creds_batch : list) {
    ...
    if (!result) {
      BLOG(ledger, ledger::LogLevel::LOG_ERROR) << "UnBlindTokens: " << error;
      continue;
    }
...
void EmptyBalance::OnSaveUnblindedCreds(
    ...
    bat_ledger::LedgerImpl* ledger) {
  BLOG(ledger, ledger::LogLevel::LOG_INFO)
      << "Finished empty balance migration with result: " << result;
  ledger->SetBooleanState(ledger::kStateEmptyBalanceChecked, true);
}
...
void EmptyBalance::ReportResults(
  ...
  BLOG(ledger, ledger::LogLevel::LOG_INFO) << "Token SUM: " << tokens_sum;

  double total = promotion_sum - contribution_sum - tokens_sum;

  if (total <= 0) {
    BLOG(ledger, ledger::LogLevel::LOG_INFO) << "Unblinded token total is OK";
    ledger->SetBooleanState(ledger::kStateEmptyBalanceChecked, true);
    return;
  }

  BLOG(ledger, ledger::LogLevel::LOG_INFO)
      << "Unblinded token total is " << total;
  ...
  ledger::WalletInfoProperties wallet_info = ledger->GetWalletInfo();
...
void EmptyBalance::Sent(
    ...
    bat_ledger::LedgerImpl* ledger) {
  ledger->LogResponse(__func__, response_status_code, response, headers);
  ...
  BLOG(ledger, ledger::LogLevel::LOG_INFO)
      << "Finished empty balance migration!";
  ledger->SetBooleanState(ledger::kStateEmptyBalanceChecked, true);
}
