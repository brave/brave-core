  ...
  database_table.GetForSegments(
      segments, [=](const Result result, const SegmentList& segments,
        ...
        if (eligible_ads.empty()) {
          ...
          GetForParentSegments(segments, size, ad_events, browsing_history,
                               callback);
          return;
        }
  ...
  database_table.GetForSegments(
      parent_segments, [=](const Result result, const SegmentList& segments,
        ...
        if (eligible_ads.empty()) {
          ...
          GetForUntargeted(size, ad_events, browsing_history, callback);
          return;
        }
...
CreativeInlineContentAdList EligibleAds::FilterIneligibleAds(
  ...
  FilterSeenAdsAndRoundRobinIfNeeded(eligible_ads, AdType::kInlineContentAd);
