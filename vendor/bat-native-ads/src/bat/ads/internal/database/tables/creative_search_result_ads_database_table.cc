...
const char kTableName[] = "creative_search_result_ads";

const int kDefaultBatchSize = 50;

int BindParameters(mojom::DBCommand* command,
                   const CreativeSearchResultAdList& creative_ads) {
  DCHECK(command);
...
CreativeSearchResultAdInfo GetFromRecord(mojom::DBRecord* record) {
  DCHECK(record);

  CreativeSearchResultAdInfo creative_ad;
...
CreativeSearchResultAdMap GroupCreativeAdsFromResponse(
    mojom::DBCommandResponsePtr response) {
  DCHECK(response);

  CreativeSearchResultAdMap creative_ads;

  for (const auto& record : response->result->get_records()) {
    const CreativeSearchResultAdInfo& creative_ad = GetFromRecord(record.get());
...
CreativeSearchResultAdList GetCreativeAdsFromResponse(
    mojom::DBCommandResponsePtr response) {
  DCHECK(response);

  const CreativeSearchResultAdMap& grouped_creative_ads =
  ...
  for (const auto& grouped_creative_ad : grouped_creative_ads) {
    const CreativeSearchResultAdInfo& creative_ad = grouped_creative_ad.second;
...
CreativeSearchResultAds::CreativeSearchResultAds()
...
CreativeSearchResultAds::~CreativeSearchResultAds() = default;

void CreativeSearchResultAds::Save(
    const CreativeSearchResultAdList& creative_ads,
  ...
  const std::vector<CreativeSearchResultAdList>& batches =
...
void CreativeSearchResultAds::Delete(ResultCallback callback) {
  mojom::DBTransactionPtr transaction = mojom::DBTransaction::New();
...
void CreativeSearchResultAds::GetForCreativeInstanceId(
  ...
  AdsClientHelper::Get()->RunDBTransaction(
      std::move(transaction),
      std::bind(&CreativeSearchResultAds::OnGetForCreativeInstanceId, this,
                std::placeholders::_1, creative_instance_id, callback));
}

void CreativeSearchResultAds::GetForSegments(
  ...
  AdsClientHelper::Get()->RunDBTransaction(
      std::move(transaction),
      std::bind(&CreativeSearchResultAds::OnGetForSegments, this,
                std::placeholders::_1, segments, callback));
}

void CreativeSearchResultAds::GetAll(
    GetCreativeSearchResultAdsCallback callback) {
  const std::string& query = base::StringPrintf(
      "SELECT "
      "cpca.creative_instance_id, "
  ...
  AdsClientHelper::Get()->RunDBTransaction(
      std::move(transaction), std::bind(&CreativeSearchResultAds::OnGetAll,
                                        this, std::placeholders::_1, callback));
}

std::string CreativeSearchResultAds::GetTableName() const {
  return kTableName;
}

void CreativeSearchResultAds::Migrate(mojom::DBTransaction* transaction,
...
void CreativeSearchResultAds::InsertOrUpdate(
...
std::string CreativeSearchResultAds::BuildInsertOrUpdateQuery(
...
void CreativeSearchResultAds::OnGetForCreativeInstanceId(
    mojom::DBCommandResponsePtr response,
    const std::string& creative_instance_id,
    GetCreativeSearchResultAdCallback callback) {
  if (!response ||
      response->status != mojom::DBCommandResponse::Status::RESPONSE_OK) {
    BLOG(0, "Failed to get creative search result ad");
    callback(/* success */ false, creative_instance_id, {});
    return;
  }
  ...
  const CreativeSearchResultAdInfo& creative_ad = creative_ads.front();

  callback(/* success */ true, creative_instance_id, creative_ad);
}

void CreativeSearchResultAds::OnGetForSegments(
    mojom::DBCommandResponsePtr response,
    const SegmentList& segments,
    GetCreativeSearchResultAdsCallback callback) {
  if (!response ||
      response->status != mojom::DBCommandResponse::Status::RESPONSE_OK) {
    BLOG(0, "Failed to get creative search result ads");
...
void CreativeSearchResultAds::OnGetAll(
    mojom::DBCommandResponsePtr response,
    GetCreativeSearchResultAdsCallback callback) {
  if (!response ||
      response->status != mojom::DBCommandResponse::Status::RESPONSE_OK) {
...
void CreativeSearchResultAds::MigrateToV22(mojom::DBTransaction* transaction) {
  DCHECK(transaction);

  util::Drop(transaction, "creative_search_result_ads");

  const std::string& query =
      "CREATE TABLE creative_search_result_ads "
