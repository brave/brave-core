...
TEST_F(BatAdsTextEmbeddingHtmlEventsTest, LogEvent) {
    ...
    LogTextEmbeddingHtmlEvent(
      ...
      [=](const bool success) {
        EXPECT_TRUE(success)
            << "Failed to log text embedding html event";
        if (success) {
          ...
          GetTextEmbeddingEventsFromDatabase(
            [=](const bool success, const TextEmbeddingHtmlEventList&
              ...
              if (success) {
                ...
                // Assert
                ASSERT_EQ(text_embedding_info.text_hashed, text_embedding_html_events[0].hashed_key);
              }
            });
        }
      });
...
TEST_F(BatAdsTextEmbeddingHtmlEventsTest, PurgeEvents) {
  ...
  int n_events = targeting::features::GetTextEmbeddingsHistorySize() + n_events_surplus;
                ...
                GetTextEmbeddingEventsFromDatabase(
                  [=](const bool success, const TextEmbeddingHtmlEventList&
                    ...
                    if (success) {
                      ...
                      // Assert
                      const int n_events_default = targeting::features::GetTextEmbeddingsHistorySize();
                      const int n_logged_events = text_embedding_html_events.size();
