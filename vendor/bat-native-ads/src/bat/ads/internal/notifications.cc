...
Notifications::Notifications(AdsImpl* ads, AdsClient* ads_client) :
  ...
  BLOG(INFO) << "Initializing notifications";
}

Notifications::~Notifications() {
  BLOG(INFO) << "Deinitializing notifications";
}
...
bool Notifications::Get(const std::string& id, NotificationInfo* info) {
...
bool Notifications::Exists(const std::string& id) {
  DCHECK(is_initialized_);
...
void Notifications::LoadState() {
  BLOG(INFO) << "Loading notifications state";
