...
#include "bat/ads/internal/account/refill_unblinded_tokens/refill_unblinded_tokens_delegate_mock.h"
...
class BatAdsRefillUnblindedTokensTest : public UnitTestBase {
 protected:
  ...
  privacy::cbr::TokenList GetTokens() {
...
TEST_F(BatAdsRefillUnblindedTokensTest, RefillUnblindedTokens) {
  ...
  const privacy::cbr::TokenList& tokens = GetTokens();
...
TEST_F(BatAdsRefillUnblindedTokensTest, VerifyAndUnblindInvalidTokens) {
  ...
  privacy::cbr::TokenList tokens = GetTokens();
  tokens[5] = Token::decode_base64("invalid_token");
