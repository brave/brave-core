...
const SupportedCountryCodeMap& GetSupportedCountryCodes() {
  ...
  // |country_code_util.cc| must be updated to reflect newly supported regions

  //   Format: { schema_version : { country code... } }
  static const base::NoDestructor<SupportedCountryCodeMap>
      kSupportedCountryCodes({
          {1,
           {
          ...
          // IMPORTANT: When adding new schema versions
          // |newly_supported_locale_| must be updated in |BraveAdsBrowserTest|
          // to reflect a locale from the latest schema version
      });
