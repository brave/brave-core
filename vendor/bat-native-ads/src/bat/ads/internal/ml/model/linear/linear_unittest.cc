...
TEST_F(BatAdsLinearTest, BinaryClassifierPredictionTest) {
  // Arrange
  const size_t expected_prediction_size = 1;
  const std::map<std::string, VectorData> weights = {
  ...
  // Assert
  ASSERT_EQ(expected_prediction_size, predictions_0.size());
  ASSERT_EQ(expected_prediction_size, predictions_1.size());
...
TEST_F(BatAdsLinearTest, TopPredictionsTest) {
  // Arrange
  const size_t prediction_limits[2] = {2, 1};
  const std::map<std::string, VectorData> weights = {
  ...
  const PredictionMap predictions_2 =
      linear_biased.GetTopPredictions(point_2, prediction_limits[0]);
  const PredictionMap predictions_3 =
      linear_biased.GetTopPredictions(point_3, prediction_limits[1]);

  // Assert
  ASSERT_EQ(weights.size(), predictions_1.size());
  ASSERT_EQ(prediction_limits[0], predictions_2.size());
  EXPECT_EQ(prediction_limits[1], predictions_3.size());
}
