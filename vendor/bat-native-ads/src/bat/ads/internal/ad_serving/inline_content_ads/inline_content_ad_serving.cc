...
void AdServing::MaybeServeAd(const std::string& size,
  ...
  if (!features::inline_content_ads::IsEnabled()) {
    callback(/* success */ false, size, inline_content_ad);
    return;
  }
  ...
  eligible_ads_->GetForSegments(
      ...
      [=](const bool was_allowed, const CreativeInlineContentAdList& ads) {
        if (was_allowed) {
          p2a::RecordAdOpportunityForSegments(AdType::kInlineContentAd,
                                              segments);
        }
        ...
        BLOG(1, "Serving inline content ad:\n"
                    ...
                    << "\n"
                    << "  imageUUrl: " << inline_content_ad.image_url << "\n"
        ...
        callback(/* success */ true, size, inline_content_ad);
      });
}
