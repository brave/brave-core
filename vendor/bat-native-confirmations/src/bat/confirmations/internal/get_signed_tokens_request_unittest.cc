...
class BatConfirmationsGetSignedTokensRequestTest : public ::testing::Test {
 protected:
  std::unique_ptr<GetSignedTokensRequest> request_;

  BatConfirmationsGetSignedTokensRequestTest()
      : request_(std::make_unique<GetSignedTokensRequest>()) {
    // You can do set-up work for each test here
  }

  ~BatConfirmationsGetSignedTokensRequestTest() override {
...
TEST_F(BatConfirmationsGetSignedTokensRequestTest,
    BuildUrl) {
  // Arrange
  WalletInfo wallet;
  ...
  // Act
  const std::string url = request_->BuildUrl(wallet, nonce);
