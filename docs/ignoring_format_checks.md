...
| **Python**           | `//brave/.yapfignore` + nested `.ruff.toml` (see below) |
...
`git cl format` currently routes Python through `ruff_chromium`, which does not
honor `.yapfignore`. For trees listed there that must stay unformatted, also add
a nested `.ruff.toml` with `extend-exclude = ["**"]` so `ruff_chromium` claims
that subtree (ahead of `//brave/.style.yapf`) and then skips every file.
...
https://github.com/swiftlang/swift-format/blob/main/Documentation/IgnoringSource.md
