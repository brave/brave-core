...
- **Some fields are routinely big** (extracted page text, tool output, image
  ...
  and a deterministic truncation policy drops fields in priority order when an
  entry still doesn't fit. Truncated fields carry a sentinel so the receiver
  preserves any locally-present value instead of overwriting it with empty.
...
Chromium has precedent for _marking_ fields as truncated so receivers know not
to trust them as a unique identifier — see
`components/sync/protocol/deletion_origin.proto`:

```protobuf
// File name (may have been truncated to fit a length limit).
optional string file_name_possibly_truncated = 5;
optional string unique_source_tag_no_pii_possibly_truncated = 6;
```

The naming convention (`*_possibly_truncated`, `*_truncated_for_sync`) and the
semantics (sentinel telling the receiver "trust the value if present, but treat
its absence as 'I didn't have room', not 'it was empty'") match what we do for
`AIChatCompressibleString.was_truncated_for_sync` and
`AIChatUploadedFile.data_truncated_for_sync`.

`components/sync/protocol/bookmark_specifics.proto` also documents a
length-based truncation policy (favicon URL is dropped when it exceeds
`kMaxFaviconUrlSize`). The difference: bookmark truncation is _destructive_ for
the receiver (the field is just gone), whereas AI Chat's truncation is
_preserve-local_ on the receiver via the sentinel.
...
`WriteCompressibleString(value, out)` picks the encoding:
...
- `std::nullopt` if `was_truncated_for_sync` is set, _or_ if gzip decompression
  failed.
...
`AIChatUploadedFile.data` is plain `optional bytes` (no gzip — image/PDF bytes
are already compressed). The companion flag
`AIChatUploadedFile.data_truncated_for_sync` is the preserve-local sentinel for
binary attachments.

### Truncation policy

`TruncateEntryForSync(Entry*)` is called after building an Entry proto and
...
fields in priority order, dropping each until the entry fits:

1. `uploaded_files[].data` — raw file bytes. Dropped first because the receiver
...
If every truncatable field has been dropped and the entry is _still_ over budget
...
A field carrying `was_truncated_for_sync = true` means "I had this locally but
couldn't fit it on the wire — don't overwrite your copy with empty."

The bridge handles preserve-local in proto space before converting to mojom:
...
ApplyRemoteRecord(specifics):
   if entry kind:
     ...
     // For each AC with was_truncated_for_sync on last_contents,
     // substitute the local content text via
     // GetArchiveContentsForConversation(conversation_uuid).
     // For each uploaded file with data_truncated_for_sync (matched by
     // file_order ordinal), substitute the local bytes via
     // GetConversationData(conversation_uuid).
     // Same for extracted_text on uploaded files.
...
and centralizes the preserve-local logic in one place.

A field that is _absent_ on the wire **and** has no sentinel is also treated as
...
| Subsequent entry                               | One Entry `Put` + one Conversation `Put` (metadata may have moved: model, last-modified)                                                |
| Streaming tool-use update on an existing entry | One Entry `Put` (full re-serialize of that entry; gzip + truncation applied if needed)                                                  |
...
- `ApplyIncrementalSyncChanges`: DELETE dispatches on prefix
  ...
  ADD/UPDATE dispatches via `ApplyRemoteRecord` (which applies the
  preserve-local merge first).

### Conflict resolution

**Last-writer-wins per record**, with one refinement: fields the sender marked
as truncated do not participate in the last-writer-wins comparison — local
always wins for those.
...
### `SyncBackend` and async bridge construction
...
To make registration succeed before the bridge is ready, the service holds a
`SyncBackend`:

```cpp
// On AIChatService, owned UI-side, bridge accessed only on db_task_runner_.
class SyncBackend : public base::RefCountedDeleteOnSequence<SyncBackend> {
  explicit SyncBackend(scoped_refptr<base::SequencedTaskRunner> task_runner);
...
ADD/UPDATE. For Entry records it first runs the preserve-local merge in proto
...
3. `AddOrUpdateAssociatedContent(conv_uuid, ac_list, contents)` — re-adds any
   ...
   has already substituted local values for any AC the sender marked as
   truncated.
...
budget once truncation has run.
...
- **`AIChatSyncConversionsTest`** (`ai_chat_sync_conversions_unittest.cc`)
  ...
  - Round-trip uploaded files (with both raw bytes and gzipped extracted text),
    permission challenge, associated content text.
  - `AIChatCompressibleString` small-value bypass, large-value gzip, truncation
    sentinel.
  - `TruncateEntryForSync` no-op below threshold, drops file bytes first, falls
    through multiple categories, refuses pathological entries.
...
- **`AIChatSyncBridgeTest`** (`ai_chat_sync_bridge_unittest.cc`)
  ...
  - Preserve-local on AC `last_contents` when the remote sender marks it
    truncated.
...
4. **Async-init race is covered by inspection, not by test.** The `SyncBackend`
   design specifically targets the case where the sync engine asks for a
   controller delegate before the bridge exists. No test simulates that
   ordering; the browser test incidentally exercises it but doesn't pin the
   ordering invariant.
...
4. **`ConversationHandler::OnRemoteSyncDataApplied` unit test** with a mock
   ...
   the UI is notified.
5. **`SyncBackend` ordering test.** Create the backend without a bridge, call
   `GetControllerDelegate()` on its sequence (expect null), then
...
3. **Chunked records for oversized fields.** When even a single field exceeds
   400 KB (e.g. a 5 MB tool fetch output, a 2 MB image), truncation drops the
   field. A future iteration could spill the offending field into ordered
   side-records keyed `k:<entry-uuid>:<chunk-index>` and reassemble on the
   receiver. Adds a third record kind to the `oneof` and a fourth stub level
   (chunk → entry → conversation), worth the complexity once telemetry indicates
   truncation fires often.
...
5. **Per-file records for multi-file uploads.** Most turns have at most one
   uploaded file. When multiple are attached, truncation drops bytes from files
   in array order. A future iteration could give each file its own 400 KB
   budget.
6. **Event-field preserve-local.** Current preserve-local rule covers AC
   `last_contents`, uploaded file `data`, and uploaded file `extracted_text`.
   Event-field truncation (e.g. a re-synced tool output) is _not_
   preserve-local: the receiver's local copy is replaced with the truncated
   value. Limited because reading the local event payload requires deserializing
   the per-event BLOB.
7. **Telemetry.** Histograms on truncation rates per field would tell us whether
