...
### Why These Files Are Generated in `brave_custom.py`

The `brave_custom.py` script is executed during `gclient runhooks` (specifically
via `npm run sync` in our workflow) as part of reclient configurator callbacks
and has access to all environment variables set in `brave/.env`.

### `.sisoenv` - Remote Execution Service Configuration

```bash
SISO_REAPI_INSTANCE=default
SISO_REAPI_ADDRESS=<RBE_service>
```

This file tells Siso:

- **Where to connect** for remote execution (the cluster address from
  `RBE_service` env var)
...
│                ├─> write .sisoenv (RBE service config)          │
│                └─> write .sisorc (ninja flags)                  │
└──────────────────────┬──────────────────────────────────────────┘
                       │
                       ▼
┌─────────────────────────────────────────────────────────────────┐
