...
TEST_F(BraveStatsUpdaterTest, SavePrefsSetsLastDailyReportedAtToNow) {
  base::Time last_daily_reported_at;
  ASSERT_TRUE(base::Time::FromUTCString("2026-04-22T15:30:00",
                                        &last_daily_reported_at));
  SetCurrentTimeForTest(last_daily_reported_at);

  brave_stats::BraveStatsUpdaterParams brave_stats_updater_params(
      GetLocalState(), kToday, kThisWeek, kThisMonth);
  brave_stats_updater_params.SavePrefs();

  EXPECT_EQ(GetLocalState()->GetTime(serp_metrics::prefs::kLastDailyReportedAt),
            last_daily_reported_at);
}

TEST_F(BraveStatsUpdaterTest, SavePrefsSetsLastWeeklyReportedAtWhenNewWeek) {
  base::Time last_weekly_reported_at;
  ASSERT_TRUE(base::Time::FromUTCString("2026-04-22T15:30:00",
                                        &last_weekly_reported_at));
  SetCurrentTimeForTest(last_weekly_reported_at);
  ...
  EXPECT_EQ(
      GetLocalState()->GetTime(serp_metrics::prefs::kLastWeeklyReportedAt),
      last_weekly_reported_at);
}

TEST_F(BraveStatsUpdaterTest, SavePrefsDoesNotSetLastWeeklyReportedAtSameWeek) {
  GetLocalState()->SetInteger(kLastCheckWOY, kThisWeek);
  brave_stats::BraveStatsUpdaterParams brave_stats_updater_params(
      GetLocalState(), kToday, kThisWeek, kThisMonth);
  brave_stats_updater_params.SavePrefs();
  ...
  EXPECT_TRUE(GetLocalState()
                  ->GetTime(serp_metrics::prefs::kLastWeeklyReportedAt)
                  .is_null());
}

TEST_F(BraveStatsUpdaterTest, SavePrefsSetsLastMonthlyReportedAtWhenNewMonth) {
  base::Time last_monthly_reported_at;
  ASSERT_TRUE(base::Time::FromUTCString("2026-04-22T15:30:00",
                                        &last_monthly_reported_at));
  SetCurrentTimeForTest(last_monthly_reported_at);
  ...
  EXPECT_EQ(
      GetLocalState()->GetTime(serp_metrics::prefs::kLastMonthlyReportedAt),
      last_monthly_reported_at);
}

TEST_F(BraveStatsUpdaterTest,
       SavePrefsDoesNotSetLastMonthlyReportedAtSameMonth) {
  GetLocalState()->SetInteger(kLastCheckMonth, kThisMonth);
  brave_stats::BraveStatsUpdaterParams brave_stats_updater_params(
      GetLocalState(), kToday, kThisWeek, kThisMonth);
  brave_stats_updater_params.SavePrefs();
  ...
  EXPECT_TRUE(GetLocalState()
                  ->GetTime(serp_metrics::prefs::kLastMonthlyReportedAt)
                  .is_null());
}
