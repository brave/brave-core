...
void BraveEmbedder::ProcessAllPassagesInParallel(TaskId task_id) {
  ...
  auto barrier_callback =
      base::BarrierCallback<std::pair<size_t, std::vector<float>>>(
          num_passages,
...
void BraveEmbedder::OnAllEmbeddingsComplete(
  ...
  DVLOG(3) << "All embeddings successful";

  // Store all embeddings
  for (const auto& result : results) {
    size_t passage_index = result.first;
    std::vector<float> embedding = result.second;
    ...
    task.embeddings[passage_index] =
        passage_embeddings::Embedding(std::move(embedding), word_count);
  }
