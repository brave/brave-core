...
void BravePassageEmbeddingsService::MaybeStartLoad() {
  ...
  // BatchEmbedder takes the remote over; ResetEmbedderState's
  // load_weak_factory_.InvalidateWeakPtrs() cancels the in-flight
  // ThreadPool reply if it races in between.
  LoadLocalModelFiles();
}
