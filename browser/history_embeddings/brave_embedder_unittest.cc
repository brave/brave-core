...
// Fake LocalAIService that returns a FakePassageEmbedder.
class FakeLocalAIService : public local_ai::mojom::LocalAIService {
 public:
  explicit FakeLocalAIService(FakePassageEmbedder* embedder)
      : embedder_(embedder) {}

  void RegisterPassageEmbedderFactory(
      mojo::PendingRemote<local_ai::mojom::PassageEmbedderFactory>) override {}
  ...
  void DisconnectAllEmbedders() { embedder_receivers_.Clear(); }

 private:
  raw_ptr<FakePassageEmbedder> embedder_;
  mojo::ReceiverSet<local_ai::mojom::PassageEmbedder> embedder_receivers_;
};
...
class BraveEmbedderTest : public testing::Test {
 protected:
  ...
  FakeLocalAIService fake_service_{&fake_worker_};
  mojo::Receiver<local_ai::mojom::LocalAIService> service_receiver_{
      &fake_service_};
...
TEST_F(BraveEmbedderTest, HighPriorityPreemptsLowPriority) {
  ...
  // Submit a passive job with 3 passages.
  base::test::TestFuture<std::vector<std::string>, std::vector<Embedding>,
                         Embedder::TaskId, ComputeEmbeddingsStatus>
      passive_future;
  embedder_->ComputePassagesEmbeddings(PassagePriority::kPassive,
  ...
  // Now submit a high-priority search query.
  base::test::TestFuture<std::vector<std::string>, std::vector<Embedding>,
                         Embedder::TaskId, ComputeEmbeddingsStatus>
      search_future;
...
TEST_F(BraveEmbedderTest, TryCancelPendingJob) {
  ...
  // task1 is in-flight, can also be cancelled.
  EXPECT_TRUE(embedder_->TryCancel(task1));
  ASSERT_TRUE(future1.IsReady());
  EXPECT_EQ(std::get<3>(future1.Take()), ComputeEmbeddingsStatus::kCanceled);
}

TEST_F(BraveEmbedderTest, CancelCurrentJobProcessesNextJob) {
  fake_worker_.set_hold_responses(true);
  ...
  base::test::TestFuture<std::vector<std::string>, std::vector<Embedding>,
                         Embedder::TaskId, ComputeEmbeddingsStatus>
      future2;
  embedder_->ComputePassagesEmbeddings(PassagePriority::kPassive, {"b"},
                                       future2.GetCallback());
  ...
  // Cancel in-flight task1 — task2 should start processing.
  EXPECT_TRUE(embedder_->TryCancel(task1));
  ASSERT_TRUE(future1.IsReady());
  EXPECT_EQ(std::get<3>(future1.Take()), ComputeEmbeddingsStatus::kCanceled);

  // task2 should now be in-flight.
  ASSERT_TRUE(base::test::RunUntil(
      [&] { return fake_worker_.passages_received().size() >= 2u; }));
  EXPECT_EQ(fake_worker_.passages_received()[1], "b");

  // Complete task2.
  fake_worker_.ReleaseNextResponse();
  ASSERT_TRUE(base::test::RunUntil([&] { return future2.IsReady(); }));
  EXPECT_EQ(std::get<3>(future2.Take()), ComputeEmbeddingsStatus::kSuccess);
}

TEST_F(BraveEmbedderTest, DisconnectFailsAllPendingJobs) {
  fake_worker_.set_hold_responses(true);
  ...
  base::test::TestFuture<std::vector<std::string>, std::vector<Embedding>,
                         Embedder::TaskId, ComputeEmbeddingsStatus>
      future1;
  embedder_->ComputePassagesEmbeddings(PassagePriority::kPassive, {"a"},
                                       future1.GetCallback());
  ...
  // Simulate PassageEmbedder disconnect.
  fake_service_.DisconnectAllEmbedders();

  // Both jobs should fail.
  ASSERT_TRUE(base::test::RunUntil([&] { return future1.IsReady(); }));
  EXPECT_EQ(std::get<3>(future1.Take()),
            ComputeEmbeddingsStatus::kExecutionFailure);
  ASSERT_TRUE(base::test::RunUntil([&] { return future2.IsReady(); }));
  EXPECT_EQ(std::get<3>(future2.Take()),
            ComputeEmbeddingsStatus::kExecutionFailure);
}
...
TEST_F(BraveEmbedderTest, EmptyPassagesReturnsImmediately) {
  ...
  EXPECT_EQ(std::get<3>(future.Take()), ComputeEmbeddingsStatus::kSuccess);
}
