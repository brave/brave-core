...
ExtensionFunction::ResponseAction BraveRewardsTipTwitterUserFunction::Run() {
  ...
  Profile* profile = Profile::FromBrowserContext(browser_context());
  if (profile->IsOffTheRecord()) {
    return RespondNow(Error("Cannot tip Twitter user in a private context"));
  }
  ...
  if (rewards_service) {
    ...
    rewards_service->SaveTwitterPublisherInfo(
        args,
        base::Bind(
            &BraveRewardsTipTwitterUserFunction::OnTwitterPublisherInfoSaved,
...
BraveRewardsIncludeInAutoContributionFunction::Run() {
  ...
  if (rewards_service) {
    rewards_service->SetContributionAutoInclude(params->publisher_key,
                                                params->exclude);
  }
  return RespondNow(NoArguments());
}
...
ExtensionFunction::ResponseAction BraveRewardsGetGrantCaptchaFunction::Run() {
  ...
  RewardsService* rewards_service =
      RewardsServiceFactory::GetForProfile(profile);
  if (rewards_service) {
    rewards_service->GetGrantCaptcha(params->promotion_id, params->type);
  }
  return RespondNow(NoArguments());
}
...
BraveRewardsGetPendingContributionsTotalFunction::Run() {
  ...
  if (!rewards_service) {
    return RespondNow(OneArgument(std::make_unique<base::Value>(0.0)));
  }
...
ExtensionFunction::ResponseAction BraveRewardsSaveSettingFunction::Run() {
  ...
  if (rewards_service) {
    if (params->key == "enabledMain") {
      rewards_service->SetRewardsMainEnabled(std::stoi(params->value.c_str()));
    }
  }
...
ExtensionFunction::ResponseAction BraveRewardsRefreshPublisherFunction::Run() {
  ...
  if (!rewards_service) {
    return RespondNow(
        TwoArguments(std::make_unique<base::Value>(false),
...
BraveRewardsGetInlineTipSettingFunction::Run() {
  ...
  rewards_service->GetInlineTipSetting(
      params->key,
      base::BindOnce(
          &BraveRewardsGetInlineTipSettingFunction::OnInlineTipSetting, this));
  return RespondLater();
}
