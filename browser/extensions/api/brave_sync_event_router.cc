...
void BraveSyncEventRouter::GotInitData(
  ...
  std::unique_ptr<base::ListValue> args(
      extensions::api::brave_sync::OnGotInitData::Create(
          arg_seed, arg_device_id, config, sync_words)
          .release());
...
void BraveSyncEventRouter::FetchSyncRecords(
  ...
  std::unique_ptr<base::ListValue> args(
      extensions::api::brave_sync::OnFetchSyncRecords::Create(
          category_names, startAt.ToJsTime(), static_cast<double>(max_records))
          .release());
  std::unique_ptr<Event> event(
      new Event(extensions::events::FOR_TEST,
...
void BraveSyncEventRouter::FetchSyncDevices() {
  std::unique_ptr<base::ListValue> args(
      extensions::api::brave_sync::OnFetchSyncDevices::Create().release());
...
void BraveSyncEventRouter::SendSyncRecords(
  ...
  std::unique_ptr<base::ListValue> args(
      extensions::api::brave_sync::OnSendSyncRecords::Create(category_name,
                                                             records)
          .release());
  std::unique_ptr<Event> event(
      new Event(extensions::events::FOR_TEST,
...
void BraveSyncEventRouter::NeedSyncWords(const std::string& seed) {
  std::unique_ptr<base::ListValue> args(
      extensions::api::brave_sync::OnNeedSyncWords::Create(seed).release());
  std::unique_ptr<Event> event(
      new Event(extensions::events::FOR_TEST,
...
void BraveSyncEventRouter::LoadClient() {
  std::unique_ptr<base::ListValue> args(
      extensions::api::brave_sync::OnLoadClient::Create().release());
