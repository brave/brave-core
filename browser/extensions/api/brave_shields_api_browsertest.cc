...
IN_PROC_BROWSER_TEST_F(BraveShieldsAPIBrowserTest,
  ...
  EXPECT_EQ(value->FindKey(content_settings_api_constants::kContentSettingKey)
                ->GetString(),
            std::string("allow"));
  ...
  RunFunctionAndReturnSingleResult(
      set_function.get(), kJavascriptSetParams, browser());
...
IN_PROC_BROWSER_TEST_F(BraveShieldsAPIBrowserTest,
  ...
  content_settings()->GetWebsiteSetting(kBraveURL,
                                        kBraveURL,
                                        CONTENT_SETTINGS_TYPE_JAVASCRIPT,
                                        std::string(),
                                        &info);
  ...
  EXPECT_EQ(info.source, content_settings::SettingSource::SETTING_SOURCE_USER);
}
...
IN_PROC_BROWSER_TEST_F(BraveShieldsAPIBrowserTest,
  ...
  HostContentSettingsMapFactory::GetForProfile(browser()->profile())
      ->SetContentSettingDefaultScope(kBraveURL,
                                      kBraveURL,
