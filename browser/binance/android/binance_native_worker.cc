...
BinanceNativeWorker::BinanceNativeWorker(JNIEnv* env,
  ...
  binance_service_ = binance::BinanceServiceFactory::GetForProfile(
...
void BinanceNativeWorker::Destroy(JNIEnv* env,
...
void BinanceNativeWorker::GetAccessToken(JNIEnv* env,
    const base::android::JavaParamRef<jobject>& jcaller) {
  if (binance_service_) {
    binance_service_->GetAccessToken(
        base::Bind(&BinanceNativeWorker::OnGetAccessToken,
...
void BinanceNativeWorker::OnGetAccessToken(bool success) {
  ...
  Java_BinanceNativeWorker_OnGetAccessToken(env,
      weak_java_binance_native_worker_.get(env), success);
}
...
void BinanceNativeWorker::OnGetAccountBalances(
  ...
  Java_BinanceNativeWorker_OnGetAccountBalances(env,
      weak_java_binance_native_worker_.get(env),
      base::android::ConvertUTF8ToJavaString(env, json_balances),
      success);
}

void BinanceNativeWorker::GetConvertQuote(JNIEnv* env,
    const base::android::JavaParamRef<jobject>& jcaller,
...
void BinanceNativeWorker::GetCoinNetworks(JNIEnv* env,
    const base::android::JavaParamRef<jobject>& jcaller) {
  if (binance_service_) {
    binance_service_->GetCoinNetworks(
        base::Bind(&BinanceNativeWorker::OnGetCoinNetworks,
            weak_factory_.GetWeakPtr()));
  }
}
...
void BinanceNativeWorker::OnGetCoinNetworks(
  ...
  Java_BinanceNativeWorker_OnGetCoinNetworks(env,
      weak_java_binance_native_worker_.get(env),
      base::android::ConvertUTF8ToJavaString(env, json_networks));
}

void BinanceNativeWorker::GetDepositInfo(JNIEnv* env,
    const base::android::JavaParamRef<jobject>& jcaller,
...
void BinanceNativeWorker::OnGetDepositInfo(
    const std::string& deposit_address,
    const std::string& deposit_tag,
    bool success) {
  JNIEnv* env = base::android::AttachCurrentThread();
  Java_BinanceNativeWorker_OnGetDepositInfo(env,
      weak_java_binance_native_worker_.get(env),
      base::android::ConvertUTF8ToJavaString(env, deposit_address),
      base::android::ConvertUTF8ToJavaString(env, deposit_tag),
      success);
}

void BinanceNativeWorker::ConfirmConvert(JNIEnv* env,
    const base::android::JavaParamRef<jobject>& jcaller,
...
void BinanceNativeWorker::OnConfirmConvert(
    bool success, const std::string& message) {
  JNIEnv* env = base::android::AttachCurrentThread();
  Java_BinanceNativeWorker_OnConfirmConvert(env,
      weak_java_binance_native_worker_.get(env),
      success,
...
void BinanceNativeWorker::GetConvertAssets(JNIEnv* env,
    const base::android::JavaParamRef<jobject>& jcaller) {
  if (binance_service_) {
    binance_service_->GetConvertAssets(
        base::Bind(&BinanceNativeWorker::OnGetConvertAssets,
            weak_factory_.GetWeakPtr()));
  }
}
...
void BinanceNativeWorker::OnGetConvertAssets(
  ...
  Java_BinanceNativeWorker_OnGetConvertAssets(env,
      weak_java_binance_native_worker_.get(env),
...
void BinanceNativeWorker::RevokeToken(JNIEnv* env,
    const base::android::JavaParamRef<jobject>& jcaller) {
  if (binance_service_) {
    binance_service_->RevokeToken(
        base::Bind(&BinanceNativeWorker::OnRevokeToken,
            weak_factory_.GetWeakPtr()));
  }
}
...
void BinanceNativeWorker::OnRevokeToken(bool success) {
  JNIEnv* env = base::android::AttachCurrentThread();
  Java_BinanceNativeWorker_OnRevokeToken(env,
      weak_java_binance_native_worker_.get(env), success);
}
