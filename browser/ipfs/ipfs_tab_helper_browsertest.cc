...
IN_PROC_BROWSER_TEST_F(IpfsTabHelperBrowserTest, IPFSFallbackInfobar) {
  ...
  GURL::Replacements replace_drweb_url;
  replace_drweb_url.SetHostStr(test_non_ipfs_url.host_piece());
  const GURL expected_test_url = test_url.ReplaceComponents(replace_drweb_url);
...

  {
    ui_test_utils::UrlLoadObserver url_observer(
        expected_test_url, content::NotificationService::AllSources());
    ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), test_url));
    url_observer.Wait();
    // Get last shown infobar
    auto* infobar = find_infobar(
  ...
  {
    ui_test_utils::UrlLoadObserver url_observer(
        test_non_ipfs_url, content::NotificationService::AllSources());
