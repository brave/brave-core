...
IN_PROC_BROWSER_TEST_F(AIChatConversationTaskBrowserTest,
  ...
  // Give some time for any potential (erroneous) async calls to happen.
  // The mock expectation would catch any violations.
  ...
  // Note: We use a mock tool in this test to control tool execution timing
  // and avoid race conditions. Since the mock tool doesn't create an actual
  // tab, we cannot verify tab release behavior here. Tab release verification
  // is covered in tests that use real navigation tools (e.g., TaskStopAction).
}
