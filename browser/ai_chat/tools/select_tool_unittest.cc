...
class SelectToolTest : public testing::Test {
 public:
  void SetUp() override {
    ...
    select_tool_ =
        std::make_unique<SelectTool>(mock_task_provider_.get(),
 ...
 protected:
  ...
  void RunWithExpectedError(const std::string& input_json,
    ...
    select_tool_->UseTool(input_json, future.GetCallback(), std::nullopt);
...
TEST_F(SelectToolTest, ValidInputWithContentNode) {
  ...
  select_tool_->UseTool(input_json, base::DoNothing(), std::nullopt);
  run_loop.Run();
...
TEST_F(SelectToolTest, ValidInputComplexValue) {
  ...
  std::string input_json = CreateValidSelectJson(target_dict, "complex-option-value-123");
  base::RunLoop run_loop;
...
TEST_F(SelectToolTest, MissingValue) {
  ...
  RunWithExpectedError(input_json, "Missing required parameter 'value' - the value attribute of the option to select.");
}
...
TEST_F(SelectToolTest, ToolMetadata) {
  ...
  EXPECT_EQ(required->size(), 1u);  // value
}
