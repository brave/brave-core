...
class MoveMouseToolTest : public testing::Test {
 ...
 protected:
  ...
  void RunWithExpectedError(const std::string& input_json,
    ...
    move_mouse_tool_->UseTool(input_json, future.GetCallback(), std::nullopt);
  ...
  void VerifyMoveMouseAction(const optimization_guide::proto::Actions& actions) {
    // Verify proto action
...
TEST_F(MoveMouseToolTest, ValidInputWithContentNode) {
  ...
  move_mouse_tool_->UseTool(input_json, base::DoNothing(), std::nullopt);
  run_loop.Run();
...
TEST_F(MoveMouseToolTest, ToolMetadata) {
  ...
  // MoveMouseTool returns nullopt for required properties since it has conditional requirements
  EXPECT_FALSE(required.has_value());
}
