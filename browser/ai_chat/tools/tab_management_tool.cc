...
class TabInsertWaiter {
 public:
  using DoneCallback = base::OnceClosure;

  static void Run(std::vector<tabs::TabHandle> tabs,
                  base::TimeDelta timeout,
                  DoneCallback done) {
    auto* self = new TabInsertWaiter(std::move(tabs), timeout, std::move(done));
    self->Arm();
  }

  ~TabInsertWaiter() = default;

 private:
  TabInsertWaiter(std::vector<tabs::TabHandle> tabs,
                  base::TimeDelta timeout,
                  DoneCallback done)
      : remaining_(tabs.size()),
        timeout_(timeout),
        done_(std::move(done)),
        tabs_(std::move(tabs)),
        weak_factory_(this) {}
  ...
  void Arm() {
    if (remaining_ == 0) {
      // Avoid reentrancy if caller does extra work in `done_`.
      base::SequencedTaskRunner::GetCurrentDefault()->PostTask(
          FROM_HERE, base::BindOnce(&TabInsertWaiter::FinishSuccess,
                                    weak_factory_.GetWeakPtr()));
      return;
    }

    subscriptions_.reserve(tabs_.size());
    for (tabs::TabHandle tab : tabs_) {
      if (auto* tab_interface = tab.Get()) {
        subscriptions_.push_back(
            tab_interface->RegisterDidInsert(base::BindRepeating(
                &TabInsertWaiter::OnOneArrived, weak_factory_.GetWeakPtr())));
      }
    }

    timer_.Start(FROM_HERE, timeout_,
                 base::BindOnce(&TabInsertWaiter::FinishTimeout,
                                weak_factory_.GetWeakPtr()));
  }

  void OnOneArrived(tabs::TabInterface* tab) {
    if (finished_) {
      return;
    }
    if (remaining_ == 0) {
  ...
  void FinishSuccess() { Finish(); }
  void FinishTimeout() { Finish(); }
  ...
  void Finish() {
    ...
    // Stop further activity and detach callbacks.
    timer_.Stop();
    subscriptions_.clear();

    // Send on the next task
    base::SequencedTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&TabInsertWaiter::SendAndDelete,
                                  weak_factory_.GetWeakPtr()));
  }
  ...
  const std::vector<tabs::TabHandle> tabs_;

  base::WeakPtrFactory<TabInsertWaiter> weak_factory_;
};

// Waiter that completes only when none of the given tab handles are valid.
// Tab closing is asynchronous since there may be a JS handler to offer the
// user the choice to close the tab.
class TabsRemovedWaiter : public content::WebContentsObserver {
 public:
  static void Wait(std::vector<tabs::TabHandle> handles,
                   base::OnceClosure on_done,
                   base::TimeDelta fallback_timeout = base::Seconds(15)) {
    auto* waiter = new TabsRemovedWaiter(std::move(handles), std::move(on_done),
                                         fallback_timeout);
    waiter->Start();
  }

  ~TabsRemovedWaiter() override = default;

 private:
  class TabObserver : public content::WebContentsObserver {
   public:
    TabObserver(TabsRemovedWaiter* waiter, content::WebContents* web_contents)
   ...
   private:
    TabsRemovedWaiter* waiter_;
  };

  TabsRemovedWaiter(std::vector<tabs::TabHandle> handles,
                    base::OnceClosure on_done,
                    base::TimeDelta timeout)
  ...
  void Start() {
    ...
    base::SequencedTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&TabsRemovedWaiter::CheckAndMaybeFinish,
                                  weak_factory_.GetWeakPtr()));
    // Fallback timer
    timer_.Start(FROM_HERE, timeout_,
                 base::BindOnce(&TabsRemovedWaiter::ForceFinish,
                                weak_factory_.GetWeakPtr()));
  }
  ...
  base::WeakPtrFactory<TabsRemovedWaiter> weak_factory_{this};
};
  ...
  for (Browser* browser : *BrowserList::GetInstance()) {
    ...
    for (int i = 0; i < tab_strip->count(); ++i) {
      ...
      if (auto* tab_interface =
        ...
        tab_info.Set("tab_id", tab_interface->GetHandle().raw_value());
      } else {
        // Fallback - this shouldn't happen for normal tabs
        tab_info.Set("tab_id", -1);
      }
...
void TabManagementTool::HandleCreateGroup(UseToolCallback callback,
  ...
  // It's possible that the function will return before the tabs have changed
  // their window. This can be observed in flakey browser tests and will result
  // in real world bugs where the returned window and tabs list does not reflect
  // the pending changes. Therefore, we will attempt to wait for all moves to
  // have their window change completed.
  TabInsertWaiter::Run(
      tabs_moved, base::Seconds(4),
      base::BindOnce(&TabManagementTool::PostTaskSendResultWithTabList,
                     weak_ptr_factory_.GetWeakPtr(), std::move(callback),
                     std::move(result), std::nullopt));
