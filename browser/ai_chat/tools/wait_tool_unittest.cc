...
class WaitToolTest : public testing::Test {
 public:
  void SetUp() override {
    ...
    wait_tool_ =
        std::make_unique<WaitTool>(mock_task_provider_.get(),
 ...
 protected:
  ...
  void RunWithExpectedError(const std::string& input_json,
    ...
    wait_tool_->UseTool(input_json, future.GetCallback(), std::nullopt);
  ...
  void VerifyWaitAction(const optimization_guide::proto::Actions& actions,
    ...
    // Note: WaitToolRequest doesn't store wait_time_ms as a member, it's converted
    // to a TimeDelta in the constructor, so we can't easily verify the exact value
...
TEST_F(WaitToolTest, ValidInputShortWait) {
  ...
  wait_tool_->UseTool(input_json, base::DoNothing(), std::nullopt);
  run_loop.Run();
...
TEST_F(WaitToolTest, MissingWaitTime) {
  ...
  RunWithExpectedError(input_json, "Invalid or missing wait_time_ms. Must be a non-negative integer.");
}
