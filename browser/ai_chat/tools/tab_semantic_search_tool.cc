...
// Per-call state passed first through the BarrierClosure that gathers per-tab
// URLID resolutions, then into HistoryEmbeddingsService::Search's
...
struct PendingCall {
  ...
  std::vector<OpenTab> tabs;
  std::string query;
  raw_ptr<history_embeddings::HistoryEmbeddingsService> embeddings_service;
...
void OnAllUrlIdsResolved(std::unique_ptr<PendingCall> pending);
...
void OnAllUrlIdsResolved(std::unique_ptr<PendingCall> pending) {
  ...
  url_id_filter.reserve(pending->tabs.size());
  for (const auto& tab : pending->tabs) {
    if (tab.url_id != 0) {
      url_id_filter.push_back(tab.url_id);
    }
  }
  if (url_id_filter.empty()) {
    std::move(pending->callback)
        .Run(CreateContentBlocksForText(EmptyResultsJson()), {});
    return;
  }
  auto* svc = pending->embeddings_service.get();
  std::string query = pending->query;
  svc->Search(
...
void TabSemanticSearchTool::UseTool(const std::string& input_json,
  ...
  pending->query = *query;
  pending->embeddings_service = embeddings_service;

  const size_t n = pending->tabs.size();
  PendingCall* pending_raw = pending.get();
  auto on_all_resolved = base::BarrierClosure(
      n, base::BindOnce(&OnAllUrlIdsResolved, std::move(pending)));

  for (size_t i = 0; i < n; ++i) {
    history_service->QueryURL(
        pending_raw->tabs[i].url,
        base::BindOnce(
            [](PendingCall* p, size_t idx, base::RepeatingClosure barrier,
               history::QueryURLResult result) {
              if (result.success) {
                p->tabs[idx].url_id = result.row.id();
              }
              barrier.Run();
            },
            pending_raw, i, on_all_resolved),
