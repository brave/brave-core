...
ClickTool::ClickTool(BrowserToolTaskProvider* task_provider,
...
void ClickTool::UseTool(const std::string& input_json,
  ...
  data_decoder::DataDecoder::ParseJsonIsolated(
      input_json,
      base::BindOnce(&ClickTool::OnParseJson, weak_ptr_factory_.GetWeakPtr(),
                     std::move(callback)));
}

void ClickTool::OnParseJson(UseToolCallback callback,
  ...
  const auto* click_type = dict.FindString("click_type");
  const auto* click_count = dict.FindString("click_count");
