...
DragAndReleaseTool::DragAndReleaseTool(BrowserToolTaskProvider* task_provider,
...
void DragAndReleaseTool::UseTool(const std::string& input_json,
  ...
  data_decoder::DataDecoder::ParseJsonIsolated(
      input_json,
      base::BindOnce(&DragAndReleaseTool::OnParseJson,
                     weak_ptr_factory_.GetWeakPtr(), std::move(callback)));
}

void DragAndReleaseTool::OnParseJson(
    UseToolCallback callback,
  ...
  const base::Value::Dict* from_dict = dict.FindDict("from");
  if (!from_dict) {
  ...
  const base::Value::Dict* to_dict = dict.FindDict("to");
  if (!to_dict) {
    std::move(callback).Run(
