...
class ScrollToolTest : public testing::Test {
 ...
 protected:
  ...
  std::string CreateValidScrollJson(const base::Value::Dict& target_dict,
    ...
    dict.Set("document_identifier", "doc123");  // Required for scroll
  ...
  void RunWithExpectedError(const std::string& input_json,
    ...
    scroll_tool_->UseTool(input_json, future.GetCallback(), std::nullopt);
  ...
  void VerifyScrollAction(
      ...
      optimization_guide::proto::ScrollAction::ScrollDirection expected_direction,
...
TEST_F(ScrollToolTest, ValidInputWithContentNodeDown) {
  ...
  scroll_tool_->UseTool(input_json, base::DoNothing(), std::nullopt);
  run_loop.Run();
...
TEST_F(ScrollToolTest, MissingDirection) {
  ...
  dict.Set("document_identifier", "doc123");
  // Note: No direction intentionally

  std::string input_json;
  base::JSONWriter::Write(dict, &input_json);

  RunWithExpectedError(input_json, "Invalid or missing direction. Must be 'left', 'right', 'up', or 'down'.");
}
...
TEST_F(ScrollToolTest, MissingDistance) {
  ...
  RunWithExpectedError(input_json, "Invalid or missing distance. Must be a positive number.");
}
...
TEST_F(ScrollToolTest, MissingDocumentIdentifier) {
  ...
  dict.Set("direction", "down");
  dict.Set("distance", 100.0);
  // Note: No document_identifier intentionally

  std::string input_json;
  base::JSONWriter::Write(dict, &input_json);

  RunWithExpectedError(input_json, "document_identifier is required");
}
...
TEST_F(ScrollToolTest, ToolMetadata) {
  ...
  EXPECT_EQ(required->size(), 3u);  // direction, distance, document_identifier
}
