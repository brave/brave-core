...
class ClickToolTest : public testing::Test {
 public:
  void SetUp() override {
    mock_task_provider_ = std::make_unique<MockBrowserToolTaskProvider>();
    click_tool_ =
 ...
 protected:
  ...
  void RunWithExpectedError(const std::string& input_json,
    ...
    click_tool_->UseTool(input_json, future.GetCallback(), std::nullopt);
  ...
  // Simplified to verify only click-specific properties and tab handle
  ...
  std::unique_ptr<MockBrowserToolTaskProvider> mock_task_provider_;
...
TEST_F(ClickToolTest, ValidInputWithContentNode) {
  ...
  click_tool_->UseTool(input_json, base::DoNothing(), std::nullopt);
  run_loop.Run();
