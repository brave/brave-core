...
class NavigationToolTest : public testing::Test {
 ...
 protected:
  ...
  void RunWithExpectedError(const std::string& input_json,
    ...
    navigation_tool_->UseTool(input_json, future.GetCallback(), std::nullopt);
  ...
  void VerifyNavigationAction(const optimization_guide::proto::Actions& actions,
    ...
    EXPECT_EQ(navigate_action.url(), expected_url);
    ...
    EXPECT_EQ(navigate_request->GetURLForJournal(), GURL(expected_url));
  }
...
TEST_F(NavigationToolTest, ValidInputHttpsUrl) {
  ...
  navigation_tool_->UseTool(input_json, base::DoNothing(), std::nullopt);
  run_loop.Run();
...
TEST_F(NavigationToolTest, MissingWebsiteUrl) {
  ...
  RunWithExpectedError(input_json, "Missing website_url parameter from input JSON.");
}

TEST_F(NavigationToolTest, InvalidUrlFormat) {
  std::string input_json = CreateValidNavigationJson("not_a_valid_url");

  RunWithExpectedError(input_json, "website_url parameter did not contain a valid URL");
}

TEST_F(NavigationToolTest, NonHttpsUrl) {
  std::string input_json = CreateValidNavigationJson("http://www.example.com");

  RunWithExpectedError(input_json, "website_url parameter must start with https://");
}
...
TEST_F(NavigationToolTest, ToolMetadata) {
  // Note: NavigationTool uses mojom::kNavigateToolName which should be "navigate"
  // but we test whatever Name() returns
