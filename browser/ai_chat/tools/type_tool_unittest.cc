...
class TypeToolTest : public testing::Test {
 public:
  void SetUp() override {
    ...
    type_tool_ =
        std::make_unique<TypeTool>(mock_task_provider_.get(),
 ...
 protected:
  ...
  void RunWithExpectedError(const std::string& input_json,
    ...
    type_tool_->UseTool(input_json, future.GetCallback(), std::nullopt);
...
TEST_F(TypeToolTest, ValidInputWithContentNode) {
  ...
  std::string input_json = CreateValidTypeJson(target_dict, "Hello World", "replace", true);
  ...
  type_tool_->UseTool(input_json, base::DoNothing(), std::nullopt);
  run_loop.Run();
...
TEST_F(TypeToolTest, ValidInputWithCoordinates) {
  ...
  std::string input_json = CreateValidTypeJson(target_dict, "Append text", "append", false);
  base::RunLoop run_loop;
...
TEST_F(TypeToolTest, ValidInputPrependMode) {
  ...
  std::string input_json = CreateValidTypeJson(target_dict, "Prepend: ", "prepend", false);
  base::RunLoop run_loop;
...
TEST_F(TypeToolTest, ValidInputCustomCoordinates) {
  ...
  std::string input_json = CreateValidTypeJson(target_dict, "Custom coordinates", "replace", true);
  base::RunLoop run_loop;
...
TEST_F(TypeToolTest, MissingMode) {
  ...
  RunWithExpectedError(input_json, "Invalid or missing mode. Must be 'replace', 'prepend', or 'append'.");
}
...
TEST_F(TypeToolTest, MissingTargetObject) {
  ...
  RunWithExpectedError(input_json, "Either content_node_id with document_identifier OR x,y coordinates must be provided.");
}
...
TEST_F(TypeToolTest, InvalidTargetValidation) {
  ...
  RunWithExpectedError(CreateInvalidTargetJson("{}"),
                       "Either content_node_id with document_identifier OR x,y coordinates must be provided.");
}
...
TEST_F(TypeToolTest, MissingDocumentIdentifierWithContentNode) {
  ...
  RunWithExpectedError(input_json, "document_identifier is required when using content_node_id.");
}
...
TEST_F(TypeToolTest, ToolMetadata) {
  ...
  EXPECT_EQ(required->size(), 3u);  // text, follow_by_enter, mode
}
