...
#include "brave/browser/ai_chat/ai_chat_browsertest_util.h"
...
class RemoteModelsIntegrationBrowserTest : public InProcessBrowserTest {
 public:
  ...
  void SetUpOnMainThread() override {
    ...
    // Side panel entry switches otherwise defer the outgoing entry's view
    // swap (and thus its WebContents-hidden signal) until the incoming
    // entry's content proxy reports ready, which is asynchronous.
  ...
  ModelService* GetModelService() {
    return GetModelServiceForBrowserTest(browser());
  }
...
IN_PROC_BROWSER_TEST_F(RemoteModelsIntegrationBrowserTest,
  ...
  // Flush every task currently queued on the UI thread and thread pool,
  // rather than guessing how long "nothing happens" takes to observe.
...
IN_PROC_BROWSER_TEST_F(RemoteModelsIntegrationBrowserTest,
  ...
  const size_t hardcoded_model_count = GetModelService()->GetModels().size();

  ShowSidePanelEntry(SidePanelEntryId::kChatUI);

  // Wait for the failed refresh to fully complete, not just for the server
  // to receive the request: a completed refresh (re)arms the timer whether
  // it succeeded or failed, so this is a real completion signal rather than
  // an arbitrary wait.
  ASSERT_TRUE(base::test::RunUntil([&] {
    return GetModelService()->IsRemoteModelsRefreshTimerRunningForTesting();
  }));

  // `kChatAutomaticModelKey` would be present even without this test
  // exercising anything, since it's a hardcoded model loaded synchronously
  // at construction. What the failed fetch must NOT do is disturb that
  // hardcoded list.
  EXPECT_EQ(GetRequestCount(), 1);
  EXPECT_EQ(GetModelService()->GetModels().size(), hardcoded_model_count);
...
IN_PROC_BROWSER_TEST_F(RemoteModelsIntegrationBrowserTest,
  ...
  // Hiding the last visible surface stops the timer directly; check that
  // signal instead of waiting past the TTL to see whether a refresh fires.
  EXPECT_FALSE(
      GetModelService()->IsRemoteModelsRefreshTimerRunningForTesting());
  EXPECT_EQ(GetRequestCount(), 1);
}
