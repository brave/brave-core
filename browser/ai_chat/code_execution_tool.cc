...
std::string_view CodeExecutionTool::Description() const {
  return "Execute JavaScript code and capture console output, charts, and "
         ...
         "data); "
         "IMPORTANT: window.createChart() returns a uPlot instance, NOT a canvas. "
         "To embed a chart in a PDF: "
         "const chart = window.createChart(opts, data); "
         "await new Promise(r => requestAnimationFrame(() => requestAnimationFrame(r))); "
         "const imgData = chart.ctx.canvas.toDataURL('image/png'); "
         "const imgBytes = Uint8Array.from(atob(imgData.split(',')[1]), c => c.charCodeAt(0)); "
         "Then in createPdf: const img = await pdfDoc.embedPng(imgBytes); "
         "const maxWidth = pageWidth - 2 * margin; "
         "const scale = Math.min(1, maxWidth / img.width); "
         "page.drawImage(img, {x: margin, y: yPos - img.height * scale, "
