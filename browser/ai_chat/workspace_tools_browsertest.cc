...
// str_replace_based_edit_tool, grep, glob, append_file.
constexpr size_t kExpectedToolCount = 4;
...
// chrome-untrusted://leo-workspace page receives a FileSystemDirectoryHandle
// for a picked folder (via launchQueue), registers its file tools via WebMCP,
// and Leo
// harvests them. Also runs the editor tool's `view` command end-to-end to
// confirm the delivered handle is usable.
class WorkspaceToolsBrowserTest : public InProcessBrowserTest {
 public:
  ...
  void SetUpCommandLine(base::CommandLine* command_line) override {
    ...
    // document.modelContext is gated by the (experimental) WebMCP runtime
...
IN_PROC_BROWSER_TEST_F(WorkspaceToolsBrowserTest,
  ...
  // The delegate started navigating to
  // chrome-untrusted://leo-workspace/<guid> in its constructor; wait for that
  // to commit before evaluating script, so the
  // poll below runs in the workspace document rather than the initial one.
  ASSERT_TRUE(content::WaitForLoadStop(web_contents));

  // Wait for the page to receive its handle via launchQueue and register its
  // tools. Poll from C++ rather than from a JS loop: the workspace page is
  // deliberately hidden, so its timers are throttled to ~1Hz and a setTimeout
  // poll would blow the test timeout.
  // -1 means the page has no `document.modelContext` at all (WebMCP not
  // enabled, or blocked for this origin), which is a different failure from
  // "the page hasn't registered its tools yet".
  int tool_count = -1;
  ASSERT_TRUE(base::test::RunUntil([&]() {
    tool_count = content::EvalJs(web_contents, R"(
        document.modelContext
            ? document.modelContext.getTools().then((t) => t.length)
            : -1
    )")
                     .ExtractInt();
    return tool_count == static_cast<int>(kExpectedToolCount);
  })) << "last observed tool count: "
      << tool_count;
  ...
  // First-party WebUI tools keep their own names verbatim (no host/path
  // prefix), via the ContentTool override for chrome-untrusted:// origins.
  EXPECT_THAT(names,
  ...
  Tool* editor = nullptr;
  for (const auto& tool : tools) {
    if (tool && tool->Name() == "str_replace_based_edit_tool") {
      editor = tool.get();
      break;
    }
  }
