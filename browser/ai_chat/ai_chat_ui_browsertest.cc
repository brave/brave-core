...
class AIChatUIBrowserTest : public InProcessBrowserTest {
 public:
  ...
  content::WebContents* OpenAIChatSidePanel() {
    ...
    auto* side_panel_web_contents = side_panel_ui->GetWebContentsForTest(SidePanelEntryId::kChatUI);
...
// - Both: Navigate to /[uuid] should load that conversation (both container and frame)
