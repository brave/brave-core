...
void RewardsTabHelper::DidFinishNavigation(
  ...
  if (!navigation_handle->IsSameDocument()) {
    ...
    creator_detection_.MaybeInjectScript(rfh);
  }

  creator_detection_.DetectCreator(
      rfh, base::BindOnce(&RewardsTabHelper::OnCreatorDetected,
...
void RewardsTabHelper::OnCreatorDetected(
  ...
  if (!result->id.empty()) {
    ...
    // table can be populated. We must also notify the utility process that a
    // "page view" has started for this creator.
