...
void CreatorDetectionScriptInjector::MaybeInjectScript(
  ...
  auto script_source = GetDetectionScript(injector_host_);
  if (!script_source) {
    return;
  }

  injector_host_->GetRemoteAssociatedInterfaces()->GetInterface(&injector_);

  bool verbose_logging =
      base::FeatureList::IsEnabled(features::kVerboseLoggingFeature);

  // Add a `braveRewards` global variable to the isolated world.
  std::string initializer =
      base::StrCat({"self.braveRewards = { verboseLogging: ",
                    verbose_logging ? "true" : "false", " }"});
  ExecuteScript(initializer, base::DoNothing());
...
void CreatorDetectionScriptInjector::DetectCreator(
  ...
  if (!rfh || rfh != injector_host_.get() || !injector_.is_bound()) {
    std::move(callback).Run(std::nullopt);
    return;
  }

  // Do not call the detection routine if it has already been run for this URL.
  auto& url = injector_host_->GetLastCommittedURL();
  if (url == last_url_) {
    std::move(callback).Run(std::nullopt);
    return;
  }
