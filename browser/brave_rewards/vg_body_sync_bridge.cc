...
void VgBodySyncBridge::WhenVgBodiesReady(
    base::OnceCallback<void(std::vector<sync_pb::VgBodySpecifics>)>
        when_vg_bodies_ready) {
  when_vg_bodies_ready_ = std::move(when_vg_bodies_ready);
}
...
void VgBodySyncBridge::OnCommitWriteBatch(
  ...
  if (error) {
    ...
    if (vg_bodies) {
      std::move(when_vg_bodies_ready_).Run(std::move(*vg_bodies));
    }
  }
}
