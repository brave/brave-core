...
absl::optional<syncer::ModelError> VgSpendStatusSyncBridge::ApplySyncChanges(
  ...
  // receive any incremental changes anyway. We only implement the initial
  // processing of remote data (see MergeSyncData above), since this is what
...
void VgSpendStatusSyncBridge::WhenVgSpendStatusesReady(
    base::OnceCallback<void(std::vector<sync_pb::VgSpendStatusSpecifics>)>
        when_vg_spend_statuses_ready) {
  when_vg_spend_statuses_ready_ = std::move(when_vg_spend_statuses_ready);
}
...
void VgSpendStatusSyncBridge::OnCommitWriteBatch(
  ...
  if (error) {
    ...
    if (vg_spend_statuses) {
      std::move(when_vg_spend_statuses_ready_)
