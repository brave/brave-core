...
const char* kAllCreativeCountDicts[] = {
...
NTPP3AHelperImpl::NTPP3AHelperImpl(PrefService* local_state,
  ...
  auto* ntp_background_images_service =
      g_brave_browser_process->ntp_background_images_service();
  if (ntp_background_images_service) {
    const auto* sr_data =
        ntp_background_images_service->GetBrandedImagesData(true);
    const auto* si_data =
        ntp_background_images_service->GetBrandedImagesData(false);
    if (sr_data) {
...
void NTPP3AHelperImpl::CleanOldCampaignsAndCreatives() {
  ...
  auto check_expiry = [&](const base::Value::Dict* dict) -> bool {
    if (!dict) {
      return false;
    }
    ...
    auto expiry_time = base::ValueToTime(*expiry_time_value);
  ...
  for (const char* dict_pref : kAllCreativeCountDicts) {
    ...
    for (auto it = creative_update->begin(); it != creative_update->end();) {
      ...
      if (!check_expiry(creative_instance_dict)) {
        it++;
        continue;
      }
