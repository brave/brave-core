...
void NewTabTakeoverInfoBarDelegate::MaybeCreate(
  ...
  if (!prefs->GetBoolean(brave_ads::prefs::kShowNewTabTakeoverInfobar)) {
    return;
  }
  prefs->SetBoolean(brave_ads::prefs::kShowNewTabTakeoverInfobar, false);

  if (infobars::ContentInfoBarManager* infobar_manager =
          infobars::ContentInfoBarManager::FromWebContents(web_contents)) {
    infobar_manager->AddInfoBar(CreateConfirmInfoBar(
        std::make_unique<NewTabTakeoverInfoBarDelegate>()));
  }
}
