...
using ntp_sponsored_images::NTPReferralImagesService;
...
NTPReferralImagesService*
BraveBrowserProcessImpl::ntp_referral_images_service() {
  if (!base::FeatureList::IsEnabled(kBraveCustomHomepage))
    return nullptr;

  if (!ntp_referral_images_service_) {
    ntp_referral_images_service_ =
        std::make_unique<NTPReferralImagesService>(component_updater(),
                                                   local_state());
  }

  return ntp_referral_images_service_.get();
}
