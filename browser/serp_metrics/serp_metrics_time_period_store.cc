...
void SerpMetricsTimePeriodStore::Set(base::ListValue data) {
  ...
  serp_metrics.Set(metric_name_, std::move(data));
...
void SerpMetricsTimePeriodStore::Clear() {
  ...
  base::DictValue updated_serp_metrics = serp_metrics->Clone();
  updated_serp_metrics.Remove(metric_name_);
  entry->SetSerpMetrics(std::move(updated_serp_metrics));
}
