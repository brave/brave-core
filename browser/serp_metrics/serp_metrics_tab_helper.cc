...
void SerpMetricsTabHelper::DidFinishNavigation(
  ...
  if (!IsSameSearchQuery(url) && ui::PageTransitionIsNewNavigation(
                                     navigation_handle->GetPageTransition())) {
    // Any navigation that doesn't match the previous serp page should reset it
    // along with any user initiated navigation (urlbar, bookmarks, etc...)
