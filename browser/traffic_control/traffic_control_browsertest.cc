...
// A new tab opened from a tab that lives in another container only inherits
// that container, so the rule wins even though the navigation is same-site.
IN_PROC_BROWSER_TEST_F(TrafficControlBrowserTest,
                       NewTabFromOtherContainerFollowsRule) {
  auto* containers_service =
      ContainersServiceFactory::GetForProfile(browser()->GetProfile());
  auto containers = containers_service->GetContainers();
  ...
  // Explicitly open the site in the other container: that tab stays put.
  OpenUrlInContainerForTest(TestUrl(), other_container);
  content::WebContents* source =
      browser()->tab_strip_model()->GetActiveWebContents();
  ASSERT_TRUE(content::WaitForLoadStop(source));
  ASSERT_EQ(containers::GetContainerIdForWebContents(source), other_id);

  // Open the same site in a new tab (middle-click style, no explicit
  // container). The rule's container must win over the inherited one.
  const GURL same_site =
      embedded_test_server()->GetURL("example.com", "/title1.html");
  content::OpenURLParams params(same_site, content::Referrer(),
                                WindowOpenDisposition::NEW_BACKGROUND_TAB,
                                ui::PAGE_TRANSITION_LINK, false);
  source->OpenURL(params, /*navigation_handle_callback=*/{});

  EXPECT_TRUE(base::test::RunUntil([&]() {
    for (int i = 0; i < browser()->tab_strip_model()->count(); ++i) {
      content::WebContents* contents =
          browser()->tab_strip_model()->GetWebContentsAt(i);
      if (contents != source && !contents->IsLoading() &&
          contents->GetLastCommittedURL() == same_site) {
  ...
  // The source tab keeps its explicitly chosen container and stays focused.
  EXPECT_EQ(containers::GetContainerIdForWebContents(source), other_id);
