...
class TestBraveWalletServiceObserverForAssetDiscoveryTask
 ...
 public:
  ...
  void OnDiscoverAssetsCompleted(
    ...
    // Nothing fatal here: an ASSERT would skip the Quit() below and hang the
    // waiting RunLoop instead of failing the test.
  ...
  void WaitForOnDiscoverAssetsCompleted(
    ...
    // Callers wait from inside the DiscoverAssets completion callback, which
    // can itself run under a RunLoop. A nested kDefault RunLoop only runs
    // system tasks, so the observer notification (an application task) would
    // never be delivered.
