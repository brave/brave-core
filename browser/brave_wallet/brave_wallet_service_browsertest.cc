...
class BraveWalletServiceTest : public InProcessBrowserTest {
 public:
  ...
  BraveWalletService* GetIncognitoWalletService() {
...
IN_PROC_BROWSER_TEST_F(BraveWalletServiceTest, IsPrivateWindow) {
  TestIsPrivateWindow(GetWalletService(), false);
  GetWalletService()->SetPrivateWindowsEnabled(true);
  TestIsPrivateWindow(GetIncognitoWalletService(), true);
  TestIsPrivateWindow(GetWalletService(), false);
}
