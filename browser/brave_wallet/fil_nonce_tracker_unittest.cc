...
TEST_F(FilNonceTrackerUnitTest, GetNonce) {
  ...
  meta.set_from(FilAddress::FromAddress(addr).EncodeAsString());
  ...
  WaitForResponse();
  EXPECT_TRUE(callback_called);
  EXPECT_EQ(nonce_result, uint64_t(3));
  ...
  WaitForResponse();
  EXPECT_TRUE(callback_called);
  EXPECT_EQ(nonce_result, uint64_t(4));
  ...
  WaitForResponse();
  EXPECT_TRUE(callback_called);
  EXPECT_EQ(nonce_result, uint64_t(5));
}
