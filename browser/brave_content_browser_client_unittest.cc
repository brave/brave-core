...
TEST_F(BraveWalleBrowserClientUnitTest,
  ...
  ASSERT_TRUE(BraveContentBrowserClient::HandleURLRewrite(
        &url, browser_context()));
...
TEST_F(BraveContentBrowserClientTest, ResolvesSync) {
  ...
  ASSERT_TRUE(
    BraveContentBrowserClient::HandleURLRewrite(&url, nullptr));
  ASSERT_STREQ(url.spec().c_str(), "chrome://settings/braveSync");

  GURL url2("brave://sync/");
  ASSERT_TRUE(
    BraveContentBrowserClient::HandleURLRewrite(&url2, nullptr));
...
// TODO(jocelyn): HandleURLReverseRewrite...?
