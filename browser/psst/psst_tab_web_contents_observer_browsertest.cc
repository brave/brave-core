...
constexpr char16_t kPolicyScriptCheckboxPrefix[] =
    u"[PSST POLICY SCRIPT] Checkbox turned off curUrl:";
    ...
    tasks: [
      {
        ...
        selector: 'input[id="testCheckbox1"]',
        turn_off: true,
      },
      {
        url: 'https://a.test:$1/a_test_2.html',
        description: 'a_test_2.html',
        selector: 'input[id="testCheckbox2"]',
        turn_off: true,
      }
    ]
  }
...
const checkCheckboxes = (resolve, reject, selector, turnOff) => {
  ...
  if (checkbox && checkbox.type === 'checkbox') {
    if (turnOff) {
      if (checkbox.checked) {
        checkbox.click();
        console.log(`[PSST POLICY SCRIPT] Checkbox turned off curUrl:${curUrl}`);
      }
    } else {
      if (!checkbox.checked) {
        checkbox.click();
        console.log(`[PSST POLICY SCRIPT] Checkbox turned on curUrl:${curUrl}`);
      }
    }
    resolve(true);
  } else {
...
(async () => {
  ...
  try {
    ...
    await waitForCheckboxToLoadWithTimeout(current_task?.selector, current_task?.turn_off)
    console.log(`[PSST] Wait finished \n psst:${JSON.stringify(psstObj)}`)
    moveCurrentTask(psstObj, null)
  } catch (error) {
    console.log(`[PSST] Error occurred \n psst:${JSON.stringify(psstObj)} \n error:${error}`)
  ...
  PsstWebContentsConsoleObserver(
    ...
    SetFilter(base::BindLambdaForTesting(
        [this](const content::WebContentsConsoleObserver::Message& message) {
          ...
          LOG(INFO) << "[PSST] Received console message: " << message.message
                    << " \n Remaining user script messages: " << user_script_messages_.size()
                    << " \n Remaining policy script messages: " << policy_script_messages_.size();
          ;
...
IN_PROC_BROWSER_TEST_F(PsstTabWebContentsObserverBrowserTest,
  ...
  PsstWebContentsConsoleObserver console_observer(
      web_contents(),
      {
        /* Executed user script when user visited the website */
        base::StrCat({kUserScriptLogPrefix,
                     CreateTestUtf16URL(https_server_, "/a_test_0.html")}),
        /* Executed the user script upon redirection to the page where the user was located at the time the PSST feature was activated */
        base::StrCat({kUserScriptLogPrefix,
                     CreateTestUtf16URL(https_server_, "/a_test_0.html")}),
        /* Executed user script when PSST redirected to the first setting page */
       base::StrCat({kUserScriptLogPrefix,
                     CreateTestUtf16URL(https_server_, "/a_test_1.html")}),
        /* Executed user script when PSST redirected to the second setting page */
       base::StrCat({kUserScriptLogPrefix,
                     CreateTestUtf16URL(https_server_, "/a_test_2.html")})},
      {
        /* Executed policy script when user visited the website */
        base::StrCat({kPolicyScriptLogPrefix,
                     CreateTestUtf16URL(https_server_, "/a_test_0.html")}),
        /* Executed policy script when psst redirected to the first setting page */
       base::StrCat({kPolicyScriptLogPrefix,
                     CreateTestUtf16URL(https_server_, "/a_test_1.html")}),
       base::StrCat({kPolicyScriptCheckboxPrefix,
                     CreateTestUtf16URL(https_server_, "/a_test_1.html")}),
          
       /* Executed policy script when psst redirected to the second setting page */
       base::StrCat({kPolicyScriptLogPrefix,
                     CreateTestUtf16URL(https_server_, "/a_test_2.html")}),
       base::StrCat({kPolicyScriptCheckboxPrefix,
                     CreateTestUtf16URL(https_server_, "/a_test_2.html")}),
