...
AdsTooltipsController::AdsTooltipsController(
#if BUILDFLAG(BRAVE_ADAPTIVE_CAPTCHA_ENABLED)
    ...
    :
#if BUILDFLAG(BRAVE_ADAPTIVE_CAPTCHA_ENABLED)
      brave_adaptive_captcha_service_(brave_adaptive_captcha_service),
#endif
      profile_(profile) {
  DCHECK(brave_adaptive_captcha_service_);
  DCHECK(profile_);
}
...
void AdsTooltipsController::ShowCaptchaTooltip(const std::string& payment_id,
  ...
  auto captcha_tooltip = std::make_unique<AdsCaptchaTooltip>(
      brave_adaptive_captcha_service_, tooltip_attributes, payment_id,
      captcha_id);
