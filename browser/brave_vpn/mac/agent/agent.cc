...
constexpr char kHelperAuthToken[] = "poc-shared-secret-do-not-use-in-production";
...
class AgentApp {
 public:
  ...
  explicit AgentApp(const std::string& server_name)
  ...
  void OnUIAppServiceDisconnected(mojo::ReceiverId id) {
    ...
    LOG(INFO) << "[Agent] UIApp client " << id << " removed (active: "
              << uiapp_clients_.size() << ").";
  }
  ...
  void CallTestUIApp(mojo::ReceiverId id) {
    ...
    if (it == uiapp_clients_.end() || !it->second.uiapp_service.is_connected()) {
      LOG(WARNING) << "[Agent] CallTestUIApp: client " << id << " gone.";
      return;
    }
    LOG(INFO) << "[Agent] Calling TestUIApp() on uiapp client " << id << ".";
    it->second.uiapp_service->TestUIApp(
        base::BindOnce(&AgentApp::OnTestUIAppResponse,
  ...
  void BroadcastTestUIApp() {
    for (auto& [id, state] : uiapp_clients_) {
      if (!state.uiapp_service.is_connected()) continue;
      LOG(INFO) << "[Agent] Broadcasting TestUIApp() to client " << id << ".";
      state.uiapp_service->TestUIApp(
          base::BindOnce(&AgentApp::OnTestUIAppResponse,
                         weak_factory_.GetWeakPtr(), id));
    }
  }
 ...
 private:
  ...
  void TryConnect() {
    ...
    helper_service_.set_disconnect_handler(
        base::BindOnce(&AgentApp::OnHelperDisconnected,
  ...
  void CallTestHelper() {
    ...
    helper_service_->TestHelper(
        base::BindOnce(&AgentApp::OnTestHelperResponse,
                       weak_factory_.GetWeakPtr()));
  }
  ...
  void Reset() {
    ...
    if (agent_receiver_.is_bound()) agent_receiver_.reset();
  }
...
void AgentUserServiceImpl::Authenticate(
  ...
  state.uiapp_service.set_disconnect_handler(
      base::BindOnce(&AgentApp::OnUIAppServiceDisconnected,
                     base::Unretained(app_), id));

  LOG(INFO) << "[Agent] UIApp client " << id << " authenticated (active: "
            << clients.size() << ").";
...
int main(int argc, char* argv[]) {
  ...
  const std::string server_name =
      std::string(kAgentServerNamePrefix) + token.ToString();
