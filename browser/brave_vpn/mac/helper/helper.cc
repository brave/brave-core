...
class HelperApp {
 public:
  ...
  void OnAgentServiceDisconnected(mojo::ReceiverId id) {
    ...
    server_.Close(id);
    clients_.erase(id);
    LOG(INFO) << "[Helper] Client " << id << " removed (active: "
              << clients_.size() << ").";
  }
  ...
  void CallTestAgent(mojo::ReceiverId id) {
    ...
    it->second.agent_service->TestAgent(
        base::BindOnce(&HelperApp::OnTestAgentResponse,
  ...
  void BroadcastTestAgent() {
    for (auto& [id, state] : clients_) {
      if (!state.agent_service.is_connected()) continue;
      LOG(INFO) << "[Helper] Broadcasting TestAgent() to client " << id << ".";
      state.agent_service->TestAgent(
          base::BindOnce(&HelperApp::OnTestAgentResponse,
                         weak_factory_.GetWeakPtr(), id));
    }
  }
 ...
 private:
  ...
  void OnClientDisconnected() {
    ...
    LOG(INFO) << "[Helper] Client " << id << (had_entry ? " removed" : " disconnected (never authenticated)")
              << " (active: " << clients_.size() << ").";
  }

  void OnTestAgentResponse(mojo::ReceiverId id, const std::string& response) {
    LOG(INFO) << "[Helper] TestAgent() from client " << id << ": \""
              << response << "\".";
  }
...
void HelperServiceImpl::Authenticate(
  ...
  state.agent_service.set_disconnect_handler(
      base::BindOnce(&HelperApp::OnAgentServiceDisconnected,
                     base::Unretained(app_), id));

  LOG(INFO) << "[Helper] Client " << id << " authenticated (active: "
            << clients.size() << ").";
