...
// TODO(bsclifton): consider persisting these to registry.
// As-is, these values won't survive the owning process being restarted.
std::optional<std::string> g_smart_proxy_url;
bool g_using_smart_proxy = false;
  ...
  if (rv == ERROR_SUCCESS && key.Valid()) {
    ...
    if (key.ReadValue(L"AutoConfigURL", &current_proxy_url) == ERROR_SUCCESS) {
      ...
      VLOG(1) << L"Set Brave VPN smart proxy routing to = \""
              << current_proxy_url << "\"";
      g_using_smart_proxy = true;
    }
  }
}

void MaybeDisableSystemProxy() {
  if (!g_using_smart_proxy) {
    return;
  }
