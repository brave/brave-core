...
bool EnableBraveVpnWireguardServiceImpl(
  ...
  if (g_smart_proxy_url.has_value()) {
    ...
    if (rv == ERROR_SUCCESS && key.Valid()) {
      ...
      // TODO(bsclifton): only make a change if value not set
      if (key.ReadValue(L"AutoConfigURL", &current_proxy_url) ==
          ERROR_SUCCESS) {
        LOG(ERROR) << L"BSC]] Current value = \"" << current_proxy_url << "\"";
      } else {
        LOG(ERROR) << L"BSC]] Unable to read value; maybe its empty";
      }
      ...
      LOG(ERROR) << L"BSC]] set to = \"" << current_proxy_url << "\"";
    }
  }
  // ------------------------------
