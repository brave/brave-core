...
class ScopedWireguardFirewall {
 public:
  // Installs the whole filter set atomically. Returns nullptr if any part of it
  // failed; callers must treat that as fatal and bring the tunnel down rather
  // than let traffic flow unprotected.
 ...
 private:
  explicit ScopedWireguardFirewall(HANDLE engine);
