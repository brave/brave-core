...
void BraveSyncServiceImplDelegate::OnDeviceInfoChange() {
  ...
  if (!found_local_device) {
    // If prefs aren't ready, we're still in early init — the empty device list
    // comes from ClearMetadataIfStopped() during StopAndClear(), not from
    // another device removing us from the chain.
    if (!sync_service_impl_->has_prefs()) {
      return;
    }
