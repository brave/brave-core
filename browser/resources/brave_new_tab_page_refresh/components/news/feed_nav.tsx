...
import {
  ...
  newsFeedTypesEqual } from '../../api/news_api'
...
export function FeedNav(props: Props) {
  ...
  const signals = useNewsState((s) => s.newsSignals)
...
function FeedButton(props: FeedButtonProps) {
  ...
  const newsFeedType = useNewsState((s) => s.newsFeedType)
  return (
    <button
      onClick={() => {
        actions.setNewsFeedType(props.feedType)
        actions.notifyNewsSidebarFilterUsage()
        if (props.onFeedSelect) {
          props.onFeedSelect()
        }
      }}
      className={classNames({
        selected: newsFeedTypesEqual(newsFeedType, props.feedType)
      })}
    >
      {props.text}
    </button>
