...
export function TopSitesGrid(props: Props) {
  ...
  const { topSites, sponsoredSites, deduplicatedTopSites } =
  ...
  React.useEffect(() => {
    dragHandler.setCallbacks({
      ...
      onDrop(from, to) {
        ...
        // draggable. `tileIndexToTopSite` offsets past them and resolves
        // back to `topSites` (not `deduplicatedTopSites`, which may have
        // fewer entries than `topSites` due to advertiser-domain
        // deduplication).
        function tileIndexToTopSite(tileIndex: number) {
          return deduplicatedTopSites[tileIndex - sponsoredSites.length]
        }

        const site = tileIndexToTopSite(from)
        if (!site) {
          return
        }
        ...
        if (to < sponsoredSites.length) {
          ...
          const targetSite = tileIndexToTopSite(to)
          position = targetSite
            ? topSites.indexOf(targetSite)
            : topSites.length - 1
        }
        if (position >= 0) {
          actions.setTopSitePosition(site.url, position)
        }
      },
    })
...
export function topSitesWithoutSponsoredSiteDuplicates(
  ...
  const advertiserDomains = sponsoredSites.map((sponsoredSite) => {
    const hostname = new URL(sponsoredSite.targetUrl.url).hostname
...
export function useDeduplicatedTopSites() {
  ...
  return { topSites, sponsoredSites, deduplicatedTopSites }
}
