...
function SponsoredRichMediaBackground(
  ...
  return (
    <IframeBackground
      ...
      onMessage={(data: SponsoredRichMediaMessage) => {
        if (!data ||
          typeof data !== 'object' ||
        ...
        switch (data.type) {
          ...
          case 'richMediaOpenBraveSearchWithQuery': {
            const value = String(data.value)
            if (value) {
              openLink(`https://search.brave.com/search?${value}`)
            }
            break
          }
          default:
...
type SponsoredRichMediaMessage =
  | { type: 'richMediaEvent'; value?: string }
  | { type: 'richMediaQueryBraveSearchAutocomplete'; value?: string }
  | { type: 'richMediaOpenBraveSearchWithQuery'; value?: string }
