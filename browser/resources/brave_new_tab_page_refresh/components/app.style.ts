...
export const style = scoped.css`
  ...
  .allow-background-pointer-events {
    ...
    /* But children will not (unless the explicitly allow it). */
    > :not(.allow-background-pointer-events) {
      pointer-events: auto;
    }

    /* And not when a popover is open. When a popover is open, pointer events
       on a background iframe will not "light-dismiss" the popover. */
