...
type TabName = 'vpn' | 'rewards' | 'talk'

const tabList: TabName[] = ['vpn', 'rewards', 'talk']

export function ProductWidgetStack() {
  const [vpnFeatureEnabled, showVpnWidget] = useVPNState((state) => [
    state.vpnFeatureEnabled,
    state.showVpnWidget
  ])

  const showRewardsWidget = useRewardsState((state) => state.showRewardsWidget)
  const showTalkWidget = useNewTabState((state) => state.showTalkWidget)
  ...
  const visibleTabs = React.useMemo(() => {
    return tabList.filter((tab) => {
      switch (tab) {
        case 'rewards': return showRewardsWidget
        case 'talk': return showTalkWidget
        case 'vpn': return vpnFeatureEnabled && showVpnWidget
      }
    })
  }, [vpnFeatureEnabled, showVpnWidget, showTalkWidget, showRewardsWidget])
...
function loadCurrentTab(): TabName | null {
  const value = localStorage.getItem('ntp-current-product-widget')
  switch (value) {
    case 'vpn':
    case 'rewards':
    case 'talk':
...
function storeCurrentTab(tab: TabName | null) {
  localStorage.setItem('ntp-current-product-widget', tab ?? '')
}
