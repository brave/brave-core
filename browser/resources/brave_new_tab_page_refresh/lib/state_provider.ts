 ...
 * @param createActions - A function that receives the state store and returns
 *   an actions object. Actions typically call `store.update()` to modify state.
 ...
 * }
 *
 * function createActions(store: StateStore<AppState>): AppActions {
 *   return {
 *     increment() {
...
export function createStateProvider<State, Actions>(
  initialState: State,
  createActions: (store: StateStore<State>) => Actions,
) {
  ...
  function Provider(props: ProviderProps) {
    const value = React.useMemo(() => {
      ...
      const actions = (props.createActions ?? createActions)(store)
