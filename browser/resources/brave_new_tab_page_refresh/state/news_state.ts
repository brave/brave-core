...
export interface NewsActions {
  ...
  subscribeToNewDirectFeed: (url: string) => void
...
export function defaultNewsActions(): NewsActions {
  return {
    ...
    async getSuggestedPublisherIds() { return [] },
    onNewsVisible() {},
    async findFeeds(url) { return [] },
...
export function feedType(
  ...
  const partial = () => {
    switch (key) {
      ...
      case 'channel': return { channel: { channel: value } }
      case 'publisher': return { publisher: { publisherId: value } }
    }
  }
  return {
    all: undefined,
