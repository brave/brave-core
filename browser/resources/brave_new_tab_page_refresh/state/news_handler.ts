...
import {
  ...
  defaultNewsActions } from './news_state'
  ...
  return {
    ...
    setPublisherEnabled(publisherId, enabled) {
      ...
      const status =
        ...
        isDirect ? mojom.UserEnabled.DISABLED :
    ...
    setChannelSubscribed(channelName, enabled) {
      ...
      if (channel) {
        if (enabled) {
          ...
          channel.subscribedLocales =
            channel.subscribedLocales.filter((locale) => locale !== newsLocale)
        }
