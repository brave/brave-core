...
RegisterPolymerTemplateModifications({
  'settings-your-saved-info-page': (templateContent) => {
    ...
    for (const selector of ['#identityManagerButton', '#travelManagerButton']) {
      ...
      if (!card) {
        throw new Error(
          `[Settings] Unable to find ${selector} on your-saved-info-page`)
      }
      card.hidden = true
    }
    ...
    if (!relatedServicesSection) {
      throw new Error('[Settings] Unable to find the related services ' +
        'section on your-saved-info-page')
    }
    ...
    if (!autofillSettingsCard) {
      throw new Error('[Settings] Unable to find ' +
        'collapsible-autofill-settings-card on your-saved-info-page')
    }
    ...
    if (loadTimeData.getBoolean('isEmailAliasesEnabled')) {
      ...
      if (!cardContainer) {
        throw new Error(
          '[Settings] Unable to find .card-container on your-saved-info-page')
      }
  ...
  'settings-your-saved-info-page-index': (templateContent) => {
    if (!loadTimeData.getBoolean('isEmailAliasesEnabled')) {
      return
    }
...
RegisterPolymerPrototypeModification({
  'settings-your-saved-info-page': (prototype) => {
...
if (loadTimeData.getBoolean('isEmailAliasesEnabled')) {
  RegisterPolymerPrototypeModification({
    ...
    'settings-your-saved-info-page-index': (prototype) => {
