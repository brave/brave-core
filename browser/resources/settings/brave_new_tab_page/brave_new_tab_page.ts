...
} from './brave_new_tab_browser_proxy.js'
...
export class SettingsBraveNewTabPageElement
extends SettingsBraveNewTabPageElementBase {
  static get is() {
  ...
  browserProxy_: BraveNewTabBrowserProxy =
    BraveNewTabBrowserProxyImpl.getInstance()

  override ready() {
    super.ready()
    this.openNewTabPage_ = this.openNewTabPage_.bind(this)

    this.browserProxy_.getNewTabShowsOptionsList().
      then((list: NewTabOption[]) => {
        this.newTabShowOptions_ = list
      })
    this.browserProxy_.shouldShowNewTabDashboardSettings().
      then((show: boolean) => {
