...
export class SettingsBraveAccountRow extends CrLitElement {
  ...
  private showToast(details?: ResendConfirmationEmailError) {
    const hideToast = (container: HTMLDivElement | null) => {
      container?.remove()
    }
  ...
  private getErrorMessage(details: ResendConfirmationEmailError): string {
    ...
    if (netErrorOrHttpStatus == null) {
      ...
      return loadTimeData.getStringF(
          ...
          errorCode != null ?
              ` (${loadTimeData.getString('braveAccountErrorDialogError')}=${
                  errorCode})` :
              '',
      )
    }
