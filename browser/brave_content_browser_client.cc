...
#endif
  handler->AddHandlerPair(&BraveContentBrowserClient::HandleURLRewrite,
                          &BraveContentBrowserClient::HandleURLReverseRewrite);
...
bool BraveContentBrowserClient::HandleURLRewrite(
  ...
  // Rewrite brave:// to chrome:// as the actual URL to be navigated, brave://
  // would be the virtual URL.
  // Note that the reverse rewrite of this can not be in a reverse rewrite
  // handler but is done in NavigationControllerImpl::UpdateVirtualURLToURL
  // chromium_src override instead. This is because upstream reverse rewrite
  // handlers will use kChromeUIScheme in their checks so we cannot change it
  // back to brave:// before upstream reverse rewrite handlers. We can not put
  // it in a reverse rewrite handler added after upstream handlers either
  // because if any of the upstream handler returns true, the added reverse
  // rewrite handler won't be run.
  if (url->SchemeIs(content::kBraveUIScheme)) {
...
// static
bool BraveContentBrowserClient::HandleURLReverseRewrite(
    GURL* url,
    content::BrowserContext* browser_context) {
  if (!url->SchemeIs(content::kBraveUIScheme) &&
      !url->SchemeIs(content::kChromeUIScheme)) {
  ...
  // brave://settings/braveSync => brave://sync
  if (url->host() == chrome::kChromeUISettingsHost && !url->path().empty() &&
      url->path().substr(1) == kBraveSyncPath) {
    GURL::Replacements replacements;
  ...
  if (url->host() == chrome::kChromeUISettingsHost && !url->path().empty() &&
      url->path().substr(1) == kContentFiltersPath) {
    return true;
  }
