...
class PlaylistRenderFrameObserverBrowserTest : public PlatformBrowserTest {
 public:
  enum class APIVisibility { kHidden, kVisible };
  ...
  template <APIVisibility visibility>
  void CheckMediaSourceAPI(const GURL& url) {
    ASSERT_TRUE(url.is_valid());

    // This is blocking call.
    ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), url));

    if constexpr (visibility == APIVisibility::kVisible) {
      EXPECT_EQ(true,
...
IN_PROC_BROWSER_TEST_F(PlaylistRenderFrameObserverBrowserTest,
                       CheckNormalSites) {
  CheckMediaSourceAPI<APIVisibility::kVisible>(
      https_server_->GetURL("www.a.com", "/"));
}

IN_PROC_BROWSER_TEST_F(PlaylistRenderFrameObserverBrowserTest, CheckYoutube) {
  CheckMediaSourceAPI<APIVisibility::kHidden>(
      https_server_->GetURL("www.youtube.com", "/"));
}
