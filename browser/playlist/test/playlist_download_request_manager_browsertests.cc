...
class PlaylistDownloadRequestManagerBrowserTest : public PlatformBrowserTest {
 public:
  ...
  GURL set_up_https_server(const std::string& html, GURL url = GURL()) {
    ...
    // ASSERT_*() only work with void functions
    ([&] {
  ...
  void LoadHTMLAndCheckResult(const std::string& html,
    ...
    EXPECT_CALL(observer, OnMediaFilesUpdated(_, _))
        .WillOnce(
            [&](const GURL& page_url,
...
// site specific descriptor is broken?
IN_PROC_BROWSER_TEST_F(PlaylistDownloadRequestManagerBrowserTest,
...
IN_PROC_BROWSER_TEST_F(PlaylistDownloadRequestManagerBrowserTest,
  ...
  EXPECT_CALL(observer, OnMediaFilesUpdated(url, IsEmpty())).Times(AtLeast(1));
  EXPECT_CALL(observer, OnMediaFilesUpdated(url, Not(IsEmpty())))
      .WillOnce(
