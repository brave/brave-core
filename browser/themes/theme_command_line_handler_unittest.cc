...
TEST_F(ThemeCommandLineHandlerTest, ProcessUserColorSwitch) {
  ...
  dark_mode::ProcessThemeCommandLineSwitchesForProfile(command_line_.get(),
                                                       profile_.get());
...
TEST_F(ThemeCommandLineHandlerTest, ProcessDefaultThemeSwitch) {
  ...
  dark_mode::ProcessBrowserWideThemeCommandLineSwitches(command_line_.get(),
                                                        profile_.get());
...
TEST_F(ThemeCommandLineHandlerTest, ProcessGrayscaleSwitch) {
  ...
  // switches::kSetGrayscaleTheme is now a boolean flag -
  // presence means true
  command_line_->AppendSwitch(switches::kSetGrayscaleTheme);
...
TEST_F(ThemeCommandLineHandlerTest, ProcessInvalidUserColor) {
  ...
  command_line_->AppendSwitchASCII(switches::kSetUserColor, "256,0,0");
...
TEST_F(ThemeCommandLineHandlerTest, ProcessMalformedUserColor) {
  ...
  command_line_->AppendSwitchASCII(switches::kSetUserColor, "not-a-color");
...
TEST_F(ThemeCommandLineHandlerTest, ProcessInvalidColorScheme) {
  ...
  command_line_->AppendSwitchASCII(switches::kSetColorScheme, "invalid");
...
TEST_F(ThemeCommandLineHandlerTest, ProcessInvalidColorVariant) {
  ...
  command_line_->AppendSwitchASCII(switches::kSetColorVariant, "invalid");
...
TEST_F(ThemeCommandLineHandlerTest, DefaultThemeTakesPrecedence) {
  // Test that switcheskSetDefaultTheme takes precedence over
  // other switches
  command_line_->AppendSwitch(switches::kSetDefaultTheme);
  command_line_->AppendSwitchASCII(switches::kSetUserColor, "255,0,0");
  command_line_->AppendSwitchASCII(switches::kSetColorScheme, "dark");

  // Process browser-wide switch first
  // (switches::kSetDefaultTheme)
  dark_mode::ProcessBrowserWideThemeCommandLineSwitches(command_line_.get(),
                                                        profile_.get());

  // Don't process per-profile switches when default theme is set
  // In production, the per-profile function would check for
  // switches::kSetDefaultTheme and return early, but since we
  // moved it to browser-wide, we skip per-profile here
