...
TEST_F(ThemeCommandLineHandlerTest, ProcessDefaultThemeSwitch) {
  ...
  ProcessBrowserWideThemeCommandLineSwitches(command_line_.get(), profile_.get());
...
TEST_F(ThemeCommandLineHandlerTest, DefaultThemeTakesPrecedence) {
  ...
  // and return early, but since we moved it to browser-wide, we skip per-profile here
