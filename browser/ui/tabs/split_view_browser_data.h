...
struct StripTile {
  ...
  bool operator<(const StripTile& other) const {
...
class SplitViewBrowserData : public BrowserUserData<SplitViewBrowserData> {
 public:
  ...
  void TileTabs(const StripTile& tile);

  void BreakTile(const tabs::TabHandle& tab);

  bool IsTabTiled(const tabs::TabHandle& tab) const;

  void SwapTabsInTile(const StripTile& tile);

  std::optional<StripTile> GetTile(const tabs::TabHandle& tab) const;

  const std::vector<StripTile>& tiles() const { return tiles_; }
 ...
 private:
  ...
  std::vector<StripTile>::iterator FindTile(const tabs::TabHandle& tab);
  std::vector<StripTile>::const_iterator FindTile(
      const tabs::TabHandle& tab) const;

  void Transfer(SplitViewBrowserData* other, std::vector<StripTile> tiles);

  SplitViewTabStripModelAdapter tab_strip_model_adapter_;

  std::vector<StripTile> tiles_;
  std::vector<StripTile> tiles_to_be_attached_to_new_window_;
