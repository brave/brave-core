...
class SplitViewTabStripModelAdapter : public TabStripModelObserver {
 public:
  ...
  void MakeTiledTabsAdjacent(const StripTile& tile, bool move_right_tab = true);
  bool SynchronizeGroupedState(const StripTile& tile,
                               const tabs::TabHandle& source,
                               std::optional<tab_groups::TabGroupId> group);
  bool SynchronizePinnedState(const StripTile& tile,
                              const tabs::TabHandle& source);
