...
void SplitViewTabStripModelAdapter::MakeTiledTabsAdjacent(const StripTile& tile,
                                                          bool move_right_tab) {
...
void SplitViewTabStripModelAdapter::TabDragEnded() {
  ...
  // the tiles.
  std::vector<StripTile> tiles_to_break;
...
void SplitViewTabStripModelAdapter::TabPinnedStateChanged(
  ...
  base::SequencedTaskRunner::GetCurrentDefault()->PostTask(
      FROM_HERE, base::BindOnce(
                     [](base::WeakPtr<SplitViewTabStripModelAdapter> adapter,
                        StripTile tile, tabs::TabHandle source_tab) {
                       if (!adapter) {
                         return;
                       }
...
void SplitViewTabStripModelAdapter::TabGroupedStateChanged(
  ...
  base::SequencedTaskRunner::GetCurrentDefault()->PostTask(
      ...
      base::BindOnce(
          [](base::WeakPtr<SplitViewTabStripModelAdapter> adapter,
             StripTile tile, const tabs::TabHandle& source,
             std::optional<tab_groups::TabGroupId> group) {
            if (!adapter) {
              return;
            }
