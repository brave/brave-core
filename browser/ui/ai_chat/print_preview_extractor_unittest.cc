 ...
 public:
  ...
  void StartExtract(
    ...
    if (auto* text_cb = std::get_if<TextCallback>(&callback)) {
      ...
      if (!expected_error_) {
        std::vector<std::vector<uint8_t>> result = {{0xde, 0xea}, {0xbe, 0xef}};
...
class PrintPreviewExtractorTest : public ChromeRenderViewHostTestHarness {
 ...
 protected:
  ...
  void VerifyPrintPreviewUIState() {
    EXPECT_TRUE(
  ...
  void RunTestCase(base::test::TestFuture<T>& future,
    ...
    if (!expect_error || extractor_error) {
      ...
      // Simulate partial composition without OnCompositeToPdfDone
      internal_extractor()->OnCompositePdfPageDone(
          0, 0, 0, printing::mojom::PrintCompositor::Status::kSuccess,
          CreatePageRegion(8).region);
      internal_extractor()->OnCompositePdfPageDone(
      ...
      // For missing preview data case, just trigger preview ready
      internal_extractor()->OnPreviewReady();
    }
    ...
    if (expect_error) {
      ...
      if constexpr (std::is_same_v<T, ImageResult>) {
        const std::vector<std::vector<uint8_t>> expected = {{0xde, 0xea},
                                                            {0xbe, 0xef}};
