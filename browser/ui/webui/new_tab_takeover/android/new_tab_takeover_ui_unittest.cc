...
// Real omnibox providers complete synchronously against a bare test
// profile, so `Start()` is overridden here to leave queries permanently
// pending instead.
class FakeAutocompleteController final : public AutocompleteController {
 public:
  explicit FakeAutocompleteController(Profile* const profile)
      : AutocompleteController(
            std::make_unique<ChromeAutocompleteProviderClient>(profile),
            AutocompleteControllerConfig{.provider_types = 0}) {}

  FakeAutocompleteController(const FakeAutocompleteController&) = delete;
  FakeAutocompleteController& operator=(const FakeAutocompleteController&) =
      delete;

  ~FakeAutocompleteController() override = default;
 ...
 public:
  void SetUp() override {
    ...
    ASSERT_TRUE(base::test::RunUntil(
        [this] { return template_url_service()->loaded(); }));
  ...
  void UseNonRespondingAutocompleteController() {
    new_tab_takeover_ui_->SetAutocompleteControllerForTesting(
        std::make_unique<FakeAutocompleteController>(profile()));
  }
  ...
  // `TemplateURLService::Remove()` forbids removing the current default
  // search provider, and Brave Search is the default out of the box.
  void SetDefaultSearchProviderToNonBraveSearchEngine() {
...
TEST_F(NewTabTakeoverUITest, SupersedingQueryResolvesPendingCallbackEmpty) {
  UseNonRespondingAutocompleteController();
...
TEST_F(NewTabTakeoverUITest,
  ...
  SetDefaultSearchProviderToNonBraveSearchEngine();
