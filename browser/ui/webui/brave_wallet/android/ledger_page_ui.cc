...
#include "brave/components/brave_wallet_page/resources/grit/brave_wallet_ledger_page_generated_map.h"
...
LedgerPageUI::LedgerPageUI(content::WebUI* web_ui, const std::string& name)
  ...
  webui::SetupWebUIDataSource(
      source,
      base::make_span(kBraveWalletLedgerPageGenerated,
                      kBraveWalletLedgerPageGeneratedSize),
      IDR_BRAVE_WALLET_LEDGER_PAGE_HTML);
...
LedgerPageUI::~LedgerPageUI() = default;

WEB_UI_CONTROLLER_TYPE_IMPL(LedgerPageUI)

void LedgerPageUI::BindInterface(
    mojo::PendingReceiver<brave_wallet::mojom::PageHandlerFactory> receiver) {
  page_factory_receiver_.reset();
  page_factory_receiver_.Bind(std::move(receiver));
}

void LedgerPageUI::CreatePageHandler(
