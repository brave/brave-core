...
IN_PROC_BROWSER_TEST_F(TabSearchPageHandlerBrowserTest, GetFocusTabs) {
  ...
  AppendTabWithTitle(browser2->GetBrowserForMigrationOnly(),
                     GURL(kBarDotComUrl1), kBarDotComTitle1);
  AppendTabWithTitle(browser2->GetBrowserForMigrationOnly(),
                     GURL(kBarDotComUrl2), kBarDotComTitle2);
  ...
  EXPECT_EQ(active_browser->GetBrowserForMigrationOnly()->user_title(),
            "topic");
  ...
  handler()->UndoFocusTabs(base::BindLambdaForTesting([&]() {
    ...
    ASSERT_TRUE(TestBrowserClosedWaiter(
                    GetLastActiveBrowserWindowInterfaceWithAnyProfile()
