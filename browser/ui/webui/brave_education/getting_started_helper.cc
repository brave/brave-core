...
std::optional<EducationContentType> GetEducationContentType() {
  if (base::FeatureList::IsEnabled(features::kShowGettingStartedPage)) {
    return features::kGettingStartedContentType.Get();
  }
  return std::nullopt;
}
...
void GettingStartedHelper::GetEducationURL(GetEducationURLCallback callback) {
  ...
  // TODO(zenparsing): We're probably going to need to include the browser
  // version in the server URLs.
  // TODO(zenparsing): Consider the effect of redirects.
  ...
  url_loader_->DownloadToString(
      ...
      base::BindOnce(&GettingStartedHelper::OnURLResponse,
                     base::Unretained(this), std::move(webui_url)),
      kMaxDownloadBytes);
}

void GettingStartedHelper::OnURLResponse(GURL webui_url,
                                         std::optional<std::string> body) {
  std::optional<GURL> result;
  if (body.has_value()) {
    result = std::move(webui_url);
  }
  RunCallbacks(std::move(result));
}
