...
// `ModelService::OnRemoteModelsSurfaceVisible()`/
// `OnRemoteModelsSurfaceHidden()`, which gate the remote model list's refresh
// timer. Side-panel "switch to a different entry" caches the `WebContents`
// alive rather than destroying it (`SidePanelEntry::CacheView()`), so
// construction/destruction of the owning `AIChatUI` is not a reliable close
// signal — visibility is.
