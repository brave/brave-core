...
#include "brave/browser/ui/webui/brave_account/allow_brave_account_tag.h"
...
void BraveAccountNavigationThrottle::MaybeCreateAndAdd(
  ...
  registry.AddThrottle(
      std::make_unique<BraveAccountNavigationThrottle>(registry));
}
...
BraveAccountNavigationThrottle::WillStartRequest() {
  return AllowBraveAccountTag::IsSet(navigation_handle()->GetWebContents())
             ? PROCEED
