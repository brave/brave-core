...
constexpr std::string_view kExternalWalletStatusKey = "status";
...
#if BUILDFLAG(ENABLE_BRAVE_REWARDS)
  std::string_view external_wallet_type =
      pref_service_->GetString(brave_rewards::prefs::kExternalWalletType);
  if (external_wallet_type.empty()) {
    return false;
  }
  const base::DictValue* const dict =
      pref_service_->GetDict(brave_rewards::prefs::kExternalWallets)
          .FindDict(external_wallet_type);
  if (!dict) {
    return false;
  }
  std::optional<int> status = dict->FindInt(kExternalWalletStatusKey);
  return status.has_value() &&
         *status != std::to_underlying(
