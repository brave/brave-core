...
#include "brave/grit/brave_origin_startup_resources.h"
#include "brave/grit/brave_origin_startup_resources_map.h"
...
void CreateAndAddDataSource(content::WebUI* web_ui, Profile* profile) {
  ...
  webui::SetupWebUIDataSource(
      source,
      base::span<const webui::ResourcePath>(kBraveOriginStartupResources),
      IDR_BRAVE_ORIGIN_STARTUP_BRAVE_ORIGIN_STARTUP_HTML);
...
void BraveOriginStartupUI::CreateHandler(
  ...
  handler_ = std::make_unique<BraveOriginStartupHandler>(
      browser_context, local_state, std::move(open_buy_window_callback),
      std::move(close_dialog_callback));
