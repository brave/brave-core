...
UntrustedCandleBertUI::UntrustedCandleBertUI(content::WebUI* web_ui)
    : ui::MojoWebUIController(web_ui), candle_service_receiver_(this) {
...
void UntrustedCandleBertUI::BindInterface(
  ...
  candle_service_receiver_.reset();
  candle_service_receiver_.Bind(std::move(receiver));
}

void UntrustedCandleBertUI::BindBert(
    mojo::PendingRemote<mojom::BertInterface> bert_interface) {
  bert_remote_.reset();
  bert_remote_.Bind(std::move(bert_interface));

  // Load model files using shared helper
