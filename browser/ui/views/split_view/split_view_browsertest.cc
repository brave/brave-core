...
IN_PROC_BROWSER_TEST_F(SplitViewBrowserTest, MiniToolbarUsesSplitBorderTest) {
  NewSplitTab();
  ...
  // The menu button acts on the split, so it is hidden when the toolbar meets
  // the contents outline instead of a split border.
  mini_toolbar->SetUsesSplitBorder(false);
  mini_toolbar->UpdateState(/*is_active*/ true, /*is_highlighted*/ false);
  EXPECT_FALSE(menu_button->GetVisible());

  mini_toolbar->SetUsesSplitBorder(true);
  mini_toolbar->UpdateState(/*is_active*/ true, /*is_highlighted*/ false);
  EXPECT_TRUE(menu_button->GetVisible());
}
