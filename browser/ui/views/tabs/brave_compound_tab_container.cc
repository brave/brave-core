...
//     // |unpinned_tab_container_| so that clean up can be done by upstream
//     code. SetScrollEnabled(false);
//   }
// }
...
//       updating
//       // tooltip. In order to avoid that, we remove the view from the old
//       parent
...
//   // Override transfer animation so that it goes well with vertical tab
//   strip. CompleteAnimationAndLayout();
...
//         contents_bounds.x(), pinned_tab_container_->bounds().bottom(),
//         width(), contents_bounds.height() - pinned_tab_container_->height());
...
//         pinned_tab_container_->bounds().bottom(),
//                   contents_bounds.width(),
//                   contents_bounds.height() -
//                   pinned_tab_container_->height()));
//   }
// }
...
//         std::min(combined_height,
//         region_view->GetTabStripViewportMaxHeight()));
//     break;
//   }
...
//   // which returns 0 and triggers a check. That check doesn't make sense
//   since
//   // if I am adding 2 pinned tabs with model_indexes 0 and 1, when checking
//   // for model_index 1 NumPinnedTabs() would still return 0 as we have not
//   added
...
//     if (scroll_view_ && pinned == TabPinned::kUnpinned &&
//     new_tab->IsActive()) {
...
// gfx::Rect
// BraveCompoundTabContainer::ConvertUnpinnedContainerIdealBoundsToLocal(
...
//         /*source=*/base::to_address(unpinned_tab_container_),
//         /*target=*/this, ideal_bounds);
//   }
