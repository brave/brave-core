...
void BraveTabContainer::StartInsertTabAnimation(int model_index) {
  ...
  // Use layout_helper_ to determine pinned state instead of data().pinned,
  // because SetData() has not been called yet at this point in the tab
  // insertion flow (TabStrip::AddTabsAt calls AddTabs/StartInsertTabAnimation
  // before SetData). This caused restored pinned tabs in vertical tab mode to
  // get incorrect initial bounds, leading to missing favicons.
  const bool is_pinned = model_index < layout_helper_->GetPinnedTabCount();
  const auto tab_width = is_pinned ? tabs::kVerticalTabMinWidth
