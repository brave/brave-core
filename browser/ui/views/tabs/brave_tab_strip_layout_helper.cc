...
void CalculateVerticalLayout(const std::vector<TabWidthConstraints>& tabs,
  ...
  for (auto iter = tabs.begin() + result->size(); iter != tabs.end(); iter++) {
    ...
    if (int level = iter->state().nesting_info().level) {
      // Base offset to be indented per level.
      constexpr int kBaseOffsetPerLevel = 20;

      // Check if the tab can be indented. If the tab is narrower than the
      // minimum width, we can't use the kBaseOffsetPerLevel and need to use
      // smaller offset that dymamically calculated.
      ...
      if (rect.width() - offset * 2 < tab_minimum_width) {
        // Calculate the new offset considering the height of the tree that
        // this tab belongs to.

        // When there's two nodes, tree_height is 1.
        const int tree_height = tab.state().nesting_info().tree_height;

        // Offset should evenly distribute the |available_width_for_tree|
