...
std::optional<containers::ContainerModel> GetContainerModelForWebContents(
  ...
  if (it != models.end()) {
    return std::optional<containers::ContainerModel>(std::move(*it));
  }
  return std::optional<containers::ContainerModel>(
      containers::ContainerModel::CreateForUnknown(container_id,
                                                   kDefaultScaleFactor));
}
