...
TEST_F(DownloadBubbleTest, BraveDownloadCommands_CopyDownloadLink) {
  ...
  BraveDownloadCommands commands(model_.GetWeakPtr());
  EXPECT_TRUE(
      commands.IsCommandEnabled(BraveDownloadCommands::COPY_DOWNLOAD_LINK));

  // Check if executing the command copies the URL to clipboard.
  auto* clipboard = ui::Clipboard::GetForCurrentThread();
  clipboard->Clear(ui::ClipboardBuffer::kCopyPaste);
  commands.ExecuteCommand(BraveDownloadCommands::COPY_DOWNLOAD_LINK);
