...
class FocusModeTopOverlay : public views::View,
 ...
 public:
  ...
  class RevealLock {
   public:
    explicit RevealLock(base::WeakPtr<FocusModeTopOverlay> owner);
    RevealLock(const RevealLock&);
    RevealLock& operator=(const RevealLock&);
    RevealLock(RevealLock&&);
    RevealLock& operator=(RevealLock&&);
    ~RevealLock();

   private:
    base::WeakPtr<FocusModeTopOverlay> owner_;
  };
  ...
  RevealLock AcquireRevealLock() {
    return RevealLock(weak_factory_.GetWeakPtr());
  }
 ...
 private:
  ...
  void SetRevealLock(std::optional<RevealLock>& lock, bool should_lock);
  void OnRevealLockCreated();
  void OnRevealLockDestroyed();
  ...
  bool active_ = false;
  size_t reveal_lock_count_ = 0;

  std::optional<RevealLock> temporary_reveal_lock_;
  std::optional<RevealLock> visible_transient_lock_;
  ...
  base::WeakPtrFactory<FocusModeTopOverlay> weak_factory_{this};
};
