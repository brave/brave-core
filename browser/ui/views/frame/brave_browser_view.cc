...
bool BraveBrowserView::IsActiveWebContentsTiled(const StripTile& tile) {
  CHECK(base::FeatureList::IsEnabled(tabs::features::kBraveSplitView));
...
void BraveBrowserView::OnTileTabs(const StripTile& tile) {
...
void BraveBrowserView::OnWillBreakTile(const StripTile& tile) {
  if (!IsActiveWebContentsTiled(tile)) {
    return;
  }
...
void BraveBrowserView::OnSwapTabsInTile(const StripTile& tile) {
  if (!IsActiveWebContentsTiled(tile)) {
    return;
  }
