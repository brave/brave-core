...
// `PrintPreviewExtractor` pipe captures through `screenshot:ScaleDownBitmap`,
// which centers the scaled content inside a fixed 1024x768 canvas and fills the
// remainder with SkColorTRANSPARENT. We crop that padding off before stitching,
// otherwise every chunk contributes empty vertical (or horizontal) bands.
...
void ScreenshotController::CaptureFullPage(content::WebContents* web_contents,
  ...
  if (!print_preview_extractor_) {
    print_preview_extractor_ = screenshot::CreatePrintPreviewExtractor(
        web_contents,
        base::BindRepeating(
...
void ScreenshotController::ShowSaveDialog(std::vector<uint8_t> png) {
  ...
  base::FilePath download_dir =
      download_dir_getter_
          ? download_dir_getter_.Run()
