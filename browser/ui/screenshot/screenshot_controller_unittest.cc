 ...
 protected:
  void SetUp() override {
    ...
    controller_ = std::make_unique<ScreenshotController>(
        ...
        base::BindRepeating(
            [](base::FilePath dir) { return dir; }, download_dir));
  }
...
TEST_F(ScreenshotControllerTest,
  ...
  // Let the first pipeline drain (kNotImplemented → empty bitmap → kCaptureFailed).
...
TEST_F(ScreenshotControllerTest,
       Pipeline_EmptyBitmap_ReturnsCaptureFailed) {
  base::test::TestFuture<Result> future;
...
TEST_F(ScreenshotControllerTest, Pipeline_FileSelected_WritesToDiskAndReturnsPath) {
  base::test::TestFuture<void> dialog_opened;
  dialog_factory_->SetOpenCallback(dialog_opened.GetRepeatingCallback());
...
TEST_F(ScreenshotControllerTest, Pipeline_WriteToUnwritablePath_ReturnsWriteFailed) {
  base::test::TestFuture<void> dialog_opened;
  dialog_factory_->SetOpenCallback(dialog_opened.GetRepeatingCallback());
