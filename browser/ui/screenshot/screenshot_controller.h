...
// Owned by BraveToolbarView, one per Browser. Orchestrates capture of a
...
class ScreenshotController : public ui::SelectFileDialog::Listener {
 public:
  ...
  // Returns false if `wc` has no live RenderWidgetHostView or a capture is
  // already in flight.
  bool CanCapture(content::WebContents* wc) const;

  void CaptureVisibleArea(content::WebContents* wc, ResultCallback done);
  void CaptureFullPage(content::WebContents* wc, ResultCallback done);
 ...
 private:
  ...
  std::unique_ptr<ai_chat::FullScreenshotter> full_screenshotter_;
#if BUILDFLAG(ENABLE_PRINT_PREVIEW)
  std::unique_ptr<ai_chat::PrintPreviewExtractor> print_preview_extractor_;
#endif
