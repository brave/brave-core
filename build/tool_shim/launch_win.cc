...
base::CommandLine::StringType CreateCmdLine(
  ...
  std::vector<base::CommandLine::StringType> quoted_argv;
  quoted_argv.reserve(argv.size());
  for (const auto& arg : argv) {
    if (arg.empty()) {
      quoted_argv.push_back(FILE_PATH_LITERAL("\"\""));
    } else {
      quoted_argv.push_back(base::CommandLine::QuoteForCommandLineToArgvW(arg));
    }
  }
  return base::JoinString(quoted_argv, FILE_PATH_LITERAL(" "));
}
...
std::ostream& operator<<(std::ostream& os, const LaunchConfig& config) {
  ...
  argv.reserve(config.argv.size());
  for (const auto& arg : config.argv) {
    argv.push_back(base::WideToUTF8(arg));
  }
  os << "Launch config:"
     << "\ncmd: " << base::JoinString(argv, " ") << "\ncwd: " << config.cwd;
  if (!config.env.empty()) {
    os << "\nenv:";
    for (const auto& [key, val] : config.env) {
      os << "\n  " << base::WideToUTF8(key) << "=" << base::WideToUTF8(val);
    }
  }
  return os;
}
