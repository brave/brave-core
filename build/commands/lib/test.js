...
const getBraveUnitTestsSuites = (config) => {
  ...
  if (config.targetOS !== 'android' && config.targetOS !== 'ios') {
    ...
    if (config.targetOS === 'ios') {
      ...
      if (options.output_xml) {
        util.run(
          'python3',
          [
            path.join('script', 'chromium_to_junit_converter.py'),
            path.join(outputDir, 'output', 'full_results.json'),
            '-o',
            `${outputFilename}.xml`,
          ],
          {
