...
const mergeWithDefault = (options: { outputDir?: any }) => {
...
async function applyPatches() {
  ...
  v8PatchStatus.forEach((s: { path: any }) => s.path = path.join('v8', s.path))
  catapultPatchStatus.forEach(
(    s: { path: any }) => s.path = path.join('third_party', 'catapult', s.path))
  devtoolsFrontendPatchStatus.forEach(
(    s: { path: any }) => s.path = path.join('third_party', 'devtools-frontend', 'src', s.path))
...
const isOverrideNewer = (original: string, override) => {
  return (fs.statSync(override).mtimeMs - fs.statSync(original).mtimeMs > 0)
}

const updateFileUTimesIfOverrideIsNewer = (original: string, override) => {
...
const deleteFileIfOverrideIsNewer = (original: string, override) => {
...
const util = {
  ...
  runAsync: (cmd, args = [], options = {}) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      prog.on('close', (statusCode: number) => {
  ...
  runGitAsync: function (repoPath, gitArgs: any[], verbose = false, logError = false) {
    return util.runAsync('git', gitArgs, { cwd: repoPath, verbose, continueOnFail: true })
      .catch((err: { message: any; stdout: any; stderr: any }) => {
        if (logError) {
  ...
  updateBranding: () => {
    ...
    if (config.targetOS === 'android') {
      ...
      let braveOverwrittenFiles: Set<any> = new Set();
      const removeUnlistedAndroidResources = (braveOverwrittenFiles: { has: (arg0: any) => any }) => {
  ...
  touchOverriddenChromiumSrcFiles: () => {
    ...
    sourceFiles.forEach((chromiumSrcFile: string | any[]) => {
    ...
    if (isDirty && config.rbeService) {
      ...
      if (fs.existsSync(reproxyCacheDir)) {
        const cacheFileFilter = (file: { endsWith: (arg0: string) => any }) => {
  ...
  touchOverriddenVectorIconFiles: () => {
    ...
    braveVectorIconFiles.forEach((braveVectorIconFile: string | any[]) => {
  ...
  runGClient: (args: string[], options = {}, gClientFile = config.gClientFile) => {
  ...
  buildArgsToString: (buildArgs: { [x: string]: any }) => {
    let args = ''
  ...
  addGitExclusion: (repoDir, exclusion: string) => {
