...
const getTestGroupDeps = (filename) => {
  const testDepFile = path.join(config.outputDir, filename)
  if (fs.existsSync(testDepFile)) {
    let suiteDepNames = JSON.parse(
...
const getTestsToRun = (config, suite) => {
  let testsToRun =
    getTestGroupDeps(path.join(config.outputDir, `${suite}.json`))

  if (config.isCI) {
    testsToRun += getTestGroupDeps(path.join(config.outputDir,
      `${suite}_ci.json`))
  }
