...
exports.update = () => {
  ...
  // Map brand-specific string files to the same name (not the chromium
  // equivalent name) for copying back. This covers both 'brave' and
  // 'brave_origin' brands.
  const brands = ['brave', 'brave_origin']
  for (const brand of brands) {
    autoGeneratedBraveToChromiumMapping[
      path.join(braveAppDir, `${brand}_strings.grd`)
    ] = path.join(chromeAppDir, `${brand}_strings.grd`)
    autoGeneratedBraveToChromiumMapping[
      path.join(braveAppDir, `settings_${brand}_strings.grdp`)
    ] = path.join(chromeAppDir, `settings_${brand}_strings.grdp`)
    autoGeneratedBraveToChromiumMapping[
      path.join(braveComponentsDir, `components_${brand}_strings.grd`)
    ] = path.join(chromeComponentsDir, `components_${brand}_strings.grd`)
  }
