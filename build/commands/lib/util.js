    ...
    return new Promise((resolve, reject) => {
      ...
      prog.on('close', (statusCode, signal) => {
        if (config.isCI && (statusCode || signal)) {
          ...
          // SIGTERM propagates to child processes and causes autoninja to exit
          // with code 1. The delay gives Node time to terminate directly from
          // the SIGTERM before we process the child's exit code.
          //
          // Hard termination is required to mark build as interrupted so we can
          // perform a cleanup on next run.
          setTimeout(() => {
