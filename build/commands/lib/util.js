...
const util = {
  ...
  buildNativeRedirectCC: async () => {
    ...
    await util.buildTargets(
      ...
      mergeWithDefault({ outputDir: config.nativeRedirectCCDir })
    )
  ...
  writeGnBuildArgs: (outputDir, buildArgs) => {
    ...
    const generatedArgsImportLine = `import("//${path.relative(config.srcDir, generatedArgsFilePath).replace(/\\/g, '/')}")`
