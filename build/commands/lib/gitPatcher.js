...
module.exports = class GitPatcher {
  ...
  async applyPatches() {
    ...
    if (!repoDirExists) {
      throw new Error(
        `Could not apply patches. Repo at path "${this.repoPath}" does not exist.`,
      )
    }
    const allFilenames = await fs.readdir(this.patchDirPath)
    const patchFilenames = allFilenames.filter((s) =>
      s.endsWith(`.${extPatch}`),
    )
    const patchInfoFilenames = allFilenames.filter((s) =>
      s.endsWith(`.${extPatchInfo}`),
    )
    ...
    for (const filename of patchFilenames) {
      const patchInfoFilename =
        filename.slice(0, extPatch.length * -1) + extPatchInfo
    ...
    for (const filename of patchInfoFilenames) {
      const patchFilename =
        filename.slice(0, extPatchInfo.length * -1) + extPatch
    ...
    try {
      if (patchesToApply.length) {
        const appliedPathsStatuses =
          await this.performApplyForPatches(patchesToApply)
        pathStatuses.push(...appliedPathsStatuses)
      }
      if (patchInfosObsolete.length) {
        const resetStatuses =
          await this.handleObsoletePatchInfos(patchInfosObsolete)
        pathStatuses.push(...resetStatuses)
      }
    } catch (err) {
      console.error(err)
      console.error(
        'There was an error applying added, modified or removed patches. Please consider running `init` to reset and re-apply all patches.',
      )
    }
    return pathStatuses
  }
  ...
  async performApplyForPatches(patchesToApply) {
    ...
    for (const patchData of patchesToApply) {
      ...
      prepOps.push(
        this.getAppliesTo(patchData.patchPath)
          ...
          .catch((err) => ({
            error: new Error(
              'Could not read data from patch file: ' + err.message,
            ),
            ...patchData,
          }))
          .then((data) => {
    ...
    // (in parallel)
    const patchInfoOps = []
    for (const { appliesTo, patchPath, patchInfoPath } of patchSets.filter(
      (p) => !p.error,
    )) {
      patchInfoOps.push(
        this.writePatchInfo(patchInfoPath, appliesTo, patchPath),
      )
    }
    ...
    return patchSets.reduce((all, { appliesTo, patchPath, error, reason }) => {
  ...
  async getAppliesTo(patchPath) {
    ...
    return (
      (await util.runGitAsync(this.repoPath, applyStatArgs))
        .split(os.EOL)
        .filter((s) => s)
        // Intrepret `--numstat -z` line format
        .map((s) => ({
  ...
  async writePatchInfo(patchInfoPath, appliesTo, patchPath) {
    for (const appliesToFile of appliesTo) {
      appliesToFile.checksum = await calculateFileChecksum(
        path.join(this.repoPath, appliesToFile.path),
      )
    }
    const patchInfo = {
    ...
    await fs.writeFile(patchInfoPath, JSON.stringify(patchInfo), {
      encoding: encodingPatchInfo,
    })
  }
  ...
  async handleObsoletePatchInfos(patchInfosObsolete) {
    ...
    for (const patchInfoPath of patchInfosObsolete) {
      ...
      const removeOp = fs
        .unlink(patchInfoPath)
        // Handle error removing patch info, not fatal error
        .catch((err) => {
          this.logProgressLine(
            `Warning: Could not remove obsolete PatchInfo file at path ${patchInfoPath}: "${err.message}"`,
          )
        })
      ops.push(removeOp)
      if (!patchInfo) {
    ...
    const resetOp = this.resetRepoFiles(allPaths).catch(() => {
