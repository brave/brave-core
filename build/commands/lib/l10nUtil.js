...
// brave_origin_strings.grd mappings (same source as brave_strings.grd but for
// Brave Origin branding)
const braveOriginStringsPath = path.resolve(
  path.join(srcDir, 'brave', 'app', 'brave_origin_strings.grd'),
)
const braveOriginSettingsPartPath = path.resolve(
  path.join(srcDir, 'brave', 'app', 'settings_brave_origin_strings.grdp'),
)
...
function getChromiumToAutoGeneratedBraveMapping() {
  if (typeof getChromiumToAutoGeneratedBraveMapping.mapping === 'undefined') {
    ...
    // organized. The first mappings are added explicitly because we change the
    // file names.
...
// Brave Origin uses the same Chromium source files as regular Brave strings,
// but outputs to different destination files. This is kept as a separate list
// since JavaScript objects can't have duplicate keys.
function getChromiumToAutoGeneratedBraveOriginMapping() {
  if (
    typeof getChromiumToAutoGeneratedBraveOriginMapping.mapping === 'undefined'
  ) {
    getChromiumToAutoGeneratedBraveOriginMapping.mapping = {
      [chromiumSettingsPartPath]: braveOriginSettingsPartPath,
      [chromiumStringsPath]: braveOriginStringsPath,

      [chromiumComponentsChromiumStringsPath]:
        braveComponentsBraveOriginStringsPath,
    }
  }
  return getChromiumToAutoGeneratedBraveOriginMapping.mapping
}
...
const l10nUtil = {
  ...
  getAutoGeneratedBraveToChromiumMapping: () => {
    if (
      ...
      const chromiumToAutoGeneratedBraveOriginMapping =
        getChromiumToAutoGeneratedBraveOriginMapping()
      ...
      // Also add Brave Origin reverse mappings
      Object.keys(chromiumToAutoGeneratedBraveOriginMapping).forEach((key) => {
        l10nUtil.getAutoGeneratedBraveToChromiumMapping.mapping[
          chromiumToAutoGeneratedBraveOriginMapping[key]
        ] = key
      })
    }
  ...
  getBraveAutoGeneratedPaths: () => {
    return Object.values(getChromiumToAutoGeneratedBraveMapping()).concat(
      Object.values(getChromiumToAutoGeneratedBraveOriginMapping()),
    )
  },
  ...
  rebaseBraveStringFilesOnChromiumL10nFiles: async () => {
    ...
    // Combine both regular Brave and Brave Origin mappings
    const allMappings = {
      ...getChromiumToAutoGeneratedBraveMapping(),
    }
    // Add Brave Origin mappings separately since they share the same sources
    const braveOriginMappings = getChromiumToAutoGeneratedBraveOriginMapping()

    const ops = Object.entries(allMappings).map(
    ...
    // Also copy for Brave Origin
    const braveOriginOps = Object.entries(braveOriginMappings).map(
      async ([sourcePath, destPath]) => {
        console.log(
          'Resetting '
            + path.relative(srcDir, destPath)
    ...
    await Promise.all([...ops, ...braveOriginOps])
    return removedMap
  },
} // const l10nUtil
