 ...
 * - Lists of files to be translated (all top level GRD and JSON files)
 * - All mappings for auto-generated Brave files from Chromium files.
 * - Top level global string replacements, such as Chromium -> Brave
 */
...
const braveOriginStringsPath = path.resolve(
  path.join(srcDir, 'brave', 'app', 'brave_origin_strings.grd'),
)
const braveStringsMainPath = path.resolve(
  path.join(srcDir, 'brave', 'app', 'brave_strings_main.grdp'),
)
...
const braveComponentsBraveOriginStringsPath = path.resolve(
  path.join(
    srcDir,
    'brave',
    'components',
...
const braveComponentsBraveStringsMainPath = path.resolve(
  path.join(
    srcDir,
    'brave',
    'components',
    'components_brave_strings_main.grdp',
  ),
)
...
function convertGrdToGrdpAndShells(
  ...
  const doc = grd.window.document

  // Extract any comment that appears before the <grit> element
  const commentMatch = grdContent.match(
    /(<\?xml[^?]*\?>)\s*(<!--[\s\S]*?-->)?\s*<grit/,
  )
  const sourceComment =
    commentMatch && commentMatch[2] ? commentMatch[2] + '\n' : ''

  // Extract messages content for GRDP using regex to preserve original
  // formatting
  const messagesMatch = grdContent.match(
    /<messages[^>]*>([\s\S]*?)<\/messages>/,
  )
  if (!messagesMatch) {
    throw new Error(`No <messages> element found in ${grdPath}`)
  }
  const messagesInnerContent = messagesMatch[1]
  const grdpFilename = path.basename(grdpPath)

  // Create GRDP content - generate comment from shell GRD filenames
  const shellGrdNames = shellGrdPaths.map((p) => path.basename(p)).join(' and ')
  const grdpContent = `<?xml version="1.0" encoding="UTF-8"?>
<!-- Common branded strings shared between ${shellGrdNames} -->
<grit-part>${messagesInnerContent}
</grit-part>
`
  console.log(`Writing GRDP: ${path.relative(srcDir, grdpPath)}`)
  fs.writeFileSync(grdpPath, grdpContent)

  // Generate shell GRD with outputs, translations, and part reference
  const outputsElement = doc.querySelector('outputs')
  const translationsElement = doc.querySelector('translations')
  const gritElement = doc.querySelector('grit')

  if (!outputsElement || !translationsElement || !gritElement) {
    throw new Error(`Missing required elements in ${grdPath}`)
  }

  // Get grit attributes
  const gritAttrs = Array.from(gritElement.attributes)
  ...
  // Create shell GRD content
  const shellGrdContent = `<?xml version='1.0' encoding='UTF-8'?>
${sourceComment}<!--This file is created by l10nUtil.js. Do not edit.-->
<grit ${gritAttrs}>
  ${outputsHtml}
  ${translationsHtml}
  <release seq="1">
    <messages fallback_to_english="true">
      <part file="${grdpFilename}" />
    </messages>
  </release>
</grit>
`

  // Write to all shell GRD paths (brave and brave_origin)
  for (const shellPath of shellGrdPaths) {
    console.log(`Writing shell GRD: ${path.relative(srcDir, shellPath)}`)
    fs.writeFileSync(shellPath, shellGrdContent)
  }
}
...
function getChromiumToAutoGeneratedBraveMapping() {
  if (typeof getChromiumToAutoGeneratedBraveMapping.mapping === 'undefined') {
    ...
    // names.
    // Note: chromiumStringsPath and chromiumComponentsChromiumStringsPath are
    // handled specially in rebaseBraveStringFilesOnChromiumL10nFiles() because
    // they generate both GRDP files and multiple shell GRD files.
...
const l10nUtil = {
  // Same as chromiumToAutoGeneratedBraveMapping but maps in reverse direction
  ...
  getBraveAutoGeneratedPaths: () => {
    // Include paths from the mapping plus the special cases handled separately
    return [
      ...Object.values(getChromiumToAutoGeneratedBraveMapping()),
      // brave_strings special case: GRDP + shell GRDs
      braveStringsMainPath,
      braveStringsPath,
      braveOriginStringsPath,
      // components_brave_strings special case: GRDP + shell GRDs
      braveComponentsBraveStringsMainPath,
      braveComponentsBraveStringsPath,
      braveComponentsBraveOriginStringsPath,
    ]
  },
  ...
  // Crowdin manages files per grd and not per grd or grdp. This is because
  // only 1 xtb is created per grd per locale even with multiple grdp files.
  ...
  logRemovedGRDParts: (mapping) => {
    if (mapping.size) {
      ...
      console.log(
        'The following GRDPs are no longer in the corresponding GRDs:\n',
      )
  ...
  // This copies Chromium files to their mapped Brave equivalents.
  // Special handling for brave_strings and components_brave_strings which
  // generate GRDP files and multiple shell GRDs (for brave and brave_origin).
  rebaseBraveStringFilesOnChromiumL10nFiles: async () => {
    ...
    console.log('Processing components_brave_strings (GRDP + shell GRDs)...')
    convertGrdToGrdpAndShells(
      chromiumComponentsChromiumStringsPath,
      braveComponentsBraveStringsMainPath,
      [braveComponentsBraveStringsPath, braveComponentsBraveOriginStringsPath],
    )

    // Handle regular mappings (simple file copies)
