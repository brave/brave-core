...
/**
 * Lazily computes and caches a value, keyed by the caller's function reference.
 * @param {Function} fn - The calling function, used as the cache key.
 * @param {() => T} compute - Factory that produces the value on first access.
 * @returns {T}
 * @template T
 */
function memoize(fn, compute) {
  if (!_memoizeCache.has(fn)) {
    _memoizeCache.set(fn, compute())
  }
  return _memoizeCache.get(fn)
}
...
function getAutoGeneratedGrdMappings() {
  return memoize(getAutoGeneratedGrdMappings, () => {
...
function getChromiumToAutoGeneratedBraveMapping() {
  return memoize(getChromiumToAutoGeneratedBraveMapping, () => {
...
const l10nUtil = {
  ...
  getAutoGeneratedBraveToChromiumMapping: () => {
    return memoize(l10nUtil.getAutoGeneratedBraveToChromiumMapping, () => {
      const chromiumToAutoGeneratedBraveMapping =
        getChromiumToAutoGeneratedBraveMapping()
      return Object.keys(chromiumToAutoGeneratedBraveMapping).reduce(
        (obj, key) => ({
          ...obj,
  ...
  getBraveNonGeneratedPaths: () => {
    return memoize(l10nUtil.getBraveNonGeneratedPaths, () => {
      return [
