...
module.exports = (program) =>
  program
    ...
    .action(async (buildConfig, args) => {
      ...
      const recordings = await Array.fromAsync(glob(`${out}/${dirsGlob}/*.profraw`))
      ...
      const allTestSuites = [...new Set(recordings
        .map(x =>x.replace(out+'/',''))
        .map(x => x.split('/'))
        .filter(x => x.length > 1)
        .map(x => x[0]))
      ...
      const hasBBT = allTestSuites.find(x => x.includes('brave_browser_tests'));
      const testSuites = hasBBT 
        ? ['brave_browser_tests']
        : allTestSuites

      if (!recordings.length) {
        console.error(`glob ${out}/${dirsGlob}/*.profraw yield any recordings!\n make sure you've built with --is_coverage and ran the appropiate tests`)
        return;
      }
      ...
      await utils.runAsync("llvm-profdata", ["merge", "-sparse", "-o", profDataPath, ...recordings])
