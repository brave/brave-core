...
import { parse } from './ifdef-loader.ts'
...
// </if>`
      ...
      expect(parse(source, { DEBUG: true, VERBOSE: true })).toContain(
        'log = true',
      )
      expect(parse(source, { DEBUG: true, VERBOSE: false })).not.toContain(
        'log = true',
      )
      expect(parse(source, { DEBUG: false, VERBOSE: true })).not.toContain(
        'log = true',
      )
    })
