...
TEST_F(ProxyConfigServiceTorTest, SetProxyAuthorization) {
  ...
  auto host_port_pair =
      info.proxy_chain().GetProxyServer(/*server_index=*/0).host_port_pair();

  EXPECT_EQ(host_port_pair.username(), anonymization_key);
  EXPECT_TRUE(info.proxy_chain().GetProxyServer(/*server_index=*/0).scheme() ==
  ...
  host_port_pair =
      info2.proxy_chain().GetProxyServer(/*server_index=*/0).host_port_pair();

  EXPECT_EQ(host_port_pair.username(), anonymization_key);
  EXPECT_EQ(host_port_pair.password(), password);
  EXPECT_TRUE(info2.proxy_chain().GetProxyServer(/*server_index=*/0).scheme() ==
              ProxyServer::SCHEME_SOCKS5);
  EXPECT_EQ(host_port_pair.host(), "127.0.0.1");
  EXPECT_EQ(host_port_pair.port(), 5566);
  ...
  host_port_pair =
      info3.proxy_chain().GetProxyServer(/*server_index=*/0).host_port_pair();

  EXPECT_EQ(host_port_pair.username(), anonymization_key);
  EXPECT_NE(host_port_pair.password(), password);
  EXPECT_TRUE(info3.proxy_chain().GetProxyServer(/*server_index=*/0).scheme() ==
              ProxyServer::SCHEME_SOCKS5);
  EXPECT_EQ(host_port_pair.host(), "127.0.0.1");
  EXPECT_EQ(host_port_pair.port(), 5566);
  ...
  host_port_pair =
      info4.proxy_chain().GetProxyServer(/*server_index=*/0).host_port_pair();

  EXPECT_EQ(host_port_pair.username(), anonymization_key);
  EXPECT_EQ(host_port_pair.password(), password);
  EXPECT_TRUE(info4.proxy_chain().GetProxyServer(/*server_index=*/0).scheme() ==
              ProxyServer::SCHEME_SOCKS5);
  EXPECT_EQ(host_port_pair.host(), "127.0.0.1");
  EXPECT_EQ(host_port_pair.port(), 5566);
  ...
  host_port_pair =
      info5.proxy_chain().GetProxyServer(/*server_index=*/0).host_port_pair();
  EXPECT_EQ(host_port_pair.username(), anonymization_key2);
  EXPECT_NE(host_port_pair.password(), password);
  EXPECT_TRUE(info5.proxy_chain().GetProxyServer(/*server_index=*/0).scheme() ==
              ProxyServer::SCHEME_SOCKS5);
  EXPECT_EQ(host_port_pair.host(), "127.0.0.1");
  EXPECT_EQ(host_port_pair.port(), 5566);
}
