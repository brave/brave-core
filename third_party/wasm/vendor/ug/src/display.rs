...
use crate::{DType, Device, LazyBuffer as LB, Result, WithDType};
use half::{bf16, f16};
...
impl<D: Device> LB<D> {
    fn fmt_dt<T: WithDType + std::fmt::Display>(
        ...
        match self.dims() {
            [] => {
                if let Ok(v) = self.data_vec::<T>() {
                    write!(f, "{}", v[0])?
                }
            }
            [s] if *s < 10 => {
                if let Ok(vs) = self.data_vec::<T>() {
...
impl<D: Device> std::fmt::Debug for LB<D> {
...
trait TensorFormatter {
    ...
    fn max_width<D: Device>(&self, to_display: &LB<D>) -> usize {
        let mut max_width = 1;
        if let Ok(vs) = to_display.data_vec() {
            for &v in vs.iter() {
    ...
    fn fmt_tensor<D: Device>(
        ...
        match dims {
            [] => {
                if let Ok(v) = t.data_vec::<Self::Elem>() {
                    self.fmt(v[0], max_w, f)?
                }
            }
            [v] if summarize && *v > 2 * edge_items => {
                if let Ok(vs) = t.narrow(0, 0, edge_items).and_then(|t| t.data_vec::<Self::Elem>())
                {
                ...
                write!(f, "...")?;
                if let Ok(vs) =
                    t.narrow(0, v - edge_items, edge_items).and_then(|t| t.data_vec::<Self::Elem>())
                {
                    for v in vs.into_iter() {
                        write!(f, ", ")?;
                        self.fmt(v, max_w, f)?;
            ...
            [_] => {
                ...
                if let Ok(vs) = t.data_vec::<Self::Elem>() {
...
{
    fn new<D: Device>(t: &LB<D>, po: &PrinterOptions) -> Result<Self> {
...
fn get_summarized_data<D: Device>(t: &LB<D>, edge_items: usize) -> Result<LB<D>> {
    let dims = t.dims();
    if dims.is_empty() {
        Ok(t.clone())
    } else if dims.len() == 1 {
...
impl<D: Device> std::fmt::Display for LB<D> {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        if let Err(err) = self.realize() {
            return write!(f, "{err:?}");
        }
