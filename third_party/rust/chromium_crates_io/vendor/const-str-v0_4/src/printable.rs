...
const fn check(
    ...
    let mut i = 0;
    while i < singletonuppers.len() {
        let (upper, lowercount) = singletonuppers[i];

        let lowerend = lowerstart + lowercount as usize;
        if xupper == upper {
            // for &lower in &singletonlowers[lowerstart..lowerend]
            let mut j = lowerstart;
            while j < lowerend {
                let lower = singletonlowers[j];
...
pub const fn is_printable(x: char) -> bool {
    let x = x as u32;
    let lower = x as u16;
