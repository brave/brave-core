...
mod tests {
  ...
  fn nested_message_serialization() {
    ...
    let snm_postcard =
      postcard::to_stdvec(&snm).expect("Should serialize to postcard");
    insta::assert_snapshot!(BASE64.encode(&snm_postcard));
  }
  ...
  fn nested_message_bad_data_load() {
    ...
    assert!(
      postcard::from_bytes::<SerializableNestedMessage>(&[7u8; 58]).is_err()
    );
    assert!(postcard::from_bytes::<SerializableNestedMessage>(&[]).is_err());
  }
