    ...
    pub fn validate_css_selector(selector: &str, accept_abp_selectors: bool) -> Result<Vec<CosmeticFilterOperator>, CosmeticFilterError> {
        ...
        fn is_procedural_operator(c: &selectors::parser::Component<SelectorImpl>) -> bool {
            ...
            match c {
                ...
                Component::NonTSPseudoClass(NonTSPseudoClass::MatchesMedia(_)) => true,
                ...
                Component::NonTSPseudoClass(NonTSPseudoClass::Others) => true,
                Component::NonTSPseudoClass(NonTSPseudoClass::Upward(_)) => true,
                Component::NonTSPseudoClass(NonTSPseudoClass::WatchAttr(_)) => true,
    ...
    impl<'i> selectors::parser::Parser<'i> for SelectorParseImpl {
        ...
        > {
            ...
            match canonical_name {
                ...
                "matches-media" => {
                    let text = to_css_nested(arguments)?;
                    return Ok(NonTSPseudoClass::MatchesMedia(text));
                }
                "matches-path" => {
                    let text = to_css_nested(arguments)?;
                ...
                "others" => {
                    ...
                    return Ok(NonTSPseudoClass::Others);
                }
                "upward" => {
                    let text = to_css_nested(arguments)?;
                ...
                "watch-attr" => {
                    let text = to_css_nested(arguments)?;
                    return Ok(NonTSPseudoClass::WatchAttr(text));
                }
                "xpath" => {
                    let text = to_css_nested(arguments)?;
    ...
    impl NonTSPseudoClass {
        fn to_procedural_operator(&self) -> Option<CosmeticFilterOperator> {
            match self {
                ...
                NonTSPseudoClass::MatchesMedia(a) => Some(CosmeticFilterOperator::MatchesMedia(a.to_owned())),
                ...
                NonTSPseudoClass::Others => Some(CosmeticFilterOperator::Others),
                NonTSPseudoClass::Upward(a) => Some(CosmeticFilterOperator::Upward(a.to_owned())),
                NonTSPseudoClass::WatchAttr(a) => Some(CosmeticFilterOperator::WatchAttr(a.to_owned())),
...
mod parse_tests {
    ...
    fn unsupported() {
        ...
        // `:others` cannot accept any arguments
        assert!(parse_cf("example.com##div:others()").is_ok());
        assert!(parse_cf("example.com##div:others(2)").is_err());
    }
