...
void RequestTracker::RegisterDocumentRequestStart(
  ...
  if (document_request_initiators_.count(frame_id) != 0) {
    CHECK(document_request_initiators_.at(frame_id) == request_id);
    return;
  }

  // If we get to this point, there should be no previous request with this
  // request ID.
  CHECK(document_requests_.count(request_id) == 0);
...
void RequestTracker::RegisterDocumentRequestComplete(
  ...
  CHECK(request_record.response_metadata.EncodedDataLength() == -1);
  CHECK(request_record.complete_timestamp == base::TimeDelta());
...
void RequestTracker::AddTracedRequestToHistory(const TrackedRequest* request) {
  CHECK(request->GetRequestId() > 0);
  CHECK(request->GetResource() != nullptr);
...
void RequestTracker::CheckTracedRequestAgainstHistory(
  ...
  CHECK(completed_requests_.count(request->GetRequestId()) == 0 ||
