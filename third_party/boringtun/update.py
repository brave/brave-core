...
Run this after bumping the pinned BoringTun rev in Cargo.toml (and any
explicit `cargo update` you want for transitive deps). It:
...
create_boringtun_licenses.py after this when you want that.
"""
...
with brave_chromium_utils.sys_path('//tools/rust'):
    ...
    CARGO = os.path.join(update_rust.RUST_TOOLCHAIN_OUT_DIR, 'bin',
                         'cargo' + ('.exe' if sys.platform == 'win32'
                                    else ''))
...
def main():
    ...
    # Isolated env. Mirrors build_boringtun.py's isolation so re-vendor
