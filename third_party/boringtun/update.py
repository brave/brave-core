...
(and any explicit `cargo update` you want for transitive deps). It:
  ...
  1. Backs up reviewer-owned files (per-crate README.chromium,
     the vendor .clang-format).
  ...
  6. Prints a list of any files in vendor/ that are excluded by git,
     which should be fixed before committing.
...
def get_excluded_files(vendor_dir: str) -> list[str]:
    """
    Returns the list of files inside `vendor` that git would exclude
    on commit.
    """
