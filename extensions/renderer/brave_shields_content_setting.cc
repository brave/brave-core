...
v8::Local<v8::Object> BraveShieldsContentSetting::Create(
  ...
  gin::Handle<BraveShieldsContentSetting> handle = gin::CreateHandle(
      isolate,
      new BraveShieldsContentSetting(
          request_handler, type_refs, access_checker, pref_name, *value_spec));
...
void BraveShieldsContentSetting::HandleFunction(const std::string& method_name,
  ...
  if (!signature->ParseArgumentsToJSON(context,
                                       argument_list,
                                       *type_refs_,
                                       &converted_arguments,
                                       &callback,
  ...
  if (method_name == "set") {
    ...
    std::string error;
    if (!value.IsEmpty() &&
        !argument_spec_.ParseArgument(
            context, value, *type_refs_, nullptr, nullptr, &error)) {
  ...
  request_handler_->StartRequest(context,
                                 "braveShields." + method_name,
                                 std::move(converted_arguments),
                                 callback,
