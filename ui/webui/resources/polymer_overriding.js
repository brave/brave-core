...
function addBraveBehaviors(component) {
  if (allBehaviorsMap[component.is]) {
    ...
    component.behaviors.push(
        ...allBehaviorsMap[component.is]) delete allBehaviorsMap[component.is]
  }
}
...
const allBraveTemplateModificationsMap =
    ...
    function addBraveTemplateModifications(component) {
      if (allBraveTemplateModificationsMap[component.is]) {
        const moduleName = component.is const domModule =
            Polymer.DomModule.import(moduleName)
        if (domModule) {
          ...
          if (template) {
            ...
            console.debug(
                `Modifying template '${component.is}' took ${t1 - t0}ms`)
          }
        }
    ...
    function OverrideIronIcons(
        iconSetName, overridingIconSetName, iconOverrides) {
      ...
      for (const chromiumIconName in iconOverrides) {
        ...
        if (!chromiumIcon) {
          console.error(`[brave overrides] Could not find chromium icon '${
              chromiumIconName}' in iconset '${iconSetName}' for replacement!`)
          continue
        }
        const braveIconName = iconOverrides[chromiumIconName] const braveIcon =
            overrideIconSet.querySelector(`#${braveIconName}`)
        if (!braveIcon) {
          console.error(`[brave overrides] Could not find brave icon '${
              braveIconName}' in iconset '${
              overridingIconSetName}' for replacement!`)
          continue
        }
        // replace
