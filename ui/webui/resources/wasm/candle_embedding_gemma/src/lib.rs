...
// Position info is injected fresh at every attention layer by rotating Q and K vectors before computing attention scores.
pub fn apply_rotary(
    x: &Tensor,
    cos: &Tensor,
    sin: &Tensor,
...
impl Gemma3Attention {
    ...
    fn repeat_kv(
        ...
        let expanded = x
            .unsqueeze(2)?
            .expand((b, h, repeat_factor, s, d))?
            .reshape((b, h * repeat_factor, s, d))?;
...
impl Gemma3Model {
    pub fn load(vb: VarBuilder, config: &Gemma3Config, model_type: ModelType) -> candle_core::Result<Self> {
        ...
        let inv_freqs = get_inv_freqs(rotary_dim, config.rope_theta, vb.device())?;
        let rotary_cache =
            get_cos_sin(config.max_position_embeddings, &inv_freqs, vb.dtype(), true)?;

        let inv_freqs_local =
            get_inv_freqs(rotary_dim, config.rope_local_base_freq, vb.device())?;
