...
namespace {
constexpr int kMaxFetchJitterInSeconds = 60;
}  // namespace
...
base::TimeDelta ObliviousHttpKeyConfig::ExpiresAfter() const {
  const base::TimeDelta expires_after =
      local_state_->GetTime(prefs::kObliviousHttpKeyConfigExpiresAt) -
      base::Time::Now();

  if (expires_after.is_negative()) {
    // Already expired.
    return {};
  }
...
void ObliviousHttpKeyConfig::FetchCallback(
  ...
  if (url_response_body->empty()) {
    VLOG(6) << "Failed to fetch OHTTP key config due to empty body";
    return FailedToFetch();
  }
...
void ObliviousHttpKeyConfig::Set(const std::string& key_config) {
  VLOG(6) << "Set OHTTP key config";
...
void ObliviousHttpKeyConfig::Remove() {
  VLOG(6) << "Removed OHTTP key config";
