...
TEST(BraveAdsAccountFeatureTest, IsEnabled) {
  ...
  // Assert
  EXPECT_TRUE(IsAccountFeatureEnabled());
}
...
TEST(BraveAdsAccountFeatureTest, IsDisabled) {
  ...
  // Assert
  EXPECT_FALSE(IsAccountFeatureEnabled());
}

TEST(BraveAdsAccountFeatureTest, GetNextPaymentDay) {
  // Arrange
...
TEST(BraveAdsAccountFeatureTest, GetMinEstimatedEarningsMultiplier) {
  // Arrange
  std::vector<base::test::FeatureRefAndParams> enabled_features;
  base::FieldTrialParams params;
