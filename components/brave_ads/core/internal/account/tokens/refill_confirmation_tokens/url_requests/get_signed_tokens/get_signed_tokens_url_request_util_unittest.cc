...
base::Value::Dict BuildUrlResponseBody() {
  return base::Value::Dict()
      .Set("batchProof", privacy::cbr::kBatchDLEQProofBase64)
      .Set("signedTokens",
           base::Value::List().Append(privacy::cbr::kSignedTokenBase64))
      .Set("publicKey", privacy::cbr::kPublicKeyBase64);
}
...
TEST_F(BraveAdsGetSignedTokensUrlRequestUtilTest, ParseAndUnblindSignedTokens) {
  ...
  // Act
  const auto result =
      ParseAndUnblindSignedTokens(dict, privacy::cbr::GetTokensForTesting(),
                                  privacy::cbr::GetBlindedTokensForTesting());
...
TEST_F(BraveAdsGetSignedTokensUrlRequestUtilTest,
  ...
  // Arrange
  base::Value::Dict dict = BuildUrlResponseBody();
  dict.Set("signedTokens",
           base::Value::List().Append(privacy::cbr::kInvalidBase64));
...
TEST_F(BraveAdsGetSignedTokensUrlRequestUtilTest,
  ...
  // Act
  BuildAndAddConfirmationTokens(privacy::cbr::GetUnblindedTokensForTesting(),
                                privacy::cbr::GetPublicKeyForTesting(),
                                GetWalletForTesting());

  // Assert
  EXPECT_FALSE(privacy::GetConfirmationTokensForTesting().IsEmpty());
}
