...
bool ConfirmationQueueItemInfo::IsValid() const {
  // All five confirmation fields map to NOT NULL DB columns, so legacy rows
  // satisfy these constraints. A type/ad_type string that no longer maps to a
  // known enum yields kUndefined and is silently dropped on the read path. It
  // is never CHECKed.
