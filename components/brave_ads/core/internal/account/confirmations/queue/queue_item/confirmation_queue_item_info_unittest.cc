...
TEST_F(BraveAdsConfirmationQueueItemInfoTest, IsValid) {
  ...
  // Act & Assert
  EXPECT_TRUE(confirmation_queue_item.IsValid());
}
...
TEST_F(BraveAdsConfirmationQueueItemInfoTest, IsNotValidIfProcessAtIsAbsent) {
  ...
  ConfirmationQueueItemInfo confirmation_queue_item =
      BuildConfirmationQueueItem(*confirmation, /*process_at=*/test::Now());
  confirmation_queue_item.process_at = std::nullopt;

  // Act & Assert
  EXPECT_FALSE(confirmation_queue_item.IsValid());
}
...
TEST_F(BraveAdsConfirmationQueueItemInfoTest, IsNotValidIfAdTypeIsUndefined) {
  ...
  ConfirmationQueueItemInfo confirmation_queue_item =
      BuildConfirmationQueueItem(*confirmation, /*process_at=*/test::Now());
  confirmation_queue_item.confirmation.ad_type = mojom::AdType::kUndefined;

  // Act & Assert
  EXPECT_FALSE(confirmation_queue_item.IsValid());
}

TEST_F(BraveAdsConfirmationQueueItemInfoTest, IsNotValidIfCreatedAtIsAbsent) {
  // Arrange
  test::DisableBraveRewards();
  ...
  ConfirmationQueueItemInfo confirmation_queue_item =
      BuildConfirmationQueueItem(*confirmation, /*process_at=*/test::Now());
  confirmation_queue_item.confirmation.created_at = std::nullopt;
