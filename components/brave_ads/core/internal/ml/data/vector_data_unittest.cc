...
TEST_F(BatAdsVectorDataTest, AddElementWise) {
  ...
  // Assert
  for (int i = 0; i < 3; i++) {
    EXPECT_NEAR(v12.at(i), v1.GetData().at(i), 0.001F);
    EXPECT_NEAR(v21.at(i), v2.GetData().at(i), 0.001F);
    EXPECT_NEAR(v34.at(i), v3.GetData().at(i), 0.001F);
  }
}
...
TEST_F(BatAdsVectorDataTest, DivideByScalar) {
  ...
  // Assert
  for (int i = 0; i < 3; i++) {
    EXPECT_NEAR(v1d.at(i), v1.GetData().at(i), 0.001F);
    EXPECT_NEAR(v2d.at(i), v2.GetData().at(i), 0.001F);
    EXPECT_NEAR(v3d.at(i), v3.GetData().at(i), 0.001F);
    EXPECT_NEAR(v4d.at(i), v4.GetData().at(i), 0.001F);
  }
}
