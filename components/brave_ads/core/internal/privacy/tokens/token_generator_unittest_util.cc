...
std::vector<cbr::Token> BuildTokens(const size_t count) {
  ...
  const size_t modulo = GetTokens().size();

  std::vector<cbr::Token> tokens;

  for (size_t i = 0; i < count; i++) {
    const std::string& token_base64 = GetTokens().at(i % modulo);
...
void MockTokenGenerator(const TokenGeneratorMock& mock, const size_t count) {
  ...
  ON_CALL(mock, Generate(count)).WillByDefault(Return(BuildTokens(count)));
}
