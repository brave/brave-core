...
class AdsClientMock : public AdsClient {
 public:
  ...
  MOCK_CONST_METHOD1(GetLocalStatePref,
                     absl::optional<base::Value>(const std::string& path));
  MOCK_METHOD2(SetLocalStatePref,
               void(const std::string& path, base::Value value));
