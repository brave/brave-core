...
#include "brave/components/brave_ads/core/internal/ads/serving/eligible_ads/exclusion_rules/daypart_exclusion_rule_util.h"
...
TEST(BraveAdsDaypartExclusionRuleUtilTest, MatchTimeSlot) {
  ...
  // Assert
  EXPECT_TRUE(MatchTimeSlot(daypart, GetMinutes(/*hours*/ 5, /*minutes*/ 35)));
}
...
TEST(BraveAdsDaypartExclusionRuleUtilTest, DoNotMatchTimeSlotAfterEndMinute) {
  ...
  CreativeDaypartInfo daypart;
  daypart.start_minute = GetMinutes(/*hours*/ 0, /*minutes*/ 0);
  daypart.end_minute = GetMinutes(/*hours*/ 5, /*minutes*/ 34);

  // Act

  // Assert
  EXPECT_FALSE(MatchTimeSlot(daypart, GetMinutes(/*hours*/ 5, /*minutes*/ 35)));
}
...
TEST(BraveAdsDaypartExclusionRuleUtilTest,
  ...
  // Arrange
  CreativeDaypartInfo daypart;
  daypart.start_minute = GetMinutes(/*hours*/ 5, /*minutes*/ 36);
  daypart.end_minute = GetMinutes(/*hours*/ 23, /*minutes*/ 59);
