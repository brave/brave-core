...
brave_adaptive_captcha::Environment AdsEnvironmentToAdaptiveCaptchaEnvironment(
  ...
  switch (environment) {
    case ads::Environment::DEVELOPMENT: {
      return brave_adaptive_captcha::DEVELOPMENT;
    }

    case ads::Environment::STAGING: {
      return brave_adaptive_captcha::STAGING;
    }

    case ads::Environment::PRODUCTION: {
...
void AdsServiceImpl::ShowScheduledCaptchaNotification(
  ...
  // Only allow one snooze per captcha; don't even show the button after that
  std::u16string cancel_button_text;
  if (snooze_count == 0) {
    cancel_button_text = l10n_util::GetStringUTF16(
  ...
  brave_tooltips::BraveTooltip captcha_tooltip(kScheduledCaptchaTooltipId,
                                               tooltip_attributes, nullptr);

  std::unique_ptr<AdTooltipPlatformBridge> platform_bridge =
      std::make_unique<AdTooltipPlatformBridge>(profile_);
  platform_bridge->ShowTooltip(captcha_tooltip);
}
