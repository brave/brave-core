...
void FakeBatAdsService::BindReceiver(
  ...
  receiver_.Bind(std::move(bat_ads_service_mojom_pending_receiver));
  ...
  receiver_.set_disconnect_handler(shutdown_callback_);
}
...
void FakeBatAdsService::Create(
  ...
  bat_ads_.BindReceiver(std::move(bat_ads_mojom_pending_associated_receiver));
