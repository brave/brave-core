...
function fillInWildcards(pattern: string, chars: string[]) {
  ...
  return [...pattern].map((char) => {
    ...
    if (char === SINGLE_CHARACTER_WILDCARD) {
...
// Splits `pattern` into a list of strings and lone wildcard characters,
// without building a RegExp from caller-supplied `chars`.
function splitOnWildcards(pattern: string, chars: string[]) {
  const parts: string[] = []
  let literal = ''
...
export function WildcardPattern({ pattern, chars, onCopy }: {
  ...
  const parts = splitOnWildcards(pattern, chars).map((part, index) => (
    chars.includes(part)
