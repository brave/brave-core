...
// static
void AdsClientMojoBridge::OnGetCreativePublisherAdsToPreCache(
...
void AdsClientMojoBridge::GetCreativePublisherAdsToPreCache(
    GetCreativePublisherAdsToPreCacheCallback callback) {
  // this gets deleted in OnGetCreativePublisherAdsToPreCache
  auto* holder = new CallbackHolder<GetCreativePublisherAdsToPreCacheCallback>(
      AsWeakPtr(), std::move(callback));

  ads_client_->GetCreativePublisherAdsToPreCache(
      std::bind(AdsClientMojoBridge::OnGetCreativePublisherAdsToPreCache,
          holder, _1, _2));
}

// static
void AdsClientMojoBridge::OnFlagPublisherAdWasPreCached(
    CallbackHolder<FlagPublisherAdWasPreCachedCallback>* holder,
    const std::string& creative_instance_id,
    const bool was_flagged) {
  if (holder->is_valid()) {
    std::move(holder->get()).Run(creative_instance_id, was_flagged);
  }

  delete holder;
}

void AdsClientMojoBridge::FlagPublisherAdWasPreCached(
    const std::string& creative_instance_id,
    FlagPublisherAdWasPreCachedCallback callback) {
  // this gets deleted in OnFlagPublisherAdWasPreCached
  auto* holder = new CallbackHolder<FlagPublisherAdWasPreCachedCallback>(
      AsWeakPtr(), std::move(callback));

  ads_client_->FlagPublisherAdWasPreCached(creative_instance_id,
      std::bind(AdsClientMojoBridge::OnFlagPublisherAdWasPreCached,
          holder, _1, _2));
}
