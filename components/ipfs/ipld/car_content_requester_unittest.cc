...
constexpr char kDefaultIpfsUrl[] =
    "ipfs://QmWiTTxzmTwHPoRsWczWJeNt8u5n3YzJBaWaUAwgmUxAEM";  //"ipfs://bafybeiakou6e7hnx4ms2yangplzl6viapsoyo6phlee6bwrg4j2xt37m3q";
...
TEST_F(CarContentRequesterUnitTest, BasicTestSteps) {
...

  {
    ...
    EXPECT_FALSE(ccr->IsStarted());
    EXPECT_EQ(data_received_callback_counter, 1);
    EXPECT_EQ(complete_callback_counter, 1);
  }
}
...
TEST_F(CarContentRequesterUnitTest, ResponseByChunks) {
  ...
  ccr->OnDataReceived("%%%", base::NullCallback());
  ccr->OnComplete(true);
  EXPECT_EQ(data_received_callback_counter, 2);
  EXPECT_EQ(complete_callback_counter, 1);
}
