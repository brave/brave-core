...
void IpfsImportWorkerBase::ImportFile(const base::FilePath upload_file_path,
  ...
  auto upload_callback =
      base::BindOnce(&IpfsImportWorkerBase::UploadData, weak_factory_.GetWeakPtr());

  base::ThreadPool::PostTaskAndReplyWithResult(FROM_HERE,
      {base::MayBlock()},
      base::BindOnce(&ipfs::CalculateFileSize, upload_file_path),
      base::BindOnce(&CreateRequestForFile, std::move(upload_callback),
                      upload_file_path, std::move(blob_storage_context_getter),
                      mime_type, filename));
}
