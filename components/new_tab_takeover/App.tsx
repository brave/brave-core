...
function useParametersFromQuery(): { placementId: string | null; creativeInstanceId: string | null } {
...
export default function App(props: React.PropsWithChildren) {
  ...
  const getCurrentWallpaper = React.useCallback(async () => {
    ...
    try {
      const response = await newTabTakeover.getCurrentWallpaper(creativeInstanceId);
