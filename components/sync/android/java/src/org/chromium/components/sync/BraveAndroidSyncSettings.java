...
public class BraveAndroidSyncSettings extends AndroidSyncSettings {
    ...
    public boolean isChromeSyncEnabled() {
        ...
        try {
            Method methodGet = Class.forName("org.chromium.chrome.browser.sync.ProfileSyncService")
                                       .getDeclaredMethod("get");
            Object profileSyncService = methodGet.invoke(null);
            if (profileSyncService == null) {
                return false;
            }
            Method methodIsSyncRequested =
                    Class.forName("org.chromium.chrome.browser.sync.ProfileSyncService")
                            .getDeclaredMethod("isFirstSetupComplete");
            Object isFirstSetupComplete = methodIsSyncRequested.invoke(profileSyncService);
            boolean bIsFirstSetupComplete = (boolean) isFirstSetupComplete;
            return bIsFirstSetupComplete;
        } catch (Exception e) {
            assert false;
