...
const char oauth_scope[] = "user:email,user:address,asset:balance,asset:ocbs";
...
std::string GetHexEncodedCryptoRandomSeed() {
  ...
  return base::HexEncode(reinterpret_cast<char*>(random_seed_bytes),
                         kSeedByteLength);
}
...
// static
std::string ContributeService::GetCodeChallenge(
    const std::string& code_verifier) {
  std::string code_challenge;
  char raw[crypto::kSHA256Length] = {0};
  crypto::SHA256HashString(code_verifier, raw, crypto::kSHA256Length);
  ...
  code_challenge.erase(
      std::find_if(code_challenge.rbegin(), code_challenge.rend(),
                   [](int ch) { return ch != '='; })
          .base(),
      code_challenge.end());
...
bool ContributeService::GetAccessToken(const std::string& code,
  ...
  auto internal_callback =
      base::BindOnce(&ContributeService::OnGetAccessToken,
  ...
  url = net::AppendQueryParameter(url, "redirect_uri", oauth_callback);
  return OAuthRequest(base_url, "POST", url.query(),
                      std::move(internal_callback));
}

bool ContributeService::GetAccountBalances(
    GetAccountBalancesCallback callback) {
  auto internal_callback =
      base::BindOnce(&ContributeService::OnGetAccountBalances,
                     base::Unretained(this), std::move(callback));
...
void ContributeService::OnGetAccountBalances(
    GetAccountBalancesCallback callback,
    const int status,
...
void ContributeService::OnGetAccessToken(
  ...
  if (status >= 200 && status <= 299) {
    ContributeJSONParser::GetTokensFromJSON(body, &access_token,
                                            "access_token");
    ContributeJSONParser::GetTokensFromJSON(body, &refresh_token,
                                            "refresh_token");
...
bool ContributeService::OAuthRequest(const GURL& url,
  ...
  iter->get()->DownloadToStringOfUnboundedSizeUntilCrashAndDie(
      url_loader_factory,
      base::BindOnce(&ContributeService::OnURLLoaderComplete,
...
bool ContributeService::SetAccessTokens(const std::string& access_token,
  ...
  Profile* profile = Profile::FromBrowserContext(context_);
  profile->GetPrefs()->SetString(kContributeAccessToken,
                                 encoded_encrypted_access_token);
...
std::string ContributeService::GetContributeTLD() {
  ...
  const int32_t us_id =
      country_codes::CountryCharsToCountryID(us_code.at(0), us_code.at(1));
  return (user_country_id == us_id) ? "us" : "com";
}

bool ContributeService::GetConvertQuote(const std::string& from,
                                        const std::string& to,
                                        const std::string& amount,
                                        GetConvertQuoteCallback callback) {
  auto internal_callback =
      base::BindOnce(&ContributeService::OnGetConvertQuote,
                     base::Unretained(this), std::move(callback));
...
void ContributeService::OnGetConvertQuote(
  ...
  if (status >= 200 && status <= 299) {
    ContributeJSONParser::GetQuoteInfoFromJSON(body, &quote_id, &quote_price,
                                               &total_fee, &total_amount);
  }
  std::move(callback).Run(quote_id, quote_price, total_fee, total_amount);
}

bool ContributeService::GetTickerPrice(const std::string& symbol_pair,
                                       GetTickerPriceCallback callback) {
  auto internal_callback =
      base::BindOnce(&ContributeService::OnGetTickerPrice,
                     base::Unretained(this), std::move(callback));
...
bool ContributeService::GetTickerVolume(const std::string& symbol_pair,
                                        GetTickerVolumeCallback callback) {
  auto internal_callback =
      base::BindOnce(&ContributeService::OnGetTickerVolume,
                     base::Unretained(this), std::move(callback));
...
bool ContributeService::GetDepositInfo(const std::string& symbol,
  ...
  auto internal_callback =
      base::BindOnce(&ContributeService::OnGetDepositInfo,
                     base::Unretained(this), std::move(callback));
...
void ContributeService::OnGetDepositInfo(
  ...
  bool success = status >= 200 && status <= 299;
  if (success) {
    ContributeJSONParser::GetDepositInfoFromJSON(body, &deposit_address,
                                                 &deposit_url);
  }

  std::move(callback).Run(deposit_address, deposit_url, success);
}

bool ContributeService::ConfirmConvert(const std::string& quote_id,
                                       ConfirmConvertCallback callback) {
  auto internal_callback =
      base::BindOnce(&ContributeService::OnConfirmConvert,
                     base::Unretained(this), std::move(callback));
...
void ContributeService::OnConfirmConvert(
  ...
  if (status >= 200 && status <= 299) {
    ContributeJSONParser::GetConfirmStatusFromJSON(body, &error_message,
                                                   &success_status);
  }
...
bool ContributeService::GetConvertAssets(GetConvertAssetsCallback callback) {
  auto internal_callback =
      base::BindOnce(&ContributeService::OnGetConvertAssets,
                     base::Unretained(this), std::move(callback));
...
void ContributeService::OnGetConvertAssets(
    GetConvertAssetsCallback callback,
    const int status,
    const std::string& body,
    const std::map<std::string, std::string>& headers) {
...
bool ContributeService::RevokeToken(RevokeTokenCallback callback) {
  auto internal_callback =
      base::BindOnce(&ContributeService::OnRevokeToken, base::Unretained(this),
                     std::move(callback));
...
void ContributeService::OnRevokeToken(
