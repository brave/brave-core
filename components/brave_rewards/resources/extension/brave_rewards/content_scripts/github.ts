...
const createBraveTipAction = (elem: Element, getMetaData: (elem: Element) => RewardsTip.GitHubMetaData | null) => {
  ...
  braveTipButton.onclick = function (event) {
    ...
    if (githubMetaData) {
      const msg = { type: 'tipGitHubUser', githubMetaData: githubMetaData }
...
const getCommentMetaData = (elem: Element): RewardsTip.GitHubMetaData | null => {
...
const getReviewItemMetaData = (elem: Element): RewardsTip.GitHubMetaData | null => {
...
const getCommitLinksMetaData = (elem: Element): RewardsTip.GitHubMetaData | null => {
...
const getStarringContainerMetaData = (elem: Element): RewardsTip.GitHubMetaData | null => {
  const ancestor = elem.closest('.d-block')
  if (ancestor) {
    const aTags = ancestor.getElementsByTagName('A')
    if (aTags.length) {
      const aTag = aTags[0]
      const userName = (aTag as HTMLAnchorElement).href.split('/')[3]
      return {
        userName: userName || ''
      }
    }
...
const getPageHeadMetaData = (elem: Element): RewardsTip.GitHubMetaData | null => {
...
const getMemberListItemMetaData = (elem: Element): RewardsTip.GitHubMetaData | null => {
  const ancestor = elem.closest('.table-list-cell')
  if (ancestor) {
    const aTags = ancestor.getElementsByTagName('A')
    if (aTags.length) {
      const aTag = aTags[0] as HTMLAnchorElement
      const userName = aTag.href.split('/')[3]
      return {
        userName: userName || ''
      }
    }
