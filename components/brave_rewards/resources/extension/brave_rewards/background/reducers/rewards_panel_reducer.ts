...
export const rewardsPanelReducer =
    (state: RewardsExtension.State|undefined, action: any) => {
      ...
      switch (action.type) {
        case types.CREATE_WALLET:
          ...
          state = {...state} state.walletCreating = true
          state.walletCreateFailed = false
          state.walletCreated = false
          state.walletCorrupted = false
          break case types.ON_WALLET_INITIALIZED:
    ...
    case types.ON_TAB_ID:
      ...
      break case types.ON_TAB_RETRIEVED: {const
                                          tab: chrome.tabs.Tab = payload.tab
      if (!tab || !tab.url || tab.incognito || !tab.active ||
          !state.walletCreated || !state.enabledMain) {
        break
      }

      const id = getWindowId(tab.windowId)
      const publishers: Record<string, RewardsExtension.Publisher> =
          state.publishers
      const publisher = publishers[id] const validKey = publisher &&
          publisher.publisher_key && publisher.publisher_key.length > 0
      ...
      if (!publisher || (publisher.tabUrl !== tab.url || !validKey)) {
        ...
        publishers[id] = { tabUrl: tab.url, tabId: tab.id }
      } else if (publisher &&
    ...
    case types.ON_PUBLISHER_DATA: {
      ...
      let publishers: Record<string, RewardsExtension.Publisher> =
          state.publishers
    ...
    case types.GET_WALLET_PROPERTIES:
      ...
      break case types.ON_WALLET_PROPERTIES:
          {state = {...state} state.walletProperties = payload.properties
      break
    }
    case types.GET_CURRENT_REPORT:
      chrome.braveRewards.getCurrentReport()
      break case types.ON_CURRENT_REPORT: {state = {...state} state.report =
                                               payload.properties
      break
    }
    case types.ON_NOTIFICATION_ADDED: {
      ...
      let notifications: Record<string, RewardsExtension.Notification> =
          state.notifications
    ...
    case types.ON_NOTIFICATION_DELETED: {
      ...
      let id = payload.id
      let notifications: Record<number, RewardsExtension.Notification> =
          state.notifications
      if (!notifications[id]) {
        id = `n_${id}`
      }
      ...
      delete notifications[id]
          ...
          if (state.currentNotification === id) {
            ...
            Object.keys(state.notifications).forEach((key: string) => {
              if (current === undefined || !notifications[current] ||
                  notifications[key].timestamp >
                      notifications[current].timestamp) {
    ...
    case types.ON_PUBLISHER_LIST_NORMALIZED: {
      ...
      for (const key in publishers) {
        let publisher = publishers[key] const updated = list.find(
            (newPublisher: RewardsExtension.PublisherNormalized) =>
    ...
    case types.SAVE_RECURRING_TIP: {
      ...
      if (newAmount < 0 || isNaN(newAmount) || publisherKey == null) {
        break
      }
    ...
    case types.ON_PUBLISHER_STATUS_REFRESHED: {
      ...
      if (publisherKey) {
        ...
        for (const key in publishers) {
          let publisher =
              publishers[key] if (publisher.publisher_key === publisherKey) {
    ...
    case types.ON_ALL_NOTIFICATIONS: {
      ...
      list.forEach((notification: RewardsExtension.Notification) => {
        ...
        id: notification.id, type: notification.type,
            timestamp: notification.timestamp, args: notification.args
        }
      })
