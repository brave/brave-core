...
chrome.braveRewards.getAllNotifications(
    (list: RewardsExtension.Notification[]) => {
        rewardsPanelActions.onAllNotifications(list)})

chrome.braveRewards.onWalletInitialized.addListener(
...
chrome.braveRewards.onPublisherData.addListener(
    (windowId: number, publisher: RewardsExtension.Publisher) => {
      ...
      if (publisher && publisher.publisher_key) {
        chrome.braveRewards.getPublisherBanner(
            publisher.publisher_key,
            ((banner: RewardsExtension.PublisherBanner) => {
                 rewardsPanelActions.onPublisherBanner(banner)}))
      }
    })

chrome.braveRewards.onWalletProperties.addListener(
    (properties: RewardsExtension.WalletProperties) => {
        rewardsPanelActions.onWalletProperties(properties)})

chrome.braveRewards.onCurrentReport.addListener(
    (properties: RewardsExtension.Report) => {
        rewardsPanelActions.onCurrentReport(properties)})

chrome.braveRewards.onGrant.addListener(
    (properties: RewardsExtension.GrantResponse) => {
        rewardsPanelActions.onGrant(properties)})

chrome.braveRewards.onGrantCaptcha.addListener(
    (captcha: RewardsExtension.Captcha) => {
        rewardsPanelActions.onGrantCaptcha(captcha)})

chrome.braveRewards.onGrantFinish.addListener(
    (properties: RewardsExtension.GrantFinish) => {
...
chrome.braveRewards.onEnabledMain.addListener(
    (enabledMain: boolean) => {rewardsPanelActions.onEnabledMain(enabledMain)})

chrome.braveRewards.onPendingContributionSaved.addListener((result: number) => {
  if (result === 0) {
    chrome.braveRewards.getPendingContributionsTotal(
        ((amount: number) => {
...
chrome.braveRewards.onExcludedSitesChanged.addListener(
...
chrome.braveRewards.onRecurringTipSaved.addListener((success: boolean) => {
  if (success) {
    chrome.braveRewards.getRecurringTips(
        (tips: RewardsExtension.RecurringTips) => {
