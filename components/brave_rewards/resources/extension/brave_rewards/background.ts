...
const iconOn = {
  path: {18: batIconOn18Url, 36: batIconOn36Url, 54: batIconOn54Url}
}
...
chrome.runtime.onInstalled.addListener(function(details) {
  if (details.reason === 'install') {
    ...
    chrome.storage.local.set(
        {'is_dismissed': initialNotificationDismissed}, function() {
...
chrome.runtime.onStartup.addListener(function() {
  ...
  chrome.runtime.onConnect.addListener(function(externalPort) {
    chrome.storage.local.set({'rewards_panel_open': 'true'})

    externalPort.onDisconnect.addListener(function() {
      chrome.storage.local.set({'rewards_panel_open': 'false'})
    })
  })
})
            ...
            chrome.braveRewards.tipTwitterUser(tabId, tweetMetaData)
                            ...
                            chrome.runtime.onMessage.addListener(
                                (msg, sender, sendResponse) => {
                                  const action =
                                      typeof msg === 'string' ? msg : msg.type
                                  switch (action) {
                                    case 'tipTwitterUser': {
                                    ...
                                    case 'rewardsEnabled': {
                                      ...
                                      chrome.braveRewards.getRewardsMainEnabled(
                                          function(enabled: boolean) {
                                            sendResponse({enabled})
                                          })
                                      // Must return true for asynchronous calls
                                      // to sendResponse
                                      return true
                                    }
                                    case 'inlineTipSetting': {
                                      // Check if inline tip is enabled
                                      chrome.braveRewards.getInlineTipSetting(
                                          msg.key, function(enabled: boolean) {
                                            sendResponse({enabled})
                                          })
                                      // Must return true for asynchronous calls
                                      // to sendResponse
