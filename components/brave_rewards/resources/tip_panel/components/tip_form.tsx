        ...
        <InfoBox title={title} style='warn'>
          ...
          </div>
          {
            state.creatorBanner.web3URL &&
              <div>{getString('web3GetStartedText')}</div>
          }
        </InfoBox>
      )
    }
    ...
    if (state.creatorWallets.length === 0 && state.creatorBanner.web3URL) {
      ...
      return (
        <InfoBox title={getString('web3OnlyTitle')}>
          <div>{getString('web3OnlyText')}</div>
          <div>{getString('web3GetStartedText')}</div>
        </InfoBox>
      )
    }
    ...
    return (
      <style.root>
        <style.successCard>
          <style.successTitle>
            {
              formatMessage(getString('contributionSentTitle'), [
                batAmountFormatter.format(sendAmount)
              ])
            }
          </style.successTitle>
          <style.successText>
        ...
        <style.buttons>
          {
            needsReconnect &&
              <button onClick={onReconnectClick}>
                {getString('reconnectButtonLabel')}
              </button>
          }
          {
            showConnect &&
