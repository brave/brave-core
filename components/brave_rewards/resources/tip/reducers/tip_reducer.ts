...
export const defaultState: RewardsTip.State = {
  ...
  recurringDonations: [],
  walletInfo: {balance: 0, choices: [], probi: '0'},
  reconcileStamp: 0
}

const publishersReducer: Reducer<RewardsTip.State> =
    (state: RewardsTip.State = defaultState, action) => {
      const payload = action.payload

      switch (action.type) {
        case types.ON_CLOSE_DIALOG:
          state = {...state} state.currentTipRecurring = false
          chrome.send('dialogClose')
          break case types.ON_TWEET: chrome.send(
              'brave_rewards_tip.tweetTip', [payload.name, payload.tweetId])
          break case types.ON_PUBLISHER_BANNER:
              {state =
               {
                 ...state
               } if (!state.publishers) {
    ...
    case types.GET_WALLET_PROPERTIES:
          ...
          .ON_TIP: {
            if (payload.publisherKey && payload.amount > 0) {
              ...
              chrome.send(
                  ...
                  [payload.publisherKey, amount, payload.recurring])
              state = {...state} state.finished = true
      ...
      break case types.ON_RECURRING_TIPS: state = {
        ...state
      ...
      break case types.GET_RECONCILE_STAMP:
