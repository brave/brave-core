...
export const formatConverted = (converted: string, currency: string = 'USD'):
    string|null => {
      return `${converted} ${currency}`
    }

export const generateContributionMonthly =
    (list: number[], rates: Record<string, number>|undefined) => {
      if (!list) {
        return []
      }
...
export const generateQR =
    (addresses: Record<Rewards.AddressesType, string>) => {
      ...
      const generate =
          (type: Rewards.AddressesType, address: string) => {
            switch (type) {
              case 'BAT':
              case 'ETH':
                url = `ethereum:${address}`
                break case 'BTC': url = `bitcoin:${address}`
                break case 'LTC': url = `litecoin:${address}`
            ...
            try {
              ...
              console.error(
                  'qr.imageSync (for url ' + url + ') error: ' + ex.toString())
            }
          }
...
export const getAddresses =
    (addresses?: Record<Rewards.AddressesType, Rewards.Address>) => {
      let result: Address[] = []
      ...
      sortedArray.forEach((type: Rewards.AddressesType) => {
        const item: Rewards.Address =
            addresses[type] if (item && item.address) {
          result.push({type, qr: item.qr, address: item.address})
        }
      })
...
export const convertProbiToFixed =
    (probi: string, places: number = 1) => {
      const result = new BigNumber(probi).dividedBy('1e18').toFixed(
          places, BigNumber.ROUND_DOWN)
