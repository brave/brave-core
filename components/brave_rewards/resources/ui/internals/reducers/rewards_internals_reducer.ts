...
const rewardsInternalsReducer: Reducer<RewardsInternals.State|undefined> =
    (state: RewardsInternals.State|undefined, action) => {
      if (state === undefined) {
        state = storage.load()
      }
      ...
      switch (action.type) {
        case types.GET_REWARDS_ENABLED:
          ...
          break case types.GET_REWARDS_INTERNALS_INFO:
              chrome.send('brave_rewards_internals.getRewardsInternalsInfo')
          break case types.ON_GET_REWARDS_ENABLED:
              state = {...state} state.isRewardsEnabled = action.payload.enabled
          break case types.ON_GET_REWARDS_INTERNALS_INFO:
              state = {...state} state.info = action.payload.info
          break default: break
      }
