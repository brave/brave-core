...
const rewardsInternalsReducer: Reducer<RewardsInternals.State | undefined> = (state: RewardsInternals.State | undefined, action) => {
  ...
  switch (action.type) {
    ...
    case types.ON_GET_REWARDS_INTERNALS_INFO:
      ...
      break
    case types.GET_ADS_INTERNALS_INFO:
      chrome.send('brave_rewards_internals.getAdsInternalsInfo')
      break
    case types.ON_GET_ADS_INTERNALS_INFO:
      state = { ...state }
      state.adsInfo = action.payload.info
      break
    ...
    case types.GET_EXTERNAL_WALLET:
      chrome.send('brave_rewards_internals.getExternalWallet', [action.payload.type])
      break
