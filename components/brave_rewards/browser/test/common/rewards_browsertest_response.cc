...
void RewardsBrowserTestResponse::LoadMocks() {
  ...
  for (auto& key : publisher_keys) {
    std::string prefix = core::publisher::GetHashPrefixRaw(key, 4);
...
void RewardsBrowserTestResponse::Get(
  ...
  if (url.find("/v0/me/cards") != std::string::npos) {
    if (base::EndsWith(
      ...
      *response_status_code = net::HTTP_ACCEPTED;
        ...
        *response =
            GetUpholdCard(external_balance_, GetUpholdExternalAddress());
    }
    return;
  }

  if (url.find("/v0/me/capabilities") != std::string::npos) {
    *response = GetUpholdCapabilities();
    return;
  }
  ...
  if (url.find("/v1/orders") != std::string::npos) {
    if (url.find("credentials") != std::string::npos) {
      ...
      *response = GetOrderCreateResponse(order_->Clone());
    }
