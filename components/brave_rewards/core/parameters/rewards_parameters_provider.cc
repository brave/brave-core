...
mojom::RewardsParametersPtr RewardsParametersProvider::LoadParameters() {
  return ValueToParameters(engine().GetState<base::Value>(state::kParameters));
}

void RewardsParametersProvider::GetParameters(GetParametersCallback callback) {
  // Return cached parameters if available.
  if (auto params = LoadParameters()) {
...
mojom::RewardsParametersPtr RewardsParametersProvider::ValueToParameters(
