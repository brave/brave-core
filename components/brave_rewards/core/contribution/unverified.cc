...
void Unverified::OnContributeUnverifiedPublishers(
  ...
  if (balance == 0) {
    ...
    ledger_->rewards_service()->OnContributeUnverifiedPublishers(
  ...
  for (const auto& item : list) {
    ...
    if (now > item->expiration_date) {
      ledger_->database()->RemovePendingContribution(
          item->id,
          std::bind(&Unverified::OnRemovePendingContribution, this, _1));
      continue;
    }
  ...
  auto get_callback = std::bind(&Unverified::WasPublisherProcessed, this, _1,
                                current->publisher_key, current->name);

  ledger_->database()->WasPublisherProcessed(current->publisher_key,
                                             get_callback);
  ...
  auto save_callback =
      std::bind(&Unverified::QueueSaved, this, _1, current->id);
...
void Unverified::WasPublisherProcessed(const mojom::Result result,
  ...
  auto save_callback = std::bind(&Unverified::ProcessedPublisherSaved, this, _1,
                                 publisher_key, name);
  ledger_->database()->SaveProcessedPublisherList({publisher_key},
                                                  save_callback);
}
