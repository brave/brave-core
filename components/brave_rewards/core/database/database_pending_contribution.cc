...
DatabasePendingContribution::DatabasePendingContribution(LedgerImpl* ledger)
...
void DatabasePendingContribution::GetReservedAmount(
  ...
  auto transaction_callback = std::bind(
      &DatabasePendingContribution::OnGetReservedAmount, this, _1, callback);
...
void DatabasePendingContribution::GetAllRecords(
  ...
  auto transaction_callback = std::bind(
      &DatabasePendingContribution::OnGetAllRecords, this, _1, callback);
...
void DatabasePendingContribution::OnGetAllRecords(
  ...
  for (auto const& record : response->result->get_records()) {
    ...
    info->expiration_date =
        info->added_date + constant::kPendingContributionExpirationInterval;
...
void DatabasePendingContribution::DeleteRecord(uint64_t id,
  ...
  auto transaction = mojom::DBTransaction::New();
  const std::string query = base::StringPrintf(
      "DELETE FROM %s WHERE pending_contribution_id = ?", kTableName);
