...
void LinkageChecker::UpdateSelfCustodyAvailableDict(
  ...
  bool previously_available = has_available_provider(
      Get<UserPrefs>().GetDict(prefs::kSelfCustodyAvailable));
  ...
  Get<UserPrefs>().SetDict(prefs::kSelfCustodyAvailable,
  ...
  if (!previously_available && currently_available) {
    if (!Get<UserPrefs>().GetBoolean(prefs::kSelfCustodyInviteDismissed)) {
