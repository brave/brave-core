...
#include "brave/components/brave_rewards/core/internal/wallet/wallet_util.h"
...
bool Wallet::SetWallet(mojom::RewardsWalletPtr wallet) {
  ...
  if (wallet->recovery_seed.size() > 1) {
    event_string = base::NumberToString(wallet->recovery_seed[0] +
                                        wallet->recovery_seed[1]);
  }
