...
#include "brave/components/brave_rewards/core/internal/state/state_migration.h"
...
void State::SetVersion(const int version) {
  engine_->database()->SaveEventLog(kVersion, base::NumberToString(version));
...
void State::SetPublisherMinVisitTime(const int duration) {
  engine_->database()->SaveEventLog(kMinVisitTime,
...
void State::SetPublisherMinVisits(const int visits) {
  engine_->database()->SaveEventLog(kMinVisits, base::NumberToString(visits));
...
void State::SetScoreValues(double a, double b) {
  engine_->database()->SaveEventLog(kScoreA, base::NumberToString(a));
  engine_->database()->SaveEventLog(kScoreB, base::NumberToString(b));
...
void State::SetCreationStamp(const uint64_t stamp) {
  engine_->database()->SaveEventLog(kCreationStamp,
