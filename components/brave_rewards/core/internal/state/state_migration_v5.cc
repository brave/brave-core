...
void StateMigrationV5::Migrate(ResultCallback callback) {
  ...
  events.insert(
      std::make_pair(kAutoContributeEnabled, base::NumberToString(enabled)));
  ...
  events.insert(std::make_pair("enabled", base::NumberToString(enabled)));

  // Next reconcile
  const auto reconcile_stamp = engine_->GetState<uint64_t>(kNextReconcileStamp);
  events.insert(std::make_pair(kNextReconcileStamp,
                               base::NumberToString(reconcile_stamp)));

  // Creation stamp
  const auto creation_stamp = engine_->GetState<uint64_t>(kCreationStamp);
  events.insert(
      std::make_pair(kCreationStamp, base::NumberToString(creation_stamp)));
