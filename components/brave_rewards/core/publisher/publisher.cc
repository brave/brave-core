...
void Publisher::SaveVisitInternal(const mojom::PublisherStatus status,
  ...
  uint64_t min_visit_time =
      engine_->Get<UserPrefs>().GetInteger(prefs::kMinVisitTime);
  ...
  if ((new_publisher || updated_publisher) &&
      (excluded || !engine_->contribution()->GetAutoContributeEnabled() ||
...
void Publisher::SynopsisNormalizer() {
  int min_visits = engine_->Get<UserPrefs>().GetInteger(prefs::kMinVisits);
  auto filter = CreateActivityFilter(
