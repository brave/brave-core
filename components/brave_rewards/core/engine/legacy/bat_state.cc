...
#include "brave/components/brave_rewards/core/engine/legacy/client_properties.h"
...
void LegacyBatState::OnLoad(ResultCallback callback,
  ...
  if (base::NumberToString(state_.reconcile_timestamp).length() > 10) {
    state_.reconcile_timestamp = state_.reconcile_timestamp / 1000;
  }

  // fix timestamp ms to s conversion
  if (base::NumberToString(state_.boot_timestamp).length() > 10) {
