...
BraveSyncClientImpl::BraveSyncClientImpl(SyncMessageHandler* handler,
  ...
  extensions::ExtensionSystem::Get(profile)->ready().Post(
      FROM_HERE,
      base::Bind(&BraveSyncClientImpl::OnExtensionSystemReady,
...
void BraveSyncClientImpl::SendGotInitData(const Uint8Array& seed,
  ...
  brave_sync_event_router_->GotInitData(
      seed, device_id, config_extension, sync_words);
}
...
void BraveSyncClientImpl::SendFetchSyncRecords(
  ...
  brave_sync_event_router_->FetchSyncRecords(
      category_names, startAt, max_records);
}
