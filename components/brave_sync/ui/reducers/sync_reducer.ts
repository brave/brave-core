...
const syncReducer: Reducer<Sync.State|undefined> =
    (state: Sync.State|undefined, action: any) => {
      if (state === undefined) {
        state = storage.load()
      }
      ...
      switch (action.type) {
        ...
        case types.SYNC_ON_PAGE_LOADED:
          ...
          break
              ...
              case types.SYNC_ON_SHOW_SETTINGS: const devices =
              payload.devices.map((device: Sync.DevicesFromBackEnd) => {
                return {
                  name: device.name, id: device.device_id,
          ...
          break
              ...
              case types.SYNC_ON_HAVE_SEED_FOR_QR_CODE: if (!payload.seed) {
            break
          }
          ...
          break
              ...
              case types.SYNC_ON_GENERATE_QR_CODE_IMAGE_SOURCE:
                  state = {
            ...state,
            seedQRImageSource: payload.imageSource
          } break

                          case types.SYNC_ON_HAVE_SYNC_WORDS: state =
          {...state,
           syncWords: payload.syncWords } break

              case types.SYNC_ON_SETUP_NEW_TO_SYNC:
                  chrome.send('setupSyncNewToSync', [payload.thisDeviceName])
          break

              case types.SYNC_ON_RESET: chrome.send('resetSync')
          // sync is reset. clear all data
          state =
          {...storage.defaultState } break

              case types.SYNC_ON_REMOVE_DEVICE:
                  if (typeof payload.id === 'undefined' ||
                      typeof payload.deviceName === 'undefined') {
            break
          }
          ...
          break
              ...
              case types.SYNC_SETUP_SYNC_HAVE_CODE: chrome.send(
                  'setupSyncHaveCode', [payload.syncWords, payload.deviceName])
          break

              case types.SYNC_SETUP_ERROR: state =
          {...state,
           error: payload.error } break
            ...
            console.info(
                '[SYNC] log message received from sync:', payload.message)
          }
          break
      }
