 ...
 * @param {Sync.DevicesFromBackEnd} devices - the sync original devices
 *     information
 */
export const onShowSettings =
    (settings: Sync.SettingsFromBackEnd, devices: Sync.DevicesFromBackEnd) => {
 ...
 */
export const onHaveSeedForQrCode =
    (seed: string) => {
...
/**
 * Dispatches a message telling the back-end that the user removed a given
 * device
 * @param {number} id - The device ID
 * @param {string} deviceName - The device name
 */
export const onRemoveDevice =
...
/**
 * Dispatches a message telling the back-end that user toggled the bookmarks
 * sync
 * @param {boolean} shouldEnable - Whether or not it should sync the bookmarks
 */
export const onSyncBookmarks =
    (shouldEnable: boolean) => {
      return action(types.SYNC_BOOKMARKS, {shouldEnable})
    }

/**
 * Dispatches a message telling the back-end that user toggled the site settings
 * sync
 * @param {boolean} shouldEnable - Whether or not it should sync the site
 *     settings
 */
export const onSyncSavedSiteSettings =
    (shouldEnable: boolean) => {
      return action(types.SYNC_SAVED_SITE_SETTINGS, {shouldEnable})
    }

/**
 * Dispatches a message telling the back-end that user toggled the browsing
 * history sync
 * @param {boolean} shouldEnable - Whether or not it should sync browsing
 *     history
 */
export const onSyncBrowsingHistory =
    (shouldEnable: boolean) => {
 ...
 */
export const onSetupSyncHaveCode =
    (syncWords: string, deviceName: string) => {
 ...
 */
export const onSyncSetupError =
    (error: Sync.SetupErrorType) => {
...
/**
 * Dispatched by the back-end to inform useful log messages for debugging
 * purposes
