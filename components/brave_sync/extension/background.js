...
chrome.braveSync.onGotInitData.addListener(function(
    seed, device_id, config, sync_words) {
  ...
  if (sync_words) {
    try {
      ...
      console.log(`"onGotInitData" sync_words=${
          JSON.stringify(sync_words)} err.message=${err.message}`);
      chrome.braveSync.syncSetupError('ERR_SYNC_WRONG_WORDS')
      return;
    }
  }
  console.log(`"got-init-data" seed=${JSON.stringify(seed)} device_id=${
      JSON.stringify(device_id)} config=${JSON.stringify(config)}`);
  callbackList['got-init-data'](null, seed, device_id, config);
});

chrome.braveSync.onFetchSyncRecords.addListener(function(
    category_names, start_at, max_records) {
  console.log(`"fetch-sync-records" category_names=${
      JSON.stringify(category_names)} start_at=${
      JSON.stringify(start_at)} max_records=${JSON.stringify(max_records)}`);
  callbackList['fetch-sync-records'](
      null, category_names, start_at, max_records);
});
...
chrome.braveSync.onResolveSyncRecords.addListener(function(
    category_name, recordsAndExistingObjects) {
  ...
  for (var i = 0; i < recordsAndExistingObjects.length; ++i) {
    ...
    if ('localRecord' in cur_rec) {
      ...
      recordsAndExistingObjectsArrArr.push(
          [cur_rec.serverRecord, cur_rec.localRecord]);
    } else {
      fixupSyncRecordBrowserToExt(cur_rec.serverRecord);
      removeLocalMeta(cur_rec.serverRecord);
  ...
  console.log(`"resolve-sync-records" category_name=${
      JSON.stringify(category_name)} recordsAndExistingObjects=${
      JSON.stringify(recordsAndExistingObjectsArrArr)}`);
  callbackList['resolve-sync-records'](
      null, category_name, recordsAndExistingObjectsArrArr);
});

chrome.braveSync.onSendSyncRecords.addListener(function(
  ...
  console.log(`"send-sync-records" category_name=${
      JSON.stringify(category_name)} records=${JSON.stringify(records)}`);
  callbackList['send-sync-records'](null, category_name, records);
});

chrome.braveSync.onSendGetBookmarksBaseOrder.addListener(function(
    deviceId, platform) {
  console.log(`"get-bookmarks-base-order" deviceId=${
      JSON.stringify(deviceId)} platform=${JSON.stringify(platform)}`);
...
chrome.braveSync.onNeedSyncWords.addListener(function(seed) {
  ...
  var words = module.exports.passphrase.fromBytesOrHex(
      buffer, /*useNiceware=*/ false /* use bip39 */);
  console.log(`"NeedSyncWords" seed=${JSON.stringify(seed)} words=${
...
function getOrder(record) {
  if ('bookmark' in record) {
    if (!record.bookmark.order) {
      ...
      console.log(
          `"get-bookmark-order" prevOrder=${prevOrder}` +
          ` nextOrder=${record.bookmark.nextOrder} parentOrder=${parentOrder}`);
      callbackList['get-bookmark-order'](
          null, prevOrder, record.bookmark.nextOrder, parentOrder);
...
function fixupBookmarkParentFolderObjectId(category_name, records) {
  // records[0].bookmark.parentFolderObjectId can be either Uint8Array or
  // Array[] Uint8Array is expanded to "binary", Array[] is expanded to "array"
  // of "integer" in schema But schema does not support "oneOf", so forcing all
  // to Uint8Array
...
function fixupSyncRecordExtToBrowser(sync_record) {
  ...
  if ('objectIdStr' in sync_record) {
    if (sync_record.objectIdStr) {
      sync_record.objectId =
          new Uint8Array(IntArrayFromString(sync_record.objectIdStr));
    }
  ...
  if ('deviceIdStr' in sync_record) {
    if (sync_record.deviceIdStr) {
      sync_record.deviceId =
          new Uint8Array(IntArrayFromString(sync_record.deviceIdStr));
    }
  ...
  if ('bookmark' in sync_record) {
    if ('parentFolderObjectId' in sync_record.bookmark) {
      if (Array.isArray(sync_record.bookmark.parentFolderObjectId)) {
        sync_record.bookmark.parentFolderObjectId =
            new Uint8Array(sync_record.bookmark.parentFolderObjectId);
      } else if (
          sync_record.bookmark.parentFolderObjectId == null ||
          sync_record.bookmark.parentFolderObjectId.length == 0) {
    ...
    if ('parentFolderObjectIdStr' in sync_record.bookmark) {
      if (sync_record.bookmark.parentFolderObjectIdStr) {
        sync_record.bookmark.parentFolderObjectId = new Uint8Array(
            IntArrayFromString(sync_record.bookmark.parentFolderObjectIdStr));
      }
    ...
    if ('prevObjectId' in sync_record.bookmark) {
      if (Array.isArray(sync_record.bookmark.prevObjectId)) {
        sync_record.bookmark.prevObjectId =
            new Uint8Array(sync_record.bookmark.prevObjectId);
      } else if (
          sync_record.bookmark.prevObjectId == null ||
          sync_record.bookmark.prevObjectId.length == 0) {
    ...
    if ('prevObjectIdStr' in sync_record.bookmark) {
      if (sync_record.bookmark.prevObjectIdStr) {
        sync_record.bookmark.prevObjectId = new Uint8Array(
            IntArrayFromString(sync_record.bookmark.prevObjectIdStr));
      }
  ...
  handleMessage(message, arg1, arg2, arg3, arg4) {
    switch (message) {
      ...
      case 'sync-setup-error':
        ...
        var errorToPass =
            (arg1 ===
             'Credential server response 400. Signed request body of the client timestamp is required.') ?
      ...
      case 'save-init-data':
        ...
        console.log(`"save-init-data" seed=${JSON.stringify(arg1)} deviceId=${
      ...
      case 'get-existing-objects':
        ...
        console.log(`"get-existing-objects" category_name=${
            JSON.stringify(
                arg1)} records=${JSON.stringify(arg2)} lastRecordTimeStamp=${
            arg3 ? arg3 : 0} isTruncated=${arg4 != undefined ? arg4 : false} `);
        chrome.braveSync.getExistingObjects(
            arg1 /*category_name*/,
            arg2 /*records*/,
            arg3 ? arg3 : 0 /*lastRecordTimeStamp*/,
            arg4 != undefined ? arg4 : false /*isTruncated*/);
        break;
      case 'resolved-sync-records':
        fixupSyncRecordsArrayExtensionToBrowser(arg2);
        console.log(`"resolved-sync-records" categoryName=${
            JSON.stringify(arg1)} records=${JSON.stringify(arg2)}`);
        chrome.braveSync.resolvedSyncRecords(
            arg1 /*categoryName*/, arg2 /*records*/);
        break;
      case 'save-bookmarks-base-order':
        console.log(
            `"save-bookmarks-base-order" order=${JSON.stringify(arg1)} `);
        chrome.braveSync.saveBookmarksBaseOrder(arg1 /*order*/);
        break;
      case 'save-bookmark-order':
        console.log(`"save-bookmark-order" order=${
            JSON.stringify(arg1)} prevOrder=${JSON.stringify(arg2)} nextOrder=${
            JSON.stringify(arg3)} parentOrder=${JSON.stringify(arg4)}`);
      ...
      default:
        console.log(
            'background.js TAGAB InjectedObject.handleMessage unknown message',
            message,
            arg1,
            arg2,
            arg3,
...
function LoadJsLibScript() {
  var sync_bundle_url =
      chrome.runtime.getURL('extension/brave-sync/bundles/bundle.js');
...
if (!self.chrome.ipc) {
  ...
  ipc.once = function(message, cb) {
    ...
    // injectedObject.handleMessage(message, '0', '0', '', false); // seems I
    // should not call it. Just save the fn here
  };
