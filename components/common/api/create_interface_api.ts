...
} & Omit<MutationOptions<NoInfer<R>>, 'mutationKey' | 'mutationFn'>
 ...
 */
export type EventDef<Args extends any[], Payload> = {
...
>(config: {
  ...
  // dedicated QueryClient for this API
  ...
  type UseQueryResult<K extends ValidKey> = BaseUseQueryResult<K> & {
    ...
    // }
    // ```
    [P in K as `${P}Data`]: RawEndpoints[K] extends { placeholderData: any }
      ? DataOf<K>
      : BaseUseQueryResult<K>['data']
    ...
    data: RawEndpoints[K] extends { placeholderData: any }
      ? DataOf<K>
      : BaseUseQueryResult<K>['data']
  }
  ...
  type APIEndpoints = {
    [K in ValidKey]: RawEndpoints[K] extends QueryEndpointDefinition<any, any>
      ? {
           ...
           */
          current: (...args: ArgsOf<K>) => DataOf<K> | undefined
        ...
        (RawEndpoints[K] extends { placeholderData: DataOf<K> }
          ? {
              ...
              // provide an even more convenient hook.
    ...
    if ('query' in endpointDef) {
      ...
      const useQ = (...args: QArgs) => {
        const useQueryResult = useQuery<QData>({
          ...
          ...queryOptions,
        }, queryClient) as UseQueryResult<typeof name>
        return {
          ...useQueryResult,
      ...
      const updater = (
        ...
        queryClient.setQueryData<QData>([...baseKey, ...args], (old) => {
          ...
          return updateFromOld(old, updateData)
        })
      }
  ...
  ;(Object.keys(config.endpoints) as Array<keyof RawEndpoints>).forEach(
    (name) => {
      ...
      if ('query' in config.endpoints[name]) {
        // @ts-expect-error: we know that `endpoints[name].useQuery` matches the signature
