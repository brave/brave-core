 ...
 public:
  TimePeriodStorageTest() : clock_(new base::SimpleTestClock) {
    ...
    state_ = std::make_unique<TimePeriodStorage>(
        &pref_service_, kPrefName, 7, std::unique_ptr<base::Clock>(clock_));
...
TEST_F(TimePeriodStorageTest, ForgetsOldSavings) {
  uint64_t saving = 10000;
  state_->AddDelta(saving);
  EXPECT_EQ(state_->GetPeriodSum(), saving);
...
TEST_F(TimePeriodStorageTest, IntermittentUsage) {
  uint64_t saving = 10000;
...
TEST_F(TimePeriodStorageTest, InfrequentUsage) {
  uint64_t saving = 10000;
  state_->AddDelta(saving);
