...
uint64_t TimePeriodStorage::GetHighestValueInPeriod() const {
  // We record only value for last N days.
  const base::Time n_days_ago = clock_->Now() - base::Days(period_days_);
  std::list<DailyValue> last_period_daily_values(daily_values_.size());
  auto copied_it =
      std::copy_if(daily_values_.begin(), daily_values_.end(),
                   last_period_daily_values.begin(),
                   [n_days_ago](auto i) { return i.day > n_days_ago; });
  last_period_daily_values.resize(
      std::distance(last_period_daily_values.begin(), copied_it));

  auto highest_it = std::max_element(last_period_daily_values.begin(),
                                     last_period_daily_values.end(),
                                     [](const auto& left, const auto& right) {
                                       return left.value < right.value;
                                     });
  if (highest_it == last_period_daily_values.end()) {
    return 0;
  }
