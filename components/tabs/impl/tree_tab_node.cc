...
const TreeTabNode& TreeTabNode::GetEmptyTreeTabNode() {
  static base::NoDestructor<TreeTabNodeTabCollection>
      empty_tree_tab_node_tab_collection(
          ...
          std::unique_ptr<TabInterface>(nullptr), base::DoNothing(),
          base::DoNothing());
...
std::vector<const TabInterface*> TreeTabNode::GetTabs() const {
  ...
  if (const base::raw_ptr<SplitTabCollection>* split_ptr =
          std::get_if<base::raw_ptr<SplitTabCollection>>(&*value)) {
  ...
  if (const base::raw_ptr<TabGroupTabCollection>* group_ptr =
          std::get_if<base::raw_ptr<TabGroupTabCollection>>(&*value)) {
