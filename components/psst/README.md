...
The `TimedScriptRepeatingCallback` class is used to handle cases when a script fails to finish running, (i.e. ["The script times out or page crashes"](#script-times-out)).</br>
Constructor:
```
...
```
Parameters:</br>
`InjectScriptCallback inject_script_callback` - callback function that injects the script into a separate world;</br>
`InsertScriptInPageTimeoutCallback timeout_callback` - callback function that is triggered if the script does not finish within the allowed time;</br>
`base::TimeDelta timeout` - the maximum time to wait for the script response;

</br>To run a script, use the `Run` method:</br>
```
void Run(const int last_committed_entry_id, const std::string& script, InsertScriptInPageCallback callback);
```
parameters:</br>
