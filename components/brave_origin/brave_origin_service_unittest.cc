...
class BraveOriginServiceTest : public testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    local_state_.registry()->RegisterBooleanPref("test.browser.pref", false);
    local_state_.registry()->RegisterBooleanPref(
        "test.browser.not_user_settable", false);
    ...
    // policy
    profile_prefs_.registry()->RegisterBooleanPref("test.profile.pref", true);
    profile_prefs_.registry()->RegisterBooleanPref(
        "test.profile.not_user_settable", true);
    ...
    service_ = std::make_unique<BraveOriginService>(
        &local_state_, &profile_prefs_, "test-profile-id",
 ...
 protected:
  ...
  BraveOriginPolicyMap CreateBrowserTestPolicies() {
    ...
    CreateTestPolicy(test_policies, "test.browser.pref", false, true);
    CreateTestPolicy(test_policies, "test.browser.not_user_settable", false,
                     false);
  ...
  BraveOriginPolicyMap CreateProfileTestPolicies() {
    BraveOriginPolicyMap test_policies;
    CreateTestPolicy(test_policies, "test.profile.pref", true, true);
    CreateTestPolicy(test_policies, "test.profile.not_user_settable", true,
                     false);
    return test_policies;
  }
...
TEST_F(BraveOriginServiceTest, SetBrowserPolicyValue_UserSettable_SetsPrefs) {
  ...
  bool result = service_->SetBrowserPolicyValue("test.browser.pref", true);
  EXPECT_TRUE(result);
  ...
  bool local_state_value = local_state_.GetBoolean("test.browser.pref");
...
TEST_F(BraveOriginServiceTest,
  ...
  bool result =
      service_->SetBrowserPolicyValue("test.browser.not_user_settable", false);
  EXPECT_TRUE(result);

  // Should be set in policy manager
  auto policy_value = BraveOriginPolicyManager::GetInstance()->GetPolicyValue(
      "test.browser.not_user_settable");
  EXPECT_TRUE(policy_value.has_value());
  EXPECT_FALSE(policy_value.value());

  // Should clear the pref in local_state since it equals default and is not
  // user-settable
  EXPECT_FALSE(local_state_.HasPrefPath("test.browser.not_user_settable"));
}
...
TEST_F(BraveOriginServiceTest,
  ...
  // default is false)
  bool result =
      service_->SetBrowserPolicyValue("test.browser.not_user_settable", true);
  EXPECT_TRUE(result);
  ...
  bool local_state_value =
      local_state_.GetBoolean("test.browser.not_user_settable");
  EXPECT_TRUE(local_state_value);
}
...
TEST_F(BraveOriginServiceTest, SetProfilePolicyValue_UserSettable_SetsPrefs) {
  ...
  bool result = service_->SetProfilePolicyValue("test.profile.pref", false);
  EXPECT_TRUE(result);

  // Should be set in both the policy manager and profile_prefs
  auto policy_value = BraveOriginPolicyManager::GetInstance()->GetPolicyValue(
      "test.profile.pref", "test-profile-id");
  EXPECT_TRUE(policy_value.has_value());
  EXPECT_FALSE(policy_value.value());

  bool profile_prefs_value = profile_prefs_.GetBoolean("test.profile.pref");
...
TEST_F(BraveOriginServiceTest,
  ...
  bool result =
      service_->SetProfilePolicyValue("test.profile.not_user_settable", true);
  EXPECT_TRUE(result);

  // Should be set in policy manager
  auto policy_value = BraveOriginPolicyManager::GetInstance()->GetPolicyValue(
      "test.profile.not_user_settable", "test-profile-id");
  EXPECT_TRUE(policy_value.has_value());
  EXPECT_TRUE(policy_value.value());

  // Should clear the pref in profile_prefs since it equals default and is not
  // user-settable
  EXPECT_FALSE(profile_prefs_.HasPrefPath("test.profile.not_user_settable"));
}
...
TEST_F(BraveOriginServiceTest,
  ...
  // default is true)
  bool result =
      service_->SetProfilePolicyValue("test.profile.not_user_settable", false);
  EXPECT_TRUE(result);
  ...
  bool profile_prefs_value =
      profile_prefs_.GetBoolean("test.profile.not_user_settable");
  EXPECT_FALSE(profile_prefs_value);
}

TEST_F(BraveOriginServiceTest, SetBrowserPolicyValue_UnknownPref_ReturnsFalse) {
  bool result = service_->SetBrowserPolicyValue("unknown.pref", true);
  EXPECT_FALSE(result);

  // Should not affect any prefs
  auto policy_value =
      BraveOriginPolicyManager::GetInstance()->GetPolicyValue("unknown.pref");
  EXPECT_FALSE(policy_value.has_value());
}

TEST_F(BraveOriginServiceTest, SetProfilePolicyValue_UnknownPref_ReturnsFalse) {
  bool result = service_->SetProfilePolicyValue("unknown.pref", true);
  EXPECT_FALSE(result);

  // Should not affect any prefs
  auto policy_value = BraveOriginPolicyManager::GetInstance()->GetPolicyValue(
      "unknown.pref", "test-profile-id");
  EXPECT_FALSE(policy_value.has_value());
}
...
TEST_F(BraveOriginServiceTest,
  ...
  BraveOriginPolicyManager::GetInstance()->SetBrowserPolicyValue(
      "test.browser.pref", true);

  // Service should return the same value
  auto result = service_->GetBrowserPrefValue("test.browser.pref");
...
TEST_F(BraveOriginServiceTest,
  ...
  BraveOriginPolicyManager::GetInstance()->SetProfilePolicyValue(
      "test.profile.pref", false, "test-profile-id");

  // Service should return the same value
  auto result = service_->GetProfilePrefValue("test.profile.pref");
  EXPECT_TRUE(result.has_value());
  EXPECT_FALSE(result.value());
}

TEST_F(BraveOriginServiceTest, GetBrowserPrefValue_UnknownPref_ReturnsNullopt) {
  auto result = service_->GetBrowserPrefValue("unknown.pref");
  EXPECT_FALSE(result.has_value());
}

TEST_F(BraveOriginServiceTest, GetProfilePrefValue_UnknownPref_ReturnsNullopt) {
  auto result = service_->GetProfilePrefValue("unknown.pref");
  EXPECT_FALSE(result.has_value());
}

TEST_F(BraveOriginServiceTest, SetThenGet_BrowserPolicy_Consistency) {
  // Set via service
  bool set_result = service_->SetBrowserPolicyValue("test.browser.pref", true);
  EXPECT_TRUE(set_result);

  // Get via service
  auto get_result = service_->GetBrowserPrefValue("test.browser.pref");
  EXPECT_TRUE(get_result.has_value());
  EXPECT_TRUE(get_result.value());

  // Change value
  set_result = service_->SetBrowserPolicyValue("test.browser.pref", false);
  EXPECT_TRUE(set_result);

  // Verify change
  get_result = service_->GetBrowserPrefValue("test.browser.pref");
  EXPECT_TRUE(get_result.has_value());
...
TEST_F(BraveOriginServiceTest, SetThenGet_ProfilePolicy_Consistency) {
  // Set via service
  bool set_result = service_->SetProfilePolicyValue("test.profile.pref", false);
  EXPECT_TRUE(set_result);

  // Get via service
  auto get_result = service_->GetProfilePrefValue("test.profile.pref");
  EXPECT_TRUE(get_result.has_value());
  EXPECT_FALSE(get_result.value());

  // Change value
  set_result = service_->SetProfilePolicyValue("test.profile.pref", true);
  EXPECT_TRUE(set_result);

  // Verify change
  get_result = service_->GetProfilePrefValue("test.profile.pref");
  EXPECT_TRUE(get_result.has_value());
  EXPECT_TRUE(get_result.value());
}
...
TEST_F(BraveOriginServiceTest, PolicyValueStoredInCorrectBraveOriginLocation) {
  // Set browser policy
  service_->SetBrowserPolicyValue("test.browser.pref", true);
  ...
  const auto* policy_info =
      BraveOriginPolicyManager::GetInstance()->GetPrefInfo("test.browser.pref");
  ...
  // Set profile policy
  service_->SetProfilePolicyValue("test.profile.pref", false);

  // Get the expected profile key
  const auto* profile_policy_info =
      BraveOriginPolicyManager::GetInstance()->GetPrefInfo("test.profile.pref");
  ASSERT_NE(profile_policy_info, nullptr);
  std::string expected_profile_key =
      GetBraveOriginProfilePrefKey(*profile_policy_info, "test-profile-id");
...
TEST_F(BraveOriginServiceTest, SetPrefBehavior_NotUserSettableWithNonDefault) {
  ...
  EXPECT_TRUE(local_state_.HasPrefPath("test.browser.not_user_settable"));
  EXPECT_TRUE(local_state_.GetBoolean("test.browser.not_user_settable"));
...
TEST_F(BraveOriginServiceDisabledTest, FeatureDisabled_NoSideEffects) {
  ...
  EXPECT_FALSE(local_state_.GetBoolean("test.browser.pref"));
  EXPECT_TRUE(profile_prefs_.GetBoolean("test.profile.pref"));
}
