...
// On `ComponentReady` it hands the install dir to
// `OnDeviceSpeechModelsState`, which the speech recognition controller
// observes. Ships as its own component (distinct from the embedding model's
// `BraveLocalAIModels`), gated by the on-device speech recognition feature.
//
