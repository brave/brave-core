...
smallEl.textContent = 'Shared per-profile service that automatically loads the model from ~/Downloads/embeddinggemma-300m/'

serviceStatus.textContent = '' // Clear existing content
...
compareBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    // Display results
    const text1Preview = `"${text1.substring(0, 50)}${text1.length > 50 ?
      '...' : ''}"`
    const text2Preview = `"${text2.substring(0, 50)}${text2.length > 50 ?
      '...' : ''}"`
