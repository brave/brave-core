...
impl RelPositionMultiHeadAttention {
    pub fn new(cfg: &NemotronConfig, vb: VarBuilder) -> Result<Self> {
        ...
        let d_k = d / h;
        Ok(Self {
            linear_q: linear_no_bias(d, d, vb.pp("linear_q"))?,
            linear_k: linear_no_bias(d, d, vb.pp("linear_k"))?,
            linear_v: linear_no_bias(d, d, vb.pp("linear_v"))?,
            linear_out: linear_no_bias(d, d, vb.pp("linear_out"))?,
