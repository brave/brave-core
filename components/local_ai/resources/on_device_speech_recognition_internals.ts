...
import { OnDeviceSpeechRecognitionService } from 'gen/brave/components/local_ai/core/local_ai.mojom.m.js'
...
async function transcribeAudio(audio: Float32Array) {
  ...
  // Balance the createAsrSession above so the controller's active-session
  // count returns to zero and its idle timer can tear the worker down
  // (freeing the loaded model). The real Web Speech engine does this in
  // EndRecognition; the direct-mojo test path must do it explicitly, or
  // the worker stays resident forever.
