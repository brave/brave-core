...
// WASM entry point for Parakeet-TDT-0.6B-v3 (Q4_K GGUF variant).
// TdtRecognizer600mQ4K owns the streaming session: GGUF loading,
...
#[wasm_bindgen]
pub struct TdtRecognizer600mQ4K {
    model: TdtState,
    tokenizer: Tokenizer,
    mel_filters: Vec<f32>,
    config: TdtConfig,
...
impl TdtRecognizer600mQ4K {
    ...
    pub fn new(
        ...
        mel_filters: Vec<u8>,
        config: Vec<u8>,
    ) -> Result<TdtRecognizer600mQ4K, JsError> {
...
impl TdtRecognizer600mQ4K {
    ...
    fn run_inference(&mut self, audio_f32: &[f32]) -> Result<String, JsError> {
        ...
        log_error(&format!(
            "[tdt-600m-q4k] timing: mel={:.0}ms enc+dec={:.0}ms tok={:.0}ms \
