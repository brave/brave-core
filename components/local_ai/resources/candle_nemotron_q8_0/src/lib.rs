...
// uses them through the `NemotronTranscriberQ80` wrapper below.
pub mod audio;
pub mod decode;
pub mod model;
...
#[wasm_bindgen]
pub struct NemotronTranscriberQ80 {
    state: NemotronState,
    tokenizer: Tokenizer,
    mel_filters: Vec<f32>,
...
impl NemotronTranscriberQ80 {
    ...
    pub fn new(
        ...
        mel_filters: Vec<u8>,
        config: Vec<u8>,
    ) -> Result<NemotronTranscriberQ80, JsError> {
    ...
    pub fn mark_done(&mut self) -> Result<String, JsError> {
        ...
        // Flush any residue: pad up to CHUNK_SAMPLES with zeros and
        // run a final chunk. nemotron's cache-aware encoder accepts
        // partial chunks via padding without disturbing future
        // sessions because we reset the cache in `reset()` between
        // utterances.
