...
import actions from '../../../../brave_extension/extension/brave_extension/background/actions/windowActions'
    ...
    interface WindowIdEvent extends chrome.events.Event<(windowId : number) => void> {emit : (windowId : number) => void}

                                                                                      interface WindowReferenceEvent extends chrome.events.Event<(window : chrome.windows.Window) => void> {emit : (window : chrome.windows.Window) => void}

                                                                                                                                                                                            describe('windowsEvents events', () => {describe('chrome.windows.onFocusChanged', () => {let spy : jest.SpyInstance
beforeEach(() => {spy = jest.spyOn(actions, 'windowFocusChanged')})
afterEach(() => {spy.mockRestore()})
  ...
  const window = {
    ...
    incognito: false,
    type: 'normal'
  } beforeEach(() => {spy = jest.spyOn(actions, 'windowCreated')})
  afterEach(() => {spy.mockRestore()})
  ...
  describe('chrome.windows.onRemoved', () => {
    let spy: jest.SpyInstance
  beforeEach(() => {spy = jest.spyOn(actions, 'windowRemoved')})
  afterEach(() => {spy.mockRestore()})
