...
import actions from '../../../../brave_extension/extension/brave_extension/background/actions/tabActions'
    ...
    interface TabActivatedEvent extends chrome.events.Event<(activeInfo : chrome.tabs.TabActiveInfo) => void> {emit : (detail : chrome.tabs.TabActiveInfo) => void}

                                                                                                               interface TabCreatedEvent extends chrome.events.Event<(activeInfo : chrome.tabs.Tab) => void> {emit : (detail : chrome.tabs.Tab) => void}

                                                                                                                                                                                                              interface TabUpdatedEvent extends chrome.events.Event<(tabId : number, changeInfo : chrome.tabs.TabChangeInfo, tab : chrome.tabs.Tab) => void> {emit : (tabId : number, changeInfo : chrome.tabs.TabChangeInfo, tab : chrome.tabs.Tab) => void}

                                                                                                                                                                                                                                                                                                                                                                      describe('tabsEvents events', () => {describe('chrome.tabs.onActivated', () => {let spy : jest.SpyInstance
beforeEach(() => {spy = jest.spyOn(actions, 'activeTabChanged')})
afterEach(() => {spy.mockRestore()})
  ...
  const inputTab = {
    ...
    incognito: false,
    selected: true
  } beforeEach(() => {spy = jest.spyOn(actions, 'tabCreated')})
  afterEach(() => {spy.mockRestore()})
  ...
  describe('chrome.tabs.onUpdated', () => {
    let spy: jest.SpyInstance
  const inputTabId = 3
  const inputChangeInfo = {} const inputTab = {
    id: 3,
    index: 0,
    pinned: false,
    highlighted: false,
    ...
    incognito: false,
    selected: true
  } beforeEach(() => {spy = jest.spyOn(actions, 'tabDataChanged')})
  afterEach(() => {spy.mockRestore()})
