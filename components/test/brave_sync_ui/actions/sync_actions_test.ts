...
it('onShowSettings', () => {
  const settings =
  {
    sync_bookmarks: true, sync_configured: true, sync_history: true,
        sync_settings: true, sync_this_device: true
  } const devices = {
    name: 'MIKE TYSON PC',
    device_id: 1,
    last_active: 0
  } expect(actions.onShowSettings(settings, devices))
                        .toEqual({
...
it('onGenerateQRCodeImageSource',
   () => {
       expect(actions.onGenerateQRCodeImageSource('someImageConvertedToBase64'))
           .toEqual({
...
it('onSyncReset', () => {expect(actions.onSyncReset()).toEqual({
                    type: types.SYNC_ON_RESET,
                    meta: undefined,
                    payload: undefined
                  })})

it('onSyncBookmarks', () => {expect(actions.onSyncBookmarks(true)).toEqual({
