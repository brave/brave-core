...
import {convertBalance, convertProbiToFixed, formatConverted} from '../../../brave_rewards/resources/ui/utils'
...
it('rates are empty', () => {expect(convertBalance('10', {})).toBe('0.00')})

it('rate is missing',
   () => {expect(convertBalance('10', {'USD': 10}, 'EUR')).toBe('0.00')})

it('all good', () => {expect(convertBalance('10', {'USD': 10})).toBe('100.00')})
  ...
  it('probi is not 10^18', () => {expect(convertProbiToFixed('9')).toBe('0.0')})
