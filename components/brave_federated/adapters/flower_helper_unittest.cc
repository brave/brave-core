...
std::map<std::string, double> config_metrics = {{"loss", 0.42}, {"accuracy", 42.0}};
...
TEST(BraveFederatedLearningFlowerHelperTest, ParseFitTaskListFromResponseBody) {
    ...
    flower::TaskIns* task_instruction = fit_task_instruction_response.add_task_ins_list();
    ...
    *fit_instruction.mutable_parameters() = GetParametersFromVectors(test_parameters);
    ...
    *task_instruction->mutable_task() = flower_task;
    ...
    const std::string response_string = fit_task_instruction_response.SerializeAsString();
    ...
    EXPECT_EQ(task_config["accuracy"], static_cast<float>(config_metrics["accuracy"]));
}

TEST(BraveFederatedLearningFlowerHelperTest, ParseEvaluateTaskListFromResponseBody) {
    // Arrange
    flower::PullTaskInsResponse eval_task_instruction_response;
    flower::TaskIns* task_instruction = eval_task_instruction_response.add_task_ins_list();
    task_instruction->set_task_id("42");
    task_instruction->set_group_id("23");
    task_instruction->set_workload_id("8");
    ...
    *eval_instruction.mutable_parameters() = GetParametersFromVectors(test_parameters);
    *eval_instruction.mutable_config() = MetricsToProto(config_metrics);
    *server_message.mutable_evaluate_ins() = eval_instruction;
    *flower_task.mutable_legacy_server_message() = server_message;
    *task_instruction->mutable_task() = flower_task;

    const std::string response_string = eval_task_instruction_response.SerializeAsString();
...
TEST(BraveFederatedLearningFlowerHelperTest, BuildPostTrainTaskResultsPayload) {
    ...
    PerformanceReport performance_report = PerformanceReport(dataset_size, loss, accuracy, test_parameters, metrics);
    ...
    std::vector<Weights> parameters = GetVectorsFromParameters(fit_result.parameters());
    EXPECT_EQ(parameters.size(), 2U);
    EXPECT_EQ(parameters, test_parameters);

    std::map<std::string, float> flower_metrics = ConfigsFromProto(fit_result.metrics());
...
TEST(BraveFederatedLearningFlowerHelperTest, BuildPostEvaluateTaskResultsPayload) {
    // Arrange
    TaskId task_id = {"42", "23", "8"};
    ...
    flower::Task flower_task = flower_task_result.task();
    flower::ClientMessage client_message = flower_task.legacy_client_message();
    flower::ClientMessage_EvaluateRes evaluate_result = client_message.evaluate_res();
    EXPECT_EQ(static_cast<size_t>(evaluate_result.num_examples()), dataset_size);
    EXPECT_EQ(evaluate_result.loss(), loss);

    std::map<std::string, float> flower_metrics = ConfigsFromProto(evaluate_result.metrics());
    EXPECT_EQ(flower_metrics.size(), 2U);
    EXPECT_EQ(flower_metrics["alpha"], static_cast<float>(metrics["alpha"]));
    EXPECT_EQ(flower_metrics["beta"], static_cast<float>(metrics["beta"]));
}
