...
struct CustomBackoffEntryPolicy : net::BackoffEntry::Policy {
    ...
    static void RegisterJSONConverter(base::JSONValueConverter<CustomBackoffEntryPolicy>* policy_converter) {
        policy_converter->RegisterIntField("num_errors_to_ignore", &CustomBackoffEntryPolicy::num_errors_to_ignore);
        policy_converter->RegisterIntField("initial_delay_ms", &CustomBackoffEntryPolicy::initial_delay_ms);
        policy_converter->RegisterDoubleField("multiply_factor", &CustomBackoffEntryPolicy::multiply_factor);
        policy_converter->RegisterDoubleField("jitter_factor", &CustomBackoffEntryPolicy::jitter_factor);
        policy_converter->RegisterCustomField<int64_t>("maximum_backoff_ms", &CustomBackoffEntryPolicy::maximum_backoff_ms, &ParseInt64);
        policy_converter->RegisterCustomField<int64_t>("entry_lifetime_ms", &CustomBackoffEntryPolicy::entry_lifetime_ms, &ParseInt64);
        policy_converter->RegisterBoolField("always_use_initial_delay", &CustomBackoffEntryPolicy::always_use_initial_delay);
    }
