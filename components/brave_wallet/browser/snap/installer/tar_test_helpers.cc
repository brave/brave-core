...
void SetOctalField(base::span<char> buf,
                   size_t offset,
                   size_t width,
                   uint64_t value) {
  CHECK_LE(offset + width, buf.size());
  std::string oct;
  do {
  ...
  field += oct;
  base::span<char> field_span = buf.subspan(offset, width);
  for (size_t i = 0; i < digits; ++i) {
    field_span[i] = field[i];
  }
  field_span[width - 1] = '\0';
}

// Computes the ustar header checksum over |header| (with the checksum field
// treated as spaces) and writes it back as "NNNNNN\0 ".
void SetChecksumField(base::span<char> header) {
  CHECK_GE(header.size(), 156u);
