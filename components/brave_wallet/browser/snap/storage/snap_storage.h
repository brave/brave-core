...
class SnapStorage {
 public:
  ...
  // Moves bundle.js and manifest.json from |unpacked_dir| (the install-time
  // temp directory) to the persistent snap directory for |snap_id|.
  // |on_done| is called on the originating sequence; argument is false on I/O
  // error.
  void MoveSnapFiles(const std::string& snap_id,
