...
TEST(Eip2930TransactionUnitTest, GetSignedTransaction) {
  ...
  Eip2930Transaction tx(
      mojom::TxData::New("0x0", "0x3b9aca00", "0x62d4",
                         "0xdf0a88b2b68c673713a8ec826003676f272e3573", "0x1",
                         std::vector<uint8_t>()),
      "0x796f6c6f763378");
  ASSERT_EQ(tx.type(), 1);
  auto* access_list = tx.access_list();
  ...
  tx.GetMessageToSign(
      ...
      base::BindLambdaForTesting([&](bool success,
        ...
        EXPECT_EQ(tx.v_, 0u);
        EXPECT_EQ(
            base::ToLowerASCII(base::HexEncode(tx.r_)),
            "294ac94077b35057971e6b4b06dfdf55a6fbed819133a6c1d31e187f1bca938d");
        EXPECT_EQ(
            base::ToLowerASCII(base::HexEncode(tx.s_)),
...
TEST(Eip2930TransactionUnitTest, GetBaseFee) {
  ...
  Eip2930Transaction tx2(
      mojom::TxData::New("0x0", "0x0", "0x0", "", "0x0", data), "0x15BE");
