...
void JsonRpcService::GetERC20TokenBalance(
  ...
  auto internal_callback =
      base::BindOnce(&JsonRpcService::OnGetERC20TokenBalance,
                     weak_ptr_factory_.GetWeakPtr(), std::move(callback));
  RequestInternal(eth::GetCallPayload(
                      /*from_address=*/std::nullopt, contract,
                      /*gas=*/std::nullopt, /*gas_price=*/std::nullopt,
                      /*value=*/std::nullopt, data, kEthereumBlockTagLatest),
                  true, network_url, std::move(internal_callback));
}
...
void JsonRpcService::GetERC20TokenAllowance(
  ...
  auto internal_callback =
      base::BindOnce(&JsonRpcService::OnGetERC20TokenAllowance,
                     weak_ptr_factory_.GetWeakPtr(), std::move(callback));
  RequestInternal(eth::GetCallPayload(
                      /*from_address=*/std::nullopt, contract_address,
                      /*gas=*/std::nullopt, /*gas_price=*/std::nullopt,
                      /*value=*/std::nullopt, data, kEthereumBlockTagLatest),
                  true, GetNetworkURL(chain_id, mojom::CoinType::ETH),
                  std::move(internal_callback));
...
void JsonRpcService::UnstoppableDomainsResolveDns(
  ...
  for (const auto [chain_id, address] :
    ...
    RequestInternal(eth::GetCallPayload(
                        /*from_address=*/std::nullopt, address,
                        /*gas=*/std::nullopt, /*gas_price=*/std::nullopt,
                        /*value=*/std::nullopt, *data, kEthereumBlockTagLatest),
                    true, NetworkManager::GetUnstoppableDomainsRpcUrl(chain_id),
                    std::move(internal_callback));
  }
}
...
void JsonRpcService::GetEthTokenUri(const std::string& chain_id,
  ...
  RequestInternal(
      eth::GetCallPayload(/*from_address=*/std::nullopt, contract_address,
                          /*gas=*/std::nullopt, /*gas_price=*/std::nullopt,
                          /*value=*/std::nullopt, function_signature,
