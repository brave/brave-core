...
TEST(BitcoinHardwareKeyringUnitTest, AddAccountFails) {
  ...
  // Index already used.
  EXPECT_FALSE(keyring.AddAccount(0, kBtcMainnetHardwareAccount1));
...
TEST(BitcoinHardwareKeyringUnitTest, GetAddress) {
  ...
  EXPECT_TRUE(testnet_keyring.AddAccount(0, kBtcTestnetHardwareAccount0));
  EXPECT_TRUE(testnet_keyring.AddAccount(1, kBtcTestnetHardwareAccount1));
  EXPECT_FALSE(testnet_keyring.GetAddress(2, BitcoinKeyId(0, 0)));
