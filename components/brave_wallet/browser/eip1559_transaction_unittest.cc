...
TEST(Eip1559TransactionUnitTest, GetMessageToSign) {
  ...
  auto tx_data = mojom::TxData::New(
      "0x0", "0x0", "0x0", "0x0101010101010101010101010101010101010101", "0x0",
      data);
  Eip1559Transaction tx(std::move(tx_data), "0x4", "0x0", "0x0");
  ASSERT_EQ(tx.type(), 2);
  ...
  tx.GetMessageToSign(
      "0x0",
      base::BindLambdaForTesting([](bool status,
...
TEST(Eip1559TransactionUnitTest, GetSignedTransaction) {
  ...
  for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); ++i) {
    ...
    auto tx = std::make_unique<Eip1559Transaction>(
        mojom::TxData::New(cases[i].nonce, "0x0", cases[i].gas_limit,
                           "0x000000000000000000000000000000000000aaaa",
                           cases[i].value, std::vector<uint8_t>()),
        "0x4", cases[i].max_priority_fee_per_gas, cases[i].max_fee_per_gas);
    ...
    tx->GetMessageToSign(
        ...
        base::BindLambdaForTesting(
            [&](bool status, const std::vector<uint8_t>& message) {
              EXPECT_TRUE(status);
              signature = key.Sign(message, &recid);
              ...
              tx->GetSignedTransaction(base::BindLambdaForTesting(
                  [&](bool status, const std::string& signed_transaction) {
                    EXPECT_TRUE(status);
                    EXPECT_EQ(signed_transaction, cases[i].signed_tx);
                  }));
              base::RunLoop().RunUntilIdle();
            }));
    base::RunLoop().RunUntilIdle();
...
TEST(Eip1559TransactionUnitTest, GetUpfrontCost) {
  Eip1559Transaction tx(
      mojom::TxData::New("0x0", "0x0", "0x64",
                         "0x0101010101010101010101010101010101010101", "0x6",
                         std::vector<uint8_t>()),
      "0x4", "0x8", "0xA");
