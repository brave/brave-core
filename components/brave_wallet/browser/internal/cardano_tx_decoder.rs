...
fn encode_tx_outputs(outputs: &[CxxSerializableTxOutput]) -> CborValue {
    let outputs_cbor: Vec<CborValue> = outputs
        .iter()
        .map(|output| {
...
fn encode_tx_vkey_witness(vkey_witness_set: &Vec<CxxSerializableVkeyWitness>) -> CborValue {
    CborValue::Array(
        vkey_witness_set
            .iter()
            .map(|witness| {
...
fn encode_tx_witness(witness: &CxxSerializableTxWitness) -> CborValue {
    ...
    witness_map.push((
        ...
        encode_tx_vkey_witness(&witness.vkey_witness_set),
    ));
