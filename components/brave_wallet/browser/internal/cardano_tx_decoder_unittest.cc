...
TEST(CardanoTxDecoderTest, ApplySignResults_ValidSignatures) {
  ...
  sign_result1.public_key =
      std::vector<uint8_t>(32, 1);  // 32 bytes filled with 1
  sign_result1.signature_bytes =
      std::vector<uint8_t>(64, 2);  // 64 bytes filled with 2
  ...
  sign_result2.public_key =
      std::vector<uint8_t>(32, 3);  // 32 bytes filled with 3
  sign_result2.signature_bytes =
      std::vector<uint8_t>(64, 4);  // 64 bytes filled with 4
  ...
  // Note: The actual comparison might need adjustment based on the exact CBOR
  // serialization This test verifies that the function returns a valid result
...
TEST(CardanoTxDecoderTest, ApplySignResults_InvalidSignatures) {
  auto tx = GetUnsignedReferenceTransaction();
  auto tx_bytes = CardanoTransactionSerializer().SerializeTransaction(tx);

  std::vector<CardanoTxDecoder::CardanoSignMessageResult> invalid_sign_results;

  // Create signature result with empty data
  CardanoTxDecoder::CardanoSignMessageResult invalid_sign_result;
  invalid_sign_result.public_key = std::vector<uint8_t>();       // Empty
  invalid_sign_result.signature_bytes = std::vector<uint8_t>();  // Empty
  invalid_sign_results.push_back(std::move(invalid_sign_result));

  auto result =
      CardanoTxDecoder::ApplySignResults(tx_bytes, invalid_sign_results);
  // Should return a result (skips invalid signatures)
  EXPECT_TRUE(result.has_value());
}
...
TEST(CardanoTxDecoderTest, CardanoSignMessageResult_DefaultConstruction) {
  CardanoTxDecoder::CardanoSignMessageResult result;

  EXPECT_TRUE(result.signature_bytes.empty());
  EXPECT_TRUE(result.public_key.empty());
}

TEST(CardanoTxDecoderTest, CardanoSignMessageResult_CopyConstruction) {
  CardanoTxDecoder::CardanoSignMessageResult original;
  original.public_key = std::vector<uint8_t>(32, 1);
  original.signature_bytes = std::vector<uint8_t>(64, 2);

  CardanoTxDecoder::CardanoSignMessageResult copy(original);

  EXPECT_EQ(copy.public_key, original.public_key);
  EXPECT_EQ(copy.signature_bytes, original.signature_bytes);
}

TEST(CardanoTxDecoderTest, CardanoSignMessageResult_MoveConstruction) {
  CardanoTxDecoder::CardanoSignMessageResult original;
  original.public_key = std::vector<uint8_t>(32, 1);
  original.signature_bytes = std::vector<uint8_t>(64, 2);

  auto original_public_key = original.public_key;
  auto original_signature_bytes = original.signature_bytes;

  CardanoTxDecoder::CardanoSignMessageResult moved(std::move(original));

  EXPECT_EQ(moved.public_key, original_public_key);
  EXPECT_EQ(moved.signature_bytes, original_signature_bytes);
}

TEST(CardanoTxDecoderTest, RestoredTransactionTypes_DefaultConstruction) {
  CardanoTxDecoder::RestoredTransactionInput input;
  EXPECT_EQ(input.index, 0u);
  EXPECT_EQ(input.amount, std::nullopt);
  EXPECT_FALSE(input.address.has_value());

  CardanoTxDecoder::RestoredTransactionOutput output;
  EXPECT_EQ(output.amount, 0u);

  CardanoTxDecoder::RestoredTransactionBody body;
  EXPECT_TRUE(body.inputs.empty());
  EXPECT_TRUE(body.outputs.empty());

  CardanoTxDecoder::RestoredTransaction tx;
  EXPECT_TRUE(tx.tx_body.inputs.empty());
  EXPECT_TRUE(tx.tx_body.outputs.empty());
  EXPECT_TRUE(tx.witness_set.empty());
  EXPECT_TRUE(tx.raw_bytes.empty());
}

TEST(CardanoTxDecoderTest, RestoredTransactionTypes_CopyConstruction) {
  CardanoTxDecoder::RestoredTransactionInput input;
  input.index = 1;
  input.amount = 1000u;
  input.address = GetMockCardanoAddress();

  CardanoTxDecoder::RestoredTransactionInput copy(input);
  EXPECT_EQ(copy.index, input.index);
  EXPECT_EQ(copy.amount, input.amount);
  EXPECT_EQ(copy.address, input.address);

  CardanoTxDecoder::RestoredTransactionOutput output;
  output.address = GetMockCardanoAddress();
  output.amount = 2000u;

  CardanoTxDecoder::RestoredTransactionOutput output_copy(output);
  EXPECT_EQ(output_copy.address, output.address);
  EXPECT_EQ(output_copy.amount, output.amount);
}

TEST(CardanoTxDecoderTest, RestoredTransactionTypes_MoveConstruction) {
  CardanoTxDecoder::RestoredTransactionBody body;
  body.inputs.resize(1);
  body.outputs.resize(1);

  CardanoTxDecoder::RestoredTransactionBody moved_body(std::move(body));
  EXPECT_EQ(moved_body.inputs.size(), 1u);
  EXPECT_EQ(moved_body.outputs.size(), 1u);

  CardanoTxDecoder::RestoredTransaction tx;
  tx.tx_body.inputs.resize(1);
  tx.tx_body.outputs.resize(1);
  tx.witness_set.resize(1);
  tx.raw_bytes.resize(10);

  CardanoTxDecoder::RestoredTransaction moved_tx(std::move(tx));
  EXPECT_EQ(moved_tx.tx_body.inputs.size(), 1u);
  EXPECT_EQ(moved_tx.tx_body.outputs.size(), 1u);
  EXPECT_EQ(moved_tx.witness_set.size(), 1u);
  EXPECT_EQ(moved_tx.raw_bytes.size(), 10u);
}

TEST(CardanoTxDecoderTest, InputWitness_TypeAlias) {
  CardanoTxDecoder::InputWitness witness;
  EXPECT_EQ(witness.size(),
            96u);  // Should be 96 bytes (32 pubkey + 64 signature)

  // Test that we can fill it with data
  witness.fill(0xFF);
