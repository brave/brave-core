...
class CardanoTxDecoder {
 public:
  // Forward declaration for InputWitness
  using InputWitness = std::array<uint8_t, 96u>;
  ...
  struct CardanoSignMessageResult {
    ...
    // The signed message bytes (signature)
  ...
  // Restored transaction types moved from cardano_cip30_serializer.h
