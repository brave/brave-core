...
TEST_F(ZCashCreateOrchardToOrchardTransactionTaskTest, ValidateAddress) {
  ...
  EXPECT_TRUE(
      ZCashCreateOrchardToOrchardTransactionTask::ValidateAddress(
  ...
  EXPECT_EQ(
      mojom::ZCashAddressError::InvalidUnifiedAddress,
      ZCashCreateOrchardToOrchardTransactionTask::ValidateAddress(false, "vv")
          .error());
}
