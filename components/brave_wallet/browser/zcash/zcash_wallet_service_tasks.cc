...
void CreateTransparentTransactionTask::WorkOnTask() {
  ...
  DCHECK_EQ(kDefaultTransparentOutputsCount,
            transaction_.transparent_part().outputs_.size());
...
bool CreateTransparentTransactionTask::PickInputs() {
  ...
  for (auto& input : all_inputs) {
    transaction_.transparent_part().inputs_.push_back(std::move(input));
    transaction_.set_fee(
        CalculateTxFee(transaction_.transparent_part().inputs_.size(), 0));
  ...
  DCHECK(!transaction_.transparent_part().inputs_.empty());
  return done;
}

bool CreateTransparentTransactionTask::PrepareOutputs() {
  auto& target_output = transaction_.transparent_part().outputs_.emplace_back();
  ...
  auto& change_output = transaction_.transparent_part().outputs_.emplace_back();
...
bool CreateShieldAllTransactionTask::CreateTransaction() {
  ...
  zcash_transaction.transparent_part().inputs_ = std::move(all_inputs);
  zcash_transaction.set_fee(CalculateTxFee(
      zcash_transaction.transparent_part().inputs_.size(),
  ...
  orchard_output.value_ =
      zcash_transaction.TotalInputsAmount() - zcash_transaction.fee();
  orchard_output.address_ = std::move(addr_bytes.value());

  zcash_transaction.orchard_part().outputs_.push_back(
      std::move(orchard_output));

  zcash_transaction.set_locktime(chain_height_.value());
  zcash_transaction.set_expiry_height(chain_height_.value() + kDefaultZCashBlockHeightDelta);
...
bool CreateShieldAllTransactionTask::CompleteTransaction() {
  CHECK(transaction_);
  CHECK_EQ(1u, transaction_->orchard_part().outputs_.size());
  CHECK(tree_state_);
  ...
  for (const auto& output : transaction_->orchard_part().outputs_) {
    outputs.push_back(
        ::brave_wallet::OrchardOutput{output.value_, output.address_});
  }
  ...
  {
    transaction_->orchard_part().digest_ =
  ...
  {
    auto orchard_raw_part = const_cast<::brave_wallet::OrchardBuilderValue&>(
                                orchard_bundle->unwrap())
                                .complete(sighash)
                                ->unwrap()
                                .raw_tx();
    transaction_->orchard_part().raw_tx_ =
