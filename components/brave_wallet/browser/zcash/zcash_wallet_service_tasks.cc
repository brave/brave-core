...
::rust::Box<::zcash::OrchardBuilderResult>
CreateShieldAllTransactionTask::CreateOrchardBuilder(
  ...
  if (random_seed_for_testing_) {
    CHECK_IS_TEST();
    return ::zcash::create_testing_orchard_builder(
        std::move(tree_state), std::move(outputs),
        random_seed_for_testing_.value());
  } else {
    return ::zcash::create_orchard_builder(std::move(tree_state),
                                           std::move(outputs));
  }
}
...
bool CreateShieldAllTransactionTask::CompleteTransaction() {
  ...
  for (const auto& output : transaction_->orchard_part().outputs_) {
    outputs.push_back(::zcash::OrchardOutput{output.value_, output.address_});
  }

  auto orchard_bundle = zcash::CreateOrchardBuilder(
