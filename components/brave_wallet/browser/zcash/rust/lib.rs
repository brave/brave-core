...
fn create_orchard_builder_internal(
    ...
    let wallet_fvk = match OrchardFVK::from_bytes(&fvk) {
        Some(fvk) => fvk,
        None => {
            return Box::new(CxxOrchardUnauthorizedBundleResult::from(Err(Error::FvkError)));
        }
    ...
    for out in outputs {
        ...
        // `add_output` instead.
        let add_output_result = if wallet_fvk.scope_for_address(&addr).is_some() {
            builder.add_change_output(
                wallet_fvk.clone(),
                Some(wallet_fvk.to_ovk(OrchardScope::Internal)),
                addr,
