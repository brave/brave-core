...
use orchard::{
    ...
    }, bundle::{commitments, Bundle}, keys::{FullViewingKey as OrchardFVK, PreparedIncomingViewingKey, Scope as OrchardScope}, note:: {
        ExtractedNoteCommitment, Nullifier, RandomSeed
    }, note_encryption:: {
        CompactAction, OrchardDomain
    }, tree::MerkleHashOrchard, zip32::{ChildIndex as OrchardChildIndex, Error as Zip32Error, ExtendedSpendingKey}
};
...
use incrementalmerkletree::frontier::{self, Frontier};
...
use crate::ffi::FfiCheckpointBundle;
...
use crate::ffi::FfiRetention;
use crate::ffi::OrchardFrontierChainState;
...
use crate::ffi::FfiCommitment;
use crate::ffi::FfiCommitments;
...
pub(crate) const PRUNING_DEPTH: u8 = 10;
...
mod ffi {
    ...
    struct OrchardMerkleHash {
        hash: [u8; 32]
    }

    struct OrchardMerklePath {
        position: u32,
        auth_path: Vec<OrchardMerkleHash>,
        root: OrchardMerkleHash
    }
    ...
    #[derive(Clone)]
    struct OrchardFrontierChainState {
        frontier_tree_state: Vec<u8>,
        frontier_block_height: u32,
        frontier_orchard_commitment_tree_size: u32
    }
    ...
    #[derive(Clone)]
    struct FfiCommitment {
        hash: [u8; 32],
        retention: FfiRetention
    }

    #[derive(Clone)]
    struct FfiCommitments {
        commitments: Vec<FfiCommitment>
    }
    ...
    extern "Rust" {
        ...
        fn batch_decode(
            ...
            frontier_chain_state: OrchardFrontierChainState,
        ...
        fn insert_commitments(self: &mut OrchardShardTreeBundle, scan_result: &mut BatchOrchardDecodeBundle) -> bool;
        fn calculate_witness(self: &mut OrchardShardTreeBundle, commitment_tree_position: u32, checkpoint: u32) -> Box<OrchardWitnessBundleResult>;
  

        fn create_mock_decode_result_with_frontier(frontier: OrchardFrontierChainState, commitments: FfiCommitments) -> Box<BatchOrchardDecodeBundleResult>;
        fn create_mock_decode_result(commitments: FfiCommitments) -> Box<BatchOrchardDecodeBundleResult>;
        fn create_testing_shard_tree(ctx: UniquePtr<ShardStoreContext>) -> Box<OrchardTestingShardTreeBundleResult>;

        fn append(self: &mut OrchardTestingShardTreeBundle, v: [u8; 32], checkpoint: bool, marked: bool) -> bool;
        fn insert_commitments(self: &mut OrchardTestingShardTreeBundle, scan_result: &mut BatchOrchardDecodeBundle) -> bool;
        fn calculate_witness(self: &mut OrchardTestingShardTreeBundle, commitment_tree_position: u32, checkpoint: u32) -> Box<OrchardWitnessBundleResult>;
        fn root(self: &mut OrchardTestingShardTreeBundle, level: u8, index: u32) -> Box<OrchardRootBundleResult>;

        //fn insert_commitment(self: &mut OrchardTestingShardTreeBundle, h: MerkleHashOrchard, checkpoint: bool, marked: bool) -> bool;
        fn insert_frontier(self: &mut OrchardTestingShardTreeBundle, frontier: OrchardFrontierChainState) -> bool;
        
        fn value(self: &OrchardRootBundle) -> [u8; 32];
        fn is_ok(self: &OrchardRootBundleResult) -> bool;
        fn error_message(self: &OrchardRootBundleResult) -> String;
        fn unwrap(self: &OrchardRootBundleResult) -> Box<OrchardRootBundle>;
    ...
    unsafe extern "C++" {
       ...
       // type ShardStoreStatusCode;
        ...
        fn orchard_get_checkpoints(
            ctx: &ShardStoreContext,
            limit: usize,
            into: &mut Vec<FfiCheckpointBundle>) -> ShardStoreStatusCode;
    }
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match &self {
            ...
            // Error::ShardTree(e) => write!(f, "Error: {}", e.to_string()),
...
pub struct BatchOrchardDecodeBundleValue {
    ...
    commitments: Vec<Option<(MerkleHashOrchard, Retention<BlockHeight>)>>,
    frontier_chain_state: Option<OrchardFrontierChainState>
}
...
type OrchardShardTreeBundleValue = OrchardGenericShardTreeBundleValue<TREE_HEIGHT, SHARD_HEIGHT, PRUNING_DEPTH>;
type OrchardTestingShardTreeBundleValue = OrchardGenericShardTreeBundleValue<TESTING_TREE_HEIGHT, TESTING_SHARD_HEIGHT, TESTING_PRUNING_DEPTH>;
...
#[derive(Clone)]
pub struct OrchardRootBundleValue {
    value: [u8; 32]
}

// impl Clone for OrchardWitnessBundleValue {
//     fn clone(&self) -> Self {
//         OrchardWitnessBundleValue { path: MerklePath::from_parts(self.path.position(), self.path.auth_path().clone()) }
//     }
// }

impl<const TREE_HEIGHT: u8, const SHARD_HEIGHT: u8, const PRUNING_DEPTH: u8> Clone for OrchardGenericShardTreeBundleValue<TREE_HEIGHT, SHARD_HEIGHT, PRUNING_DEPTH> {
    fn clone(&self) -> Self {
        OrchardGenericShardTreeBundleValue { tree : ShardTree::new(self.tree.store().clone(), PRUNING_DEPTH.into()) }
    }
}
...
#[derive(Clone)]
struct OrchardRootBundle(OrchardRootBundleValue);
...
struct OrchardRootBundleResult(Result<OrchardRootBundle, Error>);
...
impl_result!(OrchardRootBundle, OrchardRootBundleResult, OrchardRootBundleValue);
...
fn create_orchard_builder_internal(
    ...
    for spend in spends {
        ...
        let auth_path = spend.merkle_path.auth_path.iter().map(|v| {
            let v = MerkleHashOrchard::from_bytes(&v.hash);
            if v.is_some().into() {
                Ok(v.unwrap())
            } else {
        ...
        let _ = builder.add_spend(fvk.unwrap(), note.unwrap(), merkle_path);
    }
...
fn batch_decode(
    ...
    let mut note_commitments: Vec<Option<(MerkleHashOrchard, Retention<BlockHeight>)>> = vec![];
    ...
    for (output_idx, ((_, output), decrypted_note)) in
        ...
        let is_checkpoint = &output.is_block_last_action/*  && last_commitments_in_block */;
        ...
        note_commitments.push(Some((MerkleHashOrchard::from_bytes(&output.cmx).unwrap(), retention)));
        ...
        if let Some((_key_id, note)) = decrypted_note {
            ...
            found_notes.push(DecryptedOrchardOutput{
                ...
                commitment_tree_position: (output_idx as u32) + frontier_chain_state.frontier_orchard_commitment_tree_size
            });
    ...
    Box::new(BatchOrchardDecodeBundleResult::from(Ok(BatchOrchardDecodeBundleValue {
        ...
        frontier_chain_state: Some(frontier_chain_state)
     })))
}
...
fn insert_frontier<const COMMITMENT_TREE_DEPTH: u8, const SHARD_HEIGHT: u8>(
    ...
    frontier: &OrchardFrontierChainState
) -> bool {
    println!("--- 1 {:?} {:?} ", COMMITMENT_TREE_DEPTH, SHARD_HEIGHT);
    println!("--- 1 {:?} ", &frontier.frontier_tree_state);
    let frontier_commitment_tree = read_commitment_tree::<MerkleHashOrchard, _, COMMITMENT_TREE_DEPTH>(
        &frontier.frontier_tree_state[..]);

    if frontier_commitment_tree.is_err() {
        return false;
    }
    println!("--- 2 frontier size {:?}", &frontier_commitment_tree.as_ref().expect("reason").size());

    let frontier_result = tree.insert_frontier(
        frontier_commitment_tree.unwrap().to_frontier(),
        Retention::Marked,
    );
    println!("--- 3 {:?}",  frontier_result.is_ok());
...
fn insert_commitments<const COMMITMENT_TREE_DEPTH: u8, const CHUNK_SIZE: usize, const SHARD_HEIGHT: u8>(
    ...
    // Insert to the end of the tree by default
    let mut start_position : u64 = match shard_tree.max_leaf_position(0) {
        Ok(Some(pos)) => pos.into(),
        Ok(None) => 0,
        _ => return false
    };

    match &scan_result.0.frontier_chain_state {
        // Don't insert frontier if frontier_tree_state is empty
        // This is a special case when scanning is done sequentially
        Some(f) => {
            println!("---- with frontier");

            if !insert_frontier::<COMMITMENT_TREE_DEPTH, SHARD_HEIGHT>(shard_tree, f) {
                return false;
            }

            start_position = (f.frontier_orchard_commitment_tree_size).into();
            println!("--- frontier start position {:?} ", start_position)
        },
        None => ()
    }
    println!("---- start_position {:?} ", start_position);

    println!("--- insert commitments {:?} ", &scan_result.0.commitments.len());
    shard_tree.batch_insert(
        Position::from(start_position),
        scan_result.0.commitments.clone().into_iter().map(|c| {
            (
                c.unwrap().0, c.unwrap().1
                // if c.1 == 2 {
                //     Retention::Marked
                // } else if c.1 == 3 {
                //     Retention::Checkpoint { id: BlockHeight::from_u64(1), is_marked: (false) }
                // } else {
                //     Retention::Ephemeral
                // }
            )
        })).unwrap();
    // TODO(cypt4) : Check proper borders
    // let orchard_subtrees = scan_result.0.commitments
    //     .rchunks_mut(CHUNK_SIZE)
    //     .enumerate()
    //     .filter_map(|(i, chunk)| {
    //         let start =
    //             start_position + (i * CHUNK_SIZE) as u64;
    ...
    // for (tree, checkpoints) in orchard_subtrees {
    //     let _ = shard_tree.insert_tree(tree, checkpoints);
    // }
    println!("-- size test");
    println!("---shard tree size {:?}", shard_tree.max_leaf_position(0));
...
impl OrchardShardTreeBundle {
    ...
    fn calculate_witness(self: &mut OrchardShardTreeBundle, commitment_tree_position: u32, checkpoint: u32) -> Box<OrchardWitnessBundleResult> {
        match self.0.tree.witness_at_checkpoint_id_caching((commitment_tree_position as u64).into(), &checkpoint.into()) {
            Ok(witness) => Box::new(OrchardWitnessBundleResult::from(Ok(OrchardWitnessBundleValue { path: witness.path_elems().into() }))),
            Err(e) => {
                eprintln!("Error occurred: {}", e);
                Box::new(OrchardWitnessBundleResult::from(Err(Error::WitnessError)))
            }
        }
    }
}
...
impl OrchardTestingShardTreeBundle {
    ...
    fn root(self: &mut OrchardTestingShardTreeBundle, level: u8, index: u32) -> Box<OrchardRootBundleResult> {
        let address = Address::from_parts(level.into(), index.into());
        let truncate_at = self.0.tree.max_leaf_position(0).map_err(|_| Error::ShardStoreError).unwrap();
        match truncate_at {
            Some(truncate_at) => {
                let root = self.0.tree.root(address, truncate_at);
                match root {
                    Ok(root) => Box::new(OrchardRootBundleResult::from(Ok(OrchardRootBundleValue { value: root.to_bytes() }))),
                    Err(_) => Box::new(OrchardRootBundleResult::from(Err(Error::ShardStoreError)))
                }
            },
            None => Box::new(OrchardRootBundleResult::from(Err(Error::ShardStoreError)))
        }
    }

    fn append(self: &mut OrchardTestingShardTreeBundle, _v: [u8; 32], _checkpoint: bool, _marked: bool) -> bool {
        true
    }

    fn insert_frontier(self: &mut OrchardTestingShardTreeBundle, frontier: OrchardFrontierChainState) -> bool {
        let frontier_commitment_tree = read_commitment_tree::<MerkleHashOrchard, _, TESTING_TREE_HEIGHT>(
            &frontier.frontier_tree_state[..]);
        ...
        let frontier_result = self.0.tree.insert_frontier(
            frontier_commitment_tree.unwrap().to_frontier(),
            Retention::Checkpoint {
                id: BlockHeight::from_u32(frontier.frontier_block_height),
                // Check whether is_marked == true
                is_marked: false,
            },
        );
        ...
        println!("XXXZZZ frontier inserted");
        true
    }
    ...
    fn calculate_witness(self: &mut OrchardTestingShardTreeBundle, commitment_tree_position: u32, checkpoint: u32) -> Box<OrchardWitnessBundleResult> {
        println!("witness inputs {:?} {:?} {:?}", &commitment_tree_position, &checkpoint, self.0.tree.max_leaf_position(0));
        //self.0.tree.max_leaf_position(checkpoint_depth)
        match self.0.tree.witness_at_checkpoint_id_caching((commitment_tree_position as u64).into(), &checkpoint.into()) {
            Ok(witness) => {
                println!("witness {:?}", &witness.path_elems());
                Box::new(OrchardWitnessBundleResult::from(Ok(OrchardWitnessBundleValue { path: witness.into() })))
            },
            Err(e) => {
                eprintln!("Error occurred: {}", e);
                Box::new(OrchardWitnessBundleResult::from(Err(Error::WitnessError)))
...
impl From<&FfiShardAddress> for Address {
    fn from(item: &FfiShardAddress) -> Self {
        print!("item addr {:?} {:?}", item.level, item.index);
...
impl<H: HashSer> TryFrom<&FfiShardTree> for LocatedPrunableTree<H> {
    ...
    fn try_from(item: &FfiShardTree) -> Result<Self, Self::Error> {
        let shard_tree = read_shard(&mut Cursor::new(&item.data)).map_err(|_| Error::ShardStoreError)?;
...
impl<H: HashSer> TryFrom<&LocatedPrunableTree<H>> for FfiShardTree {    
    ...
    fn try_from(item: &LocatedPrunableTree<H>) -> Result<Self, Self::Error> {
        ...
        let mut result = FfiShardTree {
            address: FfiShardAddress::try_from(&item.root_addr()).map_err(|_| Error::ShardStoreError)?,
            hash: subtree_root_hash.unwrap_or_else(|| vec![]).try_into().map_err(|_| Error::ShardStoreError)?,
            data: vec![]
        };

        println!("orchard put shard {:?} {:?}", result.address.level, result.address.index);
    ...
    for CxxShardStoreImpl<H, SHARD_HEIGHT>
{
    ...
    fn get_shard(
        ...
        let result = orchard_get_shard(&*ctx.try_borrow().unwrap(),
            &FfiShardAddress::try_from(&addr).map_err(|_| Error::ShardStoreError)?,
            &mut into);
        println!("shard data {:?}", &into.data);
        if result == ShardStoreStatusCode::Ok {
            println!("tree try from");
    ...
    fn put_shard(&mut self, subtree: LocatedPrunableTree<Self::H>) -> Result<(), Self::Error> {
        let ctx = self.native_context.clone();
        let ffi_tree = FfiShardTree::try_from(&subtree).map_err(|_| Error::ShardStoreError)?;
    ...
    fn truncate(&mut self, from: Address) -> Result<(), Self::Error> {
        ...
        let result = orchard_truncate(ctx.try_borrow_mut().unwrap().pin_mut(),
          &FfiShardAddress::try_from(&from).map_err(|_| Error::ShardStoreError)?);
    ...
    fn put_cap(&mut self, cap: PrunableTree<Self::H>) -> Result<(), Self::Error> {
        ...
        write_shard(&mut ffi_cap.data, &cap).map_err(|_| Error::ShardStoreError)?;
    ...
    fn checkpoint_count(&self) -> Result<usize, Self::Error> {
        ...
        println!("--- checkpoint counts : {:?}", &input);
        if result == ShardStoreStatusCode::Ok {
            println!("-- checkpoint count ok");
    ...
    fn get_checkpoint(
        ...
        if result == ShardStoreStatusCode::Ok {
            println!("--- checkpoint {:?}", Checkpoint::from(&input_checkpoint));
    ...
    {
        let ctx = self.native_context.clone();
        let mut into : Vec<FfiCheckpointBundle> = vec![];
        let result = orchard_get_checkpoints(&*ctx.try_borrow().unwrap(), limit, &mut into);
        println!("-- with checkpoints {:?}", into.len());
        if result == ShardStoreStatusCode::Ok {
            for item in into {
...
fn convert_ffi_commitments(ffi_commitments: &FfiCommitments) -> Vec<Option<(MerkleHashOrchard, Retention<BlockHeight>)>> {
    ffi_commitments.commitments.iter().map(|c| {
        let retention:Retention<BlockHeight> = {
            if c.retention.checkpoint {
                println!("--- checkpoint {:?} ",  c.retention.checkpoint_id);
        ...
        // TODO: should values be inserted here or hash?
        let mh = MerkleHashOrchard::from_bytes(&c.hash);
        Some((mh.unwrap(), retention))
    }).collect()
}
...
fn create_mock_decode_result(commitments: FfiCommitments) -> Box<BatchOrchardDecodeBundleResult> {
    Box::new(BatchOrchardDecodeBundleResult::from(Ok(BatchOrchardDecodeBundleValue {
        ...
        frontier_chain_state: None
    })))
}

fn create_mock_decode_result_with_frontier(frontier: OrchardFrontierChainState, ffi_commitments: FfiCommitments) -> Box<BatchOrchardDecodeBundleResult> {
    Box::new(BatchOrchardDecodeBundleResult::from(Ok(BatchOrchardDecodeBundleValue {
        fvk: [0; 96],
        outputs: vec![],
        commitments: convert_ffi_commitments(&ffi_commitments),
        frontier_chain_state: Some(frontier)
    })))
}
...
fn create_mock_commitment(position: u32, seed: u32) -> [u8; 32] {
    print!("XXXZZZ create mock commitment");
    let r = MerkleHashOrchard::from_bytes(
