...
std::unique_ptr<OrchardDecodedBlocksBundle> OrchardBlockDecoder::DecodePool(
  ...
  const size_t action_count = compact_actions.size();
  ...
  if (decode_result->is_ok()) {
    ...
    LOG(ERROR) << (is_ironwood ? "Ironwood" : "Orchard") << ": decoded "
               << bundle->size() << " note(s) from " << action_count
               << " action(s)";
    return std::make_unique<OrchardDecodedBlocksBundleImpl>(
        base::PassKey<class OrchardBlockDecoder>(), std::move(bundle));
  }
  LOG(ERROR) << (is_ironwood ? "Ironwood" : "Orchard")
             << ": batch_decode failed";
  return nullptr;
}
