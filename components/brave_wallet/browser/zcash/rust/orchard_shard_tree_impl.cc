...
OrchardShardTreeImpl::CalculateWitness(const OrchardNote& note,
                                       uint32_t checkpoint) {
  auto result = orcard_shard_tree_->calculate_witness(
      note.orchard_commitment_tree_position, checkpoint);
  if (!result->is_ok()) {
  ...
  witness.position = note.orchard_commitment_tree_position;
