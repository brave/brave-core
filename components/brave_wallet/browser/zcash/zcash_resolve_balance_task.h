...
class ZCashResolveBalanceTask {
 public:
  using ZCashResolveBalanceTaskCallback = base::OnceCallback<void(base::expected<mojom::ZCashBalancePtr, std::string>)>;
 ...
 private:
  ...
  void OnDiscoveryDoneForBalance(ZCashWalletService::RunDiscoveryResult discovery_result);

  void OnUtxosResolvedForBalance(base::expected<ZCashWalletService::UtxoMap, std::string> result);

#if BUILDFLAG(ENABLE_ORCHARD)
  void OnGetSpendableNotes(base::expected<std::vector<OrchardNote>, ZCashOrchardStorage::Error> result);
