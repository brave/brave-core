...
bool ZCashTransaction::OrchardOutput::operator==(
  ...
  return std::tie(this->address_, this->value_) ==
         std::tie(other.address_, other.value_);
}
bool ZCashTransaction::OrchardOutput::operator!=(
    const ZCashTransaction::OrchardOutput& other) const {
  return !(*this == other);
...
bool ZCashTransaction::OrchardPart::operator==(const OrchardPart& other) const {
  return std::tie(this->digest_, this->outputs_, this->raw_tx_) ==
         std::tie(other.digest_, other.outputs_, other.raw_tx_);
}
bool ZCashTransaction::OrchardPart::operator!=(const OrchardPart& other) const {
  return !(*this == other);
}
...
bool ZCashTransaction::TransparentPart::operator==(
  ...
  return std::tie(this->inputs_, this->outputs_) ==
         std::tie(other.inputs_, other.outputs_);
}
bool ZCashTransaction::TransparentPart::operator!=(
    const TransparentPart& other) const {
  return !(*this == other);
...
ZCashTransaction ZCashTransaction::Clone() const {
  ...
  for (auto& input : transparent_part_.inputs_) {
    result.transparent_part_.inputs_.emplace_back(input.Clone());
  }
  for (auto& output : transparent_part_.outputs_) {
    result.transparent_part_.outputs_.emplace_back(output.Clone());
  }
...
std::optional<ZCashTransaction> ZCashTransaction::FromValue(
  ...
  for (auto& item : *inputs_list) {
    ...
    result.transparent_part_.inputs_.push_back(std::move(*input_opt));
  }
  ...
  for (auto& item : *outputs_list) {
    ...
    result.transparent_part_.outputs_.push_back(std::move(*output_opt));
  }
...
bool ZCashTransaction::IsSigned() const {
  if (transparent_part_.inputs_.empty()) {
    return false;
  }

  return base::ranges::all_of(transparent_part_.inputs_,
