...
void ZCashCreateTransparentToIronwoodTransactionTask::CreateTransaction() {
  ...
  auto pick_result = PickZCashTransparentInputs(*utxo_map_, amount_,
  ...
  zcash_transaction.transparent_part().inputs = pick_result->inputs;
  ...
  if (pick_result->change != 0) {
    ...
    change_output.amount = pick_result->change;
    auto script_pubkey = ZCashAddressToScriptPubkey(
    ...
    change_output.script_pubkey = std::move(*script_pubkey);
  }
