...
void ZCashResolveBalanceTask::WorkOnTask() {
  ...
  if (!discovery_result_) {
    LOG(ERROR) << "XXXZZZ get discovery_result";
  ...
  if (!utxo_map_) {
    LOG(ERROR) << "XXXZZZ get utxo map";
    zcash_wallet_service_->GetUtxos(chain_id_, account_id_.Clone(),
  ...
  if (!orchard_notes_) {
    LOG(ERROR) << "XXXZZZ get spendable notes";
    // TODO(cypt4) : Check whether account is shielded
  ...
  if (!result_) {
    LOG(ERROR) << "XXXZZZ create balane";
    CreateBalance();
    return;
  }

  LOG(ERROR) << "XXXZZZ fetch balance finished";
...
void ZCashResolveBalanceTask::OnGetSpendableNotes(
    base::expected<std::vector<OrchardNote>, ZCashOrchardStorage::Error> result) {
  if (!result.has_value()) {
...
void ZCashResolveBalanceTask::OnDiscoveryDoneForBalance(ZCashWalletService::RunDiscoveryResult discovery_result) {
...
void ZCashResolveBalanceTask::OnUtxosResolvedForBalance(base::expected<ZCashWalletService::UtxoMap, std::string> result) {
  if (!result.has_value()) {
    error_ = result.error();
    ScheduleWorkOnTask();
    return;
