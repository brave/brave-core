...
TEST_F(ZCashCreateTransparentToIronwoodTransactionTaskTest,
  ...
  std::unique_ptr<ZCashCreateTransparentToIronwoodTransactionTask> task =
  ...
  EXPECT_EQ(tx_result.value().v6_part().ironwood.outputs[0].value, 100000u);
...
TEST_F(ZCashCreateTransparentToIronwoodTransactionTaskTest,
  ...
  EXPECT_EQ(tx_result.value().v6_part().ironwood.outputs[0].value, 185000u);
  EXPECT_EQ(tx_result.value().v6_part().ironwood.outputs[0].addr,
            orchard_part.value());
}
...
TEST_F(ZCashCreateTransparentToIronwoodTransactionTaskTest, UtxosError) {
  ON_CALL(zcash_wallet_service(), GetUtxos(_, _))
      .WillByDefault([&](const mojom::AccountIdPtr& account_id,
                         ZCashWalletService::GetUtxosCallback callback) {
...
TEST_F(ZCashCreateTransparentToIronwoodTransactionTaskTest, LatestBlockError) {
  ON_CALL(zcash_wallet_service(), GetUtxos(_, _))
      .WillByDefault([&](const mojom::AccountIdPtr& account_id,
                         ZCashWalletService::GetUtxosCallback callback) {
        ZCashWalletService::UtxoMap utxo_map;
