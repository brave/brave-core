 ...
 public:
  ...
  void SetUp() override {
    ...
    keyring_service_->CreateWallet(kMnemonicDivideCruise, kTestWalletPassword,
                                   base::DoNothing());
    // keyring_service()->CreateWallet("gate junior chunk maple cage select
    // orange circle price air tortoise jelly art frequent fence middle ice
    // moral wage toddler attitude sign lesson grain", kTestWalletPassword,
    //                                 base::DoNothing());
    zcash_account_ =
...
TEST_F(ZCashWalletServiceUnitTest, ShieldFunds) {
  zcash_wallet_service()->set_random_seed_for_testing(70972);
  ...
  ON_CALL(*zcash_rpc(), GetLatestBlock(_, _))
      .WillByDefault(
          ::testing::Invoke([&](const std::string& chain_id,
                                ZCashRpc::GetLatestBlockCallback callback) {
            auto response = mojom::BlockID::New(
                2468414u,
  ...
  ON_CALL(*zcash_rpc(), GetLatestTreeState(_, _))
      .WillByDefault(
          ::testing::Invoke([&](const std::string& chain_id,
            ...
            auto tree_state = mojom::TreeState::New(
