...
ZCashOrchardStorage::GetCap(mojom::AccountIdPtr account_id) {
  ...
  if (!resolve_cap.Step()) {
    if (!resolve_cap.Succeeded()) {
      return base::unexpected(
          Error{ErrorCode::kFailedToExecuteStatement, database_->GetErrorMessage()});
    }
    return std::nullopt;
  }
...
base::expected<bool, ZCashOrchardStorage::Error> ZCashOrchardStorage::PutCap(
    mojom::AccountIdPtr account_id,
    OrchardCap cap) {
  LOG(ERROR) << "XXXZZZ put cap " << cap.data.size();
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
base::expected<bool, ZCashOrchardStorage::Error> ZCashOrchardStorage::PutShard(
  ...
  if (existing_shard.value()) {
    ...
    LOG(ERROR) << "XXXZZZ insert new shard";
...
ZCashOrchardStorage::GetShard(mojom::AccountIdPtr account_id,
  ...
  sql::Statement resolve_shard_statement(database_->GetCachedStatement(
      SQL_FROM_HERE,
      "SELECT root_hash, shard_data FROM " kShardTree
      " "
  ...
  auto shard = OrchardShard(address, hash.value(),
                            std::vector<uint8_t>());
  LOG(ERROR) << "XXXZZZ 3";
...
ZCashOrchardStorage::AddCheckpoint(mojom::AccountIdPtr account_id,
  ...
  LOG(ERROR) << "XXXZZZ AddCheckpoint " << checkpoint_id << " " << checkpoint.tree_state_position.value_or(0);
  ...
  if (!extant_tree_state_position) {
    LOG(ERROR) << "XXXZZZ 1 create new checkpoint";
...
ZCashOrchardStorage::RemoveCheckpoint(mojom::AccountIdPtr account_id,
                                      uint32_t checkpoint_id) {
  LOG(ERROR) << "XXXZZZ remove checkpoint " << checkpoint_id;
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
