...
constexpr uint32_t kCardanoTxHashSize = 32u;
...
class CardanoTransaction {
 public:
  ...
  // A set of inputs for cardano transaction which should be spent together. Now
  ...
  class TxInputGroup {
   public:
    ...
    const std::vector<CardanoTransaction::TxInput>& inputs() const {
      return inputs_;
    }

    void AddInput(CardanoTransaction::TxInput input);
    void AddInputs(std::vector<CardanoTransaction::TxInput> inputs);
