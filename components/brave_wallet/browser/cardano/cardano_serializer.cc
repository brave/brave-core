...
//     return BitcoinSerializer::SerializeWitness(DummySignature(),
//     DummyPubkey());
//   }();
//   return dummy_witness;
// }

// void PushOutpoint(const CardanoTransaction::Outpoint& outpoint,
...
// SHA256HashArray HashPrevouts(const CardanoTransaction& tx) {
...
// SHA256HashArray HashSequence(const CardanoTransaction& tx) {
//   DCHECK_EQ(tx.sighash_type(), kBitcoinSigHashAll);
...
// void PushOutput(const CardanoTransaction::TxOutput& output,
//                 BtcLikeSerializerStream& stream) {
...
// SHA256HashArray HashOutputs(const CardanoTransaction& tx) {
//   DCHECK_EQ(tx.sighash_type(), kBitcoinSigHashAll);
...
// void SerializeInputs(const CardanoTransaction& tx,
//                      BtcLikeSerializerStream& stream) {
...
// uint32_t GetOutpointVBytes(const CardanoTransaction::Outpoint& outpoint) {
//   uint32_t bytes = 0;
//   bytes += 32;  // txid
//   bytes += 4;   // index
//   return bytes;
...
// uint32_t GetInputVBytes(const CardanoTransaction::TxInput& input) {
//   uint32_t bytes = 0;
...
// uint32_t GetInputsVBytes(const CardanoTransaction& tx) {
//   uint32_t bytes = 0;
...
// uint32_t GetOutputVBytes(const CardanoTransaction::TxOutput& output) {
//   uint32_t bytes = 0;
//   bytes += 8;  // amount
...
// uint32_t GetOutputsVBytes(const CardanoTransaction& tx) {
//   uint32_t bytes = 0;
...
// void SerializeOutputs(const CardanoTransaction& tx,
//                       BtcLikeSerializerStream& stream) {
...
// void SerializeWitnesses(const CardanoTransaction& tx,
//                         BtcLikeSerializerStream& stream) {
//   for (const auto& input : tx.inputs()) {
...
// uint32_t GetWitnessWeightUnits(const CardanoTransaction::TxInput& input,
...
// uint32_t GetWitnessesWeightUnits(const CardanoTransaction& tx,
//                                  bool dummy_signatures) {
//   uint32_t weight = 0;
//   for (const auto& input : tx.inputs()) {
...
// // static
uint32_t BitcoinSerializer::CalcTransactionSize(const CardanoTransaction& tx,
                                                bool dummy_signatures) {
  // // TODO(apaymyshev): we support only segwit inputs by now, so script_sig
  // // should be emtpy.
  // for (const auto& input : tx.inputs()) {
