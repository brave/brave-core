...
TEST(CardanoTxDecoderTest, DecodeTransaction_ValidMinimalTransaction) {
  ...
  EXPECT_FALSE(restored_tx.tx_body.total_collateral.has_value());
}
...
TEST(CardanoTxDecoderTest, DecodeTransaction_WithWithdrawals) {
  ...
  EXPECT_EQ(withdrawals[0].address_bytes, expected_reward_account);
...
TEST(CardanoTxDecoderTest, DecodeTransaction_WithVotingProceduresRejected) {
  // Voting procedures (key 19) are not supported, so any transaction that
  // carries them must be rejected.
  // clang-format off
  std::vector<uint8_t> cbor = {
      0x82,        // [transaction_body, witness_set]
      0xa4,        // body map with 4 entries
      ...
      0x02,        // Key 2: fee
      0x1a, 0x00, 0x00, 0x27, 0x10,  // Fee: 10000
      0x13,        // Key 19: voting_procedures
      0x80,        // Empty array placeholder
      0xa0,        // Empty witness set
  };
  // clang-format on
...
TEST(CardanoTxDecoderTest, DecodeTransaction_WithProposalProceduresRejected) {
  // Proposal procedures (key 20) are not supported, so any transaction that
  // carries them must be rejected.
  // clang-format off
  std::vector<uint8_t> cbor = {
      0x82,        // [transaction_body, witness_set]
      ...
      0x02,        // Key 2: fee
      0x1a, 0x00, 0x00, 0x27, 0x10,  // Fee: 10000
      0x14,        // Key 20: proposal_procedures
      0x80,        // Empty array placeholder
      0xa0,        // Empty witness set
  };
  // clang-format on
...
TEST(CardanoTxDecoderTest, DecodeTransaction_WithCurrentTreasuryValueRejected) {
  // Current treasury value (key 21) is not supported, so any transaction
  // that carries it must be rejected.
  // clang-format off
  std::vector<uint8_t> cbor = {
      0x82,        // [transaction_body, witness_set]
      0xa4,        // body map with 4 entries
      ...
      0x02,        // Key 2: fee
      0x1a, 0x00, 0x00, 0x27, 0x10,  // Fee: 10000
      0x15,        // Key 21: current treasury value
      0x00,        // 0 placeholder
      0xa0,        // Empty witness set
  };
  // clang-format on
...
TEST(CardanoTxDecoderTest, DecodeTransaction_WithDonationRejected) {
  // Donation (key 22) is not supported, so any transaction that carries it
  // must be rejected.
  // clang-format off
  std::vector<uint8_t> cbor = {
      0x82,        // [transaction_body, witness_set]
...
TEST(CardanoTxDecoderTest, DecodeTransaction_WithCollateral) {
  // Body with collateral (13), collateral return (16), total collateral (17).
  // clang-format off
  std::vector<uint8_t> cbor = {
      0x82,        // [transaction_body, witness_set]
      ...
      0x02,        // Key 2: fee
      0x1a, 0x00, 0x00, 0x27, 0x10,
      0x0d,        // Key 13: collateral
      0x81,        // nonempty set with 1 input
      0x82, 0x58, 0x20,
      0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa,
      0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa,
      0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa,
      0x02,        // Index: 2
      0x10,        // Key 16: collateral return
      0x82, 0x58, 0x1c,
      ...
      0x1a, 0x00, 0x1e, 0x84, 0x80,  // 2000000
      0xa0,        // Empty witness set
  };
  // clang-format on
  ...
  ASSERT_EQ(body.collateral.size(), 1u);
  EXPECT_EQ(body.collateral[0].index, 2u);
  std::array<uint8_t, 32> expected_collateral_hash;
  expected_collateral_hash.fill(0xaa);
  EXPECT_EQ(body.collateral[0].tx_hash, expected_collateral_hash);

  ASSERT_TRUE(body.collateral_return.has_value());
  EXPECT_EQ(body.collateral_return->coin_value.lovelace_amount, 1000000u);
  std::vector<uint8_t> expected_return_addr(28, 0xbb);
