...
TEST_F(CardanoApiImplTest, SignTx_DeclinedByPartialSignError) {
  ...
  auto tx_bytes =
      CardanoTransactionSerializer({.ignore_sign_check_for_testing = true})
...
TEST_F(CardanoApiImplTest, SignTx) {
  ...
  auto unsigned_tx_bytes =
      CardanoTransactionSerializer({.ignore_sign_check_for_testing = true})
          .SerializeTransaction(unsigned_tx);
  ...
  EXPECT_EQ(request->change_outputs.size(), 1u);
  EXPECT_EQ(request->change_outputs[0]->address, addr1);
  EXPECT_EQ(request->change_outputs[0]->value, 24282816u);
...
TEST_F(CardanoApiImplTest, SignTx_ExistingExternalSignature) {
  ...
  auto signed_tx_bytes =
      CardanoTransactionSerializer({.ignore_sign_check_for_testing = true})
          .SerializeTransaction(signed_tx);
...
TEST_F(CardanoApiImplTest, SignTx_PartialSign) {
  ...
  auto unsinged_tx_bytes =
      CardanoTransactionSerializer({.ignore_sign_check_for_testing = true})
          .SerializeTransaction(unsiged_tx);
