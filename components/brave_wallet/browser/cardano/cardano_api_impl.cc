...
void CardanoApiImpl::OnSignTransactionRequestProcessed(
  ...
  for (const auto& input : tx.tx_body.inputs) {
    ...
    converted_result.signature_bytes.assign(
        sign_result.value().signature.begin(),
        sign_result.value().signature.end());
    converted_result.public_key.assign(sign_result.value().pubkey.begin(),
                                       sign_result.value().pubkey.end());
