...
void CardanoApiImpl::SignTx(const std::string& tx_cbor,
  ...
  auto restored_tx =
      CardanoTransactionSerializer::DeserializeTransaction(tx_cbor_bytes);
  if (!restored_tx) {
    std::move(callback).Run(
        std::nullopt,
        mojom::CardanoProviderErrorBundle::New(
...
void CardanoApiImpl::OnGetUtxosForSignTx(
  ...
  if (!InsertKnowInputAddresses(utxos.value(), tx, partial_sign)) {
    std::move(callback).Run(
        std::nullopt,
        mojom::CardanoProviderErrorBundle::New(
...
mojom::SignCardanoTransactionRequestPtr CardanoApiImpl::FromRestoredTransaction(
  ...
  std::vector<mojom::CardanoTxOutputPtr> change_outputs;
  ...
  for (const auto& output : tx.tx_body_.outputs_) {
    bool is_change = address_map->find(output.address) != address_map->end();
    if (is_change) {
      change_outputs.emplace_back(mojom::CardanoTxOutput::New(
  ...
  return mojom::SignCardanoTransactionRequest::New(
      ...
      std::move(external_outputs), std::move(change_outputs),
...
FindAddressByInput(
    ...
    const CardanoTransactionSerializer::RestoredTransactionInput& input) {
...
bool CardanoApiImpl::InsertKnowInputAddresses(
    const GetCardanoUtxosTask::UtxoMap& utxo_map,
...
void CardanoApiImpl::OnSignTransactionRequestProcessed(
  ...
  auto signed_tx = CardanoTransactionSerializer().ApplySignResults(
