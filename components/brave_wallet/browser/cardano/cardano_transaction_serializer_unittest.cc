...
TEST(CardanoTransactionSerializerTest, RestoreTransaction) {
  auto tx = GetReferenceTransaction();
  auto tx_bytes = CardanoTransactionSerializer().SerializeTransaction(tx);
  auto deserialize_result =
      CardanoTransactionSerializer().DeserializeTransaction(tx_bytes);
...
TEST(CardanoTransactionSerializerTest, ApplySignatures) {
  ...
  EXPECT_EQ(
      CardanoTransactionSerializer({.ignore_sign_check_for_testing = true})
          .ApplySignResults(tx_bytes, sign_results)
          .value(),
      CardanoTransactionSerializer({.ignore_sign_check_for_testing = true})
          .SerializeTransaction(tx_with_signatures));
}
