...
//     "assets_asset_storage_index": int,        // u8
//     "assets_metadata_storage_index": int,     // u8
//     "assets_account_storage_index": int,      // u8
...
PolkadotChainMetadataPrefs::GetChainMetadata(std::string_view chain_id) const {
  ...
  if (!ReadBool(*chain_metadata, kHasAssetsPallet, &has_assets_pallet)) {
    return std::nullopt;
  }

  uint8_t assets_pallet_index = 0;
  if (!ReadUint(*chain_metadata, kAssetsPalletIndex, &assets_pallet_index)) {
    return std::nullopt;
  }

  uint8_t assets_asset_storage_index = 0;
  if (!ReadUint(*chain_metadata, kAssetsAssetStorageIndex,
                &assets_asset_storage_index)) {
    return std::nullopt;
  }

  uint8_t assets_metadata_storage_index = 0;
  if (!ReadUint(*chain_metadata, kAssetsMetadataStorageIndex,
                &assets_metadata_storage_index)) {
    return std::nullopt;
  }

  uint8_t assets_account_storage_index = 0;
  if (!ReadUint(*chain_metadata, kAssetsAccountStorageIndex,
  ...
  return PolkadotChainMetadata::FromFields(
      ...
      spec_version, asset_tx_payment, has_assets_pallet, assets_pallet_index,
      assets_asset_storage_index, assets_metadata_storage_index,
      assets_account_storage_index, assets_transfer_keep_alive_call_index);
}
...
bool PolkadotChainMetadataPrefs::SetChainMetadata(
  ...
  int assets_asset_storage_index = 0;
  int assets_metadata_storage_index = 0;
  int assets_account_storage_index = 0;
  ...
  if (!base::CheckedNumeric<int>(metadata.GetSystemPalletIndex())
      ...
      !base::CheckedNumeric<int>(metadata.GetAssetsPalletIndex())
           .AssignIfValid(&assets_pallet_index) ||
      !base::CheckedNumeric<int>(metadata.GetAssetsAssetStorageIndex())
           .AssignIfValid(&assets_asset_storage_index) ||
      !base::CheckedNumeric<int>(metadata.GetAssetsMetadataStorageIndex())
           .AssignIfValid(&assets_metadata_storage_index) ||
      !base::CheckedNumeric<int>(metadata.GetAssetsAccountStorageIndex())
           .AssignIfValid(&assets_account_storage_index) ||
      !base::CheckedNumeric<int>(metadata.GetAssetsTransferKeepAliveCallIndex())
  ...
  value.Set(kHasAssetsPallet, metadata.HasAssetsPallet());
  value.Set(kAssetsPalletIndex, assets_pallet_index);
  value.Set(kAssetsAssetStorageIndex, assets_asset_storage_index);
  value.Set(kAssetsMetadataStorageIndex, assets_metadata_storage_index);
  value.Set(kAssetsAccountStorageIndex, assets_account_storage_index);
