...
TEST(PolkadotChainMetadataUnitTest, FromFields) {
  ...
  EXPECT_TRUE(metadata.HasAssetsPallet());
  EXPECT_EQ(metadata.GetAssetsPalletIndex(), 50u);
  EXPECT_EQ(metadata.GetAssetsAssetStorageIndex(), 0u);
  EXPECT_EQ(metadata.GetAssetsMetadataStorageIndex(), 3u);
  EXPECT_EQ(metadata.GetAssetsAccountStorageIndex(), 1u);
  EXPECT_EQ(metadata.GetAssetsTransferKeepAliveCallIndex(), 9u);
...
TEST(PolkadotChainMetadataUnitTest,
  ...
  EXPECT_FALSE(metadata->HasAssetsPallet());
  EXPECT_EQ(metadata->GetAssetsPalletIndex(), 0u);
  EXPECT_EQ(metadata->GetAssetsAssetStorageIndex(), 0u);
  EXPECT_EQ(metadata->GetAssetsMetadataStorageIndex(), 0u);
  EXPECT_EQ(metadata->GetAssetsAccountStorageIndex(), 0u);
  EXPECT_EQ(metadata->GetAssetsTransferKeepAliveCallIndex(), 0u);
}
