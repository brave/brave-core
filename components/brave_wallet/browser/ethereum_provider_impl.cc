...
void EthereumProviderImpl::AddEthereumChain(base::ListValue params,
  ...
  if (brave_wallet_service_->network_manager()
    ...
    if (base::CompareCaseInsensitiveASCII(
            json_rpc_service_->GetChainIdSync(mojom::CoinType::ETH, origin_),
            chain_id_lower) != 0) {
  ...
  auto error_message =
      json_rpc_service_->AddEthereumChainForOrigin(std::move(chain), origin_);
...
void EthereumProviderImpl::SendOrSignTransactionInternal(
  ...
  mojom::NetworkInfoPtr chain =
      json_rpc_service_->GetNetworkSync(mojom::CoinType::ETH, origin_);
...
void EthereumProviderImpl::SignMessage(const std::string& address,
  ...
  if (siwe_message) {
    ...
    if (EthAddress::From0xHex(address) !=
        EthAddress::From0xHex(siwe_message->address)) {
      brave_wallet_service_->AddSignMessageError(mojom::SignMessageError::New(
          GenerateRandomHexString(), MakeOriginInfo(origin_),
...
void EthereumProviderImpl::SetupMethodHandlers() {
  ...
  add_handler(kWalletInvokeSnap,
              &EthereumProviderImpl::HandleWalletInvokeSnapMethod);
...
void EthereumProviderImpl::HandleWalletInvokeSnapMethod(
  ...
  // wallet_invokeSnap / wallet_snap params:
  // [{ snapId: string, request: { method: string, params?: unknown } }]
  LOG(ERROR) << "XXXZZZ HandleWalletInvokeSnapMethod origin="
             << origin_.Serialize()
             << " params_count=" << request.params.size();
  const base::DictValue* params_dict =
      (request.params.size() == 1 && request.params.front().is_dict())
  ...
  LOG(ERROR) << "XXXZZZ HandleWalletInvokeSnapMethod: snap_id=" << *snap_id
             << " method=" << *method;
...
void EthereumProviderImpl::HandleWalletRequestSnapsMethod(
  ...
  LOG(ERROR) << "XXXZZZ HandleWalletRequestSnapsMethod origin="
             << origin_.Serialize()
             << " params_count=" << request.params.size();
  const base::DictValue* snaps_dict = nullptr;
  ...
  for (const auto [snap_id, opts] : *snaps_dict) {
    LOG(ERROR) << "XXXZZZ HandleWalletRequestSnapsMethod: requesting snap_id="
               << snap_id;
  }
  ...
  ss->snap_controller()->RequestSnaps(
      origin_, *snaps_dict,
      base::BindOnce(&EthereumProviderImpl::OnSnapRequestSnapsResult,
                     weak_factory_.GetWeakPtr(), std::move(request_callback),
                     std::move(request.id)));
}
...
void EthereumProviderImpl::OnSnapInvokeResult(
  ...
  LOG(ERROR) << "XXXZZZ OnSnapInvokeResult has_result=" << result.has_value()
  ...
  base::Value response_value =
      result ? std::move(*result) : base::Value(base::DictValue());
  std::move(callback).Run(mojom::EthereumProviderResponse::New(
      std::move(id), std::move(response_value), false, "", false));
}

void EthereumProviderImpl::OnSnapRequestSnapsResult(
    RequestCallback callback,
    base::Value id,
...
void EthereumProviderImpl::GetChainId(GetChainIdCallback callback) {
  if (json_rpc_service_) {
    json_rpc_service_->GetChainIdForOrigin(mojom::CoinType::ETH, origin_,
                                           std::move(callback));
  }
}
...
void EthereumProviderImpl::AddSuggestToken(mojom::BlockchainTokenPtr token,
  ...
  auto request = mojom::AddSuggestTokenRequest::New(MakeOriginInfo(origin_),
                                                    std::move(token));
