...
TEST(EthCallDataBuilderTest, TokensBalance) {
  ...
  ASSERT_FALSE(TokensBalance("invalid", {"0x0D8775F648430679A709E98d2b0Cb6250d2887EF"}));

  // Valid owner address, invalid contract address is invalid
  ASSERT_FALSE(TokensBalance("0x08A8fDBddc160A7d5b957256b903dCAb1aE512C5", {"invalid"}));
  ...
  data = TokensBalance("0xB4B2802129071b2B9eBb8cBB01EA1E4D14B34961",
                       {"0x0D8775F648430679A709E98d2b0Cb6250d2887EF", "0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48"});
  ASSERT_TRUE(data);
  EXPECT_EQ(
      data.value(),
      "0xe5da1b68000000000000000000000000B4B2802129071b2B9eBb8cBB01EA1E4D14B349"
