...
TEST(EthCallDataBuilderTest, GetEthAddr) {
  {
    auto call = GetEthAddr("test.crypto", "ETH", mojom::kMainnetChainId);
  ...
  {
    auto call = GetEthAddr("test.crypto", "USDT",
  ...
  {
    auto call = GetEthAddr("test.crypto", "QWEQWE", "0x12345");
