...
void KeyringService::CreateKeyrings(const KeyringSeed& keyring_seed) {
  ...
  if (IsKeyringEnabled(KeyringId::kPolkadotSubstrateMainnet)) {
    LOG(ERROR) << "XXXZZZ create polkadot keyring";
    polkadot_substrate_mainnet_keyring_ = std::make_unique<PolkadotSubstrateKeyring>(
        keyring_seed.entropy);
  }
}
...
template <>
PolkadotSubstrateKeyring* KeyringService::GetKeyring(mojom::KeyringId keyring_id) const {
  for (auto* keyring :
       {polkadot_substrate_mainnet_keyring_.get()}) {
    if (keyring && keyring->keyring_id() == keyring_id) {
      return keyring;
    }
  }
