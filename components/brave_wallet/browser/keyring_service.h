...
class KeyringService : public KeyedService, public mojom::KeyringService {
 public:
  ...
  void SetHardwareAccountName(const std::string& keyring_id,
                              const std::string& address,
 ...
 private:
  ...
  void NotifySelectedAccountChanged(const mojom::AccountInfo& selected_account);
  void NotifySelectedDappAccountChangedForCoin(mojom::CoinType coin);
  bool SetSelectedAccountInternalSilently(
      const mojom::AccountInfo& selected_account);

  bool ValidatePasswordInternal(const std::string& password);
  void HandleMaybeSelectedAccountRemoved(mojom::CoinType coin,
                                         const std::string& keyring_id,
                                         const std::string& address);
  mojom::AccountInfoPtr SelectFallbackAccount();
