...
TEST(HashUtilsUnitTest, DoubleSHA256Hash) {
  ...
  // NIST.1
  EXPECT_EQ(brave_wallet::HexEncodeLower(
                DoubleSHA256Hash(std::vector<uint8_t>{0x61, 0x62, 0x63})),
            "4f8b42c22dd3729b519ba6f68d2da7cc5b2d606d05daed5ad5128cc03e6c6358");

  // EMPTY
  EXPECT_EQ(
      brave_wallet::HexEncodeLower(DoubleSHA256Hash(std::vector<uint8_t>{})),
      "5df6e0e2761359d30a8275058e299fcc0381534545f55cf43e41983f5d4c9456");
}

TEST(HashUtilsUnitTest, Hash160) {
  EXPECT_EQ(brave_wallet::HexEncodeLower(
                Hash160(std::vector<uint8_t>{0x61, 0x62, 0x63})),
            "bb1be98c142444d7a56aa3981c3942a978e4dc33");

  EXPECT_EQ(brave_wallet::HexEncodeLower(Hash160(std::vector<uint8_t>{})),
...
TEST(HashUtilsUnitTest, Blake2bHash) {
  ...
  EXPECT_EQ(base::HexEncode(Blake2bHash<64>(
                {base::byte_span_from_cstring("abc")}, personalizer)),
