...
AddressContent::AddressContent(AddressType type,
                               ...
                               ChainType chainType)
    : addressType(type), hash(std::move(hash)), chainType(chainType) {}
...
std::string EncodeCashAddress(const std::string& prefix,
  ...
  std::vector<uint8_t> data =
      PackAddressData(content.hash, content.addressType);
...
static std::optional<ChainType> ChainTypeFromPrefix(const std::string& prefix) {
...
std::optional<AddressContent> DecodeCashAddress(
  ...
  return AddressContent(type, data, *chainType);
}
