...
// Short representation of the merkle tree state on some point
// Since batch inserting may contain gaps between scan ranges we insert
// frontier tree state which allows to calculate witnesses(merkle path from left
// to the tree root) even previos scan ranges are not completed.
struct FrontierChainState {
  FrontierChainState();
  ~FrontierChainState();
  FrontierChainState(const FrontierChainState&);

  uint32_t frontier_block_height;
  uint32_t frontier_orchard_tree_size;
  std::vector<uint8_t> frontier_tree_state;
};
