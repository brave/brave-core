...
std::optional<uint64_t> ReadCompactSize(base::span<const uint8_t>& data) {
  ...
  if (data.size() > 0 && data[0] < 253) {
    ...
    value = base::numerics::U16FromBigEndian(data.subspan(1).first<2u>());
    data = data.subspan(1 + 2);
  } else if (type <= 254 && data.size() >= 5) {
    value = base::numerics::U32FromBigEndian(data.subspan(1).first<4u>());
    data = data.subspan(1 + 4);
  } else if (data.size() >= 9) {
    value = base::numerics::U64FromBigEndian(data.subspan(1).first<8u>());
