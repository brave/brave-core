...
mojom::BlockchainTokenPtr ValueToBlockchainToken(const base::DictValue& value) {
  ...
  if (auto type_int = value.FindInt("zcash_token_type")) {
    ...
    token_ptr->zcash_token_type = mojom::IsKnownEnumValue(type)
                                      ? type
                                      : mojom::ZCashTokenType::kNone;
  } else {
    // Back-compat with schema that stored a bool `is_shielded`. Shielded tokens
    // are never persisted, so a legacy `true` is only defensive.
    bool legacy_shielded = value.FindBool("is_shielded").value_or(false);
    token_ptr->zcash_token_type =
        legacy_shielded
            ? mojom::ZCashTokenType::kOrchard
            : (token_ptr->coin == mojom::CoinType::ZEC
...
base::DictValue BlockchainTokenToValue(const mojom::BlockchainTokenPtr& token) {
  ...
  value.Set("zcash_token_type",
            static_cast<int>(token->zcash_token_type));
  return value;
}
