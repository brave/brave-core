...
class BraveQueryFilter_ComponentEnabled : public testing::Test {
 public:
  void SetUp() override {
    feature_list_.InitAndEnableFeature(features::kQueryFilterComponent);
    _filter_test_rules.emplace();
  }

  void TearDown() override { ResetRules(); }

  void ResetRules() {
    if (_filter_test_rules.has_value()) {
      _filter_test_rules.reset();
    }
  }

  void UpdateRules(std::string_view json) {
    ASSERT_TRUE(_filter_test_rules);
    ASSERT_TRUE(_filter_test_rules->UpdateRules(json));
  }

  QueryFilterData* instance() const { return QueryFilterData::GetInstance(); }

 private:
  std::optional<test::ScopedTestingQueryFilterRules> _filter_test_rules;
...
TEST_F(BraveQueryFilter_ComponentEnabled, FilterQueryTrackers) {
  ...
  EXPECT_EQ(MaybeApplyQueryStringFilter(GURL("https://brave.com"), GURL(),
                                        GURL("https://test.com/?fbclid=123"),
                                        "GET", false),
            GURL("https://test.com/"));
  // Expect filtering `fbclid` param when cross origin `initiator_url`
  // and invalid `redirect_source_url`
  ...
  EXPECT_EQ(MaybeApplyQueryStringFilter(GURL(), GURL("https://brave.com"),
                                        GURL("https://test.com/?mkt_tok=123"),
                                        "GET", false),
            GURL("https://test.com/"));
  ...
  EXPECT_FALSE(MaybeApplyQueryStringFilter(GURL("https://test.com"), GURL(),
                                           GURL("https://test.com/?gclid=123"),
                                           "GET", false));
  // Test nothing is filtered with same origin `redirect_source_url`
  EXPECT_FALSE(MaybeApplyQueryStringFilter(GURL(), GURL("https://test.com"),
                                           GURL("https://test.com/?gclid=123"),
                                           "GET", false));
  // Test nothing is filtered with same origin `redirect_source_url`
...
TEST_F(BraveQueryFilter_ComponentEnabled, ScopedQueryTrackingTest) {
  ...
  EXPECT_FALSE(MaybeApplyQueryStringFilter(GURL("https://brave.com"), GURL(),
                                           GURL("https://twitter.com/?t=123"),
                                           "GET", false));
  ...
  EXPECT_EQ(MaybeApplyQueryStringFilter(GURL("https://brave.com"), GURL(),
                                        GURL("https://x.com/?ref_src=123"),
                                        "GET", false),
            GURL("https://x.com/"));
  EXPECT_EQ(MaybeApplyQueryStringFilter(GURL("https://brave.com"), GURL(),
                                        GURL("https://y.com/?ref_src=123"),
                                        "GET", false),
            GURL("https://y.com/"));
  EXPECT_FALSE(MaybeApplyQueryStringFilter(GURL("https://brave.com"), GURL(),
                                           GURL("https://z.com/?ref_src=123"),
                                           "GET", false));

  // Edge cases
  EXPECT_EQ(MaybeApplyQueryStringFilter(GURL("https://example.com"), GURL(),
                                        GURL("https://brave.com/?sample1=123"),
                                        "GET", false),
...
class BraveQueryFilter_ComponentDisabled : public testing::Test {
 public:
  void SetUp() override {
    feature_list_.InitAndDisableFeature(features::kQueryFilterComponent);
