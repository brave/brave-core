...
namespace {
constexpr base::TimeDelta kMaxFetchJitter = base::Minutes(1);
}  // namespace
...
void ObliviousHttpKeyConfig::MaybeFetch() {
  if (HasExpired()) {
    VLOG(1) << "OHTTP key config has expired";
    return Fetch();
  }
...
void ObliviousHttpKeyConfig::FetchAfter(base::TimeDelta delay) {
  ...
  const base::TimeDelta randomized_delay =
      delay + base::RandTimeDeltaUpTo(kMaxFetchJitter);
  const base::Time fetch_at = base::Time::Now() + randomized_delay;
  VLOG(1) << "Fetch OHTTP key config at " << fetch_at;
...
void ObliviousHttpKeyConfig::Fetch() {
  ...
  VLOG(1) << "Fetching OHTTP key config";
...
void ObliviousHttpKeyConfig::FetchCallback(
  ...
  if (!url_response_body) {
    VLOG(0) << "Failed to fetch OHTTP key config due to missing response body";
    return FailedToFetch();
  }

  const auto* const response = url_loader->ResponseInfo();
  if (!response) {
    VLOG(0) << "Failed to fetch OHTTP key config due to missing response";
    return FailedToFetch();
  }
  ...
  if (response_headers->response_code() != net::HTTP_OK) {
    VLOG(0) << "Failed to fetch OHTTP key config due to invalid response code: "
            << response_headers->response_code();
    return FailedToFetch();
  }

  if (url_response_body->empty()) {
    VLOG(0) << "Failed to fetch OHTTP key config due to empty body";
    return FailedToFetch();
  }
...
void ObliviousHttpKeyConfig::SuccessfullyFetched(
  ...
  VLOG(1) << "Successfully fetched OHTTP key config";
...
void ObliviousHttpKeyConfig::Retry() {
  VLOG(0) << "Retry fetching OHTTP key config";
...
void ObliviousHttpKeyConfig::Remove() {
  VLOG(1) << "Removed OHTTP key config";
