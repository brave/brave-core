...
TEST(SearchQueryMetricsSearchEngineUtilTest, IsSearchEngine) {
  for (std::string_view url_string : kSearchEnginetUrls) {
    GURL url(url_string);
    ASSERT_TRUE(url.is_valid()) << url;
    EXPECT_TRUE(IsSearchEngine(url)) << url;
  }
}

TEST(SearchQueryMetricsSearchEngineUtilTest, IsNotSearchEngine) {
  for (std::string_view url_string : kGenericUrls) {
    GURL url(url_string);
    ASSERT_TRUE(url.is_valid()) << url;
    EXPECT_FALSE(IsSearchEngine(url)) << url;
  }
}

TEST(SearchQueryMetricsSearchEngineUtilTest, IsSearchEngineResultsPage) {
  for (std::string_view url_string : kSearchEnginetUrls) {
    GURL url(url_string);
    ASSERT_TRUE(url.is_valid()) << url;
...
TEST(SearchQueryMetricsSearchEngineUtilTest, IsNotSearchEngineResultsPage) {
  for (std::string_view url_string : kGenericUrls) {
    GURL url(url_string);
    ASSERT_TRUE(url.is_valid()) << url;
