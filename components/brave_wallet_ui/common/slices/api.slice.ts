...
let selectedPendingTransactionId: string = ''
  ...
  */
  export class BaseQueryCache {
    ...
    private _selectedAccount?: AccountInfoEntity
    ...
    getSelectedAccount = async () => {
      if (!this._selectedAccount) {
        const allAccounts = await this.getAllAcountsInfo()
        const info = allAccounts.accounts[allAccounts.selectedIndex];
        this._selectedAccount = accountInfoToAccountInfoEntity(info)
      }
...
export function createWalletApi (
  ...
  const walletApi = createApi({
    ...
    endpoints: ({ mutation, query }) => { return {
      ...
      >({
        queryFn: async ({ keyringId, address }, api, extraOptions, baseQuery) => {
          const {
            cache,
            // apiProxy
            data: { keyringService }
          ...
          await keyringService.setSelectedAccount(keyringId, address)
      ...
      getSelectedAccount: query<AccountInfoEntity, void>({
        queryFn: async (arg, { dispatch }, extraOptions, baseQuery) => {
          const { cache } = baseQuery(undefined)

          return {
            data: await cache.getSelectedAccount()
          }
        },
      ...
      getGasEstimation1559: query<BraveWallet.GasEstimation1559, void>({
        queryFn: async (_, { dispatch }, extraOptions, baseQuery) => {
          try {
            ...
            // TODO(apaymyshev): this should not be selected account and network,
            // but account and network for watched transaction
...
export const {
  ...
  useLazyGetSelectedAccountQuery,
