...
test('Bridge not ready', () => {
  ...
  // Use a flag instead of a real DOM element to signal that bridge creation has
  // started. A DOM element would leak across tests and cause hasBridgeCreated()
  // to return true for later transports sharing the same frameId (possible when
  // crypto.randomUUID is mocked to a fixed value by another test file).
