...
export function refreshVisibleTokenInfo (targetNetwork?: BraveWallet.NetworkInfo) {
  return async (dispatch: Dispatch, getState: () => State) => {
    ...
    /**
     * this can still return an empty array
     * when the request is already in-flight
     */
    const networks = await queryNetworksList(dispatch, 'visibleIds')
    ...
    const visibleAssets = targetNetwork
      ...
      : await Promise.all(networks.map(async (item) => await inner(item)))
