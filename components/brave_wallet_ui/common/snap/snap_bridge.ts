...
export class SnapBridge {
  ...
  // Optional override for snapsService — used by snap_host.ts which provides
  // its own SnapsService remote instead of going through getAPIProxy().
  private snapsServiceOverride: BraveWallet.SnapsServiceRemote | null = null
  ...
  setSnapsService(svc: BraveWallet.SnapsServiceRemote): void {
    this.snapsServiceOverride = svc
  }

  private getSnapsService(): BraveWallet.SnapsServiceRemote {
    if (this.snapsServiceOverride) {
      return this.snapsServiceOverride
    }
    return getAPIProxy().snapsService
  }
