...
function App () {
  ...
  const networks: BraveWallet.NetworkInfo[] = useUnsafeWalletSelector(
    WalletSelectors.networkList)
      ...
      ...fullTokenList.filter(
        asset =>
          !userVisibleTokensInfo.some(
            token => token.contractAddress.toLowerCase() ===
  ...
  React.useEffect(() => {
    ; (async () => {
      const results = await Promise.all(
        networks.map(async e =>
          (await swapServiceMojo.isSwapSupported(e.chainId)).result)
      )

      const result = networks.filter((_, index) =>
          results[index]).map(e => makeNetworkInfo(e))
    ...
    <div>
      {selectedNetwork && selectedAccount && (
        <SwapInterface
          ...
          exchanges={[]}
          // FIXME - remove getNetworkFeeEstimate, since chain-specific
          //  methods are already available in the wallet adapters.
          // This is a stub to make tsc happy.
