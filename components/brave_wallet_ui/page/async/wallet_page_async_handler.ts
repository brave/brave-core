...
handler.on(WalletPageActions.updateAccountName.type, async (store: Store, payload: UpdateAccountNamePayloadType) => {
  ...
  if (hardwareAccount && hardwareAccount.hardware) {
    const result = await keyringService.setHardwareAccountName(payload.keyringId, payload.address, payload.name)
    return result.success
  }
  const result = payload.isDerived
    ? await keyringService.setKeyringDerivedAccountName(payload.keyringId, payload.address, payload.name)
    : await keyringService.setKeyringImportedAccountName(payload.keyringId, payload.address, payload.name)
  return result.success
})
