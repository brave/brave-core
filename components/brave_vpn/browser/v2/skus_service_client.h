...
// presentation; this class lazily binds the remote, re-binds it lazily on
// disconnect, and forwards the call.
//
// Callback contract: every response callback passed in is run exactly once,
// always asynchronously. If the pipe drops while a request is in flight, the
// callback is run with a synthesized error result instead of being silently
// destroyed.
