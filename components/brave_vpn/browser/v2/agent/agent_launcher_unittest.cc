...
// current attempt, and reporting it is what used to tear down a recovering
// agent client.
...
TEST_F(AgentLauncherTest, BurstOfLaunchesReportsOnce) {
  ...
  for (const base::test::TestFuture<LaunchError>& launch_reported : reported) {
    report_count += launch_reported.IsReady() ? 1u : 0u;
  }
