...
std::optional<NTPSponsoredSite> MaybeParseSite(
    const base::DictValue& dict,
    const base::FilePath& installed_dir,
    const std::string& url_prefix) {
  std::optional<int> tile_version = dict.FindInt(kTileVersionKey);
  if (tile_version != kExpectedTileVersion) {
    return std::nullopt;
  }
  ...
  const base::FilePath image_file_path = installed_dir.Append(
...
NTPSponsoredSitesData::NTPSponsoredSitesData(
  ...
  for (const auto& value : *sponsored_sites) {
    ...
    std::optional<NTPSponsoredSite> sponsored_site =
        MaybeParseSite(*tile_dict, installed_dir, url_prefix);
    if (sponsored_site) {
