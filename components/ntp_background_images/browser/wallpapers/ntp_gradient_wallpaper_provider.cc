...
NTPGradientWallpaperProvider::NTPGradientWallpaperProvider(
    NTPCustomBackgroundDelegate& custom_background_delegate)
    : custom_background_delegate_(custom_background_delegate) {}

NTPGradientWallpaperProvider::~NTPGradientWallpaperProvider() = default;

bool NTPGradientWallpaperProvider::IsEligible() const {
  return custom_background_delegate_->IsColorBackgroundEnabled() &&
         IsGradientColor(custom_background_delegate_->GetColor());
}

void NTPGradientWallpaperProvider::MaybeGetWallpaper(
    NTPWallpaperProvider::MaybeGetWallpaperCallback callback) {
  // Gradients do not get a wallpaper type of their own; the renderer only
  // ever receives a CSS color string, so this reuses the color shape.
