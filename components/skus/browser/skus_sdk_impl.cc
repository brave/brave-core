...
void SkusSdkFetcher::BeginFetch(
  ...
  resource_request->method = request_method.c_str();
...
void shim_executeRequest(
  ...
  // TODO: does `callback` require std::move()? I think so
  fetcher = make_unique<SkusSdkFetcher>(g_SkusSdk->url_loader_factory_);
  fetcher->BeginFetch(GURL(ruststring_2_stdstring(req.url)),
                 ruststring_2_stdstring(req.method), std::move(callback),
                 std::move(ctx));
}
