...
mod ffi {
    ...
    extern "Rust" {
        ...
        fn clone_resource_storage(storage: &BraveCoreResourceStorage) -> Box<BraveCoreResourceStorage>;
        fn merge_resource_storage(storage: &BraveCoreResourceStorage, additional_resources_json: &str) -> Box<BraveCoreResourceStorage>;
    }
    extern "Rust" {
        type Engine;
