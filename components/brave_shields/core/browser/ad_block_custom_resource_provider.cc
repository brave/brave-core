...
void AdBlockCustomResourceProvider::LoadResources(
    base::OnceCallback<void(rust::Box<adblock::BraveCoreResourceStorage>)> on_load) {
...
void AdBlockCustomResourceProvider::OnCustomResourcesLoaded(
    base::OnceCallback<void(rust::Box<adblock::BraveCoreResourceStorage>)> on_load,
    rust::Box<adblock::BraveCoreResourceStorage> default_storage,
  ...
  if (custom_resources.GetList().empty()) {
    ...
    if (!custom_resources_json) {
      ...
      auto merged_storage = adblock::merge_resource_storage(*default_storage, *custom_resources_json);
