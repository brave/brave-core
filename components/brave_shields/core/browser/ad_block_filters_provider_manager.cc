...
#include "brave/components/brave_shields/core/services/filter_set/mojom/filter_set.mojom-forward.h"
...
// Translates the 2-argument `LoadFilters` callback into one that takes a single tuple argument.
// Intended for compatibility with `BarrierCallback`.
void LoadFiltersTuple(base::RepeatingCallback<void(std::tuple<std::vector<unsigned char>, uint8_t>)> cb, std::vector<unsigned char> filter_buffer, uint8_t permission_mask) {
...
AdBlockFiltersProviderManager* AdBlockFiltersProviderManager::GetInstance() {
  static mojo::Remote<filter_set::mojom::UtilParseFilterSet> filter_set_service =
    content::ServiceProcessHost::Launch<filter_set::mojom::UtilParseFilterSet>(
        content::ServiceProcessHost::Options()
            .WithDisplayName("Adblock Filter Parsing")
            .Pass());

  static base::NoDestructor<AdBlockFiltersProviderManager> instance(std::move(filter_set_service));
  return instance.get();
}

AdBlockFiltersProviderManager::AdBlockFiltersProviderManager(mojo::Remote<filter_set::mojom::UtilParseFilterSet> filter_set_service) : filter_set_service_(std::move(filter_set_service)) {
}
...
void AdBlockFiltersProviderManager::LoadFilters(
    base::OnceCallback<
        void(std::vector<unsigned char> filter_buffer, uint8_t permission_mask)>) {
  NOTREACHED();
}
...
void AdBlockFiltersProviderManager::LoadFiltersForEngine(
  ...
  for (auto* const provider : filters_providers) {
    task_tracker_.PostTask(
        ...
        base::BindOnce(&AdBlockFiltersProvider::LoadFilters,
                       provider->AsWeakPtr(), base::BindRepeating(LoadFiltersTuple, collect_and_merge)));
  }
}
...
void AdBlockFiltersProviderManager::FinishCombinating(
    base::OnceCallback<
        void(const std::vector<unsigned char>& verified_engine_dat)> cb,
    uint64_t flow_id,
    std::vector<std::tuple<std::vector<unsigned char>, uint8_t>>
        results) {
  TRACE_EVENT("brave.adblock",
  ...
  std::transform(results.cbegin(), results.cend(), std::back_inserter(inputs), [](std::tuple<std::vector<unsigned char>, uint8_t> tuple) {
