...
void AdBlockService::SourceProviderObserver::OnResourcesLoaded(
  ...
  // Clone the storage for this engine - each engine gets its own Box but shares the underlying Arc
  ...
  if (!filter_set_) {
    ...
    auto engine_load_callback = base::BindOnce(
        ...
        adblock_engine_->AsWeakPtr(), std::move(filter_set_), std::move(cloned_storage));
