...
class BlockDecision {
 public:
  ...
  AdBlockDecision* AsAdBlockDecision() {
    return IsAdBlockDecision()
        ? reinterpret_cast<AdBlockDecision*>(this)
        : nullptr;
  }
  const AdBlockDecision* AsAdBlockDecision() const {
    return IsAdBlockDecision()
        ? reinterpret_cast<const AdBlockDecision*>(this)
        : nullptr;
  }
  ...
  AsHTTPUpgradableResourceBlockDecision() const {
    return IsHTTPUpgradableResourceBlockDecision()
        ? reinterpret_cast<const HTTPUpgradableResourceBlockDecision*>(this)
        : nullptr;
  }
};
...
class AdBlockDecision
 ...
 public:
  AdBlockDecision(const std::string& rule);
...
class TrackerBlockDecision
    : public BlockDecision {
 public:
  TrackerBlockDecision(const std::string& host);
