...
function AdBlockOnlyModeNotice() {
  const onLaunchClick = () => {
    chrome.tabs.create({ url: 'chrome://settings/shields', active: true })
  }
    ...
    <div style={{ padding: '0 var(--leo-spacing-xl) var(--leo-spacing-xl)' }}>
      ...
      <Alert type='info' hideIcon hasActions={false} style={{ paddingTop: 'var(--leo-spacing-m)' }}>
        <div style={{ font: 'var(--leo-font-heading-h4)' }}>
          Is the site working correctly now?
        </div>
        ...
        >
          <Button size="medium" onClick={onLooksGoodClick}>Looks good</Button>
          <Button kind="plain" size="medium" onClick={onUndoAndReportClick}>Report site</Button>
        </div>
      </Alert>
    </div>
  )
...
function AdBlockOnlyModeControlsContent() {
  return (
    <section
      id='global-controls-content'
    >
      <AdBlockOnlyModeNotice />
      <GlobalSettings />
    </section>
  )
}
