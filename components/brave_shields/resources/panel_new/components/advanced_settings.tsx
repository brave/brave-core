    ...
    <div>
      ...
      >
        ...
        <option value={AdBlockMode.AGGRESSIVE}>
          {getString(S.BRAVE_SHIELDS_TRACKERS_AND_ADS_BLOCKED_AGG)}
        </option>
        <option value={AdBlockMode.STANDARD}>
          {getString(S.BRAVE_SHIELDS_TRACKERS_AND_ADS_BLOCKED_STD)}
        </option>
        <option value={AdBlockMode.ALLOW}>
          {getString(S.BRAVE_SHIELDS_TRACKERS_AND_ADS_ALLOW_ALL)}
        </option>
      </select>
      <button
    ...
    <div>
      ...
      >
        ...
        <option value={HttpsUpgradeMode.STRICT_MODE}>
          {getString(S.BRAVE_SHIELDS_HTTPS_UPGRADE_MODE_STRICT)}
        </option>
        <option value={HttpsUpgradeMode.STANDARD_MODE}>
          {getString(S.BRAVE_SHIELDS_HTTPS_UPGRADE_MODE_STANDARD)}
        </option>
        <option value={HttpsUpgradeMode.DISABLED_MODE}>
          {getString(S.BRAVE_SHIELDS_HTTPS_UPGRADE_MODE_DISABLED)}
        </option>
      </select>
      <div />
    </div>
  ...
  const enforcedDescription = () => {
    switch (overrideSource) {
      case ContentSettingSource.kExtension:
        return getString(
          S.BRAVE_SHIELDS_SCRIPTS_BLOCKED_OVERRIDDEN_BY_EXTENSION,
        )
      case ContentSettingSource.kPolicy:
        return getString(S.BRAVE_SHIELDS_SCRIPTS_BLOCKED_OVERRIDDEN_BY_POLICY)
      case ContentSettingSource.kAllowList:
        return getString(
          S.BRAVE_SHIELDS_SCRIPTS_BLOCKED_OVERRIDDEN_BY_ALLOWLIST,
        )
      case ContentSettingSource.kSupervised:
        return getString(
          S.BRAVE_SHIELDS_SCRIPTS_BLOCKED_OVERRIDDEN_BY_SUPERVISOR,
        )
      case ContentSettingSource.kInstalledWebApp:
        return getString(S.BRAVE_SHIELDS_SCRIPTS_BLOCKED_OVERRIDDEN_BY_PWA)
      default:
        return getString(S.BRAVE_SHIELDS_SCRIPTS_BLOCKED_OVERRIDDEN)
    }
  }
    ...
    <div>
      ...
      <div className='toggle'>
        <div className='block-scripts-text'>
          <div>
            {getString(S.BRAVE_SHIELDS_SCRIPTS_BLOCKED)}
      ...
      {showStrictMode ? (
        ...
        >
          ...
          <option value={FingerprintMode.STRICT_MODE}>
            {getString(S.BRAVE_SHIELDS_FINGERPRINTING_BLOCKED_AGG)}
          </option>
          <option value={FingerprintMode.STANDARD_MODE}>
            {getString(S.BRAVE_SHIELDS_FINGERPRINTING_BLOCKED_STD)}
          </option>
          <option value={FingerprintMode.ALLOW_MODE}>
            {getString(S.BRAVE_SHIELDS_FINGERPRINTING_ALLOW_ALL)}
          </option>
        </select>
      ) : (
        <div className='toggle'>
    ...
    <div>
      ...
      >
        ...
        <option value={CookieBlockMode.BLOCKED}>
          {getString(S.BRAVE_SHIELDS_COOKIES_BLOCKED)}
        </option>
        <option value={CookieBlockMode.CROSS_SITE_BLOCKED}>
          {getString(S.BRAVE_SHIELDS_THIRD_PARTY_COOKIES_BLOCKED)}
        </option>
        <option value={CookieBlockMode.ALLOW}>
          {getString(S.BRAVE_SHIELDS_COOKIES_ALLOWED_ALL)}
        </option>
      </select>
      <div />
    </div>
...
function ForgetFirstPartyStorageControls() {
  ...
  return (
    <div>
      <Icon name='eye-off' />
      <div className='toggle'>
        {getString(S.BRAVE_SHIELDS_FORGET_FIRST_PARTY_STORAGE_LABEL)}
        <Toggle
  ...
  return (
    <div>
      ...
      <div className='block-elements'>
        <div>{getString(S.BRAVE_SHIELDS_BLOCK_ELEMENTS)}</div>
        <button onClick={actions.resetBlockedElements}>
          {getString(S.BRAVE_SHIELDS_SHOW_ALL_BLOCKED_ELEMENTS)}
        </button>
      </div>
      <div />
    </div>
