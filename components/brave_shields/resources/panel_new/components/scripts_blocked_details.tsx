...
export function ScriptsBlockedDetails(props: Props) {
  ...
  return (
    <main data-css-scope={style.scope}>
      <DetailsHeader
        title={getString(S.BRAVE_SHIELDS_SCRIPT_DETAILS_TITLE)}
        onBack={props.onBack}
      />
        ...
        {allowedList.length > 0 && (
          <>
            <div className='list-header allow-header'>
              {getString(S.BRAVE_SHIELDS_ALLOWED_SCRIPTS_LABEL)} (
              {allowedList.length})
              <button onClick={() => actions.blockAllowedScripts(allowedList)}>
                {getString(S.BRAVE_SHIELDS_BLOCK_SCRIPTS_ALL)}
              </button>
            </div>
            <div className='resource-list'>
              <ResourceListView
                urls={allowedList}
                actionText={getString(S.BRAVE_SHIELDS_SCRIPT_BLOCK)}
        ...
        {blockedList.length > 0 && (
          <>
            <div className='list-header block-header'>
              {getString(S.BRAVE_SHIELDS_BLOCKED_SCRIPTS_LABEL)} (
              {blockedList.length})
              <button onClick={() => actions.allowScriptsOnce(blockedList)}>
                {getString(S.BRAVE_SHIELDS_ALLOW_SCRIPTS_ALL)}
              </button>
            </div>
            <div className='resource-list'>
              <ResourceListView
                urls={blockedList}
                actionText={getString(S.BRAVE_SHIELDS_ALLOW_SCRIPT_ONCE)}
