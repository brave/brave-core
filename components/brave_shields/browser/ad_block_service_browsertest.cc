...
// Attempt to request from a hostname that is blocked by a rule in the base
// adblocking engine and is excepted by another rule in the base adblocking
// engine, but has an important blocking rule in the custom adblock rules
// engine.
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest,
    CustomImportantRulesOverrideDefaultClientExceptions) {

  EXPECT_EQ(browser()->profile()->GetPrefs()->GetUint64(kAdsBlocked), 0ULL);

  UpdateAdBlockInstanceWithRules("*o.png\n@@*go.png");
  ASSERT_TRUE(g_brave_browser_process->ad_block_custom_filters_service()
                ->UpdateCustomFilters("*logo.png$important"));
