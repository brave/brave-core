...
base::FilePath GetSubscriptionPath(const base::FilePath& path,
                                   const GURL& id) {
  ...
  return path.AppendASCII(pathsafe_id);
}
...
base::FilePath AdBlockSubscriptionServiceManager::GetSubscriptionPath(
    const GURL& id) const {
  return brave_shields::GetSubscriptionPath(subscription_path_, id);
}
...
// TODO(bridiver) this needs to be a weakptr reference
void AdBlockSubscriptionServiceManager::OnGetDownloadManager(
  ...
  download_manager_->set_subscription_path_callback(base::BindRepeating(
      &brave_shields::GetSubscriptionPath, subscription_path_));
  // TODO(bridiver) - add observer for remaining methods and remove
  // AdBlockSubscriptionServiceManager from AdBlockSubscriptionDownloadManager
  ready_.Signal();
}
