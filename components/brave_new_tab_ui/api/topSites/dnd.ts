...
export const onDraggedSite =
    (state: NewTab.State, url: string, destUrl: string) => {
      const gridSitesWithoutPreview = gridAPI.getGridSites(state)
      const currentPositionIndex =
          gridSitesWithoutPreview.findIndex(site => site.url === url)
      const finalPositionIndex =
          gridSitesWithoutPreview.findIndex(site => site.url === destUrl)
      ...
      if (!pinnedMovingSite) {
        const movingTopSite = Object.assign(
            {}, gridSitesWithoutPreview.find(site => site.url === url))
      ...
      state = {...state, pinnedTopSites} state = {
        ...state,
        gridSites: gridAPI.getGridSites(state)
      } return state
    }

export const onDragEnd = (state: NewTab.State) => {
  state = {...state, gridSites: gridAPI.getGridSites(state)} return state
}
