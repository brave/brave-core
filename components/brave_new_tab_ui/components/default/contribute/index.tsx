...
import ContributeLogo from './assets/contribute-logo'
...
import { getUSDPrice } from '../../../contribute-utils'
...
interface Props {
  ...
  onContributeUserTLD: (userTLD: NewTab.ContributeTLD) => void
  ...
  onContributeAccountBalances: (balances: Record<string, string>) => void
  onContributeClientUrl: (clientUrl: string) => void
  onDisconnectContribute: () => void
  onCancelDisconnect: () => void
  onConnectContribute: () => void
  onValidAuthCode: () => void
  onUpdateActions: () => void
  onDismissAuthInvalid: () => void
}

class Contribute extends React.PureComponent<Props, State> {
  ...
  componentDidMount () {
    ...
    if (!userTLDAutoSet) {
      chrome.contribute.getUserTLD((userTLD: NewTab.ContributeTLD) => {
        this.props.onContributeUserTLD(userTLD)
  ...
  getClientURL = () => {
    chrome.contribute.getClientUrl((clientUrl: string) => {
      this.props.onContributeClientUrl(clientUrl)
    })
  }
  ...
  checkForOauthCode = () => {
    ...
    if (authCode) {
      chrome.contribute.getAccessToken(authCode, (success: boolean) => {
        if (success) {
  ...
  connectContribute = () => {
    const { contributeClientUrl } = this.props
    window.open(contributeClientUrl, '_self')
    this.props.onConnectContribute()
  }
  ...
  finishDisconnect = () => {
    chrome.contribute.revokeToken(() => {
      this.props.onDisconnectContribute()
  ...
  processConvert = () => {
    ...
    chrome.contribute.confirmConvert(currentConvertId, (success: boolean, message: string) => {
      if (success) {
        this.props.onUpdateActions()
  ...
  toggleTLD = () => {
    ...
    this.props.onContributeUserTLD(newTLD)
  ...
  shouldShowConvertPreview = () => {
    ...
    chrome.contribute.getConvertQuote(currentConvertFrom, currentConvertTo, currentConvertAmount, (quote: any) => {
  ...
  renderAuthInvalid = () => {
    ...
    return (
      <InvalidWrapper>
        <InvalidTitle>
          {getLocale('contributeWidgetAuthInvalid')}
        </InvalidTitle>
        <InvalidCopy>
          {getLocale('contributeWidgetAuthInvalidCopy')}
        </InvalidCopy>
        <GenButton onClick={onDismissAuthInvalid}>
          {getLocale('contributeWidgetDone')}
        </GenButton>
      </InvalidWrapper>
    )
  }
  ...
  renderDisconnectView = () => {
    return (
      <DisconnectWrapper>
        <DisconnectTitle>
          {getLocale('contributeWidgetDisconnectTitle')}
        </DisconnectTitle>
        <DisconnectCopy>
          {getLocale('contributeWidgetDisconnectText')}
        </DisconnectCopy>
        <DisconnectButton onClick={this.finishDisconnect}>
          {getLocale('contributeWidgetDisconnectButton')}
        </DisconnectButton>
        <DismissAction onClick={this.cancelDisconnect}>
          {getLocale('contributeWidgetCancelText')}
        </DismissAction>
      </DisconnectWrapper>
    )
  }
  ...
  renderConvertSuccess = () => {
    ...
    return (
      <InvalidWrapper>
        ...
        <InvalidTitle>
          {`${getLocale('contributeWidgetConverted')} ${currentConvertAmount} ${currentConvertFrom} to ${currentConvertTransAmount} ${currentConvertTo}!`}
        </InvalidTitle>
        <ConnectButton isSmall={true} onClick={this.finishConvert}>
          {getLocale('contributeWidgetContinue')}
        </ConnectButton>
      </InvalidWrapper>
    )
  }
  ...
  renderInsufficientFundsView = () => {
    return (
      <InvalidWrapper>
        <InvalidTitle>
          {getLocale('contributeWidgetUnableToConvert')}
        </InvalidTitle>
        <InvalidCopy>
          {getLocale('contributeWidgetInsufficientFunds')}
        </InvalidCopy>
        <GenButton onClick={this.retryConvert}>
          {getLocale('contributeWidgetRetry')}
        </GenButton>
      </InvalidWrapper>
    )
  }
  ...
  renderUnableToConvertView = () => {
    ...
    const errorMessage = convertError || getLocale('contributeWidgetConversionFailed')
      ...
      <>
        ...
        <DetailArea>
          <MemoArea>
            <MemoInfo>
              <DetailLabel>
                {`${currentDepositAsset} ${getLocale('contributeWidgetDepositAddress')}`}
              </DetailLabel>
              <DetailInfo>
                {
                  address
                  ? address
                  : getLocale('contributeWidgetAddressUnavailable')
                }
              </DetailInfo>
            </MemoInfo>
            {
              ...
              ? <CopyButton onClick={this.copyToClipboard.bind(this, address)}>
                  {getLocale('contributeWidgetCopy')}
                </CopyButton>
              : null
            }
          </MemoArea>
    ...
    return (
      <>
        <ListItem>
          ...
          <SearchInput
            type={'text'}
            placeholder={getLocale('contributeWidgetSearch')}
          ...
          <ListInfo position={'left'}>
            <TradeLabel>
              <Balance isBTC={true} hideBalance={hideBalance}>
                {this.formatCryptoBalance(accountBTCValue)} <TickerLabel>{getLocale('contributeWidgetBTCTickerText')}</TickerLabel>
              </Balance>
  ...
  renderConvertConfirm = () => {
    ...
    return (
      <InvalidWrapper>
        <InvalidTitle>
          {getLocale('contributeWidgetConfirmConversion')}
        </InvalidTitle>
        <ConvertInfoWrapper>
          <ConvertInfoItem>
            <ConvertLabel>{getLocale('contributeWidgetConvert')}</ConvertLabel>
            <ConvertValue>{`${displayConvertAmount} ${currentConvertFrom}`}</ConvertValue>
          </ConvertInfoItem>
          <ConvertInfoItem>
            <ConvertLabel>{getLocale('contributeWidgetFee')}</ConvertLabel>
            <ConvertValue>{`${displayConvertFee} ${currentConvertFrom}`}</ConvertValue>
          </ConvertInfoItem>
          <ConvertInfoItem isLast={true}>
            <ConvertLabel>{getLocale('contributeWidgetWillReceive')}</ConvertLabel>
            <ConvertValue>{`${displayReceiveAmount} ${currentConvertTo}`}</ConvertValue>
          </ConvertInfoItem>
        </ConvertInfoWrapper>
        <ActionsWrapper>
          <ConnectButton isSmall={true} onClick={this.processConvert}>
            {`${getLocale('contributeWidgetConfirm')} (${currentConvertExpiryTime}s)`}
          </ConnectButton>
          <DismissAction onClick={this.cancelConvert}>
            {getLocale('contributeWidgetCancel')}
          </DismissAction>
        </ActionsWrapper>
      </InvalidWrapper>
    )
  ...
  renderConvertView = () => {
    ...
    return (
      <>
        <Copy>
          {getLocale('contributeWidgetConvert')}
        </Copy>
        <AvailableLabel>
          {`${getLocale('contributeWidgetAvailable')} ${convertFromAmount} ${currentConvertFrom}`}
        </AvailableLabel>
        <BuyPromptWrapper>
          <FiatInputWrapper>
            <FiatDropdown
        ...
        <ActionsWrapper>
          <ConvertButton onClick={this.shouldShowConvertPreview}>
            {getLocale('contributeWidgetPreviewConvert')}
          </ConvertButton>
    ...
    return (
      <>
        <NavigationBar>
          ...
          >
            {getLocale('contributeWidgetSummary')}
          </NavigationItem>
          <NavigationItem
            isLeading={true}
          ...
          >
            {getLocale('contributeWidgetDepositLabel')}
          </NavigationItem>
          <NavigationItem
...
export const ContributeWidget = createWidget(Contribute)
