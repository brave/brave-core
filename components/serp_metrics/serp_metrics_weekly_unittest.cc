...
// searches are within the `TimePeriodStorage` window. The clock is advanced to
// the next Monday 00:00:00 UTC so that each advance lands on a precise ISO week
// boundary. Weekly usage pings are simulated at specific times to verify which
// metrics are included or excluded based on `kLastWeeklyReportedAt`. Usage
// pings report metrics from last week as well as from the stale period.
class SerpMetricsWeeklyTest : public SerpMetricsTestBase {
 public:
  ...
  // Advances the clock to one millisecond shy of the next Monday 00:00:00 UTC.
  ...
  // Simulates sending a weekly usage ping by recording `at` as the last weekly
  // reported time. `GetSearchCountForStaleWeekPeriod` uses
  // `kLastWeeklyReportedAt` to determine which searches are already reported.
  void SimulateSendingWeeklyUsagePingAt(base::Time at) {
    local_state_.SetTime(prefs::kLastWeeklyReportedAt, at);
  }
};
...
TEST_F(SerpMetricsWeeklyTest, BraveSearchCountForStalePeriod) {
  // Week 0: Stale
  serp_metrics_->RecordSearch(SerpMetricType::kBrave);
  serp_metrics_->RecordSearch(SerpMetricType::kBrave);
  AdvanceClockToNextMonday();
...
TEST_F(SerpMetricsWeeklyTest, GoogleSearchCountForStalePeriod) {
  // Week 0: Stale
  serp_metrics_->RecordSearch(SerpMetricType::kGoogle);
  serp_metrics_->RecordSearch(SerpMetricType::kGoogle);
  AdvanceClockToNextMonday();
...
TEST_F(SerpMetricsWeeklyTest, OtherSearchCountForStalePeriod) {
  // Week 0: Stale
  serp_metrics_->RecordSearch(SerpMetricType::kOther);
  serp_metrics_->RecordSearch(SerpMetricType::kOther);
  AdvanceClockToNextMonday();
...
TEST_F(SerpMetricsWeeklyTest, SearchCountForStalePeriodAcrossMultipleWeeks) {
  // Week 0: Stale (kBrave + kGoogle = 2)
  serp_metrics_->RecordSearch(SerpMetricType::kBrave);
  serp_metrics_->RecordSearch(SerpMetricType::kGoogle);
  AdvanceClockToNextMonday();

  // Week 1: Stale (kGoogle + kOther + kOther = 3)
  serp_metrics_->RecordSearch(SerpMetricType::kGoogle);
  serp_metrics_->RecordSearch(SerpMetricType::kOther);
  serp_metrics_->RecordSearch(SerpMetricType::kOther);
  AdvanceClockToNextMonday();

  // Week 2: Stale (kOther = 1)
  serp_metrics_->RecordSearch(SerpMetricType::kOther);
  AdvanceClockToNextMonday();
  ...
  // Week 4: This week
  serp_metrics_->RecordSearch(SerpMetricType::kBrave);
  EXPECT_EQ(6U, serp_metrics_->GetSearchCountForStaleWeekPeriod());
}

TEST_F(SerpMetricsWeeklyTest, SearchCountsForLastWeekAndStalePeriod) {
  // Week 0: Stale
  serp_metrics_->RecordSearch(SerpMetricType::kBrave);
  serp_metrics_->RecordSearch(SerpMetricType::kGoogle);
  serp_metrics_->RecordSearch(SerpMetricType::kGoogle);
...
TEST_F(SerpMetricsWeeklyTest, SearchCountForStalePeriodOnCuspOfWeekRollover) {
  // Week 0: Stale
  serp_metrics_->RecordSearch(SerpMetricType::kBrave);
  serp_metrics_->RecordSearch(SerpMetricType::kGoogle);
  serp_metrics_->RecordSearch(SerpMetricType::kOther);
...
TEST_F(SerpMetricsWeeklyTest, CountAllSearchesIfWeeklyUsagePingWasNeverSent) {
  ...
  // `kLastWeeklyReportedAt` defaults to null — simulating a never-sent weekly
  // ping.
  EXPECT_EQ(1U,
            serp_metrics_->GetSearchCountForLastWeek(SerpMetricType::kBrave));
  EXPECT_EQ(1U,
...
TEST_F(SerpMetricsWeeklyTest,
       SearchRecordedJustBeforeWeekRolloverCountsAsLastWeek) {
  AdvanceClockToJustBeforeNextWeek();  // Sunday 23:59:59.999 UTC
  serp_metrics_->RecordSearch(SerpMetricType::kBrave);
  task_environment_.AdvanceClock(base::Milliseconds(1));  // Monday 00:00:00 UTC
  EXPECT_EQ(1U,
            serp_metrics_->GetSearchCountForLastWeek(SerpMetricType::kBrave));
}

TEST_F(SerpMetricsWeeklyTest,
       SearchRecordedAtExactWeekRolloverCountsAsThisWeek) {
  AdvanceClockToJustBeforeNextWeek();
  task_environment_.AdvanceClock(base::Milliseconds(1));  // Monday 00:00:00 UTC
  serp_metrics_->RecordSearch(SerpMetricType::kBrave);
  EXPECT_EQ(0U,
...
TEST_F(SerpMetricsWeeklyTest, SearchAtWeekRolloverEndsUpInLastWeekNotStale) {
  SimulateSendingWeeklyUsagePingAt(base::Time::Now());
  AdvanceClockToJustBeforeNextWeek();
  task_environment_.AdvanceClock(base::Milliseconds(1));  // Monday 00:00:00 UTC
  serp_metrics_->RecordSearch(SerpMetricType::kBrave);
...
TEST_F(SerpMetricsWeeklyTest,
       SearchJustBeforeWeekRolloverEndsUpInStaleNotLastWeek) {
  AdvanceClockToJustBeforeNextWeek();
  serp_metrics_->RecordSearch(SerpMetricType::kBrave);  // Sunday 23:59:59.999
...
TEST_F(SerpMetricsWeeklyTest,
       LastCheckedAtOnLastWeekStartYieldsZeroStalePeriod) {
  // Week 0: Stale
  serp_metrics_->RecordSearch(SerpMetricType::kBrave);
  AdvanceClockToNextMonday();
  ...
  // Week 2: This week — stale range is empty because ping covered last week.
  EXPECT_EQ(0U, serp_metrics_->GetSearchCountForStaleWeekPeriod());
}

TEST_F(SerpMetricsWeeklyTest,
       SearchRecordedInLastWeekOfYearCountsAsLastWeekInNewYear) {
  base::Time time;
  ASSERT_TRUE(base::Time::FromUTCString("28 Dec 2049 12:00:00", &time));
