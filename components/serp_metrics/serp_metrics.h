...
class SerpMetrics final {
 public:
  ...
  // Returns searches from the previous complete ISO week (Monday 00:00:00 to
  // Sunday 23:59:59 UTC) for the given engine type.
  size_t GetSearchCountForLastWeek(SerpMetricType type) const;

  // Returns searches from the previous complete calendar month (first day
  // 00:00:00 to last day 23:59:59 UTC) for the given engine type.
 ...
 private:
  // Returns the start of the daily stale period. Reads `kLastDailyReportedAt`
