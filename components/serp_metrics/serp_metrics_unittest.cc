...
class TestingTimePeriodStore : public TimePeriodStore {
 public:
  TestingTimePeriodStore() = default;

  TestingTimePeriodStore(const TestingTimePeriodStore&) = delete;
  TestingTimePeriodStore& operator=(const TestingTimePeriodStore&) = delete;

  ~TestingTimePeriodStore() override = default;

  void Save(base::ListValue data) override { data_ = std::move(data); }

  const base::ListValue* Get() const override { return &data_; }

  void Clear() override { data_.clear(); }

 private:
  base::ListValue data_;
};

TimePeriodStorages BuildTestingTimePeriodStorages() {
  ...
  time_period_storages.emplace(SerpMetricType::kBrave,
                               std::make_unique<TimePeriodStorage>(
                                   std::make_unique<TestingTimePeriodStore>(),
 ...
 public:
  ...
  void SetUp() override {
    ...
    serp_metrics_ = std::make_unique<SerpMetrics>(
        &local_state_, BuildTestingTimePeriodStorages());
  }
