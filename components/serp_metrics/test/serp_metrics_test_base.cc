...
void SerpMetricsTestBase::SetUp() {
  ...
  local_state_.registry()->RegisterTimePref(prefs::kLastDailyReportedAt,
                                            /* Never reported */ base::Time());
  local_state_.registry()->RegisterTimePref(prefs::kLastWeeklyReportedAt,
                                            /* Never reported */ base::Time());
  local_state_.registry()->RegisterTimePref(prefs::kLastMonthlyReportedAt,
                                            /* Never reported */ base::Time());

  // 93 days covers three back-to-back 31-day months, ensuring month-boundary
  // metrics are not dropped before the monthly ping fires.
