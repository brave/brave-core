...
import {File, TorrentsState, TorrentState} from '../../constants/webtorrentState'
...
const focusedWindowChanged =
    (windowId: number, state: TorrentsState) => {
      return {
...
const windowRemoved =
    (windowId: number, state: TorrentsState) => {
      const {activeTabIds} = state delete activeTabIds[windowId] return {
        ...state, activeTabIds
      }
    }

const activeTabChanged =
    (tabId: number, windowId: number, state: TorrentsState) => {
...
const windowCreated =
    (window: chrome.windows.Window, state: TorrentsState) => {
...
const tabUpdated =
    (tabId: number, url: string, state: TorrentsState) => {
      const {torrentStateMap, torrentObjMap} = state
      const origTorrentState: TorrentState =
          torrentStateMap[tabId] const origInfoHash =
              origTorrentState ? origTorrentState.infoHash : undefined
      ...
      if (parsedURL.protocol === 'magnet:') {  // parse torrent
        ...
        const name =
            parsedURL.pathname.substr(parsedURL.pathname.lastIndexOf('/') + 1)
        // for .torrent case, ix (index of file) for selecting a specific file
        // in the file list is given in url like #ix=5
        ...
        const key = Object.keys(torrentStateMap)
                        .find(
      ...
      // save new torrent state and subscribe directly if torrent already
      // existed
      if (newTorrentState) {
      ...
      return {
        ...state, torrentStateMap, torrentObjMap
      }
    }

const tabRemoved =
    (tabId: number, state: TorrentsState) => {
      const {torrentStateMap, torrentObjMap} = state
      const torrentState = torrentStateMap[tabId] if (torrentState) {
...
const startTorrent =
    (torrentId: string, tabId: number, state: TorrentsState) => {
      ...
      if (torrentState && torrentState.infoHash &&
        ...
        addTorrent(
            torrentId)  // objectMap will be updated when info event is emitted
      } else if (
          torrentState && torrentState.infoHash &&
...
const updateProgress =
    (state: TorrentsState, torrent: Torrent) => {
...
const updateServer =
    (state: TorrentsState, torrent: Torrent, serverURL: string) => {
      const {torrentObjMap} = state
      torrentObjMap[torrent.infoHash] = {
        ...torrentObjMap[torrent.infoHash],
        serverURL
    ...
    (torrentId: string,
     ...
     state: TorrentsState) => {
      ...
      if (infoHash && !findTorrent(infoHash) && parsedTorrent) {
        addTorrent(parsedTorrent)  // objectMap will be updated when info event
                                   // is emitted
      }
      ...
      return {
        ...state, torrentObjMap, torrentStateMap
      }
    }

const defaultState: TorrentsState = {
  currentWindowId: -1,
  activeTabIds: {},
  torrentStateMap: {},
  torrentObjMap: {}
    ...
    (state: TorrentsState = defaultState,
     action: any) => {  // TODO: modify any to be actual action type
      const payload = action.payload
      switch (action.type) {
        case windowTypes.types.WINDOW_CREATED:
          state = windowCreated(payload.window, state)
          break case windowTypes.types.WINDOW_REMOVED: state =
              windowRemoved(payload.windowId, state)
          break case windowTypes.types.WINDOW_FOCUS_CHANGED: state =
              focusedWindowChanged(payload.windowId, state)
          ...
          if (!state.activeTabIds[state.currentWindowId] &&
              payload.tab.active) {
            state =
                activeTabChanged(payload.tab.id, payload.tab.windowId, state)
          }

          if (payload.changeInfo.url) {
            state = tabUpdated(payload.tab.id, payload.changeInfo.url, state)
          }
          break case tabTypes.types.TAB_REMOVED: state =
              tabRemoved(payload.tabId, state)
          break case tabTypes.types.TAB_RETRIEVED:
              if (payload.tab.id && payload.tab.url) {
            state = tabUpdated(payload.tab.id, payload.tab.url, state)
          }
          break case torrentTypes.types.WEBTORRENT_PROGRESS_UPDATED: state =
              updateProgress(state, payload.torrent)
          break case torrentTypes.types.WEBTORRENT_INFO_UPDATED: state =
              updateInfo(state, payload.torrent)
          break case torrentTypes.types.WEBTORRENT_SERVER_UPDATED: state =
              updateServer(state, payload.torrent, payload.serverURL)
          break case torrentTypes.types.WEBTORRENT_START_TORRENT: state =
              startTorrent(payload.torrentId, payload.tabId, state)
          break case torrentTypes.types.WEBTORRENT_STOP_DOWNLOAD: state =
              stopDownload(payload.tabId, state)
          break case torrentTypes.types.WEBTORRENT_TORRENT_PARSED: state =
              torrentParsed(
                  payload.torrentId,
                  payload.tabId,
                  payload.infoHash,
