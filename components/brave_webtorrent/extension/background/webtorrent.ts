...
export const createServer =
    (torrent: WebTorrent.Torrent, cb: (serverURL: string) => void) => {
      if (!torrent.infoHash)
        return  // torrent is not ready

            const opts = {
              // Only allow requests from this origin ('chrome-extension://...)
              // so
              // websites cannot violate same-origin policy by reading contents
              // of
              // active torrents.
...
const maybeDestroyWebTorrent =
    () => {
      if (!webTorrent || webTorrent.torrents.length !== 0)
        return webTorrent.destroy()
...
export const delTorrent = (infoHash: string) => {
  ...
  if (servers[infoHash]) {
    servers[infoHash].close() delete servers[infoHash]
  }
