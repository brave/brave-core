...
export const addTorrentEvents = (torrent: Torrent) => {
  torrent.on('done', () => {webtorrentActions.progressUpdated(torrent)})
  torrent.on('infoHash', () => {
  ...
  torrent.on('metadata', () => {
    ...
    torrent.on(
        'download',
        throttle(
            (bytes: number) => {webtorrentActions.progressUpdated(torrent)},
            1000))
    torrent.on(
        'upload',
        throttle(
  ...
  torrent.on('ready', () => {
    ...
    torrent.on(
        'warning', (e: Error|string) => {console.log('warning: ', torrent, e)})
    torrent.on(
        'error', (e: Error|string) => {console.log('error: ', torrent, e)})
}
