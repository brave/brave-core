...
void PageContentExtractor::SetContentToolsListener(
  ...
  if (listener) {
    ...
    // Treat a dropped subscription the same as an explicit unsubscribe.
...
void PageContentExtractor::DidCreateNewDocument() {
  // The ModelContext is per-document, so the listener must be re-attached.
  UpdateToolChangeListener();
}
...
void PageContentExtractor::UpdateToolChangeListener() {
  ...
  // Tools registered before the subscription was established have already
  // fired their `toolchange` event, so notify once on (re)attachment.
