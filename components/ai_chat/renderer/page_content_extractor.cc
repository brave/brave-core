  ...
  if (origin.is_valid()) {
    ...
    if (kYouTubeHosts.contains(host)) {
      ...
      auto script_callback = base::BindOnce(
          &PageContentExtractor::OnJSInnerTubeConfigResult,
...
void PageContentExtractor::OnJSInnerTubeConfigResult(
  ...
  result->content = mojom::PageContentData::NewInnerTubeConfig(
      mojom::InnerTubeConfig::New(*api_key, *video_id));
  std::move(callback).Run(std::move(result));
}
