...
void DistillPageText(
  ...
  LOG(ERROR) << "HELLO " << __FILE__;
  LOG(ERROR) << __func__;
  // ui::AXTreeID tree_id = render_frame->GetWebFrame()->GetAXTreeID();
  ...
  LOG(ERROR) << __func__
             << ": Snapshotted accessibility tree and unserialized.";
  ...
  if (contents_text.empty()) {
    LOG(ERROR) << __func__ << " Contents is empty\n";
