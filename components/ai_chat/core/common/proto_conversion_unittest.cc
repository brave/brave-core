...
TEST(ProtoConversionTest, SerializeToolUseEvent_InvalidId) {
  ...
  auto mojom_event =
      mojom::ToolUseEvent::New("test_tool", "", "{}", std::nullopt, false);
  bool success = SerializeToolUseEvent(mojom_event, &proto_event);
...
TEST(ProtoConversionTest, SerializeToolUseEvent_InvalidToolName) {
  ...
  auto mojom_event =
      mojom::ToolUseEvent::New("", "tool_id_123", "{}", std::nullopt, false);
  bool success = SerializeToolUseEvent(mojom_event, &proto_event);
...
TEST(ProtoConversionTest, SerializeDeserializeSmartModeEntry) {
  // Create mojom SmartModeEntry
  auto mojom_entry =
      mojom::SmartModeEntry::New("summarize", "Please summarize this content");

  // Serialize to proto
  store::SmartModeEntryProto proto_entry;
  SerializeSmartModeEntry(mojom_entry, &proto_entry);
  ...
  // Deserialize back to mojom
  auto deserialized_entry = DeserializeSmartModeEntry(proto_entry);
