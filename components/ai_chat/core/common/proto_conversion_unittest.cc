...
TEST(ProtoConversionTest, SerializeDeserializeToolUseEvent_ValidData) {
  const std::string kArgumentsJson = R"({
        "param1": "value1",
        "param2": 42,
        "param3": true,
        "nested": {
  ...
  auto mojom_event = mojom::ToolUseEvent::New(
      "test_tool", "tool_id_123", kArgumentsJson, base::Value::Dict(),
      std::vector<mojom::ContentBlockPtr>());

  // Add different parsed arguments to verify that they are not the source
  // or truth - argments_json should be.
  mojom_event->arguments->Set("param4", "value4");
  ...
  EXPECT_EQ(proto_event.arguments_json(), kArgumentsJson);
  ...
  // Verify deserialized data
  // Arguments should be parsed correctly from arguments_json when deserialized.
  mojom_event->arguments = base::test::ParseJsonDict(kArgumentsJson);
...
TEST(ProtoConversionTest, SerializeDeserializeToolUseEvent_NoOutput) {
  ...
  auto mojom_event = mojom::ToolUseEvent::New(
      "test_tool", "tool_id_123", "{}", base::Value::Dict(), std::nullopt);
...
TEST(ProtoConversionTest, SerializeDeserializeToolUseEvent_EmptyArguments) {
  // Create mojom ToolUseEvent with empty arguments
  ...
  EXPECT_TRUE(success);
  EXPECT_EQ(proto_event.arguments_json(), "{}");

  // Deserialize back to mojom
  auto deserialized_event = DeserializeToolUseEvent(proto_event);

  EXPECT_EQ(deserialized_event->tool_name, "test_tool");
  EXPECT_EQ(deserialized_event->id, "tool_id_123");
  EXPECT_TRUE(deserialized_event->arguments.has_value());
  EXPECT_TRUE(deserialized_event->arguments->empty());
}
