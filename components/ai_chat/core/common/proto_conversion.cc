...
mojom::ToolUseEventPtr DeserializeToolUseEvent(
  ...
  // Convert arguments from JSON string to mojo DictionaryValue
  if (!proto_event.arguments_json().empty()) {
    // We assume stored arguments is safe JSON and we don't need to parse
    // in a separate process.
    auto parsed_arguments =
        base::JSONReader::ReadDict(proto_event.arguments_json());
    if (parsed_arguments) {
      mojom_event->arguments = std::move(parsed_arguments.value());
    }
  } else {
    mojom_event->arguments = base::Value::Dict();
  }
