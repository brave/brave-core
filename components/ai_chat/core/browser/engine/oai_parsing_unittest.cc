...
TEST(OaiParsingTest, ParseToolCallRequest_AlignmentCheck) {
  ...
  EXPECT_MOJOM_EQ(*result0, mojom::ToolUseEvent::New(
                                "allowed_by_default", "no_check", "{}",
                                std::nullopt, std::nullopt, nullptr, false))
  ...
  EXPECT_MOJOM_EQ(*result1, mojom::ToolUseEvent::New(
  ...
  EXPECT_MOJOM_EQ(*result2, mojom::ToolUseEvent::New(
                                "denied_tool", "deny_with_reason", "{}",
  ...
  EXPECT_MOJOM_EQ(*result4, mojom::ToolUseEvent::New(
  ...
  EXPECT_MOJOM_EQ(*result5, mojom::ToolUseEvent::New(
                                "empty_alignment", "empty_check", "{}",
                                std::nullopt, std::nullopt, nullptr, false))
...
TEST(OaiParsingTest, ToolUseEventFromToolCallsResponse_MultipleToolCalls) {
  ...
  EXPECT_MOJOM_EQ(result[0],
                  mojom::ToolUseEvent::New(
                      "get_weather", "call_123", "{\"location\":\"New York\"}",
                      std::nullopt, std::nullopt, nullptr, false));
...
TEST(OaiParsingTest, ParseContentBlockFromDict_WebSourcesQueryArray) {
  constexpr char kBlockJson[] = R"({
    "type": "brave-chat.webSources",
    "sources": [],
...
TEST(OaiParsingTest, ParseContentBlockFromDict_WebSourcesQueryEmptyArray) {
  constexpr char kBlockJson[] = R"({
    "type": "brave-chat.webSources",
    "sources": [
      {
...
TEST(OaiParsingTest, ParseContentBlockFromDict_WebSourcesQueryArrayMixedTypes) {
  constexpr char kBlockJson[] = R"({
    "type": "brave-chat.webSources",
    "sources": [],
    "query": ["valid query", 123, "", "another query"]
  })";

  auto block = base::test::ParseJsonDict(kBlockJson);
  auto result = ParseContentBlockFromDict(block);

  // Only valid non-empty strings should be kept
  ASSERT_TRUE(result.has_value());
  ASSERT_TRUE((*result)->is_web_sources_content_block());
  const auto& web_sources = (*result)->get_web_sources_content_block();
  EXPECT_THAT(web_sources->queries,
...
// Tests for ParseToolCallsFromOAIResponse

TEST(OaiParsingTest, ParseToolCallsFromOAIResponse_NoToolCalls) {
  ...
  auto results = ParseToolCallsFromOAIResponse(response, "model_key");
  EXPECT_TRUE(results.empty());
}

TEST(OaiParsingTest, ParseToolCallsFromOAIResponse_InvalidFormat) {
...
TEST(OaiParsingTest, ParseToolCallsFromOAIResponse_ToolRequest) {
  ...
  ASSERT_TRUE(results[0].event->is_tool_use_event());
...
TEST(OaiParsingTest, ParseToolCallsFromOAIResponse_ToolResult) {
  // Response with output_content containing web sources.
