...
class GenerateRewriteParameterizedTest
    : public EngineConsumerOAIUnitTest,
      public testing::WithParamInterface<GenerateRewriteTestParam> {};

TEST_P(GenerateRewriteParameterizedTest, GenerateRewriteSuggestion) {
  ...
  std::string expected_messages = absl::StrFormat(
      ...
      expected_excerpt_text, expected_text);
  ...
  EXPECT_CALL(*client, PerformRequestWithOAIMessages(_, _, _, _, _))
      .WillOnce(
          [&](const mojom::CustomModelOptions& options,
            ...
            // serialized messages.
            std::vector<OAIMessage> first_message_only;
            first_message_only.push_back(std::move(messages[0]));
            EXPECT_EQ(client->GetMessagesJson(std::move(first_message_only)),
...
INSTANTIATE_TEST_SUITE_P(
    AllRewriteTypes,
    GenerateRewriteParameterizedTest,
    testing::Values(
