  ...
  if (*type == "completion") {
    ...
    uint64_t total_tokens =
        total_tokens_opt.has_value() && total_tokens_opt.value() >= 0
            ...
            : 0;
    // Get tokens_trimmed as uint64_t
    std::optional<int> tokens_trimmed_opt =
        response_event.FindInt("tokens_trimmed");
    uint64_t tokens_trimmed =
        tokens_trimmed_opt.has_value() && tokens_trimmed_opt.value() >= 0
            ? static_cast<uint64_t>(tokens_trimmed_opt.value())
            : 0;
    return mojom::ConversationEntryEvent::NewContentReceiptEvent(
        mojom::ContentReceiptEvent::New(total_tokens, tokens_trimmed));
  }
