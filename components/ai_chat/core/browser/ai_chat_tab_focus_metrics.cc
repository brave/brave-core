...
AIChatTabFocusMetrics::AIChatTabFocusMetrics(PrefService* local_state,
    ...
    : local_state_(local_state),
      ...
      delegate_(delegate),
      total_tab_count_storage_(local_state,
                               prefs::kTabFocusP3ATotalTabCountPrefName),
      max_tab_count_storage_(local_state,
                             prefs::kTabFocusP3AMaxTabCountPrefName),
      session_count_storage_(local_state,
                             prefs::kTabFocusP3ASessionCountPrefName) {
...
void AIChatTabFocusMetrics::RegisterPrefs(PrefRegistrySimple* registry) {
  registry->RegisterListPref(prefs::kTabFocusP3ATotalTabCountPrefName);
  registry->RegisterListPref(prefs::kTabFocusP3AMaxTabCountPrefName);
  registry->RegisterListPref(prefs::kTabFocusP3ASessionCountPrefName);
  registry->RegisterTimePref(prefs::kTabFocusP3ALastUsageTimePrefName, {});
}
...
void AIChatTabFocusMetrics::RecordUsage(size_t tab_count) {
  ...
  p3a_utils::RecordFeatureUsage(local_state_, nullptr,
                                prefs::kTabFocusP3ALastUsageTimePrefName);
  ReportCountMetrics();
  ReportLastUsageTime();
}
...
void AIChatTabFocusMetrics::RecordEnabled() {
  ...
  if (profile_prefs_->GetBoolean(prefs::kBraveAIChatTabOrganizationEnabled)) {
    status = delegate_->is_premium() ? EnabledStatus::kEnabledPremium
...
void AIChatTabFocusMetrics::ReportLastUsageTime() {
  p3a_utils::RecordFeatureLastUsageTimeMetric(
      local_state_, prefs::kTabFocusP3ALastUsageTimePrefName,
