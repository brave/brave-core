...
std::unique_ptr<EngineConsumer> ModelService::GetEngineForModel(
  ...
  if (!model) {
    ...
    // refresh, unlike before remote models existed. The automatic model is
    // the one entry OnRemoteModelsReady() guarantees is never dropped, so
    // it's the last resort before giving up entirely.
