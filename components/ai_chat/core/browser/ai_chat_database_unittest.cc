...
TEST_P(AIChatDatabaseTest, InlineSearchEvent) {
  const std::string uuid = "first";
  mojom::ConversationPtr metadata = mojom::Conversation::New(
      uuid, "title", base::Time::Now() - base::Hours(2), true, std::nullopt, 0,
      0, false, std::vector<mojom::AssociatedContentPtr>());
...
TEST_P(AIChatDatabaseTest, InlineSearchEvent_Invalid) {
  const std::string uuid = "first";
  mojom::ConversationPtr metadata = mojom::Conversation::New(
      uuid, "title", base::Time::Now() - base::Hours(2), true, std::nullopt, 0,
      0, false, std::vector<mojom::AssociatedContentPtr>());
  ...
  // Remove the invalid event from expected history
  history[1]->events->pop_back();
...
TEST_F(AIChatDatabaseSyncTest, ApplyRemoteEntryReplacesChildRows) {
  // The query entry carries uploaded files and the response entry carries
  // completion and search-query events. Re-applying a trimmed revision of
  // each must replace those child rows rather than accumulate them.
  auto history =
      CreateSampleChatHistory(1u, 0, /*num_uploaded_files_per_query=*/2u);
  ASSERT_EQ(history.size(), 2u);
  ASSERT_EQ(history[0]->uploaded_files->size(), 2u);
  ASSERT_EQ(history[1]->events->size(), 3u);
