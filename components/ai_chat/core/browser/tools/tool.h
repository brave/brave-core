...
class Tool {
 public:
  ...
  // format, e.g. `{ "location": { "type": "string", "description": "location for weather" } }`
  // TODO(petemill): Use base::Value::Dict to avoid JSON parsing in api
  // clients. Provide static helpers to build the schema, e.g. StringProperty(description, optional_values),
  // ArrayProperty(description, items), ObjectProperty(description, properties).
  ...
  // Parameters for remote-defined tools that this client provides, e.g. screen width, location, etc.
