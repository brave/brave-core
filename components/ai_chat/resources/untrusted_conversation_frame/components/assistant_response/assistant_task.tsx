...
const importantToolNames = [
  Mojom.NAVIGATE_TOOL_NAME,
  Mojom.TODO_TOOL_NAME,
]

const skipStepsToolNames = [
  Mojom.TODO_TOOL_NAME,
]
...
export default function AssistantTask(props: Props) {
  ...
  React.useEffect(() => {
    ...
    const id = API.getInstance().uiObserver.thumbnailUpdated.addListener((tabId: number, dataURI: string) => {
      console.error(dataURI);
      if (tabId === conversationContext.contentTaskTabId && props.isActiveTask) {
  ...
  return (
    <div className={styles.task}>
      <Tabs
        ...
        size='medium'
      >
        <TabItem value='progress'>{getLocale(S.AI_CHAT_TASK_PROGRESS_LABEL)}</TabItem>
        ...
        <div className={styles.taskData}>
          {showSteps && (
            props.assistantEntries.map((entry, index) => {
              ...
              const toolUseEvents = lastEntryEdit.events?.filter(event => !!event && event.toolUseEvent && !skipStepsToolNames.includes(event.toolUseEvent?.toolName ?? '')) ?? []
              const isLastEntry = index === props.assistantEntries.length - 1
              return (
                <AssistantResponse
                  key={entry.uuid || index}
                  events={toolUseEvents}
                  isEntryInteractivityAllowed={props.isActiveTask && isLastEntry}
...
function Progress(props: Props) {
  ...
  const lastCompletionEvent = props.assistantEntries.flatMap(entry => entry.events?.filter(event => !!event && event.completionEvent) ?? []).at(-1)
  // Show important tool use events that require interactivity or for info
  const lastEntryToolUseEvents = props.assistantEntries.at(-1)?.events?.filter(event => !!event && !!event.toolUseEvent) ?? []
  // Interactive tools only from most recent entry
  const interactiveToolUseEvents = lastEntryToolUseEvents.filter(event => interactiveToolNames.includes(event.toolUseEvent?.toolName ?? ''))
  // Most recent (up to 1) important tool use events from any entry
  const importantToolUseEvents = props.assistantEntries.flatMap(entry => entry.events?.filter(event => !!event && !!event.toolUseEvent && importantToolNames.includes(event.toolUseEvent?.toolName ?? '')) ?? []).slice(-1)
