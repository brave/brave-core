...
function getBrowserTheme() {
  ...
  const nala = Array.from(document.styleSheets)
    .find(s => s.href?.includes('nala.css'))!

  const baseColors = Array.from(nala.cssRules || [])
    .find(rule => rule instanceof CSSImportRule && rule.href?.includes('theme/colors.css')) as CSSImportRule
...
export default function RichSearchWidget(props: { jsonData: string }) {
  ...
  const frameHandler = React.useCallback(
    (iframe: HTMLIFrameElement | null) => {
      if (iframe) {
        const sendContent = () => {
          iframe.contentWindow?.postMessage({ type: 'theme', styles: getBrowserTheme() }, RICH_SEARCH_WIDGETS_ORIGIN)
