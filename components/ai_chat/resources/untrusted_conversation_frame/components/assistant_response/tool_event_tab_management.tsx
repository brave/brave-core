...
const ToolEventTabManagement: ToolComponent = (props) => {
  ...
  const needsUserPermission = props.toolUseEvent.requiresUserInteraction && props.isEntryActive

  if (needsUserPermission) {
    content.progressIcon = (
      <span data-testid='tool-tabmanagement-permission-icon'>
    ...
    content.toolText = (
      <div>
        {getLocale(S.AI_CHAT_TOOL_TAB_MANAGEMENT_ACTIVE_STATE_DESCRIPTION)}
      </div>
    )
  } else {
    // TODO(petemill): Alternate UI state for user having previously denied permission.
    // We could check output (but that's ugly string-matching)? Or we could preserve the client data in the
    // tool use event?
    content.toolText = (
      <div>
        {getLocale(S.AI_CHAT_TOOL_TAB_MANAGEMENT_COMPLETED_STATE_DESCRIPTION)}
      </div>
    )
  }
  ...
  if (props.toolUseEvent.output) {
    content.tooltipContent = (
      <div>
        {props.toolUseEvent.output.map((output) => (
          <div>
