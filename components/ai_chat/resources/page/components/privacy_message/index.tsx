...
function PrivacyMessage () {
  ...
  const aboutDescription = formatMessage(getLocale(StringIds.AboutDescription), {
    tags: {
      $1: (content) => createLinkWithClickHandler(content, WIKI_URL)
    }
  })

  const aboutDescription3 = formatMessage(getLocale(StringIds.AboutDescription3), {
    tags: {
  ...
  return (
    ...
    >
      <div slot="subtitle">{getLocale(StringIds.PrivacyTitle)}</div>
      <div className={styles.content}>
        <p>{aboutDescription}</p>
        <p>{getLocale(StringIds.AboutDescription2)}</p>
        <p>{aboutDescription3}</p>
      </div>
      <div slot="actions">
        <Button ref={buttonRef} onClick={context.handleAgreeClick}>{getLocale(StringIds.AcceptButtonLabel)}</Button>
      </div>
    </Dialog>
  )
}
