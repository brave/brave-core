...
import { AssociatedContent, ContentType } from 'gen/brave/components/ai_chat/core/common/mojom/ai_chat.mojom.m.js'
...
const MockContext = (props: React.PropsWithChildren<Partial<AIChatContext & ConversationContext>>) => {
  ...
  return (
    <AIChatReactContext.Provider value={{
...
describe('Attachments Component', () => {
  ...
  it('renders the attachments dialog with correct title', () => {
    ...
    expect(screen.getByText('CHAT_UI_ATTACHMENTS_TABS_TITLE')).toBeInTheDocument()
  })
  ...
  it('displays tab URLs', () => {
    ...
    expect(screen.getByText('https://github.com/brave/brave-browser')).toBeInTheDocument()
  ...
  it('displays favicons for each tab', () => {
    ...
    expect(favicons[0]).toHaveAttribute('src', expect.stringContaining('google.com'))
    expect(favicons[1]).toHaveAttribute('src', expect.stringContaining('github.com'))
    expect(favicons[2]).toHaveAttribute('src', expect.stringContaining('stackoverflow.com'))
  })
  ...
  describe('Search functionality', () => {
    ...
    it('shows search icon in input', () => {
      ...
      const searchIcon = container.querySelector('leo-input leo-icon[slot="left-icon"]')
  ...
  describe('Empty states', () => {
    it('shows no results message when there are no unassociated tabs', () => {
      ...
      expect(screen.getByText('CHAT_UI_ATTACHMENTS_SEARCH_NO_RESULTS')).toBeInTheDocument()
      // Should not show suggestion text when there are no tabs at all
      expect(screen.queryByText('CHAT_UI_ATTACHMENTS_SEARCH_NO_RESULTS_SUGGESTION')).not.toBeInTheDocument()
    })
