...
describe('Editable component', () => {
  ...
  it('displays mixed content with associated content nodes', () => {
    ...
    const associatedSpan = editable.querySelector(
      'span[data-type="associated-content"]',
    )
  ...
  it('handles complex content with multiple associated content nodes', () => {
    ...
    expect(editable).toHaveTextContent(
      'Meeting between @Alice, @Bob and @Carol scheduled for tomorrow',
    )

    // Check that all associated content spans are rendered
    const associatedSpans = editable.querySelectorAll(
      'span[data-type="associated-content"]',
    )
        ...
        <div>
          ...
          <button
            onClick={() =>
              setContent([
                'Text with ',
                createAssociatedContentNode('user-1', 'mention'),
                ' added',
              ])
            }
  ...
  it('should be able to delete associated-content nodes with backspace', async () => {
    ...
    expect(editable.querySelector('span[data-type="associated-content"]')).toBe(
      null,
    )
