...
import { convertFileToUploadedFile, FileReadError, ImageProcessingError, ApiConnectionError } from './file_utils'
...
// Mock the Mojom module with all required services for PageAPI
jest.mock('../../common/mojom', () => ({
  // Core services
  Service: {
    getRemote: jest.fn()
  },
  ...
  // Legacy (unused but mocked for completeness)
  AIChatUIHandlerFactory: {
    getRemote: jest.fn()
  },
  ...
  // Required callback router classes for PageAPI constructor
  MetricsRemote: jest.fn().mockImplementation(() => ({})),
  ServiceObserverCallbackRouter: jest.fn().mockImplementation(() => ({})),
  ChatUICallbackRouter: jest.fn().mockImplementation(() => ({})),
  ParentUIFrameCallbackRouter: jest.fn().mockImplementation(() => ({})),
  TabDataObserverCallbackRouter: jest.fn().mockImplementation(() => ({})),

  // Tab tracking service
  TabTrackerService: {
    getRemote: jest.fn().mockReturnValue({
      addObserver: jest.fn()
    })
  }
}))
...
describe('convertFileToUploadedFile', () => {
  ...
  function setupMojomServiceMocks() {
    ...
    ;(Mojom.Service.getRemote as jest.Mock).mockReturnValue(mockService)
    ;(Mojom.AIChatUIHandler.getRemote as jest.Mock).mockReturnValue(mockHandler)
  }
  ...
  describe('successful file processing', () => {
    ...
    it('converts empty file successfully', async () => {
  ...
  describe('error handling', () => {
    ...
    it('throws ImageProcessingError when processImageFile returns null',
       async () => {
      const file = createMockFile('test.png', 'image/png')
      const mockArrayBuffer = new ArrayBuffer(8)
    ...
    it('throws ApiConnectionError when processImageFile rejects', async () => {
      const file = createMockFile('test.png', 'image/png')
      const mockArrayBuffer = new ArrayBuffer(8)

      mockHandler.processImageFile.mockRejectedValue(
        new Error('Backend processing failed')
      )
      ...
      await expect(convertFileToUploadedFile(file)).rejects.toThrow(
        'Failed to connect to image processing service: Backend processing' +
        ' failed'
      )
    })

    it('throws ApiConnectionError when API factory fails', async () => {
      const file = createMockFile('test.png', 'image/png')
      const mockArrayBuffer = new ArrayBuffer(8)

      // Mock the getAPI function to throw an error
      ;(getAPI as jest.Mock).mockImplementation(() => {
        throw new Error('Handler factory failed')
      })
      ...
      await expect(convertFileToUploadedFile(file)).rejects.toThrow(
        'Failed to connect to image processing service: Handler factory failed'
      )
