...
export const convertFileToUploadedFile = async (
  ...
  try {
    // Use the existing uiHandler from API instead of creating a new one
    const api = getAPI()
    ...
    // Handle API connection or other backend errors
    if (error instanceof Error) {
      throw new ApiConnectionError(
        `Failed to connect to image processing service: ${error.message}`,
        error
      )
    }
