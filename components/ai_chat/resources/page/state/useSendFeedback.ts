  ...
  const handleRateMessage = React.useCallback(
    async (
      ...
      if (!response.ratingId) {
        showAlert({
          type: 'error',
          content: getLocale(StringIds.RatingError),
          actions: []
        })
      ...
      if (isLiked) {
        showAlert({
          type: 'info',
          content: getLocale(StringIds.AnswerLiked),
          actions: []
        })
      } else {
        ...
        showAlert({
          type: 'info',
          content: getLocale(StringIds.AnswerDisliked),
          actions: [
            {
              text: getLocale(StringIds.AddFeedbackButtonLabel),
              kind: 'plain-faint',
  ...
  async function handleFeedbackFormSubmit(
    ...
    if (feedbackId.current) {
      ...
      if (!response.isSuccess) {
        showAlert({
          type: 'error',
          content: getLocale(StringIds.FeedbackSubmitError),
          actions: []
        })
        return
      }
      ...
      showAlert({
        type: 'success',
        content: getLocale(StringIds.FeedbackSent),
        actions: []
      })
      setIsFeedbackFormVisible(false)
    }
