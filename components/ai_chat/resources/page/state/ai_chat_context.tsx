...
export default function useProvideAIChatContext(props: AIChatContextProps) {
  ...
  const { getVisibleConversationsData, isPlaceholderData: isVisibleConversationsLoading } = api.useGetVisibleConversations()

  const store = {
    api: props.api,
    initialized: api.isStandalone.current !== undefined && !isVisibleConversationsLoading,
    isMobile: loadTimeData.getBoolean('isMobile'),
    isHistoryFeatureEnabled: loadTimeData.getBoolean('isHistoryEnabled'),
    isSmartPageContentFeatureEnabled: loadTimeData.getBoolean('isSmartPageContentFeatureEnabled'),
     ...
     * @deprecated use api.useGetPremiumStatus() instead
     */
    allActions: api.useGetActionMenuList().data,

    isStandalone: api.useIsStandalone().data,

    /**
     * @deprecated use api.useGetVisibleConversations() instead
     */
    visibleConversations: getVisibleConversationsData,
