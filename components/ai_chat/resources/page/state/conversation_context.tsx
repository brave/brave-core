...
export function useProvideConversationContext(props: ContextProps) {
  ...
  const isVisible = useIsConversationVisible(conversationState.conversationUuid)
  React.useEffect(() => {
    if (!isVisible) return
  ...
  React.useEffect(() => {
    ...
    const conversationTitle = visibleConversations.find(c =>
  ...
  }, [visibleConversations, conversationState.conversationUuid])

  const actionList = useActionMenu(inputText, aiChat.allActions)
...
export function useIsNewConversation() {
  ...
  return !aiChatContext.visibleConversations.find(c => c.uuid === uuid)
}
