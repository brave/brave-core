...
ContentTool::ContentTool(const blink::mojom::ScriptTool& script_tool,
  ...
  if (url.SchemeIs(content::kChromeUIUntrustedScheme)) {
    ...
    // chrome-untrusted://leo-workspace) that registers tools via WebMCP. These
    // are system-owned, so surface the tool's own name and description
    // verbatim: no "web_" or host prefix (the page is unique and controls its
    // names) and no "website-provided" framing. The names matter: the workspace
    // editor deliberately mirrors Anthropic's `str_replace_based_edit_tool` so
    // it lands in the model's training distribution, which a prefix would
    // defeat.
