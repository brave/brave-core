...
std::optional<std::string> DecodePathSegment(std::string_view escaped) {
  ...
  // Defence in depth: GURL has already turned any backslash into a separator.
...
// and device nodes needs a real stat; on Windows none of them are reachable by
...
bool ShouldHandleWorkspaceFileRequest(const std::string& request_path) {
  // Structural only: malformed requests are claimed so that they fail in
  // HandleWorkspaceFileRequest() rather than at the default resource.
...
void HandleWorkspaceFileRequest(
  ...
  // Unknown uuids fail closed rather than falling back to the current folder.
...
std::optional<WorkspaceFileRequest> ParseWorkspaceFileRequestPath(
  ...
  // Registered uuids are generated lowercase, so anything else cannot match.
  ...
  // Nothing maps a directory to an index file, so the files root names nothing.
...
std::optional<base::FilePath> ResolveWorkspaceFileBlocking(
  ...
  // An absolute or parent-referencing path would make Append() escape the
  // folder (and DCHECK).
  ...
  // IsParent() only compares strings, so both sides must already be resolved.
