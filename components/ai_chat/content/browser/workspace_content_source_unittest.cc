...
// Inputs go through URLToRequestPath() exactly as WebUIDataSource does before
// invoking the filter, so GURL's canonicalisation is part of what is tested.
class WorkspaceFileRequestPathTest : public testing::Test {
 public:
  void SetUp() override {
    // Without the content schemes registered, that canonicalisation would
...
TEST_F(WorkspaceFileRequestPathTest, RejectsTheFilesRootWhichNamesNoFile) {
  ExpectRejected("");
  ExpectRejected("/");
  // A trailing slash leaves the directory as the target, rejected later when
  // it fails to read as a regular file.
  EXPECT_EQ(base::FilePath::FromASCII("docs"), ParsedPath("/docs/"));
}
...
TEST_F(WorkspaceFileRequestPathTest, CanonicalisationCollapsesDotSegments) {
  // Pinned so that a GURL change surfaces here, not as a traversal bug.
...
TEST_F(WorkspaceFileRequestPathTest,
  ...
  // Climbing out of the reserved segment stops it being a file request at all,
  // rather than escaping: "files" is no longer second.
...
TEST_F(WorkspaceFileRequestPathTest, CanonicalisationConvertsBackslashes) {
  // Pinned because DecodePathSegment's backslash check is only defence in depth
  // while GURL keeps treating "\" as a separator.
...
TEST_F(WorkspaceFileRequestPathTest, MalformedUuidIsStillClaimed) {
  ...
  // tool page rather than an error.
