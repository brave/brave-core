...
// chat will be opened if all the above conditions are met.
class AIChatBraveSearchThrottle : public content::NavigationThrottle {
 public:
  class Delegate {
   public:
    virtual void OpenLeo(content::WebContents* web_contents) {}
    Delegate() = default;
  ...
  explicit AIChatBraveSearchThrottle(std::unique_ptr<Delegate> delegate,
 ...
 private:
  void OnNonceValidationResult(bool valid);
  void OnPermissionPromptResult(blink::mojom::PermissionStatus status);

  void OpenLeoWithStagedConversions();
