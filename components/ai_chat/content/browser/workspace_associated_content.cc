...
WorkspaceAssociatedContent::WorkspaceAssociatedContent(
  ...
  // Make the folder servable at <this url>/files/ for as long as this
  // workspace exists, so generated files can be previewed.
...
void WorkspaceAssociatedContent::DeliverDirectoryHandle(
  ...
  // page can use the delivered handle without a permission prompt. Access is
  // scoped by the handle to the user-picked folder.
  //
  // The grant is origin-scoped and workspace files are served from that same
  // origin, so it also covers model-authored script in a preview. See
  // workspace_content_source.h.
