...
class OnDeviceAIServiceTest : public testing::Test {
 protected:
  void SetUp() override {
    feature_list_.InitAndEnableFeature(features::kOnDeviceAIModels);
    service_ = std::make_unique<OnDeviceAIService>(base::BindRepeating(
        &OnDeviceAIServiceTest::CreateFakeWebContents, base::Unretained(this)));
  }
  ...
  std::unique_ptr<OnDeviceAIService> service_;
...
TEST_F(OnDeviceAIServiceTest, GetPassageEmbedderCreatesBackgroundContents) {
...
TEST_F(OnDeviceAIServiceTest, GetPassageEmbedderWaitsForFactory) {
...
TEST_F(OnDeviceAIServiceTest, EndToEndEmbedding) {
...
TEST_F(OnDeviceAIServiceTest, MultipleConsumersEachGetResults) {
...
TEST_F(OnDeviceAIServiceTest, ShutdownDisconnectsConsumers) {
  base::test::TestFuture<mojo::PendingRemote<mojom::PassageEmbedder>> future;
  service_->GetPassageEmbedder(future.GetCallback());
  RegisterFactory();
...
TEST_F(OnDeviceAIServiceTest, ShutdownCancelsPendingCallbacks) {
...
TEST_F(OnDeviceAIServiceTest, ReinitializesAfterDestroyed) {
  base::test::TestFuture<mojo::PendingRemote<mojom::PassageEmbedder>> future;
  service_->GetPassageEmbedder(future.GetCallback());
  RegisterFactory();
...
TEST_F(OnDeviceAIServiceTest, DoubleShutdownIsIdempotent) {
  base::test::TestFuture<mojo::PendingRemote<mojom::PassageEmbedder>> future;
  service_->GetPassageEmbedder(future.GetCallback());
...
TEST_F(OnDeviceAIServiceTest, FactoryDisconnectCancelsPendingCallbacks) {
