const pageInjectionCode = function () {
  ...
  class WPAd extends BATAd {
    constructor() {
      super()
      console.log('WPAd element created', this, window.wpAd)
      if (window.wpAd) {
    ...
    setMatchingInventory() {
      const adKey = this.id.replace('slug_', '')
      console.log('setMatchingInventory', adKey, this)
      if (!adKey) {
        console.error('no id found!', this.id, this)
        return
      }
      if (!window.wpAd) {
        return
  ...
  console.log('BATSense: registering wp-ad custom elements', window.wpAd)
  window.customElements.define('wp-ad', WPAd)
  console.log('BATSense: registered wp-ad custom elements', window.wpAd)

  // Define all ads
  function ChooseAdSizesForWP() {
    console.log('ChooseAdSizesForWP', window.wpAd)
    if (wpAdQueue.length) {
      const ads = wpAdQueue
      wpAdQueue = []
      for (const ad of ads) {
        ad.setMatchingInventory()
      }
    }
  }
  // we need wpAds config to be available, so wait for it
  if (window.wpAd) {
    ChooseAdSizesForWP()
  } else {
    let _wpAd
    Object.defineProperty(window, 'wpAd', {
      configurable: true,
      set: function (val) {
        _wpAd = val
        ChooseAdSizesForWP()
