...
import {hasPortNumber, isHttpOrHttps} from '../../helpers/urlUtils'
 ...
 * @return a promise with the corresponding shields panel data for the input
 *     tabData
 */
export const getShieldSettingsForTabData =
  ...
  const cookies = details[7].setting !== details[8].setting ?
      'block_third_party' :
      details[7].setting
  const braveShields = isHttpOrHttps(origin) ? details[0].setting : 'block'
    return {
    url: url.href, origin, hostname, id: tabData.id, braveShields,
        ads: details[1].setting, trackers: details[2].setting,
        httpUpgradableResources: details[3].setting,
        javascript: details[4].setting, fingerprinting, cookies
    }
  }).catch(() => {
    return {
    url: url.href, origin, hostname, id: tabData.id, braveShields: 'block',
        ads: 0, trackers: 0, httpUpgradableResources: 0, javascript: 0,
        fingerprinting: 0
    }
  })
    }
 ...
 */
export const getTabData = (tabId: number) => chrome.tabs.getAsync(tabId)

/**
 * Obtains new information about the shields panel settings for the specified
 * tabId
 * @param {number} tabId the tabId of the tab who's content settings are of
 *     interest
...
const getPrimaryPatternForOrigin =
    (origin: string) => {
...
export const setAllowBraveShields = (origin: string, setting: string) =>
    chrome.braveShields.plugins.setAsync({
      ...
      resourceIdentifier:
          {id: resourceIdentifiers.RESOURCE_IDENTIFIER_BRAVE_SHIELDS},
      setting,
      scope: getScope()
    })
 ...
 * The tracking-protection service will come into effect if the tracker is
 * marked as blocked.
 * @param {string} origin the origin of the site to change the setting for
 * @param {string} setting 'allow' or 'block'
...
export const setAllowTrackers = (origin: string, setting: string) =>
    chrome.braveShields.plugins.setAsync({
      primaryPattern: origin + '/*',
      resourceIdentifier:
          {id: resourceIdentifiers.RESOURCE_IDENTIFIER_TRACKERS},
      setting,
      scope: getScope()
    })
 ...
 * The https-everywhere service will come into effect if the resource is marked
 * as blocked.
 * @param {string} origin the origin of the site to change the setting for
 * @return a promise which resolves when the setting is set
 */
export const setAllowHTTPUpgradableResources =
    (origin: string, setting: BlockOptions) => {
      const primaryPattern = getPrimaryPatternForOrigin(origin)
      return chrome.braveShields.plugins.setAsync({
        primaryPattern,
        resourceIdentifier: {
          id: resourceIdentifiers.RESOURCE_IDENTIFIER_HTTP_UPGRADABLE_RESOURCES
        },
        setting,
        scope: getScope()
      })
...
export const setAllowJavaScript = (origin: string, setting: string) =>
    chrome.braveShields.javascript.setAsync(
        {primaryPattern: origin + '/*', setting, scope: getScope()})

/**
 * Changes the fingerprinting at origin to be allowed or blocked.
 * The fingerprinting-protection service will come into effect if the
 * fingerprinting is marked as blocked.
 * @param {string} origin the origin of the site to change the setting for
 * @return a promise which resolves with the setting is set
 */
export const setAllowFingerprinting =
    (origin: string, setting: string) => {
...
export const setAllowCookies =
    (origin: string, setting: string) => {
      ...
      const p2 = chrome.braveShields.plugins.setAsync({
        primaryPattern: origin + '/*',
        resourceIdentifier:
            {id: resourceIdentifiers.RESOURCE_IDENTIFIER_COOKIES},
        setting: originSetting,
        scope: getScope()
      })
 ...
 */
export const setAllowScriptOriginsOnce =
    (origins: Array<string>, tabId: number) => new Promise<void>(
        (resolve) => {chrome.braveShields.allowScriptsOnce(
            origins, tabId, () => {resolve()})})
