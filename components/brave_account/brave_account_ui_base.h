...
class BraveAccountUIBase {
 ...
 private:
  void SetupWebUIDataSource(WebUIDataSource* source, const GURL& url) {
    ...
    // rows as a second page at `brave://account/settings`. Their strings are
    // provided by brave://settings on desktop, so they have to be added here
    // instead.
    source->AddResourcePath(brave_account::kSettingsPath,
                            IDR_BRAVE_ACCOUNT_BRAVE_ACCOUNT_SETTINGS_PAGE_HTML);
