            ...
            builder.build(&mut rng)
                ...
                .and_then(|builder| {
                    builder.map(|bundle| OrchardBuilder { unauthorized_bundle: bundle.0, rng: OrchardRandomSource::OsRng(rng) })
                        .ok_or(Error::from(OrchardBuildError::AnchorMismatch))
                })
        },
...
impl OrchardBuilderValue {
    ...
    fn complete(self: &OrchardBuilderValue, sighash: [u8; 32]) -> Box<OrchardBundleResult> {
        ...
        Box::new(OrchardBundleResult::from(match self.0.rng {
            OrchardRandomSource::OsRng(ref rng) => {
                self.0.unauthorized_bundle.clone()
                .create_proof(&orchard::circuit::ProvingKey::build(), rng.clone())
                .and_then(|b| {
                            b.apply_signatures(
                                rng.clone(),
                                sighash,
            ...
            OrchardRandomSource::MockRng(ref rng) => {
                self.0.unauthorized_bundle.clone()
                .create_proof(&orchard::circuit::ProvingKey::build(), rng.clone())
                .and_then(|b| {
                            b.apply_signatures(
