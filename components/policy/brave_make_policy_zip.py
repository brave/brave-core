...
def main():
  ...
  _make_policy_zip(chrome_policy_zip, dest_zip)
...
def _make_policy_zip(chrome_policy_zip, dest_zip):
  with TemporaryDirectory() as tmp_dir:
    ...
    for adm in glob(join(tmp_dir, 'windows/adm/**/*.adm'), recursive=True):
      _replace_in_file(adm, r'^([a-zA-Z_0-9]+=".*)Google Chrome(.*")$', r'\1Brave\2', encoding='utf-16')

    with ZipFile(dest_zip, 'w') as dest_zipfile:
       for dirpath, dirnames, filenames in os.walk(tmp_dir):
         for filename in filenames:
           filepath = join(dirpath, filename)
           dest_zipfile.write(filepath, arcname=relpath(filepath, tmp_dir))

def _replace_in_file(file_path, pattern, replacement, encoding='utf-8'):
  with open(file_path, encoding=encoding) as f:
    text = f.read()
  with open(file_path, 'w', encoding=encoding) as f:
    f.write(re.subn(pattern, replacement, text)[0])
