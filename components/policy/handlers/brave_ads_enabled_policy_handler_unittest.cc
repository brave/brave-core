...
#include "brave/components/policy/handlers/brave_ads_enabled_policy_handler.h"
...
constexpr std::string_view kBraveAdsTypePrefNames[] = {
...
TEST_F(BraveAdsEnabledPolicyHandlerTest,
  ...
  for (const std::string_view pref_name : kBraveAdsTypePrefNames) {
    bool pref_value;
    ASSERT_TRUE(prefs().GetBoolean(pref_name, &pref_value)) << pref_name;
    EXPECT_FALSE(pref_value) << pref_name;
  }
}
...
TEST_F(BraveAdsEnabledPolicyHandlerTest,
  ...
  for (const std::string_view pref_name : kBraveAdsTypePrefNames) {
    bool pref_value;
    EXPECT_FALSE(prefs().GetBoolean(pref_name, &pref_value)) << pref_name;
  }
}
