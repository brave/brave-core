...
// the execution engine only -- it deliberately does NOT depend on
...
// runner that reads the model from `embeddings_model_file` and runs it on
// LiteRT's CPU backend (taking the SentencePiece control-token ids), or
// nullptr on failure.
