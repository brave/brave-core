...
def MakeUpdatedProfileArchive(cfg: RunnerConfig, options: CommonOptions, extra_dirs_to_add: List[str]) -> str:
  assert cfg.version is not None
  profile_dir = GetProfilePath(cfg.profile, options.working_directory,
                               cfg.version)
  ...
  for dir in extra_dirs_to_add:
    target_dir = os.path.join(profile_dir, os.path.basename(dir))
    logging.info(f'Adding extra {dir} to {target_dir}')
...
def RunUpdateProfile(brave_config: perf_config.PerfConfig, chromium_config: perf_config.PerfConfig, options: CommonOptions) -> bool:
  ...
  if not _RunUpdateProfileForConfig(chromium_config, options, extra_dirs_to_add):
    return False
...
def _RunUpdateProfileForConfig(config: perf_config.PerfConfig,
  ...
  return MakeUpdatedProfileArchive(configurations[0], options, extra_dirs_to_add)
