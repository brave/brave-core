...
@dataclass
class PatchInfo:
    """ Manages a .pathinfo file contents.
    ...
    Pathinfo files are not committted to the repository. These files have the
    ...
    def __post_init__(self):
        ...
        # Setup patchinfo metadata file (same as the patch .patchinfo extension).
        self.patchinfo = PathChecksumPair(
            self.patch.path.with_suffix('.patchinfo'))

        # This is set relative, so it gets validated to be under BRAVE_CORE_PATH.
...
class PlasterFile:
    ...
    def apply(self, dry_run=False):
        ...
        plaster = tomllib.loads(info.plaster_contents)
        contents = repository.chromium.read_file(info.source)

        for substitution in plaster.get('substitution', []):
