...
This file is organised in three layers:

* Unit tests (`EntryLineParseTest`, `PatternMatchTest`, `RewritePlanTest`,
  `MessageWriterTest`) exercise the pure file-in / file-out transforms in the
  `rebase` module with no git involved.

* `RebaseRecommitHelperTest` covers the `brockit.Rebase.recommit_in_rebase_plan`
  todo-file transformer as a pure file-in / file-out unit test.

* `RebaseExecuteTest` covers `brockit.Rebase.execute()` end-to-end against a
  real git tree built by `FakeChromiumRepo`. The editor callbacks
...
class EntryLineParseTest(unittest.TestCase):
    """Tests for `rebase.EntryLine.parse`."""

    def test_parse_simple_pick(self):
        entry_line = rebase.EntryLine.parse('pick aaa # [cr148] Feature A\n')
    ...
    def test_parse_pure_comment_returns_none(self):
        self.assertIsNone(rebase.EntryLine.parse('# Rebase plan\n'))
        self.assertIsNone(rebase.EntryLine.parse('   # indented\n'))
    ...
    def test_parse_malformed_line_raises(self):
        ...
        with self.assertRaises(rebase.EditorRecoverableFailure):
            rebase.EntryLine.parse('garbage\n')
        with self.assertRaises(rebase.EditorRecoverableFailure):
            rebase.EntryLine.parse('pickonly # no hash\n')
    ...
    def test_entry_type_classification_and_properties(self):
        ...
        regular = rebase.EntryLine.parse('pick ccc # [cr148] Feature A\n')
    ...
    def test_reassign_target_hash_property(self):
        ...
        regular = rebase.EntryLine.parse('pick aaa # [cr148] Feature A\n')
...
class PatternMatchTest(unittest.TestCase):
    ...
    def test_strip_cr_tag_removes_simple_tag(self):
        self.assertEqual(rebase._strip_cr_tag('[cr149] Foo'), 'Foo')
...
class MessageWriterTest(unittest.TestCase):
    """Tests for `rebase.MessageWriter`.
    ...
    @staticmethod
    def _messages(writer: 'rebase.MessageWriter') -> list:
    ...
    def test_hand_off_to_editor_invokes_supplied_editor(self):
        ...
        with patch.object(rebase.terminal, 'run') as mock_run:
            rebase.hand_off_to_editor(path, reason='', editor='nano')
    ...
    def test_hand_off_to_editor_prepends_reason_as_comment(self):
        ...
        with patch.object(rebase.terminal, 'run'):
            rebase.hand_off_to_editor(
                path,
    ...
    def test_hand_off_to_editor_no_reason_leaves_file_alone(self):
        ...
        with patch.object(rebase.terminal, 'run'):
            rebase.hand_off_to_editor(path, reason='', editor='vim')
...
class RebaseRecommitHelperTest(unittest.TestCase):
...
class RebaseExecuteTest(unittest.TestCase):
    ...
    def test_no_flags_is_a_passthrough_rebase(self):
        """No flags: no plan transform, history preserved on the new
        target."""
    ...
    def test_squash_minor_bumps_with_stacked_fixup_commits(self):
        ...
        pinned target and `squash_minor_bumps` collapses the whole
    ...
    def test_defaults_from_ref_to_previous_for_same_major(self):
    ...
    def test_defaults_from_ref_to_previous_major_for_new_major(self):
    ...
    def test_rebase_failure_raises_invalid_input(self):
