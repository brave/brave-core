...
Different from a lift using `--with-github`, the command above command will
also attempt to create a PR, if none exists, however it is necessesary that the
...
class GitHubIssue(Versioned):
    ...
    def create_push_request(self, issue_url: str) -> bool:
        ...
        if is_major or upstream_branch == 'master':
            # It is not comman for upstream test to be run on uplifts of minor
            # upgrades.
        ...
        pr_url = terminal.run(cmd)
        terminal.log_task(f'GitHub PR created for this bump: {pr_url}')
        return True

    def create_or_updade_version_issue(self, with_pr: bool) -> bool:
        """Creates a github issue for the upgrade.
    ...
    def execute(self) -> bool:
        ...
        return self.create_or_updade_version_issue(with_pr=True)
    ...
    Args:
        ...
        if result is True and with_github is True:
            GitHubIssue(base_version=self.base_version,
                        target_version=self.target_version
                        ).create_or_updade_version_issue(with_pr=False)
