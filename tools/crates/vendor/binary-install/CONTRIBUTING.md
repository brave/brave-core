...
If you are trying to use `binary-install` and run into an issue- please file an
issue! We'd love to get you up and running, even if the issue you have might
not be directly related to the code in `binary-install`. This tool seeks to make
it easy for developers to get going, so there's a good chance we can do
something to alleviate the issue by making `binary-install` better documented or
...
toolchain. Run this command in the `binary-install` directory you cloned.
