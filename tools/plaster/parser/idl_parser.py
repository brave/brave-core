...
ERROR_REMAP = {
  ...
  'Unexpected "{" after keyword "class".' : 'class missing name.',
}
...
# Members
#
# A member attribute or function of a struct, interface, or class.
#
...
# Class
#
# A class is a named collection of functions, and members.
#
  def p_class_block(self, p):
    """class_block : modifiers CLASS SYMBOL '{' class_list '}' ';'"""
    p[0] = self.BuildNamed('Class', p, 3, ListFromConcat(p[1], p[5]))
    if self.parse_debug: DumpReduction('class_block', p)

  def p_class_error(self, p):
    """class_block : modifiers CLASS error '{' class_list '}' ';'"""
    p[0] = []

  def p_class_list(self, p):
    """class_list : member_function ';' class_list
                      | """
    if len(p) > 1 :
      p[0] = ListFromConcat(p[1], p[3])
      if self.parse_debug: DumpReduction('class_list', p)
...
def FindVersionError(releases, node):
  err_cnt = 0
  if node.IsA('Interface', 'Struct', 'Class'):
    comment_list = []
