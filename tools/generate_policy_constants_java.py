...
"""
    ...
    for policy_name in sorted(policy_names):
        ...
        java_content += f'    public static final String {constant_name} = "{policy_name}";\n'
...
def generate_srcjar(policy_names, output_srcjar):
    ...
    print(f"Generated {len(policy_names)} policy constants in srcjar {output_srcjar}")
...
def main():
    ...
    if not yaml_files:
        print(f"Warning: No YAML policy files found in {args.policy_dir}", file=sys.stderr)
        sys.exit(1)
