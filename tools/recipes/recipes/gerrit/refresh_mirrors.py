...
class InputProperties:
    ...
    # When True, don't refresh the git-cache mirrors (the `mirror/...` projects).
    no_mirrors_refresh: bool = False
    # When True, don't refresh the owned repos (brave-core, depot_tools fork).
    no_owned_repos_refresh: bool = False
...
def RunSteps(api: RecipeScriptApi, properties: InputProperties) -> None:
    ...
    brave_core_root = api.brave_core_shallow.deploy('tools/cr')
    ...
    if properties.no_mirrors_refresh:
        cmd.append('--no-mirrors-refresh')
    if properties.no_owned_repos_refresh:
        cmd.append('--no-owned-repos-refresh')
