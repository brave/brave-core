...
| `build`            | `pnpm run build -- Release --channel=<c> --target=brave:all`, then relaxes `ptrace_scope`  | `utils.runBuild`                   | Recipe module + `bots`                     |
...
- `isOfficialBuild()` is false for any sanitiser build, so this is a
  ...
  sanitisers;
- `isLsan()` returns `isAsan() && targetOS === 'linux'` (re-enabled in #38807,
  closing brave-browser#56047), so on this job we build ASan **with** LSan,
  matching upstream's `Linux ASan LSan Builder`.
...
thing being deleted: our `bots` reads generated data only, with no `configs`
table and no fallback lookup path.
...
| **D5**  | A new tool, **`brave/infra/bots`**, owns build configuration — a mix of lucicfg (spec files -> generated JSON) and `mb` (generated JSON -> `args.gn`, `gn gen`) in one place, since we have neither's surrounding infrastructure to split them across. Under `--builder`, `buildArgs.ts` is bypassed **entirely**: the generated `gn-args.json` is the input and `args.gn` is the output. It reads generated data only — no `mb_config.pyl` equivalent, which is the layer upstream is deleting. |
| **D6**  | The **`.gni` defaults under `brave/build/args/`** are re-expressed as data, not imported. Each file's own assignments become a `gn_args.config()`, composed the same way as the per-builder overrides. The `.gni` files themselves are untouched and still drive today's non-`--builder` builds; under `--builder`, `gn_args.py` is authoritative and the files are never read.                                                                                                                  |
...
| **D13** | **The output directory is `out/<builder>`.** It keeps a builder's tree away from a developer's `out/Release` and gives the secrets `.gni` a predictable home. Taken for now and deliberately cheap to revisit: it is one value in the builder config, read by `bots` and by `pnpm run build`, and nothing else depends on the spelling.                                                                                                                                                          |
...
| **D18** | **LSan follows ASan on Linux, as `config.ts` already does.** `isLsan()` was re-enabled ahead of this migration (#38807, closing brave-browser#56047): `is_lsan = isAsan() && targetOS === 'linux'`. The Linux `asan` config in `gn_args.py` sets `is_lsan = True` from the start — there is no flag left to flip later.                                                                                                                                                                          |
...
- D11 closes the last two ways a build could be described in more than one
  ...
  config. A developer who wants a one-off variation runs `bots lookup <builder>`
  into a build directory, edits `args.gn` by hand and builds without
  `--builder`, using the existing `use_no_gn_gen` path. `bots` itself must not
...
- D7 has a pleasant side effect: because `args.gn` only ever contains an
  `import(...)` line for secrets, `bots lookup` output and the `args.gn` in the
...
- D19 settles what looked like an ordering problem, and it rests on a detail
  ...
  recipe could therefore read it straight after `bots gen`. It should not treat
  it as the source of truth anyway: which tests a builder runs is a property of
  the builder, belongs in its `targets.json`, and wants phrasing in terms of
...
- D18 is a straight port, not a staged flip. An earlier draft of this document
  assumed `isLsan()` was still hard-coded `false` pending brave-browser#56047;
  that issue was resolved (#38807) before this migration started, so
  `gn_args.py` simply encodes today's logic — `is_lsan = True` on the Linux
  `asan` config — rather than carrying a bug reference forward. The general
  point survives on other examples: once a setting like this is data instead of
  buried in TypeScript, flipping it later for a real reason is a one-line,
  one-builder change instead of a code change.
...
- Reusing `tools/recipes/config.py` for the resolver is therefore **not** the
  ...
  counterpart. The host in `lib/config.py` is a small piece of Python: a DFS
...
brave/infra/config/                       (new; Track A)
  gn_args.py             the named GN-arg configs — our gn_args.star (D20)
                           re-expresses brave/build/args/*.gni as data (D6)
  builders/*.py          builders per group — our chromium.memory.star
      ^ spec: declarative, starlark-portable (§7.4), frozen once read (D21)
  lib/config.py          the host: registration, freeze, resolve, validate
  generated/builders/<builder>/
      gn-args.json       { gn_args, secrets } — upstream's shape, minus the
                           args_file it also allows, which we don't use (D6)
      sync.json          target os/arch, gclient overrides, project pins
      targets.json       compile targets, test targets, harness flags
        |
        |  read by
        v
brave/infra/bots/                         (new; Track A; D5: mb + lucicfg, one tool)
  bots.py snapshot         spec -> the generated/ files above; a presubmit
                             re-runs this and fails on drift (lucicfg-like)
  bots.py lookup <builder> print the args.gn it would write            (mb-like)
  bots.py gen <builder>    write args.gn (+ secrets .gni), run gn gen  (mb-like)
  bots.py validate         every builder resolves; no secret values in git
        ^
        |
...
# brave/build/args/*.gni re-expressed as data (D6), one config per file, kept
# in sync by hand: none is big enough to be worth an args_file, and inlining
# sidesteps the single-args_file-per-resolution rule below entirely.
gn_args.config(name = "brave_defaults", args = {
    "disable_fieldtrial_testing_config": True,
    "enable_precompiled_headers": False,
    # … the rest of brave_defaults.gni …
})
gn_args.config(name = "blink_platform_defaults", args = {
    "proprietary_codecs": True,
    "ffmpeg_branding": "Chrome",
    "enable_widevine": True,
    # … the rest of blink_platform_defaults.gni …
})
gn_args.config(name = "branding_defaults", args = {
    "branding_path_component": "brave",
    "branding_path_product": "brave",
})
gn_args.config(name = "desktop_defaults", args = {"safe_browsing_mode": 1})
...
gn_args.config(
    ...
    args = {
        "is_asan": True,
        # LSan runs on Linux ASan builds (re-enabled in #38807). config.ts's
        # isLsan() ties this to target_os == "linux", which this config assumes.
        "is_lsan": True,
...
`brave/infra/config/builders/sanitisers.py` mirrors
`subprojects/chromium/ci/chromium.memory.star`, down to the group defaults and
the local helper that adds a notification to every builder in the group (not
landed yet — kept here for when a builder needs one on top of the group's own):

```python
# brave/infra/config/builders/sanitisers.py
...
- **No `args_file`, even though the resolver still supports one.** Upstream
  permits exactly one `args_file` per resolved config, and `buildArgs.ts`
  currently emits up to four ordered imports (`brave_defaults`,
  `blink_platform_defaults`, a branding file, then one of desktop/android/ios).
  Rather than add a thin wrapper `.gni` per platform family under
  `brave/build/args/builders/` to preserve that invariant, `gn_args.py`
  re-expresses each `.gni`'s own assignments as a plain `gn_args.config()` (D6):
  none of the four files is more than a handful of lines, and inlining them
  sidesteps the single-`args_file` rule rather than working around it — a real
  constraint hit early in practice, where two sibling configs that each
  transitively carry the _same_ `args_file` still conflict, since the merge only
  ever sees the resolved string, not the originating node. `args_file` stays on
  `gn_args.config()` as a faithful, general primitive for the day something
  genuinely can't be expressed as data; nothing currently resolves one.
...
The generated result for our builder — no `args_file` key, since nothing
resolves to one, and the former `.gni`-only values (`proprietary_codecs`,
`branding_path_component`, …) now sit in `gn_args` alongside everything else:
...
`bots gen` renders `args.gn` from it: the secrets import when `secrets` is
non-empty (an `args_file` import too, on a builder that happens to resolve one),
then the `gn_args` assignments. The secrets file is written by `bots` from the
environment named in `secrets`, with mode `0600`. A missing secret is a hard
error on a builder that declares it, and — unlike today's `dummy` fallbacks —
that failure happens before any compilation starts. The secrets import stays
`bots`'s concern, not a config-level one: config data never contains a raw
import line.
...
- **Cleanup is still explicit, and ordered last.** The output directory survives
  ...
  test step, and recreated by the next run's `bots gen`. Deleting it earlier
  would leave `build.ninja` referring to a missing import, so any later bare
  `ninja` would fail its regeneration check. This is the direct replacement for
...
Note that `bots lookup` prints only the import line for secrets, so it is safe
in logs; `gn args --list` on such a build directory is **not** — it resolves and
...
`from lib.config import …` with `load(…)`, swaps the host, and leaves the
...
that today only get an issue (D17) — so it is not mistaken for a regression.
LSan is already enabled on this job (D18) and needs no special handling in the
fixtures: `is_lsan=true` is expected, and everything else is a pure port where
...
This stage also lands `gn_args.py`'s `.gni`-mirroring configs (D6) —
`brave_defaults`, `blink_platform_defaults`, `branding_defaults`,
`desktop_defaults` — each re-expressing one existing `.gni`'s assignments as
data, since `gn-args.json` needs their values and nothing downstream reads the
`.gni` files themselves under `--builder`.

**Deliverables.** The classification table; the three schemas; the
`.gni`-mirroring configs in `gn_args.py`; hand-written `gn-args.json`,
`sync.json` and `targets.json` for both builders, each carrying its
`out/<builder>` output directory (D13). The hand-written `gn-args.json` should
already be in the shape A5's resolver will produce — `{gn_args, secrets}`, no
`args_file` (D6) — so that A5 is verified against it rather than defining it.
...
**Work.** Replace `bots gen`'s stub (A5) with the real thing: resolve the
`secrets` map from the environment; write `<out>/brave_secrets.gni` with
restrictive permissions; fail loudly on a missing secret; provide explicit
cleanup that the recipe calls in a `finally` equivalent, after every step that
compiles or runs anything, plus a belt-and-braces cleanup in the Jenkins `post`
block. No `.gitignore` change is needed since the file lives under `out/`, but
the presubmit should still refuse a committed `*_secrets.gni` anywhere, in case
the pattern gets copied to a source-tree path later.
...
**Size.** Small to medium. **Depends on.** A5.
...
generation to `bots gen` (the existing `use_no_gn_gen` seam is the natural place
...
#### A5 — The spec files, the host, and `brave/infra/bots`

**Goal.** Stop hand-writing the generated files; make "what ASan means" exist in
exactly one place; make adding a builder a data-only change; keep the spec close
enough to upstream's starlark that either a reader or a future port can move
between them without translation; and have one tool that turns a builder name
into a configured build directory, usable from both the recipe and a developer's
shell.

**Work.** `brave/infra/config/`, split the way upstream splits it — spec on one
side, host on the other — plus `brave/infra/bots/`, the mb-and-lucicfg-in-one
tool from D5 that reads what the host resolves:
...
- **`gn_args.py`** — the analogue of `gn_args/gn_args.star` (D20). Every named
  ...
  `.gni`-mirroring configs (D6). The only file in the repository that assigns a
  GN arg value for a builder.
- **`builders/<group>.py`** — analogues of `chromium.memory.star`: group
  defaults, and builders that are a name plus the configs they reference. Our
  ASan work needs one file (`sanitisers.py`), eventually with two builders.
- **`lib/config.py`** — the host, and the only place with logic. Registration
  ...
  resolution (unused by our own configs today, D6), `target_os` and `target_cpu`
  required, plus a validation hook per resolved builder.
- **`brave/infra/bots/bots.py`**, with subcommands:

  - `snapshot` — the lucicfg-like half: emits `gn-args.json`, `sync.json` and
    `targets.json` per builder plus the locations index. A presubmit re-runs it
    and fails on drift, exactly as upstream's generator does;
  - `lookup <builder>` — the mb-like half: resolve and print the exact `args.gn`
    contents; no side effects; secrets shown as the import line only;
  - `gen <builder> [--out-dir DIR]` — write `args.gn`, then run `gn gen`,
    passing the builder's declared test targets (D19) so GN emits their
    `runtime_deps`. That costs nothing now and is the prerequisite for producing
    isolates later, which is why the plumbing belongs here from the start rather
    than in a retrofit. Secrets handling is a stub here — a builder that
    declares `secrets` gets an empty or `dummy`-valued `brave_secrets.gni` —
    real resolution is A3's job, which needs exactly this stub to exist first;
  ...
  `lookup`/`gen`/`validate`/`describe` read the generated files only; they know
  nothing of configs or how they resolved — only `snapshot` touches the spec and
  the host. Unit tests compare `lookup` output against the A0 fixtures.

`freeze`/`thaw`/`FrozenDict` live in `lib/config.py` itself rather than in
`tools/recipes/`, trimmed to their stdlib-only core from upstream's
`recipes-py/recipe_engine/engine_types.py`: `brave/infra/config` and
`brave/infra/bots` are meant to work standalone, without a dependency on the
recipes engine, and reaching into `tools/recipes` from here would invert that.
...
config fails; the lint rejects a spec file that steps outside §7.4;
`bots lookup linux-x64-asan-brave` matches the A0 `args.gn` fixture byte for
byte; and `bots gen` produces a build directory that `ninja` accepts.

**Size.** Medium to large. **Depends on.** A1.
...
`A0 → A1 → A5 → A3 → A4 → B1 → B2 → A6 → B3 → B4 → B5 → B6`. B1 and B2 can start
as soon as A1 fixes the schema, using hand-written generated files.
...
1. **Test isolates, sharding and off-node execution** (D19). The builder
   declares test targets and `bots gen` emits their `runtime_deps`, so producing
   isolates is an added `bots` subcommand rather than a redesign. Everything
   that follows from isolates — sharding, separate tester builders, running
   tests off the build node — needs a scheduler we do not have on Jenkins, so it
...
| `buildArgs.ts` (for CI)                                     | `bots lookup` / `bots gen` (A5)                                           |
