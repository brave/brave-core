...
class BraveContentSettingsObserverFlashBrowserTest
...
IN_PROC_BROWSER_TEST_F(BraveContentSettingsObserverFlashBrowserTest,
  ...
  int len;
  ASSERT_TRUE(ExecuteScriptAndExtractInt(contents(), kFlashPluginExists, &len));
  ASSERT_EQ(len, 0);
}
...
IN_PROC_BROWSER_TEST_F(BraveContentSettingsObserverFlashBrowserTest,
  ...
  bool value;
  EXPECT_TRUE(
      ExecuteScriptAndExtractBool(contents(), "triggerPrompt();", &value));
  EXPECT_TRUE(value);
