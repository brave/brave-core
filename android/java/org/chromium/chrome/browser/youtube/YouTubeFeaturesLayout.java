...
/**
 * YouTube layout extending a linear layout used to draw extra buttons to switch into fullscreen or
 * picture in picture mode. The layout needs to be instantiated programmatically and will take care
 * of drawing two buttons with rounded corners to each side. The buttons will provide a consistent
 * ripple effect that will not go over their boundaries and a custom outline for the elevation
 * effect.
 */
...
public class YouTubeFeaturesLayout extends LinearLayout {
    ...
    @NonNull private final Runnable mShowRunnable;
    @NonNull private final Runnable mHideRunnable;
    ...
    public YouTubeFeaturesLayout(@NonNull final Context context, @NonNull final Callback callback) {
                ...
                new ViewOutlineProvider() {
                    ...
                    public void getOutline(View view, Outline outline) {
                        ...
                        outline.setRoundRect(
                                0, 0, view.getWidth(), view.getHeight(), view.getHeight() / 2f);
                    }
                });
        ...
        initLayoutButton(
                fullscreenButton,
                paddingPx,
                R.drawable.fullscreen_on,
                R.drawable.youtube_button_ripple_left);

        final ImageButton pictureInPictureButton = new ImageButton(context);
        initLayoutButton(
                pictureInPictureButton,
                paddingPx,
                R.drawable.picture_in_picture,
                R.drawable.youtube_button_ripple_right);
    ...
    private void initLayoutButton(
        ...
        imageButton.setForeground(
                ContextCompat.getDrawable(imageButton.getContext(), rippleEffectDrawableRes));

        // Set layout weight to 0.5 to give each button half of the layout width.
        final LinearLayout.LayoutParams params =
                new LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 0.5f);
