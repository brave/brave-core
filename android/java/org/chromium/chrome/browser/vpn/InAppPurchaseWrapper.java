...
public class InAppPurchaseWrapper {
    ...
    public static final List<String> SUBS_SKUS = new ArrayList<>(Collections.singletonList("nightly.bravevpn.monthly"));

    public static InAppPurchaseWrapper getInstance() {
        if (inAppPurchaseWrapper == null)
            inAppPurchaseWrapper = new InAppPurchaseWrapper();
        ...
        if (!purchase.isAcknowledged()) {
            billingClient.acknowledgePurchase(acknowledgePurchaseParams, billingResult -> {
                if (billingResult.getResponseCode() == OK) {
                    ...
                    Toast.makeText(context, "Subscription successfully consumed", Toast.LENGTH_LONG).show();
                } else {
                    Toast.makeText(context, "Failed to acknowledge purchase :" + billingResult, Toast.LENGTH_LONG).show();
                }
            });
        } else {
            BraveVpnUtils.setSubscriptionPurchased();
    ...
    PurchasesUpdatedListener purchasesUpdatedListener = (billingResult, purchases) -> {
        if (billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK) {
            ...
            Toast.makeText(context, "You are already subscribed to the subscription. Try restoring the subscription.", Toast.LENGTH_LONG).show();
        } else if (billingResult.getResponseCode() == BillingClient.BillingResponseCode.SERVICE_DISCONNECTED) {
            InAppPurchaseWrapper.getInstance().connectToBillingService();
        } else if (billingResult.getResponseCode() == BillingClient.BillingResponseCode.USER_CANCELED) {
            Toast.makeText(context, "ERROR!!\nCaused by a user cancelling the purchase flow.", Toast.LENGTH_LONG).show();
        } else {
