...
public class NewTabTakeoverSafeAreaReporter extends RecyclerView.OnScrollListener
    ...
    private @Nullable Integer mTopNtpItemsHeight;
    ...
    private void measureAndReportSafeArea() {
        ...
        maybeUpdateTopNtpItemsHeight();
        if (mTopNtpItemsHeight == null) {
            return;
        }

        final int safeAreaTop =
                mRecyclerView.getTop() - mRichMediaContainer.getTop() + mTopNtpItemsHeight;
    ...
    private void maybeUpdateTopNtpItemsHeight() {
        ...
        mTopNtpItemsHeight = topNtpItemsHeight;
    }
     ...
     * pixels in a WebView without a custom viewport scale), inset by the standard NTP content
     * margin and clamped to the viewport.
     */
