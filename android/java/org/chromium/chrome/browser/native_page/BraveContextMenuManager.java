...
public class BraveContextMenuManager extends ContextMenuManager {
    ...
    protected void customizeMenuModel(MVCListAdapter.ModelList menuModel, Delegate delegate) {
        ...
        if (!afterItems.isEmpty()) {
            if (!menuModel.isEmpty()) {
                menuModel.add(
                        new MVCListAdapter.ListItem(ListItemType.DIVIDER, new PropertyModel()));
            }
    ...
    /**
     * NTP widget control options appear at the bottom of the menu, behind a divider if anything
     * precedes them (see {@link #customizeMenuModel}).
     */
