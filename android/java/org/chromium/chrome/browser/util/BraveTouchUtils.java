...
public class BraveTouchUtils {
    ...
    /**
     * Ensures that the touchable area of [view] equal [minTarget] by expanding the touch area of a
     * view beyond its actual view bounds.
     *
     * <p>This function was taken from Trackr Android App (https://github.com/android/trackr) and
     * adapted to Java. minTarget is in dp instead of px.
     *
     * <p>Copyright (C) 2021 The Android Open Source Project
     *
                                    ...
                                    if (view.isShown()) {
                                        ...
                                        DisplayMetrics metrics =
                                                view.getContext()
                                                        ...
                                                        .getDisplayMetrics();
                                        int height =
                                                (int) Math.round(bounds.height() / metrics.density);
                                        int width =
                                                (int) Math.round(bounds.width() / metrics.density);

                                        int extraSpaceStart = 0;
                                        int extraSpaceEnd = 0;
                                        if (height < minTarget) {
                                            extraSpaceStart =
                                                    (int) Math.floor((minTarget - height) / 2f);
                                        ...
                                        if (width < minTarget) {
                                            extraSpaceStart =
                                                    (int) Math.floor((minTarget - width) / 2f);
