 ...
 * prepends "Add site", and appends the NTP top-sites widget-control items (behind a divider, but
 * only when something else precedes them) when the delegate is a {@link BraveNtpDelegate}; (3)
 * clicks on Brave items are dispatched correctly.
 */
...
public class BraveContextMenuManagerTest {
    ...
    public void testCustomizeMenuModel_selectedModeGetsEndIcon() {
        ...
        // No divider since nothing precedes these items (e.g. the "Add new" long-press menu).
        // [0]=frequent (not selected), [1]=shortcuts (selected), [2]=hide.
        assertEquals(0, menuModel.get(0).model.get(ListMenuItemProperties.END_ICON_ID));
        assertEquals(
                R.drawable.ic_check_circle_filled,
                menuModel.get(1).model.get(ListMenuItemProperties.END_ICON_ID));
        assertEquals(0, menuModel.get(2).model.get(ListMenuItemProperties.END_ICON_ID));
    }
    ...
    public void testCustomizeMenuModel_partiallySupported_onlyIncludesSupportedItems() {
        ...
        // No divider since nothing precedes it; just the single supported item.
        assertEquals(1, menuModel.size());
        assertEquals(
                BraveNtpDelegate.BRAVE_SHOW_FREQUENT,
                menuModel.get(0).model.get(ListMenuItemProperties.MENU_ITEM_ID));
    ...
    @Test
    public void testCustomizeMenuModel_widgetItemsOnly_noDividerWhenNothingPrecedes() {
        // Simulates the "Add new" long-press menu: no standard items, no "Add site" — just the
        // widget-control items. A leading divider there would have nothing to separate from.
        doReturn(true)
                .when(mBraveDelegate)
                .isBraveItemSupported(BraveNtpDelegate.BRAVE_SHOW_FREQUENT);
        doReturn(true)
        ...
        assertEquals(3, menuModel.size());
        for (int i = 0; i < menuModel.size(); i++) {
            assertFalse(menuModel.get(i).type == ListItemType.DIVIDER);
        }
    }

    @Test
    public void testCustomizeMenuModel_widgetItemsAfterStandardItem_dividerPresent() {
        doReturn(true)
                .when(mBraveDelegate)
                .isBraveItemSupported(BraveNtpDelegate.BRAVE_SHOW_FREQUENT);
        doReturn(true)
    ...
    @Test
    public void testCustomizeMenuModel_widgetItemsAfterAddSiteOnly_dividerPresent() {
        doReturn(true).when(mBraveDelegate).isBraveItemSupported(BraveNtpDelegate.BRAVE_ADD_SITE);
        doReturn(true)
                .when(mBraveDelegate)
                .isBraveItemSupported(BraveNtpDelegate.BRAVE_SHOW_FREQUENT);
        ...
        // [0]="Add site", [1]=divider, [2]=frequent. Even with no standard items, "Add site"
        // itself is something the divider correctly separates the widget items from.
        assertEquals(3, menuModel.size());
        assertEquals(
                BraveNtpDelegate.BRAVE_ADD_SITE,
                menuModel.get(0).model.get(ListMenuItemProperties.MENU_ITEM_ID));
