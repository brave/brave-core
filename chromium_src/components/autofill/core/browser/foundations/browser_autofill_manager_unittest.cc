...
TEST_F(BrowserAutofillManagerTest, Onion_NonMixedForm1) {
  // Set up our form data.
  FormData form;
  form.set_name(u"MyForm");
  form.set_url(GURL("http://myform.onion/form.html"));
...
TEST_F(BrowserAutofillManagerTest, Onion_NonMixedForm2) {
  // Set up our form data.
  FormData form;
  form.set_name(u"MyForm");
  form.set_url(GURL("http://myform.myonion/form.html"));
  form.set_action(GURL("http://myform.com/submit.html"));
  form.set_fields({CreateTestFormField("Name on Card", "nameoncard", "",
                                       FormControlType::kInputText)});
...
TEST_F(BrowserAutofillManagerTest, Onion_NonMixedForm3) {
  // Set up our form data.
  FormData form;
  form.set_name(u"MyForm");
  form.set_url(GURL("http://myform.onion.com/form.html"));
  form.set_action(GURL("http://myform.com/submit.html"));
  form.set_fields({CreateTestFormField("Name on Card", "nameoncard", "",
                                       FormControlType::kInputText)});
