...
void BraveTabStripCollection::AddTabRecursive(
  ...
  // If the new |tab|'s opener is the previous tab, we need to insert the new
  // tab into the opener's TreeTabNode as a child node
  if (const auto* opener = tab->GetOpener();
      opener && *GetIndexOfTabRecursive(opener) + 1 == index) {
