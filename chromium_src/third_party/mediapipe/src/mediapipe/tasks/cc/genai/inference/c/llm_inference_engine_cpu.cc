...
// CHROMIUM_SRC OVERRIDE: Added for Chromium's unsafe buffer warnings in C API
// code Related patches for Chromium compatibility:
...
// clang-format on
// CHROMIUM_SRC OVERRIDE: Fixed sentencepiece include paths for Chromium build
// Upstream uses: "sentencepiece/src/sentencepiece_processor.h" (doesn't work)
// Fixed to use:  "src/sentencepiece_processor.h" (works with Chromium's
// sentencepiece setup) Similar fix applied in:
// //third_party/mediapipe/patches/0001-Fix-sentencepiece-include.patch
#include "src/sentencepiece_processor.h"  // from @com_google_sentencepiece
#include "src/util.h"                     // from @com_google_sentencepiece
...
int LlmInferenceEngine_Session_Clone(
    LlmInferenceEngine_Session* session,
    LlmInferenceEngine_Session** cloned_session,
    char** error_msg) {
  *error_msg = strdup("Not implemented");
  return 12;
}
...
int LlmInferenceEngine_GetSentencePieceProcessor(
    ...
    const SentencePieceProcessor** processor_out,
    char** error_msg) {
  *error_msg = strdup("Not implemented");
  return 12;
}
