...
// CHROMIUM_SRC OVERRIDE: Added for Chromium's unsafe buffer warnings
// Related patches for Chromium compatibility:
// //third_party/mediapipe/patches/replace-absl-log-usages.patch
...
// clang-format off
// CHROMIUM_SRC OVERRIDE: Fixed upstream syntax error - removed trailing comma
...
PackWeightsCache::~PackWeightsCache() {
  xnn_weights_cache = nullptr;
}
...
absl::Status PackWeightsCache::Finalize() {
  ...
  // CHROMIUM_SRC OVERRIDE: Fixed FlatBuffers usage - set size during creation
  // instead of trying to mutate afterwards (which doesn't work)
  size_t estimated_size = builder_->GetSize() + 1000;  // Rough estimate
  while (estimated_size % 64 != 0) {
    estimated_size += 1;
  }
  const size_t flatbuffer_size = estimated_size;

  auto named_buffers = CreateNamedBuffers(
      *builder_, builder_->CreateVector(buffers), flatbuffer_size);
  ...
  // Ensure 64 aligned.
  while (serialized.size() % 64 != 0) {
    serialized += '\0';
  }

  // CHROMIUM_SRC OVERRIDE: Upstream tries to call GetMutableNamedBuffers() and
  // mutate_flatbuffer_size() which don't exist. FlatBuffers are immutable after
  // creation, so we set the flatbuffer_size during creation above instead.
  // Similar compilation fixes in:
  // //third_party/mediapipe/patches/compile-with-gpu-on-windows.patch
...
bool PackWeightsCache::ShouldDoubleCheckCompatibility(
  ...
  if (key_sent_for_double_check_.has_value()) {
    return false;
  }
