...
export function NetworkRequestNodePatch(NetworkRequestNode: NetworkRequestNodeType): any {
    return class extends NetworkRequestNode {
        override updateBackgroundColor(): void {
            ...
            if (networkManager && networkManager.dispatcher) {
                const info = (networkManager.dispatcher as any).adblockInfoForId(this.request().requestId());
