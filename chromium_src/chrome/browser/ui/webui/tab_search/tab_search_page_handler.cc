  ...
  if (!ai_chat_engine_) {
    ...
    ai_chat_engine_ =
        ai_chat_service->IsPremiumStatus()
            ...
            : ai_chat_service->GetEngineForModel(ai_chat::kClaudeHaikuModelKey);
  ...
  for (const auto& window : profile_data->windows) {
    for (const auto& tab : window->tabs) {
      ...
      tabs.push_back(ai_chat::Tab(base::NumberToString(tab->tab_id), tab->title,
                                  url::Origin::Create(tab->url).Serialize()));
    }
  }
...
tab_search::mojom::ErrorPtr TabSearchPageHandler::GetError(
  ...
  error->message = l10n_util::GetStringUTF8(IDS_CHAT_UI_ERROR_GENERAL);
  return error;
}
...
void TabSearchPageHandler::OnGetFocusTabs(
  ...
  Browser* new_browser =
      Browser::Create(Browser::CreateParams(Profile::FromWebUI(web_ui_), true));
