  ...
  handler()->GetSuggestedTopics(
      base::BindLambdaForTesting([&](const std::vector<std::string>& topics,
        ...
        EXPECT_EQ(
            error,
            tab_search::mojom::Error::New(
                l10n_util::GetStringUTF8(IDS_CHAT_UI_ERROR_GENERAL), nullptr));
      }));
...
TEST_F(TabSearchPageHandlerTest, UndoFocusTabs) {
  ...
  // Close tab_id6 to have a non-exisitng tab ID inside the response, for
  // example, tab is closed in the new window.
