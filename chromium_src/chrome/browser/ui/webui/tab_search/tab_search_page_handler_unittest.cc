...
// constexpr char kFooDotCom[] = "https://foo.com";
constexpr char kBarDotCom[] = "https://bar.com";
constexpr char kCatDotCom[] = "https://cat.com";
...
TEST_F(TabSearchPageHandlerTest, GetSuggestedTopics) {
  ...
  const int tab_id1 = browser1()->tab_strip_model()->GetTabAtIndex(0)->GetHandle().raw_value();
  const int tab_id2 = browser1()->tab_strip_model()->GetTabAtIndex(1)->GetHandle().raw_value();
  const int tab_id3 = browser2()->tab_strip_model()->GetTabAtIndex(0)->GetHandle().raw_value();

  std::vector<ai_chat::Tab> expected_tabs = {
      {base::NumberToString(tab_id1), kCatDotComTitle2, kCatDotCom},
      {base::NumberToString(tab_id2), kCatDotComTitle1, kCatDotCom},
      {base::NumberToString(tab_id3), kBarDotComTitle2, kBarDotCom},
  };

  std::vector<std::string> expected_topics = {"topic1", "topic2", "topic3", "topic4", "topic5"};
  auto* mock_engine = static_cast<ai_chat::MockEngineConsumer*>(
      handler()->GetAIChatEngineForTesting());
  EXPECT_CALL(*mock_engine,
              GetSuggestedTopics(testing::UnorderedElementsAreArray(expected_tabs), _))
  ...
  handler()->GetSuggestedTopics(base::BindLambdaForTesting(
      [&](const std::vector<std::string>& topics) {
        EXPECT_EQ(topics, expected_topics);
      }));

  // Test private window.
  // Test Tor window. (Or just private is enough?)
}
