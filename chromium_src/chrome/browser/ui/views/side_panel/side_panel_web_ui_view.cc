...
bool SidePanelWebUIView::HandleContextMenu(
  ...
  if (contents_wrapper_ &&
      contents_wrapper_->web_contents()->GetLastCommittedURL().host() ==
          kAIChatUIHost) {
    return false;  // Allow context menu
  }

  // For all other side panels, use the default behavior (disable context menu)
  return WebContentsDelegate::HandleContextMenu(render_frame_host, params);
}
