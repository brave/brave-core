...
// Add a member variable for brave_browser_provider_ so we can Init when
// it's time
#define local_test_provider_                                                  \
  local_test_provider_;                                                       \
  raw_ptr<brave_policy::BraveBrowserPolicyProvider> brave_browser_provider_ = \
      nullptr

#include <chrome/browser/policy/chrome_browser_policy_connector.h>  // IWYU pragma: export

#undef local_test_provider_
