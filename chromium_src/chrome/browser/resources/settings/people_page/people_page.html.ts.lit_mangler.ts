...
// We don't support Google sign-in/sync, so hiding the sync account control
// promo.
mangle((root) => {
  const syncAccountControl = root.querySelector('settings-sync-account-control')
  if (!syncAccountControl) {
    throw new Error(
      `[Settings] People page: couldn't find settings-sync-account-control`)
  }
  syncAccountControl.remove()
}, (t) => t.text.includes('<settings-sync-account-control'))
