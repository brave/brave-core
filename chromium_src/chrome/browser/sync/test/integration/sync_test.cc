...
#define BRAVE_GET_TIMESTAMP_URL \
  profile_sync_service->GetSyncServiceUrlForDebugging().Resolve("v2/timestamp")

#define BRAVE_INITIALIZE_PROFILE                         \
  test_url_loader_factory_.AddResponse(                  \
      BRAVE_GET_TIMESTAMP_URL.spec(),                    \
      R"({"timestamp": "123456", "expires_in": 3600})"); \
...
#define BRAVE_SET_OAUTH2_TOKEN_RESPONSE_2 \
  BRAVE_GET_TIMESTAMP_URL, std::move(response_head),
