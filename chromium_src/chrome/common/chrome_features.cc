...
OVERRIDE_FEATURE_DEFAULT_STATES({{
    ...
    // Actor: Show handoff button when user takes control until we implement
    // a resume mechanism in the AI Chat UI itself, if this flag ever gets
    // removed and is in the "enabled" state. Whilst it's in disabled state,
    // a "resume control" button is shown on the Tab.
