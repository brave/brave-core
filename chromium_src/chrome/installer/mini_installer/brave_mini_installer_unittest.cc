...
TEST_F(BraveMiniInstallerTest, HasStandardReferralCode) {
  ReferralCodeString referral_code;
  EXPECT_TRUE(
      ParseReferralCode(L"BraveBrowserSetup-FOO123.exe", referral_code));
...
TEST_F(BraveMiniInstallerTest,
  ...
  EXPECT_TRUE(ParseReferralCode(
      L"c:/foo/bar/BraveBrowserSetup-FOO123   (1).exe", referral_code));
  EXPECT_STREQ(referral_code.get(), L"FOO123");
}
...
TEST_F(BraveMiniInstallerTest, HasInvalidStandardReferralCodeReversed) {
  ReferralCodeString referral_code;
  EXPECT_FALSE(
      ParseReferralCode(L"BraveBrowserSetup-123FOO.exe", referral_code));
}
...
TEST_F(BraveMiniInstallerTest, HasInvalidStandardReferralCodeTooManyDigits) {
  ReferralCodeString referral_code;
  EXPECT_FALSE(
      ParseReferralCode(L"BraveBrowserSetup-FOO1234.exe", referral_code));
}

TEST_F(BraveMiniInstallerTest, HasInvalidStandardReferralCodeTooFewDigits) {
  ReferralCodeString referral_code;
  EXPECT_FALSE(
      ParseReferralCode(L"BraveBrowserSetup-FOO12.exe", referral_code));
}

TEST_F(BraveMiniInstallerTest, HasInvalidStandardReferralCodeTooManyLetters) {
  ReferralCodeString referral_code;
  EXPECT_FALSE(
      ParseReferralCode(L"BraveBrowserSetup-FOOO123.exe", referral_code));
}

TEST_F(BraveMiniInstallerTest, HasInvalidStandardReferralCodeTooFewLetters) {
  ReferralCodeString referral_code;
  EXPECT_FALSE(
      ParseReferralCode(L"BraveBrowserSetup-FO123.exe", referral_code));
}

TEST_F(BraveMiniInstallerTest, HasExtendedReferralCode) {
  ReferralCodeString referral_code;
  EXPECT_TRUE(
      ParseReferralCode(L"BraveBrowserSetup-extended-code.exe", referral_code));
...
TEST_F(BraveMiniInstallerTest,
  ...
  ReferralCodeString referral_code;
  EXPECT_FALSE(ParseReferralCode(L"BraveBrowserSetup-invalid-extended-c0de.exe",
                                 referral_code));
}
