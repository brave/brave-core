...
#define observers_ observers_; \
  std::vector<std::unique_ptr<web::WebUIIOS>> brave_web_uis_

#define CancelDialogs CancelDialogs(); \
    void TearDownBraveWebUI(); \
    void CreateBraveWebUI(const GURL& url); \
    bool HasBraveWebUI() const;  \
    void HandleBraveWebUIMessage(const GURL& source_url, std::string_view message, const base::Value::List& args); \
