...
void MojoFacade::OnWatcherCallback_BraveImpl(int callback_id,
  ...
  // There's no frame with that ID. Only other frame is the main-frame
  // This is the default behaviour in Chromium
  if (!frame) {
    frame = web_state_->GetPageWorldWebFramesManager()->GetMainWebFrame();
  }
