...
// routing in speech_recognition_manager_impl.cc handed this engine to
// BraveOnDeviceSpeechRecognitionEngine. Asks the one definition both layers
// share, so the Core is skipped for exactly the sessions Brave serves. False
// where local AI is not built, since there is then no model to serve anything.
