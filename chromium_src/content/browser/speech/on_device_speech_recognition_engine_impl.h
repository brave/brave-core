...
// Inject `friend class BraveSpeechRecognitionEngine;` into
// `OnDeviceSpeechRecognitionEngine` so our subclass can touch the
// base's private `asr_stream_` and `asr_stream_responder_` members
// when binding them against Brave's controller-owned session.
