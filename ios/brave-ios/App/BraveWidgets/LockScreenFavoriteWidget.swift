      ...
      if let fav = entry.favorite {
        Group {
          if let attributes = fav.favicon, let image = attributes.image {
            // includePadding forced to false here since we are providing our own padding below
            FaviconImage(image: image, contentMode: .scaleAspectFit, includePadding: false)
              .background(
                attributes.backgroundColor.cgColor.alpha == 0
                  ? .white : Color(attributes.backgroundColor)
              )
