...
/// A controller that plays the "Set Brave as Default" walkthrough video in a
/// picture-in-picture session so that it stays visible while the user is in the
/// system Settings app.
@MainActor
public final class DefaultBrowserPictureInPictureController: NSObject,
  AVPictureInPictureControllerDelegate
{
  ...
  /// Presents the bundled "Set Brave as Default" video in a picture-in-picture session
  /// within the given window scene. Does nothing if PiP is unsupported or the session
  /// fails to start.
  public static func present(in windowScene: UIWindowScene) async {
    let colorScheme = windowScene.traitCollection.userInterfaceStyle == .dark ? "dark" : "light"
