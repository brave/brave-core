...
public final class BlockedResource: NSManagedObject, CRUD {
  ...
  public static func mostBlockedTracker(
    inLastDays days: Int?,
    completion: @escaping (CountableEntity?) -> Void
  ) {
    ...
    DataController.perform { context in
      ...
      do {
        ...
        for result in results {
          ...
          let result = try distinctValues(
            property: hostKeyPath,
  ...
  public static func allTimeMostFrequentTrackers(
    completion: @escaping (Set<CountableEntity>) -> Void
  ) {
    ...
    DataController.perform { context in
      ...
      do {
        ...
        for result in results {
          ...
          // Finding on how many websites a given tracker was found.
          // Due to CD limitations this can't be fetched with a single query.
          let shieldsCount = try distinctValues(
            property: hostKeyPath,
            propertyToFetch: domainKeyPath,
            value: host,
            daysRange: nil,
  ...
  /// If the `days` parameter is nil, all data is considered including consolidated records.
  /// Returns nil in case of error or if nothing was found.
  public static func riskiestWebsite(
    inLastDays days: Int?,
    completion: @escaping (CountableEntity?) -> Void
  ) {
  ...
  public static func allTimeMostRiskyWebsites(
    completion:
    ...
    DataController.perform { context in
      ...
      do {
        ...
        expression.expression = .init(
          ...
          arguments: [NSExpression(forKeyPath: "faviconUrl")]
        )
        ...
        for result in results {
          guard let domain = result[domainKeyPath] as? String,
  ...
  public static func consolidateData(olderThan days: Int) {
    ...
    DataController.perform { context in
      ...
      let predicate = NSPredicate(
        format: "\(timestampKeyPath) <= %@ AND \(timestampKeyPath) != nil",
      ...
      oldItems?.forEach {
        set.insert(
          DomainTrackerPair(domain: $0.domain, tracker: $0.host, faviconUrl: $0.faviconUrl)
        )
      }
      ...
      set.forEach {
        let predicate =
          NSPredicate(
            format: "\(domainKeyPath) == %@ AND \(hostKeyPath) == %@ AND \(timestampKeyPath) = nil",
            $0.domain,
            $0.tracker
          )
  ...
  private static func groupByFetch(
    property: String,
    daysRange days: Int?,
  ...
  private static func distinctValues(
    ...
    if let days = days {
      predicate = NSPredicate(
        format: "\(timestampKeyPath) >= %@ AND \(property) == %@",
        getDate(-days) as CVarArg,
        value
      )
    } else {
