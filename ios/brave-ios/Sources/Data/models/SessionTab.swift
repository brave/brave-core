...
public final class SessionTab: NSManagedObject, CRUD {
  ...
  public init(
    ...
    if let sessionTabGroup = sessionTabGroup,
      sessionTabGroup.sessionWindow?.windowId != sessionWindow.windowId
    {
      fatalError(
        "Cannot add a tab to a different window and group. The group must belong to the same window as the tab"
      )
    }
...
extension SessionTab {
  ...
  public static func all(noOlderThan timeInterval: TimeInterval) -> [SessionTab] {
    ...
    let sortDescriptors = [NSSortDescriptor(key: #keyPath(SessionTab.index), ascending: true)]
    let predicate = NSPredicate(
      format: "\(lastUpdatedKeyPath) = nil OR \(lastUpdatedKeyPath) > %@",
      date
    )
  ...
  public static func createIfNeeded(
    ...
    DataController.perform { context in
      guard !SessionTab.exists(tabId: tabId, in: context),
        let window = SessionWindow.from(windowId: windowId, in: context)
      else { return }
