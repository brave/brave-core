...
class WebKitTabState: TabState, TabStateImpl {
  ...
  private func webViewURLDidChange(_ newURL: URL?) {
    ...
    if url?.origin == newURL?.origin {
      ...
      if isVisible, url?.displayURL?.scheme == "about", !isLoading {
        if let newURL {
          url = newURL
          notifyObservers = true
        }
  ...
  var currentUserAgentType: UserAgentType {
    if let urlString = url?.baseDomain, let override = userAgentOverrides[urlString] {
      return override
    }
  ...
  func reloadWithUserAgentType(_ userAgentType: UserAgentType) {
    if let urlString = url?.baseDomain {
