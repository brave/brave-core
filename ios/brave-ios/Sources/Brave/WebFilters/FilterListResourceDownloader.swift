...
public actor FilterListResourceDownloader {
  ...
  public func loadFilterListSettingsAndCachedData() async {
    ...
    async let startedCustomFilterListsDownloader: Void = FilterListCustomURLDownloader.shared
      .startIfNeeded()
  ...
  private func compileDefaultFilterList(resourcesInfo: CachedAdBlockEngine.ResourcesInfo) async {
    guard
      let defaultFilterListFolderURL = await FilterListSetting.makeFolderURL(
        forComponentFolderPath: Preferences.AppState.lastFilterListCatalogueComponentFolderPath
          .value
    ...
    await compileEngine(
      filterListFolderURL: defaultFilterListFolderURL,
      resourcesInfo: resourcesInfo,
      engineSource: .adBlock,
    ...
    Task { @MainActor in
      for await folderURL in adBlockService.defaultComponentStream() {
        ...
        if let resourcesInfo = await AdBlockStats.shared.resourcesInfo {
          await compileEngine(
            filterListFolderURL: folderURL,
            resourcesInfo: resourcesInfo,
            engineSource: .adBlock,
            isAlwaysAggressive: false,
            // This is set to false because we unfortunately have to use the slim list version of
            // these lists
            compileContentBlockers: false
          )
        }
  ...
  private func compileFilterListEngineIfNeeded(
    fromComponentId componentId: String,
    folderURL: URL,
    isAlwaysAggressive: Bool,
    resourcesInfo: CachedAdBlockEngine.ResourcesInfo,
    compileContentBlockers: Bool
    ...
    adBlockServiceTasks[filterList.entry.componentId] = Task { @MainActor in
      for await folderURL in adBlockService.register(filterList: filterList) {
        ...
        await self.loadShields(
          fromComponentId: filterList.entry.componentId,
          folderURL: folderURL,
          relativeOrder: filterList.order,
  ...
  private func loadShields(
    ...
    await self.compileFilterListEngineIfNeeded(
      ...
      isAlwaysAggressive: isAlwaysAggressive,
      resourcesInfo: resourcesInfo,
      compileContentBlockers: loadContentBlockers
    )
  }
}
...
extension AdblockService {
  @MainActor fileprivate func defaultComponentStream() -> AsyncStream<URL?> {
