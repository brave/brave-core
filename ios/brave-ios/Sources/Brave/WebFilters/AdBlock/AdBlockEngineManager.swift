...
@MainActor class AdBlockEngineManager {
  ...
  func update(resourcesInfo: GroupedAdBlockEngine.ResourcesInfo) {
    do {
      try engine?.useResources(from: resourcesInfo)
    } catch {
      ContentBlockerManager.log.error(
  ...
  private func cache(engine: GroupedAdBlockEngine) {
    ...
    do {
      ...
      let serializedEngine = try engine.serialize()
