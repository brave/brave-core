...
class ContentBlockerHelper {
  ...
  @MainActor func set(ruleLists: Set<WKContentRuleList>) {
    ...
    for ruleList in setRuleLists.subtracting(ruleLists) {
      ...
      tab?.webView?.underlyingWebView?.configuration.userContentController.remove(ruleList)
    ...
    for ruleList in ruleLists.subtracting(setRuleLists) {
      tab?.webView?.underlyingWebView?.configuration.userContentController.add(ruleList)
