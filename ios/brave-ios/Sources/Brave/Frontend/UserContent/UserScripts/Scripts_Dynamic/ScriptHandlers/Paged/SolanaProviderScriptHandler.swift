...
class SolanaProviderScriptHandler: TabContentScript {
  ...
  @MainActor func connect(tab: TabState, args: String?) async -> (Any?, String?) {
  ...
  @MainActor func signAndSendTransaction(tab: TabState, args: String?) async -> (Any?, String?) {
    guard let args = args,
  ...
  /// or an error dictionary for failures
  @MainActor func signMessage(tab: TabState, args: String?) async -> (Any?, String?) {
    guard let args = args,
  ...
  @MainActor func request(tab: TabState, args: String?) async -> (Any?, String?) {
    guard let args = args,
  ...
  @MainActor func signTransaction(tab: TabState, args: String?) async -> (Any?, String?) {
    guard let args = args,
      let arguments = MojoBase.Value(jsonString: args)?.dictionaryValue,
      let serializedMessage = arguments[Keys.serializedMessage.rawValue],
      let signatures = arguments[Keys.signatures.rawValue],
  ...
  /// Given args `[{serializedMessage: Buffer, signatures: {publicKey: String, signature: Buffer}}]`,
  /// will encoded the response as a json object for success or provide an error dictionary for failures
  @MainActor func signAllTransactions(tab: TabState, args: String?) async -> (Any?, String?) {
    guard let args = args,
  ...
  @MainActor private func emitConnectEvent(tab: TabState, publicKey: String) async {
    let script =
