...
class PlaylistScriptHandler: NSObject, TabContentScript, TabObserver {
  ...
  private class func processPlaylistInfo(
    ...
    // The script reports `window.top.location.href` for same-origin frames (which already
    // equals `tab.visibleURL`) and falls back to the iframe's own URL when blocked by SOP.
    // Only override in that cross-origin case so the stored `pageSrc` matches the page the
    // user thinks they added the item from, instead of an opaque embedder URL.
    ...
    // Drop captures whose `pageSrc` is structurally non-resolvable (e.g. YouTube homepage,
    // search results, feed). The detected blob `src` is bound to the live MediaSource in
    // this tab and cannot be re-extracted later by re-loading `pageSrc` in a fresh webview;
    // persisting such an item just gives the user a permanent "problem loading the resource!"
    // alert when they try to play it.
  ...
  /// Returns `true` when `pageSrc` is the kind of URL we can reasonably re-resolve later by
  /// re-loading it in a `LivePlaylistWebLoader`. For known sites whose feed/home/search pages
  /// have no stable `<video>` element to extract from, returns `false` so we don't capture
  /// items that are guaranteed to fail resolution. Unknown sites fall through and are
  /// trusted; we only special-case sites we have evidence for.
