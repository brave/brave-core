    ...
    do {
      ...
      Task { @MainActor in
        if let pageData = tab.currentPageData {
          ...
          let isAdblockModeAggressive: Bool
          if BraveShields.isUseContentSettingsForShieldsEnabled {
            let isBraveShieldsEnabled = braveShieldsUtils.isBraveShieldsEnabled(for: frameURL)
            let adBlockMode = braveShieldsUtils.adBlockMode(for: frameURL)
            isAdBlockEnabled = isBraveShieldsEnabled && adBlockMode != .allow
            isAdblockModeAggressive = isBraveShieldsEnabled && adBlockMode == .aggressive
          } else {
            let domain = pageData.domain(persistent: !tab.isPrivate)
            isAdBlockEnabled = domain.globalBlockAdsAndTrackingLevel.isEnabled
            isAdblockModeAggressive = domain.globalBlockAdsAndTrackingLevel.isAggressive
          }
          guard isAdBlockEnabled,
          ...
          let models = await AdBlockGroupsManager.shared.cosmeticFilterModels(
            ...
            isAdBlockEnabledForURL: isAdBlockEnabled
          )
          ...
          let setup = UserScriptType.ContentCosmeticSetup.makeSetup(
            from: models,
            isAggressive: isAdblockModeAggressive,
