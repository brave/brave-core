...
try {
{
  const args = ["playerResponse.adPlacements playerResponse.playerAds playerResponse.adSlots adPlacements playerAds adSlots important", "{{2}}", "{{3}}", "{{4}}", "{{5}}", "{{6}}", "{{7}}", "{{8}}", "{{9}}"];
...
try {
{
  const args = ["overlay", "overlay.shortsAdsRenderer", "propsToMatch", "url:/reel_item_watch?", "{{5}}", "{{6}}", "{{7}}", "{{8}}", "{{9}}"];
  let last_arg_index = 0;
  for (const arg_index in args) {
    if (args[arg_index] === '{{' + (Number(arg_index) + 1) + '}}') {
      break;
...
try {
{
  const args = ["playerResponse.playerConfig.ssapConfig playerResponse.streamingData.serverAbrStreamingUrl playerConfig.ssapConfig streamingData.serverAbrStreamingUrl", "{{2}}", "{{3}}", "{{4}}", "{{5}}", "{{6}}", "{{7}}", "{{8}}", "{{9}}"];
  let last_arg_index = 0;
  for (const arg_index in args) {
    if (args[arg_index] === '{{' + (Number(arg_index) + 1) + '}}') {
      break;
...
try {
{
  const args = ["script", "/^(\\(function serverContract\\(\\))/s", "/*start*/(function(){const wrapper=(target,thisArg,args)=>{let content=Reflect.apply(target,thisArg,args);if(content.includes(\"ssapPrerollEnabled\")){const modifiedContent=content.replace(/\\n.\\n.playerConfig\\.ssapConfig\\.ssapPrerollEnabled.{2}(?:tru|fals)e/,\"\");return modifiedContent;}return content;};const handler={apply:wrapper};window.atob=new Proxy(window.atob,handler);document.currentScript.textContent=document.currentScript.textContent.replace(/\\/\\*start\\*\\/(.*)\\/\\*end\\*\\//g,\"\");}());/*end*/$1", "{{4}}", "{{5}}", "{{6}}", "{{7}}", "{{8}}", "{{9}}"];
  let last_arg_index = 0;
  for (const arg_index in args) {
    if (args[arg_index] === '{{' + (Number(arg_index) + 1) + '}}') {
      break;
...
try {
{
  const args = ["script", "/^(\\(function serverContract\\(\\))/s", "/*start*/(function(){const wrapper=(target,argumentsList,newTarget)=>{const result=Reflect.construct(target,argumentsList,newTarget);try{const decoded=(new TextDecoder).decode(result);if(decoded.includes(\"ssapPrerollEnabled\")){const modifiedContent=decoded.replace(/\\n.\\n.playerConfig\\.ssapConfig\\.ssapPrerollEnabled.{2}(?:tru|fals)e/,\"\");const modifiedUint8Array=(new TextEncoder).encode(modifiedContent);return modifiedUint8Array}}catch(e){}return result};const handler={construct:wrapper};window.Uint8Array=new Proxy(Uint8Array,handler);document.currentScript.textContent=document.currentScript.textContent.replace(/\\/\\*start\\*\\/(.*)\\/\\*end\\*\\//g,\"\")})();/*end*/$1", "{{4}}", "{{5}}", "{{6}}", "{{7}}", "{{8}}", "{{9}}"];
  let last_arg_index = 0;
  for (const arg_index in args) {
    if (args[arg_index] === '{{' + (Number(arg_index) + 1) + '}}') {
      break;
...
try {
{
  const args = ["ytInitialPlayerResponse.streamingData.serverAbrStreamingUrl", "undefined", "{{3}}", "{{4}}", "{{5}}", "{{6}}", "{{7}}", "{{8}}", "{{9}}"];
  let last_arg_index = 0;
  for (const arg_index in args) {
    if (args[arg_index] === '{{' + (Number(arg_index) + 1) + '}}') {
      break;
...
try {
{
  const args = ["reelWatchSequenceResponse.entries.[-].command.reelWatchEndpoint.adClientParams.isAd entries.[-].command.reelWatchEndpoint.adClientParams.isAd", "", "propsToMatch", "url:/reel_watch_sequence?", "{{5}}", "{{6}}", "{{7}}", "{{8}}", "{{9}}"];
  let last_arg_index = 0;
  for (const arg_index in args) {
    if (args[arg_index] === '{{' + (Number(arg_index) + 1) + '}}') {
      break;
...
try {
{
  const args = ["ytInitialPlayerResponse.playerConfig.ssapConfig", "undefined", "{{3}}", "{{4}}", "{{5}}", "{{6}}", "{{7}}", "{{8}}", "{{9}}"];
  let last_arg_index = 0;
  for (const arg_index in args) {
    if (args[arg_index] === '{{' + (Number(arg_index) + 1) + '}}') {
      break;
