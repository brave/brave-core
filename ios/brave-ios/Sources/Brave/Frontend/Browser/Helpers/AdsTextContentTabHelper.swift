...
class AdsTextContentTabHelper: TabObserver, TabPolicyDecider {
  ...
  /// Bumped on every `tabDidStartNavigation`. Lets a pending `reportTextContentDidChange` task
  /// detect that a new navigation started while it was awaiting `evaluateJavaScript`, so it
  /// doesn't pair freshly-distilled text from the new document with the previous navigation's
  /// `redirectChain`.
  ...
  func tab(
    ...
    if requestInfo.isMainFrame {
      // Mirrors `ui::PageTransitionIsNewNavigation`, used by
      // `AdsTabHelper::MaybeNotifyTabTextContentDidChange` on the native path, which excludes
      // reload and back/forward but treats a resubmitted form as a new navigation.
      isNewNavigation = ![.reload, .backForward].contains(requestInfo.navigationType)
    }
    return .allow
  }
  ...
  func tabDidStopLoading(_ tab: some TabState) {
    ...
    let navigationGeneration = navigationGeneration
    Task { [weak self, weak tab, redirectChain] in
      guard let self, let tab else { return }
      await reportTextContentDidChange(
        in: tab,
  ...
  private func reportTextContentDidChange(
    ...
    // A new navigation may have started while `evaluateJavaScript` above was suspended, in which
    // case the distilled text reflects the new document and must not be paired with this
    // navigation's `redirectChain`.
    guard navigationGeneration == self.navigationGeneration else {
      return
    }
