...
extension PlaylistListViewController {
  fileprivate func shareItem(_ item: PlaylistInfo, anchorView: UIView?) {
  ...
  fileprivate func cacheItem(
    ...
    case .downloaded:
      ...
      let alert = UIAlertController(
        ...
        message: Strings.PlayList.removePlaylistOfflineDataAlertMessage,
        preferredStyle: style
      )
  ...
  fileprivate func deleteItem(itemId: String, indexPath: IndexPath) {
    let style: UIAlertController.Style =
      UIDevice.current.userInterfaceIdiom == .pad ? .alert : .actionSheet
    let alert = UIAlertController(
      title: Strings.PlayList.removePlaylistVideoAlertTitle,
      message: Strings.PlayList.removePlaylistVideoAlertMessage,
      preferredStyle: style
    )
  ...
  fileprivate func openInPrivateTabWithAuthentication(_ item: PlaylistInfo) {
...
extension PlaylistListViewController: UITableViewDelegate {
  ...
  func tableView(
    ...
    let shareAction = UIContextualAction(
      ...
      handler: { [weak self] (action, view, completionHandler) in
        ...
        if !isSharedFolder {
          alert.addAction(
            UIAlertAction(
              title: Strings.PlayList.sharePlaylistMoveActionMenuTitle,
              style: .default,
              handler: { [weak self] _ in
    ...
    cacheAction.image =
      cacheState == .invalid
      ? UIImage(braveSystemNamed: "leo.cloud.download")!
      : UIImage(braveSystemNamed: "leo.cloud.off")!
            ...
            children: {
              ...
              if PlaylistFolder.getOtherFoldersCount() > 0 {
                actions.append(
                  UIAction(
                    title: Strings.PlayList.sharePlaylistMoveActionMenuTitle,
                    image: UIImage(braveSystemNamed: "leo.folder"),
                    handler: { [weak self] _ in
                      self?.moveItems(indexPaths: [indexPath])
                    }
                  )
