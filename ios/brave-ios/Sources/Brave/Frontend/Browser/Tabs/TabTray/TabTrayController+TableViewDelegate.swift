...
extension TabTrayController: UITableViewDataSource, UITableViewDelegate {
  ...
  private func configureCell(_ cell: UITableViewCell, atIndexPath indexPath: IndexPath) {
    ...
    cell.imageIconView.do {
      ...
      $0.loadFavicon(
        for: distantTab.url,
        isPrivateBrowsing: tabManager.privateBrowsingManager.isPrivateBrowsing
      )
    }
  }
  ...
  func tableView(
    ...
    return UIContextMenuConfiguration(identifier: indexPath as NSCopying, previewProvider: nil) {
      ...
      return UIMenu(
        title: distantTab.url.absoluteString,
        identifier: nil,
...
extension Date {
  ...
  var formattedSyncSessionPeriodDate: String {
    ...
    {
      return String(
        format: Strings.OpenTabs.openTabsItemLastSyncedTodayTitle,
    ...
    {
      return String(
        format: Strings.OpenTabs.openTabsItemLastSyncedYesterdayTitle,
        hourFormatter.string(from: self)
      )
    ...
    {
      return String(
        format: Strings.OpenTabs.openTabsItemLastSyncedLastWeekTitle,
        hourDayFormatter.string(from: self)
      )
    }
