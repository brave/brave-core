...
struct ClearDataSectionView: View {
  ...
  private func clearPrivateData() {
    Preferences.Privacy.clearPrivateDataToggles.value = settings.clearableSettings.map({
      $0.isEnabled
    })

    Task { @MainActor in
      await settings.clearPrivateData(
        settings.clearableSettings.compactMap({
