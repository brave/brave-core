...
class SettingsViewController: TableViewController, BraveAccountAuthenticationObserver {
  ...
  private func resendConfirmationEmailAlertMessage(
    ...
    switch failure.tag {
    case .clientError:
      let errorCode = failure.clientError?.errorCode.rawValue ?? 0
      ...
      return L10nUtils.formatString(
        ...
        argument: " (\(errorLabel)=\(errorCode))"
      )
    case .serverError:
      guard let serverError = failure.serverError else {
        return L10nUtils.string(messageId: .BRAVE_ACCOUNT_SERVER_ERROR)
      }
      switch serverError.errorCode {
      case .maximumEmailSendAttemptsExceeded:
        return L10nUtils.string(
          messageId: .BRAVE_ACCOUNT_RESEND_CONFIRMATION_EMAIL_MAXIMUM_SEND_ATTEMPTS_EXCEEDED
        )
      case .emailAlreadyVerified:
        return L10nUtils.string(
          messageId: .BRAVE_ACCOUNT_RESEND_CONFIRMATION_EMAIL_ALREADY_VERIFIED
        )
      default:
        let netErrorOrHttpStatus = serverError.netErrorOrHttpStatus
        let scheme = netErrorOrHttpStatus > 0 ? "HTTP" : "NET"
        var argument = "\(scheme)=\(netErrorOrHttpStatus)"
        if serverError.errorCode != .null {
          let errorLabel = L10nUtils.string(messageId: .BRAVE_ACCOUNT_ERROR)
          argument += ", \(errorLabel)=\(serverError.errorCode.rawValue)"
        }
        return L10nUtils.formatString(
