...
class RewardsInternalsContributionListController: TableViewController {
      ...
      .init(
        ...
        rows: [
          ...
          Row(
            ...
            detailText:
              "\(batFormatter.string(from: NSNumber(value: cont.amount)) ?? "0.0") \(Strings.batSymbol)"
          ),
          ...
          cont.publishers.count > 1
            ? Row(
              ...
              selection: { [unowned self] in
                let controller = RewardsInternalsContributionPublishersListController(
                  publishers: cont.publishers
                )
  ...
  @objc private func tappedShare() {
    let controller = RewardsInternalsShareController(
      ...
      initiallySelectedSharables: [.contributions]
    )
