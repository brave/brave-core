...
class ShieldsSettingsViewModel: ObservableObject {
  ...
  init(tab: some TabState, domain: Domain, url: URL, braveShieldsUtils: BraveShieldsUtilsIOS) {
    ...
    if BraveShields.isUseContentSettingsForShieldsEnabled {
      ...
      let adBlockMode = braveShieldsUtils.adBlockMode(for: url)
      switch adBlockMode {
      case .aggressive: self.blockAdsAndTrackingLevel = .aggressive
      case .standard: self.blockAdsAndTrackingLevel = .standard
      case .allow: self.blockAdsAndTrackingLevel = .disabled
      @unknown default: self.blockAdsAndTrackingLevel = .standard
      }
      self.blockScripts = braveShieldsUtils.isBlockScriptsEnabled(for: url)
      self.fingerprintProtection = braveShieldsUtils.isBlockFingerprintingEnabled(for: url)
      self.stats = .init()
    } else {
