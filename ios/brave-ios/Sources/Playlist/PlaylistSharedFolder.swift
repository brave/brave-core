...
public struct PlaylistSharedFolderNetwork {
  ...
  public static func fetchPlaylist(folderUrl: String) async throws -> PlaylistSharedFolderModel {
    guard
      let playlistURL = URL(string: folderUrl)?.appendingPathComponent("playlist")
  ...
  @MainActor
  public static func createInMemoryStorage(
    for model: PlaylistSharedFolderModel
  ) async -> PlaylistFolder {
    await withCheckedContinuation { continuation in
      // Create a local shared folder
      PlaylistFolder.addInMemoryFolder(
  ...
  public static func saveToDiskStorage(memoryFolder: PlaylistFolder) async -> String {
    await withCheckedContinuation({ continuation in
      PlaylistFolder.saveInMemoryFolderToDisk(folder: memoryFolder) { folderId in
        PlaylistItem.saveInMemoryItemsToDisk(
          items: Array(memoryFolder.playlistItems ?? []),
          folderUUID: folderId
        ) {
  ...
  public static func fetchMediaItemInfo(
    item: PlaylistSharedFolderModel,
    viewForInvisibleWebView: UIView,
    webLoaderFactory: any PlaylistWebLoaderFactory
    ...
    return try await withThrowingTaskGroup(of: PlaylistInfo.self, returning: [PlaylistInfo].self) {
      group in
