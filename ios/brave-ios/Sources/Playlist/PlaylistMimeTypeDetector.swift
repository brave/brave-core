...
public class PlaylistMimeTypeDetector {
  ...
  init(url: URL) {
    ...
    } else if let fileExtension = PlaylistMimeTypeDetector.supportedAVAssetFileExtensions().first(
      where: { $0.lowercased() == possibleFileExtension })
    {
  ...
  init(mimeType: String) {
    if let fileExtension = mimeTypeMap[mimeType.lowercased()] {
      ...
      self.fileExtension = fileExtension
    } else if let mimeType = PlaylistMimeTypeDetector.supportedAVAssetMimeTypes().first(where: {
  ...
  init(data: Data) {
    ...
    if findHeader(offset: 4, data: data, header: [0x66, 0x74, 0x79, 0x70, 0x4D, 0x53, 0x4E, 0x56])
      ...
      || findHeader(
        offset: 4,
        data: data,
        header: [0x66, 0x74, 0x79, 0x70, 0x6D, 0x70, 0x34, 0x32]
    ...
    if findHeader(
      offset: 0,
      data: data,
      header: [0x30, 0x26, 0xB2, 0x75, 0x8E, 0x66, 0xCF, 0x11, 0xA6, 0xD9]
    ) {
  ...
  private static func supportedAVAssetFileExtensions() -> [String] {
    ...
    return types.compactMap({ UTType($0.rawValue)?.preferredFilenameExtension }).filter({
      !$0.isEmpty
    })
  }
