...
@MainActor class TopBarToolbarVisibilityViewModelTests: XCTestCase {
  ...
  func testSmallContentNoCollapseAllowed() {
    ...
    viewModel.send(
      action: .dragged(snapshot: snapshot, panData: .init(yTranslation: -128, yVelocity: 0))
    )
    XCTAssertEqual(viewModel.toolbarState, .expanded)
    XCTAssertNil(viewModel.interactiveTransitionProgress)
  }
  ...
  func testCustomMinimumContentHeightRequirementToCollapse() {
    ...
    XCTAssertNil(viewModel.interactiveTransitionProgress)
    viewModel.send(
      action: .endedDrag(snapshot: snapshot, panData: .init(yTranslation: -100, yVelocity: -1000))
    )
    XCTAssertEqual(viewModel.toolbarState, .expanded)
  ...
  func testExpandedToCollapsedFromTop() {
    ...
    viewModel.send(
      action: .dragged(snapshot: snapshot, panData: .init(yTranslation: -75, yVelocity: 0))
    )
  ...
  func testRubberBandingBottomEdge() {
    ...
    snapshot.isDecelerating = true
    viewModel.send(
      action: .endedDrag(snapshot: snapshot, panData: .init(yTranslation: -50, yVelocity: -250))
    )
    XCTAssertNil(viewModel.interactiveTransitionProgress)
    XCTAssertEqual(viewModel.toolbarState, .expanded)
  }
  ...
  func testZoomingSmallPage() {
    ...
    viewModel.send(
      action: .dragged(snapshot: snapshot, panData: .init(yTranslation: -150, yVelocity: 0))
    )
    viewModel.send(
      action: .endedDrag(snapshot: snapshot, panData: .init(yTranslation: -150, yVelocity: 0))
    )
