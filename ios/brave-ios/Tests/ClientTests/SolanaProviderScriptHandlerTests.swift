...
@MainActor class SolanaProviderScriptHandlerTests: XCTestCase {
  ...
  func testConnectFailure() async {
    ...
    guard let error = error, // dictionary's are unordered, need to check each value
          ...
          let code = errorDict["code"]?.intValue, let message = errorDict["message"]?.stringValue else {
  ...
  func testSignAndSendTransactionFailure() async {
    let argDict: [String: MojoBase.Value] = [
      "serializedMessage": MojoBase.Value(listValue: kSerializedMessage.map(MojoBase.Value.init(intValue:))),
  ...
  func testSignAndSendTransactionSuccess() async {
    ...
    let provider: BraveWallet.TestSolanaProvider = .init()
    provider._signAndSendTransaction = { [kTestPublicKey, kTestSignature] signTransactionParam, sendOptions, completion in
      completion(.success, "", [
  ...
  func testSignMessageSuccess() async {
    ...
    let provider: BraveWallet.TestSolanaProvider = .init()
    provider._signMessage = { [kTestPublicKey, kTestSignature] blobMsg, displayEncoding, completion in
      completion(.success, "", [
        "publicKey": MojoBase.Value(stringValue: kTestPublicKey),
        "signature": MojoBase.Value(stringValue: kTestSignature)
      ])
    ...
    let signature: [UInt8] = result["signature"]?.listValue?.map { $0.intValue }.map(UInt8.init) ?? []
    let expectedSignature: [UInt8] = (NSData(base58Encoded: kTestSignature) as? Data)?.getBytes() ?? []
  ...
  func testSignMessageWithDisplayEncodingSuccess() async {
    ...
    let provider: BraveWallet.TestSolanaProvider = .init()
    provider._signMessage = { [kTestPublicKey, kTestSignature] blobMsg, displayEncoding, completion in
      XCTAssertNotNil(displayEncoding) // displayEncoding provided as optional arg, should be non-nil
      completion(.success, "", [
        "publicKey": MojoBase.Value(stringValue: kTestPublicKey),
        "signature": MojoBase.Value(stringValue: kTestSignature)
      ])
  ...
  private let kTestSignature = "As4N6cok5f7nhXp56Hdw8dWZpUnY8zjYKzBqK45CexE1qNPCqt6Y2gnZduGgqASDD1c6QULBRy"
