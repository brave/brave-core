...
class NSURLExtensionsTests: XCTestCase {
  ...
  func testWidgetAttribution_replacesIosSourceOnBraveSearch() {
    let url = URL(string: "https://search.brave.com/search?q=test&source=ios")!
    XCTAssertEqual(
      url.updatingSourceWithWidgetAttribution().absoluteString,
      "https://search.brave.com/search?q=test&source=ios-widget"
    )
  }

  func testWidgetAttribution_isCaseInsensitiveOnHost() {
    let url = URL(string: "https://Search.Brave.com/search?q=test&source=ios")!
    let result = url.updatingSourceWithWidgetAttribution()
  ...
  func testWidgetAttribution_preservesQueryOrder() {
  ...
  func testWidgetAttribution_leavesAlreadyWidgetSourceUnchanged() {
    let url = URL(string: "https://search.brave.com/search?q=test&source=ios-widget")!
    XCTAssertEqual(url.updatingSourceWithWidgetAttribution(), url)
  }

  func testWidgetAttribution_leavesUrlWithoutSourceParamUnchanged() {
    let url = URL(string: "https://search.brave.com/search?q=test")!
    XCTAssertEqual(url.updatingSourceWithWidgetAttribution(), url)
  }

  func testWidgetAttribution_leavesOtherSourceValuesUnchanged() {
    let url = URL(string: "https://search.brave.com/search?q=test&source=spotlight")!
    XCTAssertEqual(url.updatingSourceWithWidgetAttribution(), url)
  }

  func testWidgetAttribution_leavesNonBraveHostsUnchanged() {
    let url = URL(string: "https://www.google.com/search?q=test&source=ios")!
    XCTAssertEqual(url.updatingSourceWithWidgetAttribution(), url)
  }

  func testWidgetAttribution_doesNotMatchSpoofedHosts() {
