...
class SettingsStoreTests: XCTestCase {
  ...
  private func setupServices() -> (
    BraveWallet.TestKeyringService, BraveWallet.TestBraveWalletService,
    BraveWallet.TestJsonRpcService, BraveWallet.TestTxService, IpfsAPI
  ) {
  ...
  func testSetup() {
    ...
    let ensOffchainResolveMethodExpectation = expectation(
      description: "setup-ensOffchainResolveMethod"
    )
  ...
  func testReset() {
    ...
    Preferences.Wallet.displayWeb3Notifications.value = !Preferences.Wallet.displayWeb3Notifications
      .defaultValue
    XCTAssertEqual(
