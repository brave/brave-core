...
class WalletAmountFormatterTests: XCTestCase {
  func testWeiToDecimal() throws {
    ...
    // Below 0
    XCTAssertEqual(
      try XCTUnwrap(formatter.decimalString(for: "50382080585020020", decimals: 18)),
      "0.05038"
    )
    // Different decimals
    XCTAssertEqual(
      try XCTUnwrap(formatter.decimalString(for: "50382080585020020", decimals: 10)),
      "5038208.05850"
    )
    // Hex
    XCTAssertEqual(
      try XCTUnwrap(formatter.decimalString(for: "16345785d8a0000", radix: .hex, decimals: 18)),
      "0.10000"
    )
  }
  func testWeiToBalance() throws {
    ...
    // Round down
    XCTAssertEqual(
      try XCTUnwrap(formatter.decimalString(for: "31050332080585020020", decimals: 18)),
      "31.0503"
    )
  }
  ...
  func testDecimalToWei() throws {
    ...
    XCTAssertEqual(
      try XCTUnwrap(formatter.weiString(from: 1.0, radix: .decimal, decimals: 18)),
      "1000000000000000000"
    )
    // Maximum number of digits
    XCTAssertEqual(
      try XCTUnwrap(
        formatter.weiString(from: "0.000000000000000001", radix: .decimal, decimals: 18)
      ),
      "1"
    )
    ...
    // Hex
    XCTAssertEqual(
      try XCTUnwrap(formatter.weiString(from: 0.1, radix: .hex, decimals: 18)),
      "16345785d8a0000"
    )
  }
