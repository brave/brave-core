...
@MainActor class NetworkSelectionStoreTests: XCTestCase {
  ...
  private func setupServices() -> (
    ...
    rpcService.allCustomNetworks = []
    rpcService.hiddenNetworks = []
  ...
  func testSetSelectedNetworkNoAccounts() async {
    ...
    XCTAssertFalse(
      success,
      "Expected failure for selecting Solana because we have no Solana accounts."
    )
    XCTAssertTrue(
      store.isPresentingNextNetworkAlert,
      "Expected to set isPresentingNextNetworkAlert to true to show alert asking user to create Solana account"
    )
  }
  ...
  func testAlertResponseCreateAccount() async {
    ...
    XCTAssertFalse(
      store.isPresentingNextNetworkAlert,
      "Expected to set isPresentingNextNetworkAlert to false to hide alert"
    )
    XCTAssertTrue(
      store.isPresentingAddAccount,
      "Expected to set isPresentingAddAccount to true to present add network"
    )
  }
  ...
  func testAlertResponseDontCreateAccount() async {
    ...
    XCTAssertNil(
      store.nextNetwork,
      "Expected to reset nextNetwork to nil as user does not want to create an account"
    )
  }
  ...
  func testDismissAddAccountAfterCreation() async {
    ...
    XCTAssertTrue(
      didSwitchToFilecoinMainnet,
      "Expected to switch networks as an account was created"
    )
  }
}
