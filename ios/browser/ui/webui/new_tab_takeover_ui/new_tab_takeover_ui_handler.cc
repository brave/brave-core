...
NewTabTakeoverUIHandler::NewTabTakeoverUIHandler(ntp_background_images::NTPBackgroundImagesService*
...
void NewTabTakeoverUIHandler::GetCurrentWallpaper(const std::string& creative_instance_id,
  ...
  LOG(ERROR) << "FOOBAR.GetCurrentWallpaper " << creative_instance_id;
  ...
  if (!sponsored_images_data) {
    LOG(ERROR) << "FOOBAR.GetCurrentWallpaper sponsored_images_data is null";
    return failed();
  }
  ...
  if (!creative) {
    LOG(ERROR) << "FOOBAR.GetCurrentWallpaper creative is null";
    return failed();
  }

  LOG(ERROR) << "FOOBAR.GetCurrentWallpaper creative->url " << creative->url.spec();
  LOG(ERROR) << "FOOBAR.GetCurrentWallpaper creative->should_metrics_fallback_to_p3a " << creative->should_metrics_fallback_to_p3a;
  LOG(ERROR) << "FOOBAR.GetCurrentWallpaper creative->logo.destination_url " << creative->logo.destination_url;
...
void NewTabTakeoverUIHandler::NavigateToUrl(const GURL& url) {
  //ntp_background_images_service_->NavigateToUrl(url);
}
