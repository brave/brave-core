...
AIChatRemoteModelsVisibilityObserver::~AIChatRemoteModelsVisibilityObserver() {
  // `WebStateDestroyed()` already handled this if the `WebState` went away
  // first; `web_state_` is null in that case, and touching it here would use
  // a `WebState` that is no longer guaranteed to be valid.
  if (web_state_) {
...
void AIChatRemoteModelsVisibilityObserver::WebStateDestroyed(
    web::WebState* web_state) {
  web_state_->RemoveObserver(this);
  web_state_ = nullptr;
  if (is_surface_visible_) {
