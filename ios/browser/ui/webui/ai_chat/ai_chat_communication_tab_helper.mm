...
AIChatCommunicationController* AIChatCommunicationTabHelper::GetController()
    const {
  return controller_;
}

void AIChatCommunicationTabHelper::SetController(
    AIChatCommunicationController* controller) {
...
+ (AIChatCommunicationController*)fromWebState:(web::WebState*)webState {
  if (auto* tab_helper = AIChatCommunicationTabHelper::FromWebState(webState)) {
    if (!tab_helper->GetController()) {
      tab_helper->SetController(
          [[AIChatCommunicationController alloc] initWithWebState:webState]);
    }
