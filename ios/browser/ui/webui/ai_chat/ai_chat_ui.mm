...
BraveWebUIIOSDataSource* CreateAndAddWebUIDataSource(web::WebUIIOS* web_ui,
                                                     const std::string& name,
                                                     base::span<const webui::ResourcePath> resource_paths,
                                                     int html_resource_id) {
  BraveWebUIIOSDataSource* source = static_cast<BraveWebUIIOSDataSource*>(BraveWebUIIOSDataSource::Create(name));
...
web::WebState* GetActiveWebState(web::WebUIIOS* web_ui) {
  ...
  for (Browser* browser : browser_list->BrowsersOfType(BrowserList::BrowserType::kRegular)) {
    web::WebState* active_web_state = browser->GetWebStateList()->GetActiveWebState();
    if (active_web_state) {
...
AIChatUI::AIChatUI(web::WebUIIOS* web_ui, const GURL& url)
    : web::WebUIIOSController(web_ui, url.host()), profile_(ProfileIOS::FromWebUIIOS(web_ui)) {
  DCHECK(profile_);
  //DCHECK(profile_->IsRegularProfile());
  DCHECK(!profile_->IsOffTheRecord());

  // Create a URLDataSource and add resources.
  BraveWebUIIOSDataSource* source = CreateAndAddWebUIDataSource(web_ui, url.host(), kAiChatUiGenerated, IDR_AI_CHAT_UI_HTML);
...
void AIChatUI::BindInterfaceTabTracker(
  ...
  auto* service =
      ai_chat::TabTrackerServiceFactory::GetForProfile(profile_);
  CHECK(service);
