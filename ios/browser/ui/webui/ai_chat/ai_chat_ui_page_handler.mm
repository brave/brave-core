...
#include "brave/ios/browser/api/ai_chat/core/browser/ai_chat_service.h"
...
class WaitForCommit : public web::WebStateObserver {
 public:
  WaitForCommit(
      ...
      : web::WebStateObserver(web_state), on_loaded_(std::move(on_loaded)) {}
  ~WaitForCommit() override = default;

  void DidFinishNavigation(web::WebState* web_state,
                           web::NavigationContext* navigation_context) override {
    if (handle->HasCommitted()) {  // TODO: Not sure how to check main frame here...
...
void EnsureWebStateLoaded(
  ...
  new WaitForCommit(web_state, std::move(on_loaded));   // Maybe we should use WebStateObserver::DidStartLoading to determine when the load has finished :S
//  contents->GetController().LoadIfNecessary();   // Can't be done on iOS???
}

web::WebState* GetTabFromId(int32_t tab_id) {
...
AIChatUIPageHandler::ChatContextObserver::ChatContextObserver(
    ...
    : web::WebStateObserver(web_state), page_handler_(page_handler) {}
...
//  ai_chat::HandleVoiceRecognition(owner_web_state_.get(), conversation_uuid);
}

void AIChatUIPageHandler::ShowSoftKeyboard() {
//  ai_chat::HandleShowSoftKeyboard(owner_web_state_.get());
}
...
void AIChatUIPageHandler::ChatContextObserver::WebContentsDestroyed(web::WebState* web_state) {
