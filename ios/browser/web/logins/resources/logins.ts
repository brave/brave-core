...
): [HTMLInputElement | null, HTMLInputElement | null, HTMLInputElement | null] {
...
function fillForm(
  ...
  if (
    usernameField
    && (usernameField.value || usernameField.disabled || usernameField.readOnly)
  ) {
  ...
  if (selectedLogin && autofillForm && !isFormDisabled) {
    if (usernameField) {
      ...
      const userNameDiffers = selectedLogin.username !== usernameField.value
      const userEnteredDifferentCase =
        userTriggered
        && userNameDiffers
...
async function asyncFindLogins(form: HTMLFormElement): Promise<LoginData[]> {
