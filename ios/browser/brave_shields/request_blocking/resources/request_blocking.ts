...
const sendMessage = (resourceURL: URL) => {
  ...
  const originDisplay =
    window.location.href !== window.origin ? ` (${window.origin})` : ``
  if (blockingCache.has(resourceURL.href)) {
    return Promise.resolve(blockingCache.get(resourceURL.href)!).then(
      (blocked) => {
        if (blocked) {
          console.info(
            `Brave prevented frame displaying ${window.location.href}${originDisplay} from loading a resource from ${resourceURL.href} (cached)`,
          )
        }
        return blocked
      },
  ...
  return sendWebKitMessageWithReply('RequestBlockingMessageHandler', {
    ...
    if (blocked) {
      console.info(
        `Brave prevented frame displaying ${window.location.href}${originDisplay} from loading a resource from ${resourceURL.href}`,
      )
    }
...
const patchProgressEvent = (progressEvent: ProgressEvent) => {
  Object.setPrototypeOf(
    progressEvent,
    (window as any).XMLHttpRequestProgressEvent.prototype,
  )
...
XMLHttpRequest.prototype.open = function (
  ...
  const [_method, url, isAsync] = args
