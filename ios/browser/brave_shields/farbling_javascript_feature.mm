...
void FarblingJavaScriptFeature::ScriptMessageReceivedWithReply(
  ...
  if (!request_url.is_valid() || !request_url.SchemeIsHTTPOrHTTPS() ||
  ...
  bool is_farbling_enabled = service->GetFingerprintMode(request_url) !=
  ...
  if (is_farbling_enabled) {
    ...
    const base::Token farbling_token = brave_shields::GetFarblingToken(
        map, request_url, /*additional_entropy=*/{});
