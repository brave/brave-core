 ...
 * party status
 */
const getPartiness = (urls: Array<string>): Promise<Map<string, boolean>> => {
  let url_data = {
    urls: urls,
  }
  ...
  return new Promise<Map<string, boolean>>((resolve) => {
    // TODO: send to native-side for real response
    resolve(new Map(urls.map((url) => [url, true] as const)))
  })
}
...
const timeInMSBeforeStart: number = 0
...
const minAdTextChars: number = 30
const minAdTextWords: number = 5
const returnToMutationObserverIntervalMs: number = 10000
const selectorsPollingIntervalMs: number = 500
...
// |scoreCalcIntervalMs|.
let currentMutationScore: number = 0
// The time frame used to calc |currentMutationScore|.
const scoreCalcIntervalMs: number = 1000
// The begin of the time frame to calc |currentMutationScore|.
let currentMutationStartTime: number = performance.now()
...
const classIdWithoutHtmlOrBody: string =
...
const CC = {
  allSelectors: new Set<string>(),
  pendingSelectors: { ids: new Set<string>(), classes: new Set<string>() },
  alwaysHiddenSelectors: new Set<string>(),
  hiddenSelectors: new Set<string>(),
  unhiddenSelectors: new Set<string>(),
  allStyleRules: new Array<string>(),
  runQueues: [
  ...
  // URLS
  pendingOrigins: new Set<string>(),
  // A map of origin strings and their isFirstParty results
  urlFirstParty: new Map<string, boolean>(),
  ...
  proceduralActionFilters: new Array<ProceduralActionFilter>(),
  // Tells us if procedural filtering is available
  hasProceduralActions: false,
  cosmeticStyleSheet: new CSSStyleSheet(),
}
...
const sendPendingOriginsIfNeeded = async () => {
  ...
  for (const origin of origins) {
    const isFirstParty = results.get(origin)
    if (isFirstParty !== undefined) {
      CC.urlFirstParty.set(origin, isFirstParty)
    } else {
...
const sendPendingSelectorsIfNeeded = async () => {
  ...
  if (
    CC.pendingSelectors.ids.size === 0
 ...
 */
const extractIDSelectorIfNeeded = (element: Element) => {
 ...
 * @returns True or false indicating if anything was extracted
 */
const extractClassSelectorsIfNeeded = (element: Element) => {
 ...
 */
const extractNewSelectors = (element: Element) => {
 ...
 */
const extractOriginIfNeeded = (element: Element) => {
 ...
 */
const idleize = (onIdle: () => void, timeout: number) => {
...
const queueSelectorsFromMutations = (mutations: MutationRecord[]) => {
  let mutationScore = 0
  for (const mutation of mutations) {
    const changedElm = mutation.target as Element
 ...
 */
const extractOriginFromURLString = (urlString: string) => {
  try {
 ...
 */
const isFirstPartyURL = (urlString: string) => {
  ...
  if (origin !== undefined) {
    const isFirstParty = CC.urlFirstParty.get(origin)
...
const stripChildTagsFromText = (
  ...
  for (let _i = 0, childElms1 = childElms; _i < childElms1.length; _i++) {
    ...
    localText = localText.replaceAll((anElm as HTMLElement).innerText, '')
  }
  return localText
}
 ...
 */
const showsSignificantText = (elm: Element) => {
...
type QueryResult = {
  ...
  pendingSrcAttributes: Array<string>
}
 ...
 */
const subTreePartyInfo = (elm: Element, queryResult?: QueryResult) => {
...
const shouldUnhideElementAsync = async (element: Element) => {
  const pendingSrcAttributes: Array<string> = []
 ...
 */
const unhideSelectors = (selectors: Array<string>) => {
...
const pumpIntervalMinMs: number = 40
const pumpIntervalMaxMs: number = 100
const maxWorkSize: number = 60
let queueIsSleeping: boolean = false
  ...
  for (let index = 0; index < CC.runQueues.length; index += 1) {
    ...
    for (const aMatchingElm of matchingElms) {
      ...
      for (const selector of currentWorkLoad) {
        ...
        if (
          CC.hiddenSelectors.has(selector)
 ...
 */
const querySelectorsFromElement = (element: Element) => {
  extractNewSelectors(element)
...
const queryAttrsFromDocument = async (
  switchToMutationObserverAtTime: number | undefined,
  sendSelectorsImmediately: boolean,
) => {
  // query selectors from document
  querySelectorsFromElement(document.documentElement)
 ...
 */
const hiddenSelectorsForElement = (element: Element) => {
  if (element.hasAttribute === undefined) {
    return []
  }
 ...
 * otherwise undefined.
 */
const unhideSelectorsMatchingElementIf1P = (element: Element) => {
 ...
 */
const unhideSelectorsMatchingElementAndItsParents = (node: Element) => {
 ...
 */
const unhideSelectorsMatchingElementsAndTheirParents = (
  ...
  for (const nodeRef of nodes) {
    ...
    const element = node as Element
    if (element) {
      const newSelectors = unhideSelectorsMatchingElementAndItsParents(element)
 ...
 */
const onURLMutations = async (
  mutations: MutationRecord[],
  _observer: MutationObserver,
) => {
  const elementsWithURLs: Array<WeakRef<Node>> = []
  mutations.forEach((mutation) => {
    if (mutation.type === 'attributes') {
      ...
      switch (mutation.attributeName) {
        case 'src':
          if (
            changedElm instanceof Element
            && extractOriginIfNeeded(changedElm)
          ) {
      ...
      for (const node of mutation.addedNodes) {
        ...
        const element = node as Element
        if (extractOriginIfNeeded(element)) {
 ...
 */
const queryURLOriginsInElement = (element: Element) => {
  const possibleAdChildNodes: Array<WeakRef<Node>> = []
  const elmWithClassOrId = element.querySelectorAll('[src]')

  elmWithClassOrId.forEach((node: Node) => {
    if (extractOriginIfNeeded(node as Element)) {
 ...
 */
const processHideSelectors = (
  selectors: Array<string>,
  canUnhide1PElements: boolean,
) => {
  let hasChanges = false
...
const executeProceduralActions = (added?: Element[]) => {
  ...
  const getStyleAttr = (style: string) => {
  ...
  const performAction = (element: Element, action: any) => {
    if (action === undefined) {
  ...
  for (const { selector, action } of CC.proceduralActionFilters) {
    try {
      ...
      if (startOperator === selector.length) {
        ...
        // remain
        matchingElements.forEach((elem: Element) => {
