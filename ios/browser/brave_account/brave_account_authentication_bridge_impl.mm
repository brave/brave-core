...
NSString* GetAlertMessage(
  ...
  if (error->is_client_error()) {
    const auto error_code = error->get_client_error()->error_code;
  ...
  const auto net_error_or_http_status = server_error->net_error_or_http_status;
  // `kNull == 0` is the sentinel for "server returned null code".
  const bool has_code =
      server_error->error_code !=
      brave_account::mojom::ResendConfirmationEmailServerErrorCode::kNull;
  return l10n_util::GetNSStringF(
      IDS_BRAVE_ACCOUNT_SERVER_ERROR,
      base::UTF8ToUTF16(absl::StrFormat(
          "%s=%d", net_error_or_http_status > 0 ? "HTTP" : "NET",
          net_error_or_http_status)),
      has_code
          ? base::UTF8ToUTF16(absl::StrFormat(
                ", %s=%d", l10n_util::GetStringUTF8(IDS_BRAVE_ACCOUNT_ERROR),
                static_cast<int>(server_error->error_code)))
          : u"");
}
