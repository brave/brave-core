...
+ (nullable id)serviceForProfile:(ProfileIOS*)profile {
  ...
  // Prefs passed in here must be for the original profile as thats where
  // the private window search engine choice is stored.
  // TemplateURLServiceFactory also redirects OTR profiles to original profile
  return [[TemplateURLServiceBridgeImpl alloc]
      initWithTemplateURLService:service
                           prefs:profile->GetOriginalProfile()->GetPrefs()];
}
