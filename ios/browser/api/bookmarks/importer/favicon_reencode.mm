...
bool ReencodeFavicon(base::span<const uint8_t> src,
  ...
  NSData* image_data = [NSData dataWithBytes:src.data() length:src.size()];
  if (!image_data) {
    return false;
  }
  ...
  png_data = base::ToVector(base::apple::NSDataToSpan(ios_png_data));
  return true;
}
