...
static void AIChatTabHelper::BindPageContentExtractorHost(
                                                          mojo::PendingAssociatedReceiver<mojom::PageContentExtractorHost>
                                                          receiver) {
  auto* tab_helper = AIChatTabHelper::FromWebState(sender);
  if (!tab_helper) {
    return;
  }
...
void AIChatTabHelper::DidStartNavigation(web::WebState* web_state,
...
void AIChatTabHelper::DidFinishNavigation(web::WebState* web_state,
                                          web::NavigationContext* navigation_context) {
...
void AIChatTabHelper::GetSearchSummarizerKey(GetSearchSummarizerKeyCallback callback) {
...
void AIChatTabHelper::OnFetchPageContentComplete(GetPageContentCallback callback,
                                                 std::string content,
                                                 bool is_video,
...
void AIChatTabHelper::SetPendingGetContentCallback(GetPageContentCallback callback) {
  if (pending_get_page_content_callback_) {
    std::move(pending_get_page_content_callback_).Run("", false, "");
  }
