...
const char kBraveSyncV2V1Migrated[] =
    "brave_sync_v2.v1_migrated";  // mapped key (original location)
const char kBraveRewardsBraveSyncV2V1Migrated[] =
    "brave.rewards.brave_sync_v2.v1_migrated";  // kBraveSyncV2V1Migrated's
                                                // mapped location
...
TEST_F(RewardsSyncScopedPersistentPrefStoreTest, Observer) {
  ...
  overlay_->SetValue(kBraveSyncV2V1Migrated,
                     std::make_unique<base::Value>(false),
                     WriteablePrefStore::DEFAULT_PREF_WRITE_FLAGS);
  obs.VerifyAndResetChangedKey(kBraveSyncV2V1Migrated);
  ...
  overlay_->SetValueSilently(kBraveSyncV2V1Migrated,
                             std::make_unique<base::Value>(true),
                             WriteablePrefStore::DEFAULT_PREF_WRITE_FLAGS);
  EXPECT_TRUE(obs.changed_keys.empty());
  ...
  overlay_->RemoveValue(kBraveSyncV2V1Migrated,
                        WriteablePrefStore::DEFAULT_PREF_WRITE_FLAGS);
  obs.VerifyAndResetChangedKey(kBraveSyncV2V1Migrated);
...
TEST_F(RewardsSyncScopedPersistentPrefStoreTest, GetValue) {
  ...
  EXPECT_FALSE(overlay_->GetValue(kBraveSyncV2V1Migrated, nullptr));
  const base::Value* value = nullptr;
  EXPECT_FALSE(overlay_->GetValue(kBraveSyncV2V1Migrated, &value));
  EXPECT_EQ(value, nullptr);

  // existing mapped key
  underlay_->SetValue(kBraveRewardsBraveSyncV2V1Migrated,
                      std::make_unique<base::Value>(true),
                      WriteablePrefStore::DEFAULT_PREF_WRITE_FLAGS);
  EXPECT_TRUE(overlay_->GetValue(kBraveSyncV2V1Migrated, nullptr));
  EXPECT_TRUE(overlay_->GetValue(kBraveSyncV2V1Migrated, &value));
...
TEST_F(RewardsSyncScopedPersistentPrefStoreTest, GetValues) {
  ...
  EXPECT_TRUE(base::Value(true).Equals(value));
  value = nullptr;
  EXPECT_TRUE(values->Get(kBraveSyncV2V1Migrated, &value));
...
TEST_F(RewardsSyncScopedPersistentPrefStoreTest, RemoveValue) {
  ...
  underlay_->SetValue(kBraveRewardsBraveSyncV2V1Migrated,
                      std::make_unique<base::Value>(true),
                      WriteablePrefStore::DEFAULT_PREF_WRITE_FLAGS);
  EXPECT_TRUE(underlay_->GetValue(kBraveRewardsBraveSyncV2V1Migrated, nullptr));
  overlay_->RemoveValue(kBraveSyncV2V1Migrated,
                        WriteablePrefStore::DEFAULT_PREF_WRITE_FLAGS);
  EXPECT_FALSE(
      underlay_->GetValue(kBraveRewardsBraveSyncV2V1Migrated, nullptr));
...
TEST_F(RewardsSyncScopedPersistentPrefStoreTest, GetMutableValue) {
  ...
  underlay_->SetValue(
      ...
      WriteablePrefStore::DEFAULT_PREF_WRITE_FLAGS);
  EXPECT_TRUE(overlay_->GetMutableValue(kSyncBookmarks, nullptr));
  EXPECT_TRUE(overlay_->GetMutableValue(kSyncBookmarks, &overlay_value));
  ASSERT_TRUE(overlay_value->is_dict());
  static_cast<base::DictionaryValue*>(overlay_value)
      ->SetBoolean(kBraveRewardsSyncBookmarks, true);

  const base::Value* underlay_value = nullptr;
  EXPECT_TRUE(underlay_->GetValue(kBraveRewardsSyncBookmarks, &underlay_value));
...
INSTANTIATE_TEST_SUITE_P(
    ...
    Values(MapSyncParamType("", ""),
           ...
           MapSyncParamType(kBraveSyncV2V1Migrated,
                            kBraveRewardsBraveSyncV2V1Migrated)),
