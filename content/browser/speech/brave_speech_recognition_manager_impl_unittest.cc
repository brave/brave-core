...
class BraveSpeechRecognitionManagerImplTest : public testing::Test {
 protected:
  ...
  // reach the NOTREACHED in the error path rather than pass unnoticed.
  void CreateOnDeviceSession(
      SpeechRecognitionAudioForwarderConfig* forwarder = nullptr) {
    ...
    manager_->CreateSession(
        ...
        forwarder
            ? std::optional<SpeechRecognitionAudioForwarderConfig>(*forwarder)
            : std::nullopt,
