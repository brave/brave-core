...
// which is what ContentBrowserClient's own GetAsrSession returns. Binding one
// trips a DCHECK in Remote, so the engine has to drop it instead.
TEST_F(BraveOnDeviceSpeechRecognitionEngineTest, InvalidSessionIsNotBound) {
  CreateEngine(/*session=*/nullptr);
  SetAudioParameters();
  ASSERT_TRUE(client_->requested.Wait());

  // The answer is posted back to this sequence inside the same task that served
  // the request, and this fixture runs both on the main thread, so it is
  // already queued by now and a marker posted here runs behind it. Asserting
  // any sooner would pass whether or not the engine got the answer.
  base::test::TestFuture<void> replied;
  base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
      FROM_HERE, replied.GetCallback());
  ASSERT_TRUE(replied.Wait());

  // TryCreateSession sets this before sending the only Start the engine ever
  // sends, so nothing was started on a session that was never bound.
