...
//
// Note that the base constructor still builds its UI thread Core, which
// asks the optimization guide broker for assets. That never reaches a
// session here because the sample rate is not forwarded to it, but it is
// not free either.
//
