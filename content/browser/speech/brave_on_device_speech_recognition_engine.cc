...
void BraveOnDeviceSpeechRecognitionEngine::OnAsrSessionReady(
  ...
  if (!pending.is_valid()) {
    // No session to be had, because the frame is gone, the feature is off
    // or no model is installed. Nothing is reported from here: the
    // recognizer ignores engine errors until it starts recording, and
    // AudioChunksEnded already ends a session that has no worker. The
    // base engine is equally silent when its own model client fails.
    return;
  }
...
void BraveOnDeviceSpeechRecognitionEngine::SetAudioParameters(
  ...
  // Call the grandparent to set audio_parameters_ without forwarding the
  // sample rate to the base's UI thread Core, whose ModelBroker session
  // path Brave does not use.
